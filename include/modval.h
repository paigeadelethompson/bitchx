...
#define check_ext_mail_status                                                  \
  (global[CHECK_EXT_MAIL_STATUS].check_ext_mail_status_slot)
...
#define BUILT_IN_DLL(x)                                                        \
  void x(IrcCommandDll *intp __attribute__((unused)),                          \
...
#define check_module_version                                                   \
  (global[MODULE_VERSION_CHECK].module_version_check_slot)
...
/* Changed these to cast the function pointer rather than the arguments and
 * result.  The old method wasn't portable to some 64 bit platforms.  Next step
 * - change instances of global[XYZ] to &xyzfunc in the code.
 *
 * Should also think about standardising types (eg Screen instead of struct
 * ScreenStru, get rid of u_char).
 */
/* ircaux.c */
#define new_malloc(x)                                                          \
  (global[NEW_MALLOC].new_malloc_slot)((x), MODULENAME, __FILE__, __LINE__)
#define new_free(x)                                                            \
  (*(x) =                                                                      \
       (global[NEW_FREE].new_free_slot)(*(x), MODULENAME, __FILE__, __LINE__))
#define RESIZE(x, y, z)                                                        \
  ((x) = (global[NEW_REALLOC].new_realloc_slot)(                               \
       (x), sizeof(y) * (z), MODULENAME, __FILE__, __LINE__))
#define malloc_strcpy(x, y)                                                    \
  (global[MALLOC_STRCPY].malloc_strcpy_slot)((x), (y), MODULENAME, __FILE__,   \
                                             __LINE__)
#define malloc_strcat(x, y)                                                    \
  (global[MALLOC_STRCAT].malloc_strcat_slot)((x), (y), MODULENAME, __FILE__,   \
                                             __LINE__)
...
#define remove_trailing_spaces                                                 \
  (global[REMOVE_TRAILING_SPACES].remove_trailing_spaces_slot)
...
#define time_to_next_minute                                                    \
  (global[TIME_TO_NEXT_MINUTE].time_to_next_minute_slot)
...
#define strcpy_nocolorcodes                                                    \
  (global[STRCPY_NOCOLORCODES].strcpy_nocolorcodes_slot)
...
#define m_strdup(x)                                                            \
  (global[M_STRDUP].m_strdup_slot)((x), MODULENAME, __FILE__, __LINE__)
#define m_strcat_ues(x, y, z)                                                  \
  (global[M_STRCAT_UES].m_strcat_ues_slot)((x), (y), (z), MODULENAME,          \
                                           __FILE__, __LINE__)
#define m_strndup(x, y)                                                        \
  (global[M_STRNDUP].m_strndup_slot)((x), (y), MODULENAME, __FILE__, __LINE__)
...
#define remove_from_list_ext                                                   \
  (global[REMOVE_FROM_LIST_EXT].remove_from_list_ext_slot)
#define removewild_from_list                                                   \
  (global[REMOVEWILD_FROM_LIST].removewild_from_list_slot)
...
#define remove_all_from_array                                                  \
  (global[REMOVE_ALL_FROM_ARRAY].remove_all_from_array_slot)
...
#define find_fixed_array_item                                                  \
  (global[FIND_FIXED_ARRAY_ITEM].find_fixed_array_item_slot)
...
#define convert_output_format                                                  \
  (global[CONVERT_OUTPUT_FORMAT].convert_output_format_slot)
...
#define set_lastlog_msg_level                                                  \
  (global[SET_LASTLOG_MSG_LEVEL].set_lastlog_msg_level_slot)
...
#define cursor_not_in_display(x) ((void)
(global[CURSOR_IN_DISPLAY].cursor_in_display_slot((Screen *)x))) #define
cursor_in_display(x) ((void)
(global[CURSOR_IN_DISPLAY].cursor_in_display_slot((Screen *)x)))
*/
#define cursor_not_in_display                                                  \
  (global[CURSOR_NOT_IN_DISPLAY].cursor_not_in_display_slot)
#define cursor_in_display (global[CURSOR_IN_DISPLAY].cursor_in_display_slot)
#define is_cursor_in_display                                                   \
  (global[IS_CURSOR_IN_DISPLAY].is_cursor_in_display_slot)
...
#define create_additional_screen                                               \
  (global[CREATE_ADDITIONAL_SCREEN].create_additional_screen_slot)

/* window.c */
#define free_formats (global[FREE_FORMATS].free_formats_slot)
#define set_screens_current_window                                             \
  (global[SET_SCREENS_CURRENT_WINDOW].set_screens_current_window_slot)
#define new_window (global[NEW_WINDOW].new_window_slot)
#define delete_window (global[DELETE_WINDOW].delete_window_slot)
#define traverse_all_windows                                                   \
  (global[TRAVERSE_ALL_WINDOWS].traverse_all_windows_slot)
#define add_to_invisible_list                                                  \
  (global[ADD_TO_INVISIBLE_LIST].add_to_invisible_list_slot)
#define remove_window_from_screen                                              \
  (global[REMOVE_WINDOW_FROM_SCREEN].remove_window_from_screen_slot)
#define recalculate_window_positions                                           \
  (global[RECALCULATE_WINDOW_POSITIONS].recalculate_window_positions_slot)
...
#define recalculate_windows                                                    \
  (global[RECALCULATE_WINDOWS].recalculate_windows_slot)
...
#define swap_last_window                                                       \
  (global[FUNC_SWAP_LAST_WINDOW].func_swap_last_window_slot)
#define swap_next_window                                                       \
  (global[FUNC_SWAP_NEXT_WINDOW].func_swap_next_window_slot)
#define swap_previous_window                                                   \
  (global[FUNC_SWAP_PREVIOUS_WINDOW].func_swap_previous_window_slot)
#define show_window (global[SHOW_WINDOW].show_window_slot)
#define get_status_by_refnum                                                   \
  (global[GET_STATUS_BY_REFNUM].get_status_by_refnum_slot)
#define get_visible_by_refnum                                                  \
  (global[GET_VISIBLE_BY_REFNUM].get_visible_by_refnum_slot)
#define get_window_by_desc (global[GET_WINDOW_BY_DESC].get_window_by_desc_slot)
#define get_window_by_refnum                                                   \
  (global[GET_WINDOW_BY_REFNUM].get_window_by_refnum_slot)
...
#define update_window_status                                                   \
  (global[UPDATE_WINDOW_STATUS].update_window_status_slot)
#define update_all_status (global[UPDATE_ALL_STATUS].update_all_status_slot)
#define update_window_status_all                                               \
  (global[UPDATE_WINDOW_STATUS_ALL].update_window_status_all_slot)
#define status_update (global[STATUS_UPDATE].status_update_slot)

#define set_prompt_by_refnum                                                   \
  (global[SET_PROMPT_BY_REFNUM].set_prompt_by_refnum_slot)
#define get_prompt_by_refnum                                                   \
  (global[GET_PROMPT_BY_REFNUM].get_prompt_by_refnum_slot)
#define get_target_by_refnum                                                   \
  (global[GET_TARGET_BY_REFNUM].get_target_by_refnum_slot)
#define get_target_cmd_by_refnum                                               \
  (global[GET_TARGET_CMD_BY_REFNUM].get_target_cmd_by_refnum_slot)
#define get_window_target_by_desc                                              \
  (global[GET_WINDOW_TARGET_BY_DESC].get_window_target_by_desc_slot)
#define is_current_channel (global[IS_CURRENT_CHANNEL].is_current_channel_slot)
#define set_current_channel_by_refnum                                          \
  (global[SET_CURRENT_CHANNEL_BY_REFNUM].set_current_channel_by_refnum_slot)
#define get_current_channel_by_refnum                                          \
  (global[GET_CURRENT_CHANNEL_BY_REFNUM].get_current_channel_by_refnum_slot)
#define get_refnum_by_window                                                   \
  (global[GET_REFNUM_BY_WINDOW].get_refnum_by_window_slot)
#define is_bound_to_window (global[IS_BOUND_TO_WINDOW].is_bound_to_window_slot)
#define get_window_bound_channel                                               \
  (global[GET_WINDOW_BOUND_CHANNEL].get_window_bound_channel_slot)
...
#define window_check_servers                                                   \
  (global[WINDOW_CHECK_SERVERS].window_check_servers_slot)
#define change_window_server                                                   \
  (global[CHANGE_WINDOW_SERVER].change_window_server_slot)
#define set_level_by_refnum                                                    \
  (global[SET_LEVEL_BY_REFNUM].set_level_by_refnum_slot)
...
#define clear_window_by_refnum                                                 \
  (global[CLEAR_WINDOW_BY_REFNUM].clear_window_by_refnum_slot)
#define unclear_window_by_refnum                                               \
  (global[UNCLEAR_WINDOW_BY_REFNUM].unclear_window_by_refnum_slot)
#define set_scroll_lines (global[SET_SCROLL_LINES].set_scroll_lines_slot)
#define set_continued_lines                                                    \
  (global[SET_CONTINUED_LINES].set_continued_lines_slot)
#define current_refnum (global[CURRENT_REFNUM].current_refnum_slot)
#define number_of_windows_on_screen                                            \
  (global[NUMBER_OF_WINDOWS_ON_SCREEN].number_of_windows_on_screen_slot)
#define set_scrollback_size                                                    \
  (global[SET_SCROLLBACK_SIZE].set_scrollback_size_slot)
#define is_window_name_unique                                                  \
  (global[IS_WINDOW_NAME_UNIQUE].is_window_name_unique_slot)
#define get_nicklist_by_window                                                 \
  (global[GET_NICKLIST_BY_WINDOW].get_nicklist_by_window_slot)
#define scrollback_backwards_lines                                             \
  (global[SCROLLBACK_BACKWARDS_LINES].scrollback_backwards_lines_slot)
#define scrollback_forwards_lines                                              \
  (global[SCROLLBACK_FORWARDS_LINES].scrollback_forwards_lines_slot)
#define scrollback_forwards                                                    \
  (global[SCROLLBACK_FORWARDS].scrollback_forwards_slot)
#define scrollback_backwards                                                   \
  (global[SCROLLBACK_BACKWARDS].scrollback_backwards_slot)
...
#define unstop_all_windows                                                     \
  (global[FUNC_UNSTOP_ALL_WINDOWS].func_unstop_all_windows_slot)
#define reset_line_cnt (global[RESET_LINE_CNT].reset_line_cnt_slot)
#define toggle_stop_screen                                                     \
  (global[FUNC_TOGGLE_STOP_SCREEN].func_toggle_stop_screen_slot)
#define flush_everything_being_held                                            \
  (global[FLUSH_EVERYTHING_BEING_HELD].flush_everything_being_held_slot)
#define unhold_a_window (global[UNHOLD_A_WINDOW].unhold_a_window_slot)
#define recalculate_window_cursor                                              \
  (global[RECALCULATE_WINDOW_CURSOR].recalculate_window_cursor_slot)
#define make_window_current                                                    \
  (global[MAKE_WINDOW_CURRENT].make_window_current_slot)
...
#define save_display_target                                                    \
  (global[SAVE_DISPLAY_TARGET].save_display_target_slot)
#define restore_display_target                                                 \
  (global[RESTORE_DISPLAY_TARGET].restore_display_target_slot)
#define reset_display_target                                                   \
  (global[RESET_DISPLAY_TARGET].reset_display_target_slot)
...
#define lookup_nickcompletion                                                  \
  (global[LOOKUP_NICKCOMPLETION].lookup_nickcompletion_slot)
#define add_completion_type                                                    \
  (global[ADD_COMPLETION_TYPE].add_completion_type_slot)
...
#define remove_from_channel                                                    \
  (global[REMOVE_FROM_CHANNEL].remove_from_channel_slot)
...
#define create_channel_list                                                    \
  (global[CREATE_CHANNEL_LIST].create_channel_list_slot)
#define flush_channel_stats                                                    \
  (global[FLUSH_CHANNEL_STATS].flush_channel_stats_slot)
#define lookup_channel (global[LOOKUP_CHANNEL].lookup_channel_slot)

/* hash.c */
#define find_nicklist_in_channellist                                           \
  (global[FIND_NICKLIST_IN_CHANNELLIST].find_nicklist_in_channellist_slot)
#define add_nicklist_to_channellist                                            \
  (global[ADD_NICKLIST_TO_CHANNELLIST].add_nicklist_to_channellist_slot)
#define next_nicklist (global[NEXT_NICKLIST].next_nicklist_slot)
#define next_namelist (global[NEXT_NAMELIST].next_namelist_slot)
#define add_name_to_genericlist                                                \
  (global[ADD_NAME_TO_GENERICLIST].add_name_to_genericlist_slot)
#define find_name_in_genericlist                                               \
  (global[FIND_NAME_IN_GENERICLIST].find_name_in_genericlist_slot)
#define add_whowas_userhost_channel                                            \
  (global[ADD_WHOWAS_USERHOST_CHANNEL].add_whowas_userhost_channel_slot)
#define find_userhost_channel                                                  \
  (global[FIND_USERHOST_CHANNEL].find_userhost_channel_slot)
...
#define clear_sorted_nicklist                                                  \
  (global[CLEAR_SORTED_NICKLIST].clear_sorted_nicklist_slot)
#define add_name_to_floodlist                                                  \
  (global[ADD_NAME_TO_FLOODLIST].add_name_to_floodlist_slot)
#define find_name_in_floodlist                                                 \
  (global[FIND_NAME_IN_FLOODLIST].find_name_in_floodlist_slot)

#define remove_oldest_whowas_hashlist                                          \
  (global[REMOVE_OLDEST_WHOWAS_HASHLIST].remove_oldest_whowas_hashlist_slot)
...
#define get_wset_string_var                                                    \
  (global[GET_WSET_STRING_VAR].get_wset_string_var_slot)
#define set_wset_string_var                                                    \
  (global[SET_WSET_STRING_VAR].set_wset_string_var_slot)
...
#define queue_send_to_server                                                   \
  (global[QUEUE_SEND_TO_SERVER].queue_send_to_server_slot)
...
#define connect_to_server_by_refnum                                            \
  (global[CONNECT_TO_SERVER_BY_REFNUM].connect_to_server_by_refnum_slot)
#define close_server (global[CLOSE_SERVER].close_server_slot)
#define is_server_connected                                                    \
  (global[IS_SERVER_CONNECTED].is_server_connected_slot)
#define flush_server (global[FLUSH_SERVER].flush_server_slot)
#define server_is_connected                                                    \
  (global[SERVER_IS_CONNECTED].server_is_connected_slot)
...
#define display_server_list                                                    \
  (global[DISPLAY_SERVER_LIST].display_server_list_slot)
...
#define find_in_server_list                                                    \
  (global[FIND_IN_SERVER_LIST].find_in_server_list_slot)
#define parse_server_index (global[PARSE_SERVER_INDEX].parse_server_index_slot)

#define set_server_redirect                                                    \
  (global[SET_SERVER_REDIRECT].set_server_redirect_slot)
#define get_server_redirect                                                    \
  (global[GET_SERVER_REDIRECT].get_server_redirect_slot)
#define check_server_redirect                                                  \
  (global[CHECK_SERVER_REDIRECT].check_server_redirect_slot)
...
#define update_server_umode                                                    \
  (global[UPDATE_SERVER_UMODE].update_server_umode_slot)
#define set_server_motd (global[SET_SERVER_MOTD].set_server_motd_slot)
#define set_server_operator                                                    \
  (global[SET_SERVER_OPERATOR].set_server_operator_slot)
...
#define set_server_password                                                    \
  (global[SET_SERVER_PASSWORD].set_server_password_slot)
#define set_server_nickname                                                    \
  (global[SET_SERVER_NICKNAME].set_server_nickname_slot)
...
#define get_server_nickname                                                    \
  (global[GET_SERVER_NICKNAME].get_server_nickname_slot)
...
#define get_server_operator                                                    \
  (global[GET_SERVER_OPERATOR].get_server_operator_slot)
...
#define get_pending_nickname                                                   \
  (global[GET_PENDING_NICKNAME].get_pending_nickname_slot)
...
#define get_server_channels                                                    \
  (global[GET_SERVER_CHANNELS].get_server_channels_slot)

#define set_server_last_ctcp_time                                              \
  (global[SET_SERVER_LAST_CTCP_TIME].set_server_last_ctcp_time_slot)
#define get_server_last_ctcp_time                                              \
  (global[GET_SERVER_LAST_CTCP_TIME].get_server_last_ctcp_time_slot)
#define set_server_trace_flag                                                  \
  (global[SET_SERVER_TRACE_FLAG].set_server_trace_flag_slot)
#define get_server_trace_flag                                                  \
  (global[GET_SERVER_TRACE_FLAG].get_server_trace_flag_slot)
#define get_server_read (global[GET_SERVER_READ].get_server_read_slot)
#define get_server_linklook                                                    \
  (global[GET_SERVER_LINKLOOK].get_server_linklook_slot)
#define set_server_linklook                                                    \
  (global[SET_SERVER_LINKLOOK].set_server_linklook_slot)
#define get_server_stat_flag                                                   \
  (global[GET_SERVER_STAT_FLAG].get_server_stat_flag_slot)
#define set_server_stat_flag                                                   \
  (global[SET_SERVER_STAT_FLAG].set_server_stat_flag_slot)
#define get_server_linklook_time                                               \
  (global[GET_SERVER_LINKLOOK_TIME].get_server_linklook_time_slot)
#define set_server_linklook_time                                               \
  (global[SET_SERVER_LINKLOOK_TIME].set_server_linklook_time_slot)
#define get_server_trace_kill                                                  \
  (global[GET_SERVER_TRACE_KILL].get_server_trace_kill_slot)
#define set_server_trace_kill                                                  \
  (global[SET_SERVER_TRACE_KILL].set_server_trace_kill_slot)
#define add_server_channels                                                    \
  (global[ADD_SERVER_CHANNELS].add_server_channels_slot)
#define set_server_channels                                                    \
  (global[SET_SERVER_CHANNELS].set_server_channels_slot)
#define send_msg_to_channels                                                   \
  (global[SEND_MSG_TO_CHANNELS].send_msg_to_channels_slot)
...
#define destroy_local_stack                                                    \
  (global[DESTROY_LOCAL_STACK].destroy_local_stack_slot)
...
#define remove_all_dcc_binds                                                   \
  (global[REMOVE_ALL_DCC_BINDS].remove_all_dcc_binds_slot)
...
#define dcc_chat_socketread                                                    \
  (global[DCC_CHAT_SOCKETREAD].dcc_chat_socketread_slot)
#define dcc_send_socketread                                                    \
  (global[DCC_SEND_SOCKETREAD].dcc_send_socketread_slot)
...
#define connected_to_server                                                    \
  (*global[CONNECTED_TO_SERVER].connected_to_server_slot)
#define primary_server (*global[PRIMARY_SERVER].primary_server_slot)
#define parsing_server_index                                                   \
  (*global[PARSING_SERVER_INDEX].parsing_server_index_slot)
...
#define default_output_function                                                \
  (global[DEFAULT_OUTPUT_FUNCTION].default_output_function_slot)
...
#ifdef GUI
#ifndef MAIN_SOURCE
#define lastclicklinedata                                                      \
  ((char *)*global[LASTCLICKLINEDATA].lastclicklinedata_slot)
