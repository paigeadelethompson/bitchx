...
typedef enum NoiseEnum { UNKNOWN = 0, SILENT, QUIET, NORMAL, NOISY } Noise;
...
typedef struct {
  ...
  void (*func)(char *, char *, char *,
               char *); /* function that is the command */
  unsigned flags;
  char *help;
} IrcCommand;
...
typedef struct _IrcCommandDll {
  ...
  char *server_func; /* what gets sent to the server
                      * (if anything) */
  void (*func)(struct _IrcCommandDll *, char *, char *, char *,
               char *); /* function that is the command */
  unsigned flags;
  char *result;
  char *help;
...
typedef char *((*CTCP_DllHandler)(struct _CtcpEntryDll *, char *, char *,
                                  char *));
...
typedef struct WindowStru {
  ...
  Display *top_of_scrollback,          /* Start of the scrollback buffer */
      ...
      *scrollback_point, *screen_hold; /* Where t_o_d was at start of sb */
...
typedef struct ScreenStru {
  ...
  HVPS hvps, hvpsnick;
#endif
  HWND hwndFrame, hwndClient, hwndMenu, hwndLeft, hwndRight, hwndnickscroll,
      hwndscroll;
  int VIO_font_width, VIO_font_height, spos, mpos, codepage;
  char aviokbdbuffer[256];
#elif defined(GTK)
  GtkWidget *window, *viewport, *menubar, *scroller, *clist, *scrolledwindow,
      *box;
  GdkFont *font;
  char *fontname;
  ...
  gint gtkio;
  int pipe[2];
  int maxfontwidth, maxfontheight;
#endif
...
typedef struct {
  ...
  void (*func)(Window *, char *,
               int);    /* function to do every time variable is set */
