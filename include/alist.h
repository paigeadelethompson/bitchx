...
static inline u_32int_t ci_alist_hash(const char *s, u_32int_t *mask) {
  ...
  if (s[0] == 0) {
    ...
    *mask = 0xffff0000;
    ...
    x = (stricmp_table[(int)s[0]] << 24) | (stricmp_table[(int)s[1]] << 16) |
        (stricmp_table[(int)s[2]] << 8) | stricmp_table[(int)s[3]];
    *mask = 0xffffff00 | (s[3] ? 0xff : 0x00);
  }
...
typedef enum { HASH_INSENSITIVE, HASH_SENSITIVE } hash_type;
...
void *BX_find_fixed_array_item(void *Array, size_t size, int siz, const char *,
                               int *cnt, int *loc);
