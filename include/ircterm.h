...
#else
static inline int xlate_char(int c) { return c; }
#endif
...
static inline void term_flush(void) { fflush(current_ftarget); }
...
struct term_struct {
  ...
  int TI_hls;   /* hl  / hls       terminal uses only HLS color notation
                   (Tektronix) */
  int TI_xhpa;  /* YA  / xhpa      only positive motion for hpa/mhpa caps */
  int TI_crxm;  /* YB  / crxm      using cr turns off micro mode */
  int TI_daisy; /* YC  / daisy     printer needs operator to change character
                   set */
  ...
  char *TI_cmdch; /* CC  / cmdch     terminal settable cmd character in
                     prototype !? */
  ...
  char *TI_cnorm; /* ve  / cnorm     make cursor appear normal (undo
                     civis/cvvis) */
  ...
  char
      *TI_invis; /* mk  / invis     turn on blank mode (characters invisible) */
  ...
  char
      *TI_pfkey; /* pk  / pfkey     program function key #1 to type string #2 */
  char *TI_pfloc; /* pl  / pfloc     program function key #1 to execute string
                     #2 */
  ...
  char *TI_initp; /* Ip  / initp     Initialize color pair #1 to fg=(#2,#3,#4),
                     bg=(#5,#6,#7) */
  ...
  char *TI_smgbp;  /* Zl  / smgbp     Set bottom margin at line #1 or #2 lines
                      from bottom */
  char
      *TI_smglp; /* Zm  / smglp     Set left (right) margin at column #1 (#2) */
  ...
  char
      *TI_docr; /* Zw  / docr      Printing any of these characters causes CR */
  ...
  char *TI_pfxl;  /* xl  / pfxl      Program function key #1 to type string #2
                     and show string #3 */
  ...
  char
      *TI_smglr; /* ML  / smglr     Set both left and right margins to #1, #2 */
  char *
      TI_smgtb; /* MT  / smgtb     Sets both top and bottom margins to #1, #2 */
  ...
  char *
      TI_sgr1; /* sA  / sgr1      Define second set of video attributes #1-#6 */
  char *TI_slength; /* sL  / slength   YI Set page length to #1 hundredth of an
                       inch */
...
#endif
#define term_underline_on()                                                    \
  if (get_int_var(UNDERLINE_VIDEO_VAR))                                        \
  outcap(UNDL_ON)
#define term_underline_off()                                                   \
  if (get_int_var(UNDERLINE_VIDEO_VAR))                                        \
  outcap(UNDL_OFF)
#define term_standout_on()                                                     \
  if (get_int_var(INVERSE_VIDEO_VAR))                                          \
  outcap(REV_ON)
#define term_standout_off()                                                    \
  if (get_int_var(INVERSE_VIDEO_VAR))                                          \
  outcap(REV_OFF)
#define term_blink_on()                                                        \
  if (get_int_var(BLINK_VIDEO_VAR))                                            \
  outcap(BLINK_ON)
#define term_blink_off()                                                       \
  if (get_int_var(BLINK_VIDEO_VAR))                                            \
  outcap(BLINK_OFF)
#define term_bold_on()                                                         \
  if (get_int_var(BOLD_VIDEO_VAR))                                             \
  outcap(BOLD_ON)
#define term_bold_off()                                                        \
  if (get_int_var(BOLD_VIDEO_VAR))                                             \
  outcap(BOLD_OFF)
#define term_altcharset_on()                                                   \
  if (get_int_var(ALT_CHARSET_VAR))                                            \
  outcap(ALTCHAR_ON)
#define term_altcharset_off()                                                  \
  if (get_int_var(ALT_CHARSET_VAR))                                            \
  outcap(ALTCHAR_OFF)
