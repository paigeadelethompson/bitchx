...
static int check_cdrom_str(void) {
  ...
  put_it("%s: /SET CD_DEVICE  - The name of the CDROM device",
         cparse(cdrom_prompt));
  return 0;
}
...
unsigned char msf1_ent[3];
  ...
  if (((drive = open(dev, 0)) < 0)) {
    if (errno == EACCES)
      put_it("%s: you don't have access to the cdrom driver",
             cparse(cdrom_prompt));
    return (-1);
  }
  ...
  for (i = 1; i <= hdr.cdth_trk1 + 1; i++) {
    ...
    if (ioctl(drive, CDROMREADTOCENTRY, &TocEntry[i]))
      ...
      TocEntry[i].m_length = TocEntry[i].cdte_addr.msf.minute * 60 +
                             TocEntry[i].cdte_addr.msf.second;
      TocEntry[i].m_start =
          TocEntry[i].m_length * 75 + TocEntry[i].cdte_addr.msf.frame;
    }
  }
  ...
  for (i = 1; i <= hdr.cdth_trk1 + 1; i++) {
    ...
    if (TocEntry[i].avoid)
      TocEntry[i].m_length =
          (TocEntry[i + 1].m_start - TocEntry[i + 1].m_start) * 2;
  }
...
void set_cd_device(Window *win, char *str, int blah) {
  ...
  if (!str || !check_mount(str)) {
    put_it("%s: ERROR: CDROM is already mounted, please unmount, and try again",
           cparse(cdrom_prompt));
  ...
  if (cd_init(str) < 0) {
    put_it(
        "%s: ERROR: Could not initialize the CDROM, check if a disk is inside",
        cparse(cdrom_prompt));
    set_string_var(CD_DEVICE_VAR, NULL);
    return;
  }
...
BUILT_IN_COMMAND(cd_play) {
  ...
  if (args && *args) {
    ...
    if (TocEntry[tn].avoid == 0) {
      ...
      put_it("%s: Cannot play track #%d (Might be data track)",
             cparse(cdrom_prompt), tn);
  } else
...
BUILT_IN_COMMAND(cd_list) {
...
#endif
  {
    put_it("%s: Track #%02d: %02d:%02d:%02d %02d:%02d:%02d",
           cparse(cdrom_prompt), i, TocEntry[i].m_length / (60 * 75),
           (TocEntry[i].m_length % (60 * 75)) / 75, TocEntry[i].m_length % 75,
           TocEntry[i].m_start / (60 * 75),
           (TocEntry[i].m_start % (60 * 75)) / 75, TocEntry[i].m_start % 75);
  }
}
...
BUILT_IN_COMMAND(cd_pause) {
  ...
  if (ioctl(drive, !cpause ? CDROMPAUSE : CDROMRESUME))
    ...
    put_it("%s: %s", cparse(cdrom_prompt),
           !cpause ? "Your cdrom has been paused"
                   : "Your cdrom has been resumed");
  cpause ^= 1;
}

BUILT_IN_COMMAND(cd_help) {
  put_it("%s: CDPLAY            - Play a CDROM Track Number",
         cparse(cdrom_prompt));
  put_it("%s: CDSTOP            - Make the CDROM Stop playing",
         cparse(cdrom_prompt));
  put_it("%s: CDEJECT           - Eject the CDROM Tray", cparse(cdrom_prompt));
  put_it("%s: CDVOL             - Set's the CDROM Volume",
         cparse(cdrom_prompt));
  put_it("%s: CDLIST            - List of CDROM tracks", cparse(cdrom_prompt));
  put_it("%s: CDPAUSE           - Pause/resume the CDROM",
         cparse(cdrom_prompt));
}
