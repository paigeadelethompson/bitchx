...
extern WhowasList *check_whowas_buffer(char *nick __attribute__((unused)), char *userhost,
                                       char *channel) {
  WhowasList *tmp;
...
/* Search the whowas buffer for a nick and channel combination.  NULL channel
 * matches any channel. */
WhowasList *check_whowas_nick_buffer(const char *nick, const char *channel) {
  WhowasList *tmp = NULL;

  for (tmp = next_userhost(&whowas_userlist_list, NULL); tmp;
       tmp = next_userhost(&whowas_userlist_list, tmp)) {
    if (!my_stricmp(tmp->nicklist->nick, nick) &&
        (!channel || !my_stricmp(tmp->channel, channel))) {
      return tmp;
    }
  }
  for (tmp = next_userhost(&whowas_reg_list, NULL); tmp;
       tmp = next_userhost(&whowas_reg_list, tmp)) {
    if (!my_stricmp(tmp->nicklist->nick, nick) &&
        (!channel || !my_stricmp(tmp->channel, channel))) {
      return tmp;
    }
...
extern WhowasList *check_whosplitin_buffer(char *nick __attribute__((unused)), char *userhost,
                                           char *channel, int unlink) {
  WhowasList *tmp = NULL;
  tmp = find_userhost_channel(userhost, channel, unlink, &whowas_splitin_list);
  return tmp;
}

void add_to_whowas_buffer(NickList *nicklist, char *channel, char *server1,
                          char *server2) {
  WhowasList *new;
  ...
  if (nicklist->userlist) {
    if (whowas_userlist_count >= WHOWAS_USERLIST_MAX) {
      whowas_userlist_count -= remove_oldest_whowas(
          &whowas_userlist_list, 0,
    ...
    if (whowas_reg_count >= WHOWAS_REG_MAX) {
      whowas_reg_count -= remove_oldest_whowas(
          &whowas_reg_list, 0, (whowas_reg_count + 1) - WHOWAS_REG_MAX);
    }
    new = (WhowasList *)new_malloc(sizeof(WhowasList));
    new->has_ops = nick_isop(nicklist);
...
void add_to_whosplitin_buffer(NickList *nicklist, char *channel, char *server1,
                              char *server2) {
  WhowasList *new;
...
void clean_whowas_list(void) {
  if (whowas_userlist_count)
    whowas_userlist_count -=
        remove_oldest_whowas_hashlist(&whowas_userlist_list, 20 * 60, 0);
  if (whowas_reg_count)
    whowas_reg_count -=
        remove_oldest_whowas_hashlist(&whowas_reg_list, 10 * 60, 0);
...
void sync_whowas_adduser(UserList *added) {
  ...
  for (tmp = next_userhost(&whowas_userlist_list, NULL); tmp;
    ...
    if (check_channel_match(added->channels, tmp->channel))
      if (wild_match(added->host, tmp->nicklist->host) &&
          wild_match(added->nick, tmp->nicklist->nick))
...
void sync_whowas_unuser(UserList *entry) {
  ...
  for (tmp = next_userhost(&whowas_userlist_list, NULL); tmp;
       tmp = next_userhost(&whowas_userlist_list, tmp)) {
    if (!tmp->nicklist->userlist)
      continue;
    if (!my_stricmp(tmp->nicklist->userlist->host, entry->host) &&
        !my_stricmp(tmp->nicklist->userlist->nick, entry->nick) &&
...
void sync_whowas_addshit(ShitList *added) {
  ...
  for (tmp = next_userhost(&whowas_userlist_list, NULL); tmp;
       tmp = next_userhost(&whowas_userlist_list, tmp)) {
    if (check_channel_match(added->channels, tmp->channel)) {
      snprintf(user, sizeof user, "%s!%s", tmp->nicklist->nick,
               tmp->nicklist->host);
...
extern WhowasChanList *check_whowas_chan_buffer(char *channel, int refnum,
                                                int unlink) {
...
void add_to_whowas_chan_buffer(ChannelList *channel) {
  ...
  if (whowas_chan_count >= WHOWAS_CHAN_MAX) {
    whowas_chan_count -= remove_oldest_chan_whowas(
        &whowas_chan_list, 0, (whowas_chan_count + 1) - WHOWAS_CHAN_MAX);
  }
...
void clean_whowas_chan_list(void) {
  whowas_chan_count -=
      remove_oldest_chan_whowas(&whowas_chan_list, 24 * 60 * 60, 0);
}
...
void clear_whowas(void) {
  ...
  if (whowas_userlist_count)
    whowas_userlist_count -=
        remove_oldest_whowas_hashlist(&whowas_userlist_list, 0, 0);
  if (whowas_reg_count)
...
void show_wholeft(char *channel __attribute__((unused))) {
  int count = 0;
  int hook = 0;
  time_t ltime = now;
  reset_display_target();
...
#endif
  hook = show_wholeft_hashtable(&whowas_userlist_list, ltime, &count, &hook,
                                "Splitin");
  hook =
      show_wholeft_hashtable(&whowas_reg_list, ltime, &count, &hook, "Splitin");
  if (count && hook && fget_string_var(FORMAT_WHOLEFT_FOOTER_FSET))
    put_it("%s", convert_output_format(
                     fget_string_var(FORMAT_WHOLEFT_FOOTER_FSET), NULL));
}
