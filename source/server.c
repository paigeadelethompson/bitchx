...
#if defined(WINNT) || defined(__EMX__) || defined(__CYGWIN__) ||               \
    defined(WANT_IDENTD)
...
void BX_close_server(int cs_index, char *message) {
  ...
  if (serv_close_func)
    (*serv_close_func)(cs_index, server_list[cs_index].local_addr,
                       server_list[cs_index].port);
...
#if defined(WINNT) || defined(__EMX__) || defined(CYGWIN) ||                   \
    defined(WANT_IDENTD)
  already_identd = 0;
#endif
}
...
int timed_server(void *args, char *sub __attribute__((unused))) {
  char *p = (char *)args;
  static int retry = 0;
  int serv = -1;
  if (!p || !*p)
...
int advance_server(int i) {
  ...
  if (!get_int_var(AUTO_RECONNECT_VAR) &&
      (server_list[server].req_server != server ||
       server_list[server].retries > 1)) {
    close_server(server, empty_string);
    from_server = -1;
    if (do_hook(DISCONNECT_LIST, "No Connection"))
      put_it("%s", convert_output_format(
                       fget_string_var(FORMAT_DISCONNECT_FSET), "%s %s",
                       update_clock(GET_TIME), "No connection"));
    return -1;
  } else if (server == server_list[i].req_server &&
             server_list[server].retries > 1)
...
static void scan_nonblocking(void) {
  ...
  for (i = 0; i < number_of_servers; i++) {
    if (((server_list[i].read > -1) || (server_list[i].write > -1)) &&
  ...
  for (i = 0; i < number_of_servers && i > -1; i++) {
    ...
    if (is_server_connected(i)) {
      ...
      if (lag_check_interval > 0 &&
        ...
        my_send_to_server(
            i, "PING LAG!%lu.%ld.%ld :%s", server_list[i].lag_cookie,
            (long)server_list[i].lag_sent.tv_sec,
            (long)server_list[i].lag_sent.tv_usec, get_server_itsname(i));
      }
      if (server_list[i].lag != -1 &&
...
void do_server(fd_set *rd, fd_set *wr) {
  ...
  for (i = 0; i < number_of_servers; i++) {
#ifdef NON_BLOCKING_CONNECTS
    if (((des = server_list[i].write) > -1) && FD_ISSET(des, wr) &&
        !(server_list[i].login_flags & SF_LOGGED_IN)) {
      ...
      if (getpeername(des, (struct sockaddr *)&sa, &salen) != -1) {
        ...
        if (!server_list[i].ssl_fd ||
            server_list[i].ssl_error == SSL_ERROR_WANT_WRITE) {
#endif
          server_list[i].connect_wait = 0;
    ...
    if (((des = server_list[i].read) > -1) && FD_ISSET(des, rd)) {
      ...
      if (serv_input_func)
        ...
        if (get_server_ssl(i)) {
          ...
          if (!(server_list[i].login_flags & SF_LOGGED_IN)) {
            if (!server_list[i].ssl_fd ||
                server_list[i].ssl_error == SSL_ERROR_WANT_READ) {
              server_list[i].connect_wait = 0;
              finalize_server_connect(i, server_list[i].c_server, i);
            }
          } else
#endif
            junk =
                dgets(bufptr, des, 1, BIG_BUFFER_SIZE, server_list[i].ssl_fd);
        } else
#endif
            ...
            if ((server_list[i].from_server != -1)) {
              if ((server_list[server_list[i].from_server].read != -1) &&
                ...
                say("Connection to server %s resumed...",
                    server_list[server_list[i].from_server].name);
 ...
 */
void server_lag_reply(int s, unsigned long cookie, struct timeval lag_recv,
                      struct timeval lag_sent) {
...
int BX_find_server_refnum(char *server, char **rest) {
  int refnum;
  int port = irc_port;
  char *cport = NULL, *password = NULL, *nick = NULL, *snetwork = NULL;
   ...
   */
    ...
    if (!server || !*server)
      return from_server;
    for (i = 0; i < number_of_servers; i++) {
      if (server && server_list[i].snetwork &&
          !my_stricmp(server, server_list[i].snetwork))
        return i;
    }
  }
  /*
 ...
 */
void BX_add_to_server_list(char *server, int port, char *password, char *nick,
                           char *snetwork, int ssl, int overwrite) {
 ...
 * "server:portnum:password:nickname:snetwork".  It the points port to the
 * portnum portion and password to the password portion.  This chews up the
 * original string, so * upon return, name will only point the the name.  If
 * portnum or password are missing or empty,  their respective returned value
 * will point to null.
 *
 * With IPv6 patch it also supports comma as a delimiter.
 */
void BX_parse_server_info(char *name, char **port, char **password, char **nick,
                          char **snetwork) {
  char *ptr, delim;
...
int BX_build_server_list(char *servers) {
  ...
  while (servers) {
    ...
    while ((host = new_next_arg(servers, &servers)) != NULL) {
      ...
      add_to_server_list(host, port_num, password, nick,
                         snetwork ? snetwork : default_network, do_use_ssl, 0);
      i++;
    }
    servers = rest;
  }
...
/*
 * read_server_file: reads hostname:portnum:password:network server information
 * from a file and adds this stuff to the server list.  See build_server_list()/
 */
...
/*
 * write_server_file: writes hostname:portnum:password:network server
...
noidentwd:
...
#ifdef NON_BLOCKING_CONNECTS
  new_des = connect_by_number(server_name, &this_sucks, SERVICE_CLIENT,
                              PROTOCOL_TCP, 1);
#else
  new_des = connect_by_number(server_name, &this_sucks, SERVICE_CLIENT,
                              PROTOCOL_TCP, 0);
  ...
  if (new_des < 0) {
    ...
    say("Unable to connect to port %d of server %s: %s", port, server_name,
  ...
  if (*server_name != '/') {
    ...
    if ((server_list[from_server].local_addr.sf_family =
             localaddr->sf_family) == AF_INET)
      memcpy(&server_list[from_server].local_addr.sf_addr, &localaddr->sf_addr,
             sizeof(struct in_addr));
#ifdef IPV6
    else
      memcpy(&server_list[from_server].local_addr.sf_addr6,
             &localaddr->sf_addr6, sizeof(struct in6_addr));
#endif
  }
#ifdef WDIDENT
...
int finalize_server_connect(int refnum, int c_server, int my_from_server) {
  if (serv_open_func)
    (*serv_open_func)(my_from_server, server_list[my_from_server].local_addr,
                      server_list[my_from_server].port);
  ...
  if (get_server_ssl(refnum)) {
    ...
    if (err < 1) {
      server_list[refnum].ssl_error =
          SSL_get_error(server_list[refnum].ssl_fd, err);

      /* The SSL_connect can't complete yet.  Return without calling
       * register_server(), and this function will be called again later.
       */
  ...
  *server_list[refnum].umode = 0;
  ...
  set_window_server(server_list[refnum].server_change_refnum, my_from_server,
                    0);
...
int next_server_internal(int server, int depth, int original) {
  ...
  if (get_int_var(SERVER_GROUPS_VAR) && server_list[original].snetwork) {
    while (
        !server_list[server].snetwork ||
        strcmp(server_list[server].snetwork, server_list[original].snetwork)) {
      server++;
      if (server == number_of_servers) {
        server = 0;
        if (been_here)
 ...
 */
int next_server(int server) { return next_server_internal(server, 0, server); }
...
void BX_get_connected(int server, int old_server) {
  ...
  if (!is_server_connected(spawned_server) ||
      !get_server_nickname(spawned_server))
...
void BX_display_server_list(void) {
  ...
  if (server_list) {
    if (from_server != -1)
      say("Current server: %s %d", server_list[from_server].name,
          server_list[from_server].port);
    else
      say("Current server: <None>");

    if (primary_server != -1)
      say("Primary server: %s %d", server_list[primary_server].name,
    ...
    for (i = 0; i < number_of_servers; i++) {
      if (!netw && server_list[i].snetwork) {
        ...
        say("[%s]", netw);
                 ...
                 my_stricmp(netw, server_list[i].snetwork)) {
        netw = server_list[i].snetwork;
        say("[%s]", netw);
      }
      if (!server_list[i].nickname) {
        if (server_list[i].read == -1)
          say("\t%2d) %s %d", i, server_list[i].name, server_list[i].port);
        else
          say("\t%2d) %s %d", i, server_list[i].name, server_list[i].port);
      } else {
        if (server_list[i].read == -1)
          say("\t%2d) %s %d (was %s)", i, server_list[i].name,
              server_list[i].port, server_list[i].nickname);
        else
          say("\t%2d) %s %d (%s)", i, server_list[i].name, server_list[i].port,
...
BUILT_IN_COMMAND(servercmd) {
   ...
   */
           ...
           !my_strnicmp(server, "-SEND", strlen(server))) {
    if (!(server = next_arg(args, &args)))
      return;
    if ((i = parse_server_index(server)) == -1)
      if ((i = find_in_server_list(server, 0)) == -1)
   ...
   */
           ...
           !my_strnicmp(server, "-SAVE", strlen(server))) {
...
void reinstate_user_modes(int server) {
  ...
  if (modes && *modes)
    my_send_to_server(server, "MODE %s +%s", get_server_nickname(server),
                      modes);
}
    ...
    if (fget_string_var(FORMAT_AWAY_FSET) && !silent) {
      ...
      if (get_int_var(SEND_AWAY_MSG_VAR)) {
        snprintf(buffer, BIG_BUFFER_SIZE, "ACTION %s",
                 stripansicodes(convert_output_format(
                     fget_string_var(FORMAT_AWAY_FSET),
                     "%s [\002BX\002-MsgLog %s] %s", update_clock(GET_TIME),
                     get_int_var(MSGLOG_VAR) ? "On" : "Off", message)));
      ...
      send_to_server("%s :%s", "AWAY",
                     stripansicodes(convert_output_format(
                         ...
                         get_int_var(MSGLOG_VAR) ? "On" : "Off", message)));
      strncpy(buffer,
              convert_output_format(fget_string_var(FORMAT_SEND_ACTION_FSET),
                                    "%s %s $C ", update_clock(GET_TIME),
                                    server_list[ssa_index].nickname),
              BIG_BUFFER_SIZE);
      strlcat(buffer,
              convert_output_format(
                  fget_string_var(FORMAT_AWAY_FSET),
      ...
      send_to_server("%s :%s", "AWAY",
                     stripansicodes(convert_output_format(message, NULL)));
  } else {
...
int BX_server_list_size(void) { return number_of_servers; }
...
void clear_sent_to_server(int servnum) { server_list[servnum].sent = 0; }

int sent_to_server(int servnum) { return server_list[servnum].sent; }
...
char *BX_get_server_redirect(int s) { return server_list[s].redirect; }
 ...
 */
void got_my_userhost(UserhostItem *item, char *nick __attribute__((unused)), char *stuff __attribute__((unused))) {
...
void send_from_server_queue(void) {
  QueueSend *tmp;
  if ((tmp = serverqueue)) {
    if (now - server_list[tmp->server].last_sent >=
        get_int_var(QUEUE_SENDS_VAR)) {
...
static void vsend_to_server(int server, int type, const char *format,
                            va_list args) {
  int des, len;
  char buffer[IRCD_BUFFER_SIZE + 1];
...
BUILT_IN_COMMAND(disconnectcmd) {
  ...
  if (args && *args && isdigit((unsigned char)*args) &&
      (server = next_arg(args, &args)) != NULL) {
...
void BX_fudge_nickname(int servnum) {
  ...
  if (s->fudge_factor < (int)strlen(l_nickname))
...
void start_identd(void) {
  ...
  if ((sock = connect_by_number(NULL, &port, SERVICE_SERVER, PROTOCOL_TCP, 1)) >
      -1)
...
Sping *get_server_sping(int server, char *sname) {
  if (server <= -1)
    return NULL;
  if (sname)
    return (Sping *)find_in_list((List **)&server_list[server].in_sping, sname,
                                 0);
...
void clear_server_sping(int server, char *name) {
  ...
  if (server <= -1)
    return;
  if (name) {
    if ((tmp = (Sping *)remove_from_list((List **)&server_list[server].in_sping,
                                         name))) {
      new_free(&tmp->sname);
      new_free(&tmp);
    }
...
int BX_get_server_read(int server) { return server_list[server].read; }
...
BUILT_IN_COMMAND(do_map) {
  ...
  if (server_list[from_server].link_look == 0) {
    ...
    bitchsay("Wait until previous %s is done",
             server_list[from_server].link_look == 2 ? "MAP" : "LLOOK");
}
...
void show_server_map(void) {
  ...
  for (tmp = map; tmp; tmp = map) {
    ...
    if (!tmp->hopcount || tmp->hopcount != prevdist)
      strlcpy(tmp1, convert_output_format("%K[%G$0%K]", "%d", tmp->hopcount),
              sizeof tmp1);
    else
      *tmp1 = 0;
    snprintf(tmp2, sizeof tmp2, "$G %%W$[-%d]1%%c $0 %s", tmp->hopcount * 3,
             tmp1);
    put_it("%s", convert_output_format(
                     tmp2, "%s %s", tmp->name,
...
irc_server *add_server(irc_server **serv1, char *channel, char *arg, int hops,
                       time_t t) {
  irc_server *serv2;
...
void add_split_server(char *name, char *link, int hops) {
  ...
  if (from_server < 0)
    return;
  temp =
      add_server(&(server_list[from_server].split_link), name, link, hops, now);
  temp->status = SPLIT;
}

irc_server *check_split_server(char *server) {
  register irc_server *temp;
  if (!server || from_server < 0)
    return NULL;
...
int is_server_valid(char *name, int server) {
  if (server > -1 && server < number_of_servers &&
      ((server_list[server].snetwork &&
        my_stricmp(name, server_list[server].snetwork) == 0) ||
       (server_list[server].name &&
        my_stricmp(name, server_list[server].name) == 0)))
    return 1;
  return 0;
}
...
void parse_364(char *channel, char *args, char *subargs) {
  ...
  add_server(&server_list[from_server].tmplink, channel, args, atol(subargs),
             now);
}

void parse_365(char *channel __attribute__((unused)), char *args __attribute__((unused)), char *subargs __attribute__((unused))) {
  register irc_server *serv1;
  if (from_server < 0)
    return;
  for (serv1 = server_list[from_server].server_last; serv1;
       serv1 = serv1->next) {
    if (!find_server(server_list[from_server].tmplink, serv1->name)) {
      ...
      if (serv1->count)
        continue;
      serv1->time = now;
      if (do_hook(LLOOK_SPLIT_LIST, "%s %s %d %lu", serv1->name, serv1->link,
                  serv1->hopcount, serv1->time))
        put_it("%s", convert_output_format(
                         fget_string_var(FORMAT_NETSPLIT_FSET), "%l %s %s %d",
                         now - serv1->time, serv1->name, serv1->link,
                         serv1->hopcount));
      serv1->count++;
    } else {
      if (serv1->status & SPLIT) {
        serv1->status = ~SPLIT;
        if (do_hook(LLOOK_JOIN_LIST, "%s %s %d %lu", serv1->name, serv1->link,
                    serv1->hopcount, serv1->time))
          put_it("%s", convert_output_format(
                           fget_string_var(FORMAT_NETJOIN_FSET), "%l %s %s %d",
                           now - serv1->time, serv1->name, serv1->link,
                           serv1->hopcount));
        serv1->count = 0;
      }
  ...
  for (serv1 = server_list[from_server].tmplink; serv1; serv1 = serv1->next) {
    if (!find_server(server_list[from_server].server_last, serv1->name)) {
      if (first_time == 1) {
        if (do_hook(LLOOK_ADDED_LIST, "%s %s %d", serv1->name, serv1->link,
                    serv1->hopcount))
          put_it("%s", convert_output_format(
                           fget_string_var(FORMAT_NETADD_FSET), "%l %s %s %d",
                           now - serv1->time, serv1->name, serv1->link,
                           serv1->hopcount));
        serv1->count = 0;
      }
      add_server(&server_list[from_server].server_last, serv1->name,
                 serv1->link, serv1->hopcount, now);
    }
  }
  first_time = 1;
...
BUILT_IN_COMMAND(linklook) {
  ...
  while (serv) {
    if (serv->status & SPLIT) {
      if (!count)
        put_it("%s", convert_output_format(
                         fget_string_var(FORMAT_NETSPLIT_HEADER_FSET),
                         "%s %s %s %s", "time", "server", "uplink", "hops"));
      if (do_hook(LLOOK_SPLIT_LIST, "%s %s %d %lu", serv->name, serv->link,
                  serv->hopcount, serv->time))
        put_it("%s",
               convert_output_format(fget_string_var(FORMAT_NETSPLIT_FSET),
                                     "%l %s %s %d", now - serv->time,
                                     serv->name, serv->link, serv->hopcount));
      count++;
    }
    serv = serv->next;
  }
...
WhoEntry *who_queue_top(int server) { return server_list[server].who_queue; }

void set_who_queue_top(int server, WhoEntry *item) {
  server_list[server].who_queue = item;
}

IsonEntry *ison_queue_top(int server) { return server_list[server].ison_queue; }
...
Server *BX_get_server_list(void) { return server_list; }
...
void BX_send_msg_to_channels(int server, const char *msg) {
  ...
  for (chan = get_server_channels(server), count = 1; chan;
       chan = chan->next, count++) {
...
void BX_send_msg_to_nicks(ChannelList *chan, int server, char *msg) {
  ...
  for (n = next_nicklist(chan, NULL), count = 1; n;
       n = next_nicklist(chan, n), count++) {
    m_s3cat(&p, ",", n->nick);
    if (count > 3) {
...
int save_servers(FILE *fp) {
  ...
  for (i = 0; i < number_of_servers; i++) {
    ...
    fprintf(fp, "SERVER -ADD %s:%d:%s:%s\n", server_list[i].name,
            server_list[i].port,
            server_list[i].password ? server_list[i].password : empty_string,
            server_list[i].nickname ? server_list[i].nickname : empty_string);
  }
  return i;
}
