...
void funny_match(char *stuff) { malloc_strcpy(&match_str, stuff); }
...
void funny_print_widelist(void) {
  ...
  *buffer1 = '\0';
  for (i = 1; i < wl_elements; i++) {
    ...
    if (strlen(buffer1) + strlen(buffer2) + 5 > (size_t)cols) {
      if (do_hook(WIDELIST_LIST, "%s", buffer1))
        put_it("%s",
               convert_output_format(fget_string_var(FORMAT_WIDELIST_FSET),
                                     "%s %s", update_clock(GET_TIME), buffer1));
      *buffer1 = 0;
  ...
  if (*buffer1 && do_hook(WIDELIST_LIST, "%s", buffer1))
    put_it("%s",
           convert_output_format(fget_string_var(FORMAT_WIDELIST_FSET), "%s %s",
                                 update_clock(GET_TIME), buffer1));
...
void funny_list(char *from, char **ArgList) {
  ...
  if (funny_flags & FUNNY_WIDE) {
    ...
    wide_list[wl_elements] = (WideList *)new_malloc(sizeof(WideList));
  ...
  if (do_hook(current_numeric, "%s %s %s %s", from, channel, user_cnt, line) &&
    ...
    if (channel && user_cnt)
      put_it("%s", convert_output_format(fget_string_var(FORMAT_LIST_FSET),
  ...
  if (line && *line) {
    *buffer = 0;
    ...
    while ((t = next_arg(line, &line))) {
       ...
       * precede it. */
      nick = t + strspn(t, "@%+~-");
      nick_format = fget_string_var(isme(nick) ? FORMAT_NAMES_NICK_ME_FSET
                                               : FORMAT_NAMES_NICK_FSET);
      strlcpy(nick_buffer, convert_output_format(nick_format, "%s", nick),
              sizeof nick_buffer);
        ...
        if (special == '+')
          strlcat(buffer,
                  convert_output_format(
                      fget_string_var(FORMAT_NAMES_USER_VOICE_FSET), "%c %s",
                      special, nick_buffer),
                  sizeof buffer);
        else
          strlcat(buffer,
                  convert_output_format(
                      fget_string_var(FORMAT_NAMES_USER_CHANOP_FSET), "%c %s",
                      special, nick_buffer),
                  sizeof buffer);
      } else
        strlcat(buffer,
                convert_output_format(fget_string_var(FORMAT_NAMES_USER_FSET),
                                      ". %s", nick_buffer),
                sizeof buffer);
...
void funny_namreply(char *from, char **Args) {
  ...
  if (in_join_list(channel, from_server)) {
    ...
    if (do_hook(current_numeric, "%s %s %s %s", from, type, channel, line) &&
      ...
      put_it("%s", convert_output_format(fget_string_var(FORMAT_NAMES_FSET),
                                         "%s %s %d %d", update_clock(GET_TIME),
  ...
  if (type && channel) {
    ...
    if (do_hook(current_numeric, "%s %s %s %s", from, type, channel, line) &&
      ...
      if (fget_string_var(FORMAT_NAMES_FSET)) {
        put_it("%s",
               convert_output_format(fget_string_var(FORMAT_NAMES_FSET),
                                     "%s %s %d %d", update_clock(GET_TIME),
                                     channel, user_count, user_count));
        print_funny_names(line);
      } else {
        switch (*type) {
        case '=':
          if (last_width && (strlen(channel) > (size_t)last_width)) {
  ...
  if (in_join_list(channel, from_server)) {
    ...
    if (do_hook(current_numeric, "%s %s %s", from, channel, mode))
      put_it("%s", convert_output_format(
                       fget_string_var(FORMAT_MODE_CHANNEL_FSET),
