...
#ifdef GUI
char *mouse_actions[] = {"RCLICK",    "STATUSRCLICK",    "NICKLISTRCLICK",
                         "LCLICK",    "STATUSLCLICK",    "NICKLISTLCLICK",
                         "MCLICK",    "STATUSMCLICK",    "NICKLISTMCLICK",
                         "RDBLCLICK", "STATUSRDBLCLICK", "NICKLISTRDBLCLICK",
                         "LDBLCLICK", "STATUSLDBLCLICK", "NICKLISTLDBLCLICK",
                         "MDBLCLICK", "STATUSMDBLCLICK", "NICKLISTMDBLCLICK"};
#endif
...
static KeyMapNames key_names[] = {
    ...
    {"REFRESH_SCREEN", refresh_screen_wrapper},
    {"REFRESH_STATUS", BX_update_all_status_wrapper},
...
static void new_key(int meta, unsigned char chr, int type, int change,
                    char *stuff) {
  /*
...
#endif
      say("%s%s is bound to META%d_CHARACTER", meta_str, display_key(c),
          -(KEY(meta, c)->key_index));
    else
      say("%s%s is bound to %s %s", meta_str, display_key(c),
          key_names[KEY(meta, c)->key_index].name, SAFE(KEY(meta, c)->stuff));
  } else
 ...
 */
void save_bindings(FILE *fp, int do_all __attribute__((unused))) {
  int meta, j;
...
static int parse_key(const char *sequence, char *term) {
  ...
  for (mouse = 0; mouse < MAX_MOUSE; mouse++) {
    if (!my_strnicmp(sequence, mouse_actions[mouse],
                     strlen(mouse_actions[mouse]))) {
      *term = (char)mouse;
      return MAX_META;
    }
  }
   ...
   */
  while (copy[0] && copy[1]) {
    if (keys[meta] && KEY(meta, *copy) && KEY(meta, *copy)->key_index < 0) {
  ...
  *end-- = 0;
  ...
  if (x_debug & DEBUG_AUTOKEY) {
    yell("FIND_META_MAP says we can put [%c] in META [%d]", last_character,
         return_meta);
  }
...
static int lookup_function(const char *orig_name, int *lf_index) {
  ...
  *lf_index = -1;
  ...
  for (cnt = 0, i = 0; i < (int)NUMBER_OF_FUNCTIONS; i++) {
...
void disable_stop(void) { snew_key(0, 26, "SELF_INSERT"); }
