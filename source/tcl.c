...
extern void got_tcluserhost(UserhostItem *stuff, char *nick, char *userhost) {
  char *buffer = NULL;
  if (stuff && strcmp(stuff->host, "<UNKNOWN>") &&
      strcmp(stuff->user, "<UNKNOWN>")) {
    malloc_sprintf(&buffer, "%s %s %s", nick, stuff->user, stuff->host);
    Tcl_SetVar(tcl_interp, "userhost", buffer,
               TCL_APPEND_VALUE | TCL_LIST_ELEMENT);
    new_free(&buffer);
    return;
  }
  Tcl_SetVar(tcl_interp, "un-userhost", nick,
             TCL_APPEND_VALUE | TCL_LIST_ELEMENT);
  return;
}
...
int tcl_msg STDVAR {
  ...
  for (i = 2; i < argc; i++)
    m_s3cat(&buffer, space, argv[i]);
  send_text(argv[1], buffer,
            (!strcmp(argv[0], "notice") ? STXT_NOTICE : 0) | STXT_LOG);
  new_free(&buffer);
  return TCL_OK;
}
...
int tcl_setcomment STDVAR {
  ...
  for (tmp = next_userlist(NULL, &size, &tmp2); tmp;
       tmp = next_userlist(tmp, &size, &tmp2)) {
...
int tcl_pushmode STDVAR {
  ...
  if ((argc < 4) &&
      (!strchr(mode, 'o') || !strchr(mode, 'v') || !strchr(mode, 'b'))) {
    Tcl_AppendResult(irp, "modes b/v/o require an argument", NULL);
    return TCL_ERROR;
  }
  num_modes = (*mode == 'b') ? get_int_var(NUM_BANMODES_VAR)
...
int tcl_onchansplit STDVAR {
  ...
  for (tmp = next_userhost(&whowas_splitin_list, NULL); tmp;
       tmp = next_userhost(&whowas_splitin_list, tmp)) {
    if (!my_stricmp(tmp->nicklist->nick, argv[1]) &&
        !my_stricmp(tmp->channel, argv[2])) {
      found = 1;
      break;
    }
  }
...
int tcl_servers STDVAR {
  ...
  for (i = 0; i < server_list_size(); i++) {
    malloc_sprintf(&s, "%d %s %d %d", i, get_server_name(i), get_server_port(i),
...
void chan_struct(Tcl_Interp *irp, ChannelList *c) {
  ...
  malloc_sprintf(
      &s,
      "%s %d %lu %s %d %d %d %lu %lu %d %d %d %d %d %d %d %d %d %d %d %d %d %d "
      "%d %d %d %d %d %d %d %d %d %d %d %lu %d %lu %d %lu %d %lu %d %lu %d %d "
      "%d %d %d %d %d %lu %d %d %lu %d %d",
      c->channel, c->server, c->mode, c->s_mode, c->limit, (int)c->have_op,
      (int)c->voice, c->channel_create.tv_sec, c->join_time.tv_sec,
      c->stats_ops, c->stats_dops, c->stats_bans, c->stats_unbans,
      c->stats_sops, c->stats_sdops, c->stats_sbans, c->stats_sunbans,
      c->stats_topics, c->stats_kicks, c->stats_pubs, c->stats_parts,
      c->stats_signoffs, c->stats_joins, c->csets->set_ainv,
      c->csets->set_auto_rejoin, c->csets->set_deop_on_deopflood,
      c->csets->set_deop_on_kickflood, c->csets->set_deopflood,
      c->csets->set_deopflood_time, c->csets->set_hacking,
      c->csets->set_kick_on_deopflood, c->csets->set_kick_on_joinflood,
      c->csets->set_kick_on_kickflood, c->csets->set_kick_on_nickflood,
      c->csets->set_kick_on_pubflood, c->csets->set_kickflood,
      c->csets->set_kickflood_time, c->csets->set_nickflood,
      c->csets->set_nickflood_time, c->csets->set_joinflood,
      c->csets->set_joinflood_time, c->csets->set_pubflood,
      c->csets->set_pubflood_time, c->csets->set_pubflood_ignore,
      c->csets->set_userlist, c->csets->set_shitlist, c->csets->set_lamelist,
      c->csets->set_kick_if_banned, c->totalnicks, c->maxnicks, c->maxnickstime,
      c->totalbans, c->maxbans, c->maxbanstime, c->csets->set_aop,
      c->csets->bitch_mode);
  Tcl_AppendElement(irp, s);
  new_free(&s);
}
...
int tcl_channel_modify(Tcl_Interp *irp, char *channel, ChannelList *chan,
                       int argc, char **argv) {
...
int tcl_finduser STDVAR {
  ...
  if ((nick = lookup_userlevelc("*", host, "*", NULL)))
    malloc_sprintf(&s, "%d %s%s%s", nick->flags, nick->channels,
                   nick->password ? " " : "",
...
int tcl_chanlist STDVAR {
  ...
  if (isdigit(*argv[2])) {
    ...
    for (nick = next_nicklist(chan, NULL); nick;
         nick = next_nicklist(chan, nick)) {
      if ((level <= 0) ||
          (level && nick->userlist && nick->userlist->flags >= level))
        Tcl_AppendElement(irp, nick->nick);
    }
...
int check_tcl_msg(char *cmd, char *nick, char *uhost, char *hand, char *args) {
  ...
  if (x >= 10) {
    if (internal_debug & DEBUG_TCL)
      debugyell("Tcl func [%s] with [%s %s %s %s]", C_msg[x - 10].name, hand,
                nick, uhost, args);
...
int check_tcl_dcc(char *cmd, char *nick, char *host, int idx) {
  ...
  Tcl_SetVar(tcl_interp, "_a", args, TCL_GLOBAL_ONLY);
  x = check_tcl_bind(&H_dcc, c, atr, " $_n $_i $_a",
                     MATCH_PARTIAL | BIND_HAS_BUILTINS | BIND_USE_ATTR, C_dcc);
...
int check_tcl_ctcp(char *nick, char *uhost, char *hand, char *dest,
...
int check_on_hook(int which, char *buffer) {
  ...
  return check_tcl_bind(&H_hook, name, -1, " $_a $_aa",
...
void check_tcl_sign(char *nick, char *uhost, char *hand, char *chname,
                    char *reason) {
  int atr = 0;
  char args[BIG_BUFFER_SIZE + 1];
  UserList *n;
...
void check_tcl_topc(char *nick, char *uhost, char *hand, char *chname,
                    char *topic) {
  int atr = 0;
  char args[BIG_BUFFER_SIZE + 1];
  UserList *n;
...
void check_tcl_nick(char *nick, char *uhost, char *hand, char *chname,
                    char *newnick) {
  int atr = 0;
...
void check_tcl_kick(char *nick, char *uhost, char *hand, char *chname,
                    char *dest, char *reason) {
  char args[BIG_BUFFER_SIZE];
  int atr = 0;
  UserList *n;
...
int check_tcl_raw(char *raw, char *comm) {
  ...
  if (raw && *raw) {
    ...
    ret = check_tcl_bind(&H_raw, comm, -1, " $_a",
                         MATCH_MASK | BIND_STACKABLE | BIND_WANTRET, NULL);
  }
...
char *check_tcl_alias(char *command, char *args) {
  ...
  if (check_tcl_bind(&H_functions, command, -1, " $_a",
                     MATCH_MASK | BIND_STACKABLE, NULL))
...
int check_tcl_input(char *raw) {
  ...
  if (raw && *raw) {
    Tcl_SetVar(tcl_interp, "_a", raw, TCL_GLOBAL_ONLY);
    ret = check_tcl_bind(&H_input, raw, -1, " $_a", MATCH_MASK | BIND_STACKABLE,
                         NULL);
  }
  return (ret == BIND_EXEC_LOG) ? 1 : 0;
}
...
int check_help_bind(char *command) {
  ...
  if (command && *command) {
    ...
    x = check_tcl_bind(&H_help, comm, -1, NULL, MATCH_EXACT | BIND_HAS_BUILTINS,
                       NULL);
    new_free(&comm);
  }
  return (x ? 0 : 1);
...
void check_tcl_mode(char *nick, char *uhost, char *hand, char *chname,
                    char *mode) {
  char args[BIG_BUFFER_SIZE + 1];
  UserList *n;
...
int check_tcl_ctcr(char *nick, char *uhost, char *hand, char *dest,
                   char *keyword, char *args) {
  int atr = 0;
  UserList *n;
...
int get_bind_type(char *name) {
  ...
  if (!my_stricmp(name, "dcc"))
    tp = CMD_DCC;
  else if (!my_stricmp(name, "msg"))
    tp = CMD_MSG;
  else if (!my_stricmp(name, "fil"))
    tp = CMD_FIL;
  else if (!my_stricmp(name, "pub"))
    tp = CMD_PUB;
  else if (!my_stricmp(name, "msgm"))
    tp = CMD_MSGM;
  else if (!my_stricmp(name, "pubm"))
    tp = CMD_PUBM;
  else if (!my_stricmp(name, "join"))
    tp = CMD_JOIN;
  else if (!my_stricmp(name, "part"))
    tp = CMD_PART;
  else if (!my_stricmp(name, "sign"))
    tp = CMD_SIGN;
  else if (!my_stricmp(name, "kick"))
    tp = CMD_KICK;
  else if (!my_stricmp(name, "topc"))
    tp = CMD_TOPC;
  else if (!my_stricmp(name, "mode"))
    tp = CMD_MODE;
  else if (!my_stricmp(name, "ctcp"))
    tp = CMD_CTCP;
  else if (!my_stricmp(name, "ctcr"))
    tp = CMD_CTCR;
  else if (!my_stricmp(name, "nick"))
    tp = CMD_NICK;
  else if (!my_stricmp(name, "bot"))
    tp = CMD_BOT;
  else if (!my_stricmp(name, "chon"))
    tp = CMD_CHON;
  else if (!my_stricmp(name, "chof"))
    tp = CMD_CHOF;
  else if (!my_stricmp(name, "sent"))
    tp = CMD_SENT;
  else if (!my_stricmp(name, "rcvd"))
    tp = CMD_RCVD;
  else if (!my_stricmp(name, "chat"))
    tp = CMD_CHAT;
  else if (!my_stricmp(name, "link"))
    tp = CMD_LINK;
  else if (!my_stricmp(name, "disc"))
    tp = CMD_DISC;
  else if (!my_stricmp(name, "rejn"))
    tp = CMD_REJN;
  else if (!my_stricmp(name, "splt"))
    tp = CMD_SPLT;
  else if (!my_stricmp(name, "filt"))
    tp = CMD_FILT;
  else if (!my_stricmp(name, "raw"))
    tp = CMD_RAW;
  else if (!my_stricmp(name, "cmd"))
    tp = CMD_CMD;
  else if (!my_stricmp(name, "input"))
    tp = CMD_INPUT;
  else if (!my_stricmp(name, "alias"))
    tp = CMD_ALIAS;
  else if (!my_stricmp(name, "help"))
    tp = CMD_HELP;
  else if (!my_stricmp(name, "func"))
    tp = CMD_FUNCTION;
  else if (!my_stricmp(name, "hook"))
    tp = CMD_HOOK;
  return tp;
}
...
int tcl_bind STDVAR {
  ...
  if (tp < 0) {
    Tcl_AppendResult(
        irp, "bad type, should be one of: dcc, msg, fil, pub, ",
      ...
      for (he = Tcl_FirstHashEntry(ht, &srch); (he != NULL);
        ...
        while (tt != NULL) {
          ...
          if ((proc[0] != '*') || (strcmp(s, proc + 1) != 0))
            put_it("  %-4s %-16s %-20s %s", typ,
                   convert_flags_to_str(tt->flags_needed), s, tt->func_name);
          tt = tt->next;
        }
      }
      if (!fnd) {
...
BUILT_IN_COMMAND(tcl_version) {
  put_it("%s",
         convert_output_format(
             "BitchX Tcl Interpreter %GVersion $1%n (c)1997 Colten Edwards",
             "%s", tcl_versionstr));
}
