...
static DNS_QUEUE *build_dns_entry(char *, void (*)(DNS_QUEUE *), char *,
                                  void *);
 ...
 */
void add_to_dns_queue(char *userhost, void (*callback)(DNS_QUEUE *), char *cmd,
  ...
  if (userhost && *userhost) {
    ...
    if (split && *split) {
      ...
      if (urgency == DNS_URGENT)
        dns_enqueue_urgent(&PendingQueueHead, &PendingQueueTail, tmp);
      else
        dns_enqueue(&PendingQueueHead, &PendingQueueTail, tmp);
...
void check_dns_queue() {
  ...
  while (pthread_mutex_trylock(&finished_queue_mutex) == 0) {
    ...
    if (dns) {
#ifdef MY_DEBUG
      fprintf(stderr, "DNS IN: %s: %s <-> %s", dns->in ? dns->in : "<NULL>",
              dns->in ? dns->in : "<NULL>", dns->out ? dns->out : "<NULL>");
#endif
      if (dns->alias) {
        char buffer[BIG_BUFFER_SIZE + 1];
        snprintf(buffer, BIG_BUFFER_SIZE, "%s %s ",
                 dns->in ? dns->in : "<NULL>", dns->out ? dns->out : "<NULL>");
        parse_line("NSLOOKUP", dns->alias, buffer, 0, 0, 1);
      } else if (dns->callback)
        dns->callback(dns);
      else
        fprintf(stderr, "%s:%d error: No callback!", __FILE__, __LINE__);
      free_dns_entry(dns);
    } else
      return;
  }
...
static void kill_dns_thread(int notused) { /* Empty */ }
...
my_hostent *duphostent(struct hostent *orig) {
  ...
  for (z = 0; z < MAXADDRS && orig->h_addr_list[z]; z++)
    memcpy(&tmp->h_addr_list[z], orig->h_addr_list[z],
           sizeof(*orig->h_addr_list));
...
DNS_QUEUE *dns_dequeue(DNS_QUEUE **headp, DNS_QUEUE **tailp) {
  DNS_QUEUE *tmp = NULL;
...
DNS_QUEUE *dns_enqueue(DNS_QUEUE **headp, DNS_QUEUE **tailp, DNS_QUEUE *tmp) {
  Q_NEXT(tmp) = NULL;
  if (*headp == NULL)
    *headp = *tailp = tmp;
  else {
 ...
 */
DNS_QUEUE *dns_enqueue_urgent(DNS_QUEUE **headp, DNS_QUEUE **tailp,
                              DNS_QUEUE *tmp) {
  Q_NEXT(tmp) = *headp;
  *headp = tmp;
  if (*tailp == NULL)
...
static DNS_QUEUE *build_dns_entry(char *text, void (*callback)(DNS_QUEUE *),
                                  char *cmd, void *data) {
