...
#ifndef HAVE_GETPASS
char *getpass(char *);
char *get_string_var(int var) { return NULL; }
#endif

#ifdef WINNT
void refresh_screen(int i, char *u) { return; }
#endif
  ...
  while ((dir = readdir(dptr))) {
    ...
    if (unl) {
      ...
      fprintf(stderr, "%30s %s\r\n", dir->d_name,
              ((st.st_mode & 0700) == 0600) ? "detached" : "Attached or dead");
  }
  if (!count)
...
char *find_detach_socket(const char *path, char *name) {
  ...
  while ((dir = readdir(dptr))) {
    ...
    if (((st.st_mode & 0700) == 0600) && (st.st_uid == getuid()) &&
        !S_ISDIR(st.st_mode)) {
      count++;
      free(ret);
      ret = path_name;
...
SIGNAL_HANDLER(handle_ctrlc) { ctrl_c++; }
