...
BUILT_IN_COMMAND(csay) {
    ...
    if (s->flags & DCC_ACTIVE) {
      ...
      if ((s->flags & DCC_TYPES) == DCC_BOTMODE) {
        n->bytes_sent += dcc_printf(i, "chan %s %d %s@%s %s\n",
                                    get_server_nickname(from_server), 0,
                                    get_server_nickname(from_server),
                                    get_server_nickname(from_server), args);
        found++;
      } else if (((s->flags & DCC_TYPES) == DCC_CHAT) &&
                 (s->flags & DCC_BOTCHAT)) {
        n->bytes_sent +=
            dcc_printf(i, "[%s@%s] %s\n", get_server_nickname(from_server),
                       get_server_nickname(from_server), args);
  ...
  if (!found)
    ...
    put_it("%s", convert_output_format("%K(%R$1%K(%rxlink%K))%n $2-",
  ...
  for (i = 0; i < get_max_fd() + 1; i++) {
    ...
    if (s->flags & DCC_ACTIVE) {
      if ((s->flags & DCC_TYPES) == DCC_BOTMODE) {
        if (whom)
          dcc_printf(
              i, "whom %d:%s@%s %s %d\n", -1, get_server_nickname(from_server),
              get_server_nickname(from_server), args ? args : empty_string, 0);
        else
...
BUILT_IN_COMMAND(cmsg) {
  ...
  for (i = 0; i < get_max_fd() + 1; i++) {
    if (!check_dcc_socket(i))
      continue;
    s = get_socket(i);
    if ((s->flags & DCC_ACTIVE) && ((s->flags & DCC_TYPES) == DCC_CHAT) &&
        (s->flags & DCC_BOTCHAT))
...
int handle_tcl_chan(int idx, char *user, char *host, char *text) {
  ...
  put_it("%s", convert_output_format("%K(%R$1%K(%rxlink%K))%n $2-", "%s %s %s",
                                     update_clock(GET_TIME), user, text));
  return 0;
}
...
int tand_chan(int idx, char *par) {
  ...
  char *uhost;
  /*
           tand_chan (idx=7, par=0x8140009 "panasync 0 sabina@botnick
     testing\r")
  */
  ...
  if ((uhost = strchr(who, '@')))
    *uhost++ = '\0';
  put_it("%s", convert_output_format("%K[%RxLink%Y$1%K(%r$2%K)] %n$3-",
                                     "%s %s %s %s", update_clock(GET_TIME), who,
                                     uhost ? uhost : "u@h", par));
  return 0;
}
...
int tand_zapf(int idx, char *par) {
  ...
  to = next_arg(par, &par);
  if (!(s = find_dcc(from, "chat", NULL, DCC_BOTMODE, 0, 1, -1)) ||
      s->is_read != idx)
    return 0;
#ifdef WANT_TCL
...
int handle_dcc_bot(int idx, char *param) {
  ...
  while (C_tandbot[i].access != -1) {
    if (!my_stricmp(code, C_tandbot[i].name)) {
      ((int (*)(int, char *))C_tandbot[i].func)(idx, param);
      found = 1;
      break;
    }
    i++;
...
int tand_join(int idx, char *args) {
  ...
  put_it("%s", convert_output_format("%K[%RxLink%Y$1%K(%r$2%K)] %n$3-",
                                     ...
                                     nick, bot, "entered the TwilightZone"));
  return 0;
}
...
int tand_part(int idx, char *args) {
  ...
  put_it("%s", convert_output_format("%K[%RxLink%Y$1%K(%r$2%K)] %n$3-",
                                     "%s %s %s %s", update_clock(GET_TIME),
                                     nick, bot, "left the TwilightZone"));
  return 0;
}
    ...
    if (((s->flags & DCC_TYPES) == DCC_CHAT) && (s->flags & DCC_BOTCHAT)) {
      ...
      if (!found++)
        dcc_printf(idx, "priv %s %s %-10s  %-10s  %-25s\n",
                   get_server_nickname(from_server), from, "Who", "Bot",
                   "Host");
      dcc_printf(idx, "priv %s %s %-10s  %-10s  %-25s\n",
                 get_server_nickname(from_server), from, s->server,
                 get_server_nickname(from_server), n->userhost);
    }
  }
  return 0;
}
...
int tand_who(int idx, char *args) {
  ...
  if (!(p = strchr(from, '@'))) {
    strmopencat(buffer, IRCD_BUFFER_SIZE, from, "@",
                get_server_nickname(from_server), NULL);
    from = buffer;
  }
  ...
  if (arg && *arg) {
    if (!my_stricmp(arg, get_server_nickname(from_server)))
      ...
      for (i = 0; i < get_max_fd() + 1; i++) {
        ...
        s = get_socket(i);
        if (!(s->flags & DCC_ACTIVE) ||
            !((s->flags & DCC_TYPES) == DCC_BOTMODE))
          continue;
        if (C->info && !my_stricmp(s->server, arg)) {
          dcc_printf(i, "who %d:%s@%s %s %d\n", idx, C->server,
                     get_server_nickname(from_server), arg, 0);
          found = 1;
        }
      }
      if (!found)
...
int tand_boot(int idx, char *args) {
  ...
  if ((s = find_dcc(nick, "chat", NULL, DCC_CHAT, 0, 1, -1))) {
    chanout_but(s->is_read,
                "%s has booted %s out of the TwilightZone for (%s)\n", from,
                s->server, reason);
  ...
  if (nick) {
    ...
    if (s)
      dcc_printf(s->is_read, "[%s@%s] %s\n", s1->server,
                 user ? user : get_server_nickname(from_server), par);
    else
  ...
  if (my_stricmp(nick, get_server_nickname(from_server))) {
    ...
    if (!s)
      ...
      chanout_but(s->is_read, "%s booting %s out of the TwilightZone (%s)\n",
                  s1 ? s1->server : get_server_nickname(from_server), s->server,
                  reason);
      erase_dcc_info(s->is_read, 0, NULL);
    }
  } else
...
int cmd_tcl(int idx __attribute__((unused)),
            char *par __attribute__((unused))) {
#ifdef WANT_TCL
...
int cmd_ops(int idx, char *par) {
  ...
  if (n && n->ul) {
    ...
    if (!nick || !ch || !ch->have_op) {
      dcc_printf(idx,
                 "No Nick specified, not on that channel or not channel op\n");
      return TCL_ERROR;
    }
...
int cmd_chat(int idx, char *par) {
  ...
  if (check_dcc_socket(idx) && !(s->flags & DCC_BOTCHAT)) {
    ...
    s->flags |= DCC_BOTCHAT;
    snprintf(buffer, IRCD_BUFFER_SIZE, "%s %s %s",
             get_server_nickname(from_server), s->server, n->userhost);
...
int cmd_quit(int idx __attribute__((unused)),
             char *par __attribute__((unused))) {
  SocketList *s;
  char buffer[IRCD_BUFFER_SIZE + 1];
  s = get_socket(idx);
...
int cmd_adduser(int idx __attribute__((unused)),
                char *par __attribute__((unused))) {
  /*	add_user();*/
  return TCL_OK;
}

int cmd_whoami(int idx __attribute__((unused)),
               char *par __attribute__((unused))) {
  char *host;
  unsigned long atr = 0;
  UserList *n;
...
int cmd_help(int idx, char *par) {
  ...
  if (!command) {
    ...
    for (i = 1, j = 1; C_dcc[i - 1].name; i++) {
      if (C_dcc[i - 1].access != 0)
        if (!n->ul || !(n->ul->flags & C_dcc[i - 1].access))
          continue;
#ifdef WANT_USERLIST
      dcc_printf(idx, "%6s %-10s", convert_flags_to_str(C_dcc[i - 1].access),
                 C_dcc[i - 1].name);
#else
    ...
    if (C_dcc[i].name)
#ifdef WANT_USERLIST
      dcc_printf(idx, "%6s %-10s %s\n", convert_flags_to_str(C_dcc[i].access),
