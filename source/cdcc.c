...
BUILT_IN_COMMAND(cdcc) {
  ...
  if (!cmd) {
    l_list(NULL, NULL);
    put_it("%s: CDCC is [\002%s\002]. Use \002/cdcc help\002 to get help with "
           "cdcc",
           cparse(get_string_var(CDCC_PROMPT_VAR)),
           on_off(get_int_var(CDCC_VAR)));
    return;
  }
  if (!my_stricmp(cmd, "ON") || !my_stricmp(cmd, "OFF")) {
    set_int_var(CDCC_VAR, !my_stricmp(cmd, "ON") ? 1 : 0);
    put_it("%s: offers \002%s\002", cparse(get_string_var(CDCC_PROMPT_VAR)),
           cmd);
    return;
  }
  ...
  put_it("%s: unknown command \002%s\002",
         cparse(get_string_var(CDCC_PROMPT_VAR)), cmd);
  return;
}
...
static int l_help(char *cmd, char *args __attribute__((unused))) {
  ...
  if (!cmd) {
    ...
    *buffer = 0;
    ...
    if (!done)
      put_it("%s",
             convert_output_format("$G CDCC - No such command", NULL, NULL));
  }
  return 0;
}
...
#ifdef WANT_USERLIST
    if (!(tmp = lookup_userlevelc("*", FromUserHost, "*", NULL)) ||
        !(tmp->flags & ADD_DCC))
      return args;
#else
    return args;
...
got_good_pass:
  ...
  queue_send_to_server(from_server, "NOTICE %s :try /ctcp %s cdcc help", from,
  ...
  if (rest && *rest) {
    ...
    if (ptr)
      queue_send_to_server(
          ...
          args, ptr->numfiles, plural(ptr->numfiles), ptr->gets, ptr->size,
          ptr->minspeed, (long)ptr->timeadded);
    else
      queue_send_to_server(from_server, "NOTICE %s :Invalid info request",
                           args);
  }
  return 0;
}
    ...
    *buffer = 0;
    ...
    if (num) {
      ...
      queue_send_to_server(from_server,
                           "NOTICE %s :You have %d packs queued at %s", args,
                           num, buffer);
    } else
      queue_send_to_server(from_server, "NOTICE %s :You have no packs queued.",
                           args);
  }
  return 0;
}
...
static int do_local_send(char *command, char *args, char *rest) {
  ...
  while (1) {
    ...
    if (isdigit((unsigned char)*temp) ||
        (*(temp + 1) && isdigit((unsigned char)*(temp + 1)))) {
      if (*temp == '#')
        temp++;
      for (ptr = offerlist; ptr; ptr = ptr->next)
    ...
    if (ptr) {
      if (maxdcc && get_active_count() >= maxdcc) {
        if (maxqueue && (numqueue >= maxqueue)) {
          put_it("%s: all dcc and queue slots full",
                 cparse(get_string_var(CDCC_PROMPT_VAR)));
          if (queued_files)
            put_it("%s: Queued %d files",
                   cparse(get_string_var(CDCC_PROMPT_VAR)), queued_files);
          return count + queued_files;
        }
        queued_files += add_to_queue(args, command, ptr);
        do_hook(CDCC_SEND_NICK_LIST, "%s %s %s %d %d %d %s %s", args, "unknown",
                command, ptr->num, ptr->numfiles, ptr->gets, ptr->file,
                ptr->desc);

        continue;
      }

      put_it("%s: %s %s%s%s pack #\002%d\002 (\002%d\002 file%s)",
             cparse(get_string_var(CDCC_PROMPT_VAR)),
             !my_stricmp(command, "SEND") || !my_stricmp(command, "TSEND")
                 ? "sending"
                 : "resending",
      ...
      for (p = new_next_arg(file, &file); p && *p;
           p = new_next_arg(file, &file)) {
      ...
      if (offerlist && (isdigit((unsigned char)*temp) ||
                        (*(temp + 1) && (isdigit((unsigned char)*(temp + 1))))))
        put_it("%s: No such pack number",
               cparse(get_string_var(CDCC_PROMPT_VAR)));
      else {
  ...
  if (queued_files)
    put_it("%s: Queued %d files", cparse(get_string_var(CDCC_PROMPT_VAR)),
           queued_files);
  new_free(&dccinfo);
  new_free(&q);
  return count + queued_files;
...
static int do_dcc_sends(char *command, char *from, char *args) {
  ...
  while (1) {
    ...
    if (ptr) {
      if (ptr->password && (!password || strcmp(ptr->password, password))) {
        put_it("%s: Attempted get of secure pack %d from %s failed. [%s]",
               cparse(get_string_var(CDCC_PROMPT_VAR)), ptr->num, from,
               !password ? "No Password" : "Invalid Password");
        queue_send_to_server(
            from_server,
            "NOTICE %s :\002CDCC\002: Failed attempt to get secure pack %d",
            from, ptr->num);
        continue;
      }
      if (maxdcc && ((maxdcc - get_active_count()) < ptr->numfiles ||
                     get_active_count() >= maxdcc)) {
        if (maxqueue && (numqueue >= maxqueue)) {
          if (queued_files) {
            put_it("%s: Queued %d files for %s",
                   cparse(get_string_var(CDCC_PROMPT_VAR)), queued_files, from);
            queue_send_to_server(
                from_server,
                "NOTICE %s :\002CDCC\002: all slots full... Some requests "
                "ignored. Added to queue for %d requests",
                from, queued_files);
          } else
            queue_send_to_server(from_server,
                                 "NOTICE %s :\002CDCC\002: all dcc and queue "
                                 "slots full... Try again later",
                                 from);
        ...
        count++;
        do_hook(CDCC_SEND_NICK_LIST, "%s %s %s %d %d %d %s %s", from,
                FromUserHost, command, ptr->num, ptr->numfiles, ptr->gets,
                ptr->file, ptr->desc);
        continue;
      }
      ...
      put_it("%s: %s %s%s%s pack #\002%d\002 (\002%d\002 file%s)",
             ...
             !my_stricmp(command, "SEND") || !my_stricmp(command, "TSEND")
                 ? "sending"
             : !my_stricmp(command, "RESUME") ? "resuming"
                                              : "resending",
      ...
      queue_send_to_server(
          from_server, "NOTICE %s :\002CDCC\002: invalid pack number", from);
    count++;
    new_free(&q);
    file = NULL;
  }
  if (queued_files) {
    put_it("%s: Queued %d files for %s",
           cparse(get_string_var(CDCC_PROMPT_VAR)), queued_files, from);
    queue_send_to_server(from_server,
                         "NOTICE %s :\002CDCC\002: all slots full... Added to "
                         "the queue for %d requests",
                         from, queued_files);
  }
  new_free(&dccinfo);
  new_free(&q);
...
static int r_list(char *from, char *args __attribute__((unused))) {
  ...
  for (ptr = offerlist; ptr; ptr = ptr->next) {
    if (!once &&
        do_hook(CDCC_PREPACK_LIST, "%s %s %s %u %d %d %d %d %s %s %s %s %lu %s",
                "NOTICE", from, get_server_nickname(from_server), cdcc_numpacks,
      ...
      queue_send_to_server(
          from_server,
          "NOTICE %s :Files Offered: /ctcp %s CDCC send #N for pack N", from,
          get_server_nickname(from_server));
      if (get_int_var(DCC_SEND_LIMIT_VAR))
        queue_send_to_server(
            from_server, "NOTICE %s :    [%u pack%s %d/%d slots open]", from,
            cdcc_numpacks, plural(cdcc_numpacks),
            get_int_var(DCC_SEND_LIMIT_VAR) - get_active_count(),
            get_int_var(DCC_SEND_LIMIT_VAR));
      else
        queue_send_to_server(from_server, "NOTICE %s :    [%u pack%s]", from,
                             cdcc_numpacks, plural(cdcc_numpacks));
    }
    if (ptr->size / 1024 > 999)
      snprintf(size, sizeof(size), "\002%4.1f\002mb",
               (((double)ptr->size) / 1024) / 1024);
    else
      snprintf(size, sizeof(size), "\002%4.1f\002kb",
               (((double)ptr->size) / 1024));

    if (do_hook(CDCC_PACK_LIST, "%s %s %d %d %lu %d %s", "NOTICE", from,
                ptr->num, ptr->numfiles, ptr->size, ptr->gets, ptr->desc)) {
      queue_send_to_server(
          from_server,
          "NOTICE %s :#%d \037(\037%10s\037:\037\002%4d\002 get%s\037)\037 %s",
          from, ptr->num, size, ptr->gets, plural(ptr->gets),
          ptr->desc ? ptr->desc : "no description");
    }
    if (ptr->notes &&
        do_hook(CDCC_NOTE_LIST, "%s %s %s", "NOTICE", from, ptr->notes))
...
static int r_help(char *from, char *args) {
  ...
  if (args && *args) {
    ...
    queue_send_to_server(from_server, "NOTICE %s :\002CDCC\002: %s", from,
                         buffer);
  }
#endif
  return 0;
...
static int l_doffer(char *args, char *rest) {
  if (!cdcc_numpacks) {
    put_it("%s: you have no packs offered",
           cparse(get_string_var(CDCC_PROMPT_VAR)));
    return 0;
  }
  if (args && *args) {
    ...
    l_list(NULL, NULL);
    add_wait_prompt("Remove pack [* for all packs]: ", del_pack, empty_string,
                    WAIT_PROMPT_LINE, 1);
  }
  return 0;
}
  ...
  if (args) {
    ...
    for (ptr = offerlist; ptr; ptr = ptr->next) {
      if (it == ptr->num) {
        ...
        while (temp && *temp) {
          ...
          put_it("#%-2d %-2d  %-2d   %s", ptr->num, ptr->gets, i++, p);
        }
    ...
    put_it("%s",
           convert_output_format(
               "#   files    size     gets minspeed  description", NULL, NULL));
    for (ptr = offerlist; ptr; ptr = ptr->next) {
      sprintf(temp, "%4.1f", _GMKv(ptr->size));
      /* buggy SUNOS doesn't like this next line at all. Why?
                              sprintf(temp2, "%4.1f",
         (double)(ptr->minspeed));*/
      put_it("%-2d    \002%3d\002  %6s%-4s \002%4d\002     0.0  %s", ptr->num,
             ptr->numfiles, temp, _GMKs(ptr->size), /*temp2*/ ptr->gets,
             ptr->desc);
    }
  }
  return 0;
...
static int l_offer(char *args, char *rest) {
  ...
  if (args && *args) {
    ...
    malloc_sprintf(&tmp, "Add file(s) to pack%s #%u : ", plural(cdcc_numpacks),
                   cdcc_numpacks + 1);
...
int l_plist(char *args, char *rest __attribute__((unused))) {
  const unsigned stxt_flags =
      (do_notice_list ? STXT_NOTICE : 0) | (do_cdcc_echo ? 0 : STXT_QUIET);
  ...
  if (!get_current_channel_by_refnum(0) || !cdcc_numpacks ||
      (args && *args && !is_channel(args))) {
    put_it("%s: you %s", cparse(get_string_var(CDCC_PROMPT_VAR)),
           cdcc_numpacks ? "are not on a channel!" : "have no packs offered!");
    return 0;
  }
  ...
  snprintf(bytes_in, sizeof(bytes_in), "%1.3g", dcc_bytes_in);
  snprintf(speed_out, sizeof(speed_out), "%1.3g", cdcc_minspeed);
  if (do_hook(CDCC_PREPACK_LIST, "%s %s %s %u %d %d %d %d %s %s %s %s %lu %s",
              ...
              bytes_in, total_size_of_packs, speed_out)) {
    char *msg1 = m_sprintf(
        "\037[\037cdcc\037]\037 \002%u\002 file%s offered\037-\037 /ctcp "
        "\002%s\002 cdcc send #x for pack #x",
        cdcc_numpacks, plural(cdcc_numpacks), get_server_nickname(from_server));
    char *msg2 =
        m_sprintf("\037[\037cdcc\037]\037 dcc block size\037:\037 \002%d\002, "
                  "slots open\037:\037 \002%2d\002/\002%2d\002, dcc "
                  "queue\037:\037 \002%2d\002/\002%2d\002",
                  (blocksize) ? blocksize : 1024, maxdccs - get_active_count(),
                  maxdccs, maxqueue - numqueue, maxqueue);
    if (get_int_var(QUEUE_SENDS_VAR)) {
      queue_send_to_server(from_server, "%s %s :%s", type_msg, chan, msg1);
      queue_send_to_server(from_server, "%s %s :%s", type_msg, chan, msg2);
    } else {
  ...
  for (ptr = offerlist; ptr; ptr = ptr->next) {
    if (ptr->size / 1024 > 999)
      snprintf(size, sizeof(size), "\002%3.2f\002mb",
               (float)(ptr->size / 1024) / 1024);
    else
      snprintf(size, sizeof(size), "\002%3.2f\002kb", (float)ptr->size / 1024);

    if (do_hook(CDCC_PACK_LIST, "%s %s %d %d %lu %d %s", type_msg, chan,
                ptr->num, ptr->numfiles, ptr->size, ptr->gets, ptr->desc)) {
      char *msg =
          m_sprintf("\037%%\037 #%-2d \037(\037%10s\037:\037\002%4d\002 "
                    "get%s\037)\037 %s",
                    ptr->num, size, ptr->gets, plural(ptr->gets),
                    ptr->desc ? ptr->desc : "no description");
      if (get_int_var(QUEUE_SENDS_VAR)) {
        queue_send_to_server(from_server, "%s %s :%s", type_msg, chan, msg);
      } else {
    ...
    if (ptr->notes &&
        do_hook(CDCC_NOTE_LIST, "%s %s %s", type_msg, chan, ptr->notes)) {
...
static int l_notice(char *args, char *rest __attribute__((unused))) {
  ...
  if (do_hook(CDCC_PREPACK_LIST, "%s %s %s %u %d %d %d %d %s %s %s %s %lu %s",
              ...
              bytes_in, total_size_of_packs, speed_out)) {
    char *msg = m_sprintf(
        "\037[\037cdcc\037]\037 \002%u\002 file%s offered\037-\037 "
        "\037\"\037/ctcp \002%s\002 cdcc list\037\"\037 for pack list",
        cdcc_numpacks, plural(cdcc_numpacks), get_server_nickname(from_server));
    if (get_int_var(QUEUE_SENDS_VAR))
...
static int l_queue(char *args, char *rest) {
  ...
  if (!queuelist) {
    put_it("%s: there are no queue entries",
           cparse(get_string_var(CDCC_PROMPT_VAR)));
    return 0;
  }
  ...
  if ((command == NULL) || !my_stricmp(command, "LIST")) {
    for (ptr = queuelist; ptr; ptr = ptr->next, num++) {
      if (command && rest && *rest && !wild_match(rest, ptr->nick))
        continue;
      if (command) {
        if (do_hook(CDCC_QUEUE_LIST, "%s %s %d %d %s", ptr->nick,
                    my_ctime(ptr->time), ptr->num, ptr->numfiles, ptr->desc))
          put_it("#\002%2d\002 nick: \037%9s\037 (\002%d\002 file%s)", num,
                 ptr->nick, ptr->numfiles, plural(ptr->numfiles));
      } else
        count++;
    }
    if (count && !command)
      put_it("%s: %d entries in the queue",
             cparse(get_string_var(CDCC_PROMPT_VAR)), count);
    return 0;
    ...
    put_it("%s: deleted %d of %d entries from the queue",
           cparse(get_string_var(CDCC_PROMPT_VAR)), success, count);

  } else
    put_it("%s: /Cdcc queue remove #|nick /Cdcc queue list [nick]",
           cparse(get_string_var(CDCC_PROMPT_VAR)));
  return 0;
}
...
static int l_save(char *args, char *rest __attribute__((unused))) {
...
#endif
  {
    put_it("%s: couldn't open \"%s\"", cparse(get_string_var(CDCC_PROMPT_VAR)),
  ...
  for (ptr = offerlist; ptr; ptr = ptr->next) {
    ...
    fprintf(file, "%d %lu 0.00 %d %lu %s\n", ptr->gets, ptr->size, ptr->server,
            (unsigned long)ptr->timeadded,
            ptr->password ? ptr->password : empty_string);
    count++;
  }

  fclose(file);
  put_it("%s: \002%d\002 pack%s saved to %s",
         cparse(get_string_var(CDCC_PROMPT_VAR)), count, plural(count), name);
  new_free(&name);
...
static int l_load(char *args, char *rest __attribute__((unused))) {
  ...
  while (fgets(buffer, BIG_BUFFER_SIZE, file)) {
    ...
    if (!temp || !isdigit((unsigned char)*temp)) {
      put_it("%s: not a cdcc pack aborting",
             cparse(get_string_var(CDCC_PROMPT_VAR)));
      new_free(&ptr->file);
      new_free((char **)&ptr);
      fclose(file);
  ...
  fclose(file);
  put_it("%s: \002%d\002 pack%s loaded",
         cparse(get_string_var(CDCC_PROMPT_VAR)), count, plural(count));
  ...
  *f_path = 0;
  ...
  while ((thefile = new_next_arg(path, &path))) {
    ...
    if ((expand = expand_twiddle(f_path)))
      dptr = opendir(expand);
    if (!dptr) {
      put_it("%s: you cannot access dir %s. Attempting to continue",
             cparse(get_string_var(CDCC_PROMPT_VAR)),
             expand ? expand : thefile);
      new_free(&expand);
      new_free(&filebuf);
      continue;
    }
  ...
  if (!newpack->numfiles) {
    put_it("%s: no files found, aborting...",
           cparse(get_string_var(CDCC_PROMPT_VAR)));
    new_free(&expand);
    new_free(&filebuf);
...
static void add_note(char *args __attribute__((unused)), char *rest) {
  if (rest && *rest) {
    ...
    put_it("%s: added note to pack #\002%d\002 %s",
           cparse(get_string_var(CDCC_PROMPT_VAR)), newpack->num,
           newpack->notes);
  }
}
...
static void add_desc(char *args __attribute__((unused)), char *rest) {
  ...
  if (newpack->size / 1024 > 999)
    ...
    snprintf(size, sizeof(size), "\002%3.2f\002kb",
             (double)newpack->size / 1024);
  put_it("%s: added pack #\002%d\002, \002%d\002 file%s (%s)",
         cparse(get_string_var(CDCC_PROMPT_VAR)), newpack->num,
         newpack->numfiles, plural(newpack->numfiles == 1), size);
...
static void del_pack(char *args __attribute__((unused)), char *rest) {
  ...
  if (!rest || !*rest) {
    put_it("%s: No pack specified for removal",
           cparse(get_string_var(CDCC_PROMPT_VAR)));
    return;
  }
  if (*rest == '*') {
    ...
    put_it("%s: removed all packs from offer list",
           cparse(get_string_var(CDCC_PROMPT_VAR)));
    return;
  }
  ...
  while (rest && *rest) {
    ...
    if (packnum <= 0) {
      put_it("%s: invalid pack specification",
             cparse(get_string_var(CDCC_PROMPT_VAR)));
      continue;
    }
    ...
    for (ptr = offerlist; ptr; ptr = ptr->next) {
      if (ptr->num == packnum) {
        ...
        cdcc_numpacks--;
        set_int_var(_CDCC_PACKS_OFFERED_VAR, cdcc_numpacks);
        put_it("%s: removed pack \002%d\002 from offer list",
               cparse(get_string_var(CDCC_PROMPT_VAR)), packnum);
        num++;
        break;
      }
      last = ptr;
  ...
  if (num) {
    ...
    put_it("%s: pack \002%s\002 does not exist",
           cparse(get_string_var(CDCC_PROMPT_VAR)), rest);
  return;
}
...
int BX_add_to_queue(char *nick, char *command, pack *sendpack) {
  ...
  if (!sendpack || !sendpack->file) {
    put_it("%s: ERROR occurred in cdcc add to queue",
           cparse(get_string_var(CDCC_PROMPT_VAR)));
    return 0;
  }
  if (queuelist) {
    for (new = queuelist, count = 1; new; new = new->next) {
      if (!my_stricmp(nick, new->nick) &&
          !my_stricmp(sendpack->file, new->file)) {
        if (!lastnick || my_stricmp(lastnick, nick))
          queue_send_to_server(from_server,
                               "NOTICE %s :\002CDCC\002: You're already Queued "
                               "for %s at position %d",
                               nick, new->desc, count);
        put_it("%s: Already queued %d files for %s at %d",
               cparse(get_string_var(CDCC_PROMPT_VAR)), sendpack->numfiles,
               nick, count);
        return 0;
      }
    }
  ...
  if (last)
    ...
    queuelist = new;
  numqueue++;
  put_it("%s: Queue position %d queuing %d files for %s",
         cparse(get_string_var(CDCC_PROMPT_VAR)), numqueue, sendpack->numfiles,
         nick);
...
void dcc_sendfrom_queue(void) {
  ...
  put_it("%s: sending \037%s\037 \002%d\002 file%s from queue",
         cparse(get_string_var(CDCC_PROMPT_VAR)), ptr->nick, ptr->numfiles,
         plural(ptr->numfiles));
  ...
  while ((temp = new_next_arg(file, &file))) {
    ...
    if (ptr->command) {
      if (!my_stricmp(ptr->command, "RESEND") ||
          !my_stricmp(ptr->command, "TRESEND"))
...
static void get_minspeed(char *args __attribute__((unused)), char *rest) {
  ...
  if (cdcc_minspeed) {
    put_it("%s: Minspeed value to %1.4g KB/s",
           cparse(get_string_var(CDCC_PROMPT_VAR)), cdcc_minspeed);
    if (last && *last && isdigit((unsigned char)*last)) {
      cdcc_mintime = strtod(last, NULL);
      set_int_var(_CDCC_MINSPEED_TIME_VAR, cdcc_mintime);
      put_it("%s: Minspeed time to %5d seconds",
             cparse(get_string_var(CDCC_PROMPT_VAR)), cdcc_mintime);
    } else if (!get_int_var(_CDCC_MINSPEED_TIME_VAR))
      put_it("%s: Make sure and /set _CDCC_MINSPEED_TIME as well",
             cparse(get_string_var(CDCC_PROMPT_VAR)));
  } else
    cdcc_minspeed = 0.0;
}
...
static int l_minspeed(char *args, char *rest __attribute__((unused))) {
  char *temp = NULL;
  malloc_sprintf(&temp, "%s min-speed (0 to disable): ",
                 cparse(get_string_var(CDCC_PROMPT_VAR)));
  if (args && *args)
...
static void get_ptimer(char *args __attribute__((unused)), char *rest) {
  ...
  ptimer *= 60;
  if (ptimer)
    put_it("%s: Ptimer interval %d minutes",
           cparse(get_string_var(CDCC_PROMPT_VAR)), ptimer / 60);
  else
    plist_last_time = 0;
}
...
static int l_timer(char *args, char *rest __attribute__((unused))) {
  char *temp = NULL;
  malloc_sprintf(&temp, "%s p-timer interval(s) (0 to disable): ",
                 cparse(get_string_var(CDCC_PROMPT_VAR)));
  if (args && *args)
...
static void get_pchannel(char *args __attribute__((unused)), char *rest) {
  if (rest && *rest && is_channel(rest))
    ...
    new_free(&public_channel);
  if (public_channel)
    put_it("%s: Public timer channel(s) are [%s]",
           cparse(get_string_var(CDCC_PROMPT_VAR)), public_channel);
  else
    put_it("%s: Disabled %s public timer channel(s)",
           cparse(get_string_var(CDCC_PROMPT_VAR)),
           cparse(get_string_var(CDCC_PROMPT_VAR)));
}
...
static int l_channel(char *args, char *rest __attribute__((unused))) {
  ...
  else if (public_channel)
    put_it("%s: Public timer channel is [%s]",
           cparse(get_string_var(CDCC_PROMPT_VAR)), public_channel);
  else
    put_it("%s: Disabled %s public timer channel",
           cparse(get_string_var(CDCC_PROMPT_VAR)),
           cparse(get_string_var(CDCC_PROMPT_VAR)));
  return 0;
}
...
static void add_note1(unsigned long pnum, char *note) {
  ...
  unsigned long i;
  if (pnum && note) {
    for (i = 1, this_pack = offerlist; this_pack;
         this_pack = this_pack->next, i++)
      if (i == pnum)
        break;
    if (this_pack) {
      if (note && *note)
      ...
      put_it("%s: Invalid pack number %ld",
             cparse(get_string_var(CDCC_PROMPT_VAR)), pnum);
...
static void get_pnote(char *args __attribute__((unused)), char *rest) {
  ...
  if (rest && *rest)
    ...
    got_pnum = pnum;
    malloc_sprintf(&temp, "%s note to add to pack %ld ",
                   cparse(get_string_var(CDCC_PROMPT_VAR)), pnum);
...
static void get_describe(char *args __attribute__((unused)), char *rest) {
  ...
  if (rest && *rest)
    ...
    got_dnum = pnum;
    malloc_sprintf(&temp, "%s description to add to pack %ld ",
                   cparse(get_string_var(CDCC_PROMPT_VAR)), pnum);
...
static int l_note(char *args, char *rest __attribute__((unused))) {
  ...
  if (!offerlist)
    return 0;
  malloc_sprintf(&temp, "%s add note to pack #: ",
                 cparse(get_string_var(CDCC_PROMPT_VAR)));
  if (args && *args)
    get_pnote(NULL, args);
  else
...
static int l_describe(char *args, char *rest __attribute__((unused))) {
  ...
  if (!offerlist)
    return 0;
  malloc_sprintf(&temp, "%s change description of pack #: ",
                 cparse(get_string_var(CDCC_PROMPT_VAR)));
  if (args && *args)
...
static int l_type(char *args, char *rest __attribute__((unused))) {
  ...
  put_it("%s: type of output for offer set to [%s]",
         cparse(get_string_var(CDCC_PROMPT_VAR)),
...
static int l_echo(char *args, char *rest __attribute__((unused))) {
  ...
  put_it("%s: local echo set to [%s]", cparse(get_string_var(CDCC_PROMPT_VAR)),
         on_off(do_cdcc_echo));
  return 0;
}

static int l_stats(char *args __attribute__((unused)),
                   char *rest __attribute__((unused))) {
  char cdcc_minspeed_s[80];
  snprintf(cdcc_minspeed_s, sizeof(cdcc_minspeed_s), "%1.3f", cdcc_minspeed);
  put_it("%s",
         convert_output_format("       %G╒═══════════════════════%K[%C    cdcc "
                               "stat     %K]%G══════════════════════╕",
                               NULL));
  put_it("%s", convert_output_format("       %G│                               "
                                     ...
                                     NULL));
  put_it("%s", convert_output_format(
                   "       %G│%g╓─%K[%Cp%ctimer  %K]%g─╓-%K[%Ct%cype     "
                   "%K]%g─╖─%K[%Ct%cotal %Cp%cacks%K]%g─╓─%K[%Cs%cent  "
                   "%K]%g─╖─[%Cq%cueue%K]%g─╖%G│",
                   NULL));
  put_it("%s",
         convert_output_format("       %G│%g║ %W$[-10]0 %g║  %W$[-10]1 %g║    "
                               "%W$[-10]2 %g║ %W$[-8]3 %g║ %W$[-7]4 %g║%G│",
                               "%d %s %d %d %d", ptimer,
                               do_notice_list ? "notice" : "privmsg",
  ...
  put_it("%s", convert_output_format("       %G│ %W$[63]0-%G │", "%s",
                                     !public_channel ? "current channel"
                                                     : public_channel));
  put_it("%s", convert_output_format(
                   "       %g╓────%K[%C %c  %C %c    %K]%g───╓───%K[%C %c   %C "
                   "%c    %K]%g───╖──────────%K[%Ct%coggles%K]%g──────────╖",
                   NULL));
  put_it("%s",
         convert_output_format(
             "       %g║ %C %n    %W$[-6]0%n%R     %g║ %C %n    %W$[-6]1%n%R   "
             "  %g║   %Ct%nimer:   %W$[-3]2%n   %Ce%ncho:  %W$[-3]3 %g║",
             "1 1 %s %s", on_off(ptimer), on_off(do_cdcc_echo)));
  put_it("%s",
         convert_output_format(
             "       %g║ %C %n    %W$[-6]0%n%R     %g║ %C %n    %W$[-6]1%n%R   "
             "  %g║ %Cm%ninspeed:  %W$[-3]2%n   %Cs%necure:%W$[-3]3 %g║",
...
BUILT_IN_FUNCTION(function_cdcc) {
  ...
  for (ptr = offerlist; ptr; ptr = ptr->next) {
    if (it == ptr->num)
      return m_sprintf("%d %d %lu %d %s", ptr->num, ptr->numfiles, ptr->size,
                       ptr->gets, ptr->desc);
  }
  return m_strdup(empty_string);
}
  ...
  if (pnum && password) {
    ...
    if (this_pack) {
      if (password && *password)
        ...
        put_it("%s: Removed passwd from %ld",
               cparse(get_string_var(CDCC_PROMPT_VAR)), pnum);
      }
    } else
      put_it("%s: Invalid pack number %ld",
...
static void get_password(char *args __attribute__((unused)), char *rest) {
  ...
  if (rest && *rest)
    ...
    got_dnum = pnum;
    malloc_sprintf(&temp, "%s password to add to pack %ld: ",
                   cparse(get_string_var(CDCC_PROMPT_VAR)), pnum);
...
static int l_secure(char *args, char *rest __attribute__((unused))) {
  ...
  if (!offerlist)
    return 0;
  malloc_sprintf(&temp, "%s change password of pack #: ",
                 cparse(get_string_var(CDCC_PROMPT_VAR)));
  if (args && *args)
...
int BX_get_num_queue(void) { return numqueue; }

#else /* WANT_CDCC */

/* this is required for functions.c to compile properly */

BUILT_IN_FUNCTION(function_cdcc) { return m_strdup(empty_string); }

BUILT_IN_FUNCTION(function_sendcdcc) { return m_strdup(empty_string); }

int BX_get_num_queue(void) { return 0; }
