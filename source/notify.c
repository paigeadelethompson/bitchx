    ...
    if (tmp->flag) {
      if (count == 0) {
        if (do_hook(NOTIFY_HEADER_LIST, "%s", "Online users")) {
          ...
          put_it("%s",
                 convert_output_format(fget_string_var(FORMAT_NOTIFY_ON_FSET),
                                       "%s %s %s %s %s", "Nick", "UserHost",
                                       "Times", "Period", "Last seen"));
        }
      }
      ...
      if (do_hook(NOTIFY_LIST, "%s %s %d %s %s %s", tmp->nick,
                  tmp->host ? tmp->host : "unknown@unknown", tmp->flag, timeson,
                  period, lastseen))
        put_it("%s",
               convert_output_format(fget_string_var(FORMAT_NOTIFY_ON_FSET),
                                     "%s %s %s %s %s", tmp->nick,
                                     tmp->host ? tmp->host : tmp->looking,
                                     timeson, lastseen, "now"));
      count++;
    }
  }
  count = 0;
    ...
    if ((all && !tmp->flag) || (tmp->times && !tmp->flag)) {
      if (count == 0) {
        if (do_hook(NOTIFY_HEADER_LIST, "%s", "Offline users")) {
          ...
          put_it("%s",
                 convert_output_format(fget_string_var(FORMAT_NOTIFY_OFF_FSET),
                                       "%s %s %s %s %s", "Nick", "UserHost",
                                       "Times", "Period", "Last seen"));
        }
      }
        ...
        if (*nick) {
          ...
          if (strchr(nick, '*'))
            ...
            for (servnum = 0; servnum < server_list_size(); servnum++) {
              ...
              new_n->host = NULL;
              new_n->flag = 0;
              add_to_array((Array *)NOTIFY_LIST(servnum), (Array_item *)new_n);
              added = 1;
            }
            if (added) {
...
static void check_auto_invite(const char *nick, const char *userhost) {
  ...
  for (chan = get_server_channels(from_server); chan; chan = chan->next) {
    if (chan->have_op && get_cset_int_var(chan->csets, AINV_CSET)) {
      const UserList *ul =
          lookup_userlevelc("*", userhost, chan->channel, NULL);

      if (ul && (ul->flags & ADD_INVITE) &&
          !find_nicklist_in_channellist(nick, chan, 0)) {
        bitchsay("Auto-inviting %s to %s", nick, chan->channel);
        send_to_server("NOTICE %s :Auto-invite from %s", nick,
                       get_server_nickname(from_server));
        send_to_server("INVITE %s %s%s%s", nick, chan->channel,
...
void dispatch_notify_userhosts(void) {
  if (batched_notify_userhosts) {
    if (x_debug & DEBUG_NOTIFY)
      yell("Dispatching notifies to server [%d], [%s]", from_server,
           batched_notify_userhosts);
...
void notify_userhost_dispatch(UserhostItem *stuff, char *nick __attribute__((unused)), char *text __attribute__((unused))) {
...
void notify_userhost_reply(char *nick, char *userhost) {
  ...
  if ((tmp = (NotifyItem *)array_lookup((Array *)NOTIFY_LIST(from_server), nick,
                                        0, 0))) {
    ...
    if ((do_hook(NOTIFY_SIGNON_LIST, "%s %s", tmp->nick,
                 tmp->host ? tmp->host : empty_string))) {
      put_it("%s", convert_output_format(
                       fget_string_var(FORMAT_NOTIFY_SIGNON_FSET), "%s %s %s",
                       update_clock(GET_TIME), tmp->nick,
                       tmp->host ? tmp->host : empty_string));
      logmsg(LOG_NOTIFY, tmp->nick, 0, "%s(%s) has logged on",
             tmp->host ? tmp->host : empty_string, tmp->looking);
    }
 ...
 */
void notify_mark(char *nick, char *userhost __attribute__((unused)), int flag, int doit) {
  NotifyItem *tmp;
  int count = 0;
  int loc = 0;

  if ((tmp = (NotifyItem *)find_array_item((Array *)NOTIFY_LIST(from_server),
                                           nick, &count, &loc)) &&
      count < 0) {
    if (flag) {
      ...
      if (tmp->flag == 1) {
        ...
        if ((do_hook(NOTIFY_SIGNOFF_LIST, "%s %s", tmp->nick,
                     tmp->host ? tmp->host : empty_string)) &&
            doit)
          put_it("%s", convert_output_format(
                           fget_string_var(FORMAT_NOTIFY_SIGNOFF_FSET),
                           "%s %s %s", update_clock(GET_TIME), tmp->nick,
                           tmp->host ? tmp->host : empty_string));
        tmp->period += now - tmp->added;
        tmp->lastseen = now;
        logmsg(LOG_NOTIFY, tmp->nick, 0, "%s(%s) has logged off",
               tmp->host ? tmp->host : empty_string, tmp->looking);
...
void save_notify(FILE *fp) {
  ...
  if (server_list_size() && NOTIFY_MAX(0)) {
    ...
    for (i = 0; i < NOTIFY_MAX(0); i++)
      fprintf(fp, " %s!%s", NOTIFY_ITEM(0, i)->nick,
              NOTIFY_ITEM(0, i)->looking);
    fprintf(fp, "\n");
  }
...
void set_notify_handler(Window *win __attribute__((unused)), char *value, int unused __attribute__((unused))) {
  int len;
  int i;
  char *s;
...
void make_notify_list(int servnum) {
  ...
  for (i = 0; i < NOTIFY_MAX(0); i++) {
    ...
    add_to_array((Array *)NOTIFY_LIST(servnum), (Array_item *)tmp);
...
char *get_notify_nicks(int showserver, int showon, char *nick, int extra __attribute__((unused))) {
  char *list = NULL;
  int i = 0;
...
char *get_watch_nicks(int showserver, int showon, char *nick, int extra __attribute__((unused))) {
  char *list = NULL;
  int i = 0;
...
void make_watch_list(int servnum) {
  ...
  for (i = 0; i < WATCH_MAX(0); i++) {
    if (!(tmp = (NotifyItem *)array_lookup((Array *)WATCH_LIST(servnum),
                                           WATCH_ITEM(0, i)->nick, 0, 0))) {
      tmp = (NotifyItem *)new_malloc(sizeof(NotifyItem));
...
char *list = NULL;
  ...
  for (i = 0; i < WATCH_MAX(from_server); i++) {
    ...
    if (tmp->flag) {
      ...
      strlcpy(timeson, ltoa(tmp->times), sizeof(timeson));
      strlcpy(lastseen, ltoa(now - tmp->added), sizeof(lastseen));
      if (do_hook(WATCH_LIST, "%s %s %d %s %s %s", tmp->nick,
                  tmp->host ? tmp->host : "unknown@unknown", tmp->flag, timeson,
                  period, lastseen)) {
        if (!count) {
          put_it("%s", convert_output_format("$G Online Users", NULL, NULL));
      ...
      if (*nick == '+') {
        ...
        if (strchr(nick, '*'))
          ...
          for (servnum = 0; servnum < server_list_size(); servnum++) {
            ...
            if (WATCH_MAX(servnum) >= (is_server_connected(servnum)
                                           ? get_server_watch(servnum)
                                           : 128)) {
              bitchsay("Too many WATCH entries [%d]",
                       get_server_watch(servnum));
              continue;
            }
            new_n = (NotifyItem *)new_malloc(sizeof(NotifyItem));
            new_n->nick = m_strdup(nick);
            new_n->flag = 0;
            add_to_array((Array *)WATCH_LIST(servnum), (Array_item *)new_n);
            added = 1;
          }
          if (added) {
            if (!list)
        ...
        for (servnum = 0; servnum < server_list_size(); servnum++) {
          if (!get_server_watch(servnum))
            continue;
          while ((new_n = (NotifyItem *)array_pop((Array *)WATCH_LIST(servnum),
                                                  0))) {
            new_free(&new_n->nick);
            new_free(&new_n->looking);
            new_free(&new_n->host);
...
void show_watch_notify(char *from __attribute__((unused)), int online, char **args) {
  NotifyItem *new_n;
    ...
    if (online) {
      ...
      if (!new_n->flag) {
        ...
        if (!new_n->added)
          new_n->added = now;
        new_n->times++;
        logmsg(LOG_NOTIFY, new_n->nick, 0, "%s has logged on",
               new_n->host ? new_n->host : empty_string);
      }
      ...
      if (new_n->flag) {
        ...
        new_n->lastseen = now;
        logmsg(LOG_NOTIFY, new_n->nick, 0, "%s has logged off",
               new_n->host ? new_n->host : empty_string);
      }
    }
    new_n->flag = online;
    put_it("%s", convert_output_format(
                     fget_string_var(online ? FORMAT_WATCH_SIGNON_FSET
...
void send_watch(int server) {
  ...
  for (i = 0; i < WATCH_MAX(server); i++) {
    if (!(tmp =
              (NotifyItem *)array_lookup((Array *)WATCH_LIST(server),
                                         WATCH_ITEM(server, i)->nick, 0, 0))) {
      tmp = (NotifyItem *)new_malloc(sizeof(NotifyItem));
