...
static void get_flood_val(ChannelList *chan, enum flood_type type,
                          int *flood_count, int *flood_rate) {
...
static int set_flood(enum flood_type type, time_t flood_time, int reset,
                     NickList *tmpnick) {
  if (!tmpnick)
    return 0;
  switch (type) {
...
int BX_is_other_flood(ChannelList *channel, NickList *tmpnick,
                      enum flood_type type, int *t_flood) {
    ...
    if (!is_user && (count >= flood_count)) {
      ...
      if (count >= flood_count) {
        if (!diff || (flood_rate && (diff < flood_rate))) {
          *t_flood = diff;
          flooded = 1;
          do_hook(FLOOD_LIST, "%s %s %s %s", tmpnick->nick,
                  flood_table[type].text, channel ? channel->channel : zero,
                  tmpnick->host);
        }
...
/* Prune any flood entries older than FLOOD_RATE_VAR (or from the future, in
 * case the clock has gone backwards). */
  ...
  for (x = 0; x < FLOOD_HASHSIZE; x++) {
    ...
    while (ptr) {
      ...
      if (elapsed > flood_rate || elapsed < 0) {
        if (!(ptr = find_name_in_floodlist(ptr->name, ptr->host, flood_list,
                                           FLOOD_HASHSIZE, 1)))
          continue;
...
int BX_check_flooding(char *nick, enum flood_type type, char *line,
                      char *channel) {
  Flooding *tmp;
  int flood_rate;
  int flood_count;
  ...
  if (find_name_in_genericlist(nick, no_flood_list, FLOOD_HASHSIZE, 0))
    return 1;
  if (!(tmp = find_name_in_floodlist(nick, FromUserHost, flood_list,
                                     FLOOD_HASHSIZE, 0))) {
    remove_oldest_flood_hashlist(&flood_list[0]);
    tmp = add_name_to_floodlist(nick, FromUserHost, channel, flood_list,
                                FLOOD_HASHSIZE);
  ...
  if (!flood_count || !flood_rate)
    return 1;
  tmp->cnt++;
  if (tmp->cnt > (unsigned long)flood_count) {
    int ret;
    ...
    if (!diff || this_flood > allow_flood) {
      if (tmp->flood == 0) {
        tmp->flood = 1;
        if ((ret = do_hook(FLOOD_LIST, "%s %s %s %s", nick,
                           flood_table[type].text, channel ? channel : zero,
                           line)) != 1)
          return ret;
        if (flood_prot(nick, FromUserHost, type, get_int_var(IGNORE_TIME_VAR),
                       channel))
          return 0;
        if (get_int_var(FLOOD_WARNING_VAR))
          put_it("%s", convert_output_format(
                           fget_string_var(FORMAT_FLOOD_FSET), "%s %s %s %s %s",
                           update_clock(GET_TIME), flood_table[type].text, nick,
                           FromUserHost, channel ? channel : "unknown"));
      }
      return 1;
    } else {
      tmp->flood = 0;
...
static void check_ctcp_ban_flood(char *channel __attribute__((unused)), char *nick) {
  NickList *Nick = NULL;
  ...
  if (chan && chan->have_op &&
      get_cset_int_var(chan->csets, CTCP_FLOOD_BAN_CSET) && Nick) {
    if (!Nick->userlist ||
        (Nick->userlist && !(Nick->userlist->flags & ADD_FLOOD))) {
...
int BX_flood_prot(char *nick, char *userhost, enum flood_type flood_type,
                  int ignoretime, char *channel) {
  ChannelList *chan;
  NickList *Nick;
  ...
  if ((flood_type == CDCC_FLOOD || flood_type == CTCP_FLOOD ||
       flood_type == CTCP_ACTION_FLOOD) &&
      !get_int_var(CTCP_FLOOD_PROTECTION_VAR))
    return 0;
  ...
  case PUBLIC_FLOOD:
    if (channel) {
      if ((chan = lookup_channel(channel, from_server, 0))) {
        ...
        if (kick_on_flood &&
            (Nick = find_nicklist_in_channellist(nick, chan, 0))) {
          if (chan->have_op &&
              (!Nick->userlist ||
               (Nick->userlist && !(Nick->userlist->flags & ADD_FLOOD))))
            if (!nick_isop(Nick) ||
                get_cset_int_var(chan->csets, KICK_OPS_CSET))
              send_to_server("KICK %s %s :\002%s\002 flooder", chan->channel,
                             nick, flood_table[flood_type].text);
        }
      }
    }
    break;
  ...
  bitchsay("Auto-ignoring %s for %d minutes [\002%s\002 flood]", nick,
           ignoretime / 60, flood_table[flood_type].text);
  return 1;
}

void clean_flood_list() { remove_oldest_flood_hashlist(&flood_list[0]); }
