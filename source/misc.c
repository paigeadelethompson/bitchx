...
char *color_str[] = {
    "[0m",    "[0;34m", "[0;32m", "[0;36m", "[0;31m", "[0;35m", "[0;33m",
    "[0;30m", "[1;37m", "[1;34m", "[1;32m", "[1;36m", "[1;31m", "[1;35m",
    "[1;33m", "[1;30m", "[0m",    "[0;47m", "[0;41m", "[0;42m", "[0;43m",
    "[0;44m", "[0;45m", "[0;46m", "[0;40m", "[1;47m", "[1;41m", "[1;42m",
    "[1;43m", "[1;44m", "[1;45m", "[1;46m", "[1;40m", "[7m",    "[1m",
    "[5m",    "[4m"};

#else

char *color_str[] = {
    "[0;30m", "[0;34m", "[0;32m", "[0;36m", "[0;31m", "[0;35m", "[0;33m",
    "[0m",    "[1;30m", "[1;34m", "[1;32m", "[1;36m", "[1;31m", "[1;35m",
    "[1;33m", "[1;37m", "[0m",    "[0;40m", "[0;41m", "[0;42m", "[0;43m",
    "[0;44m", "[0;45m", "[0;46m", "[0;47m", "[1;40m", "[1;41m", "[1;42m",
    "[1;43m", "[1;44m", "[1;45m", "[1;46m", "[1;47m", "[7m",    "[1m",
    "[5m",    "[4m"};
#endif
...
char *convert_time(time_t ltime) {
  ...
  *buffer = '\0';
  ...
  hours = ltime % 24;
  days = (ltime - hours) / 24;
  snprintf(buffer, sizeof(buffer), "%2lud %2luh %2lum %2lus", days, hours,
           minutes, seconds);
...
BUILT_IN_COMMAND(do_uptime) {
  ...
  put_it("%s",
         convert_output_format("%G--[ %WBitchX%g-%wClient%g-%RStatistics "
                               "%G]------------------------------------------",
                               NULL));
  ...
  put_it("%s", convert_output_format("%G| %CCurrent RealName: %W$0-", "%s",
                                     realname));
  put_it("%s", convert_output_format("%G| %CLast Recv Message: %W$0-", "%s",
                                     last_msg[0].last_msg ? last_msg[0].last_msg
                                                          : "None"));
  put_it("%s", convert_output_format(
                   "%G| %CLast Recv Notice: %W$0-", "%s",
                   last_notice[0].last_msg ? last_notice[0].last_msg : "None"));
  put_it("%s", convert_output_format("%G| %CLast Sent Msg: %W$0-", "%s",
                                     last_sent_msg[0].last_msg
                                         ? last_sent_msg[0].last_msg
                                         : "None"));
  put_it("%s", convert_output_format("%G| %CLast Sent Notice: %W$0-", "%s",
...
int timer_unban(void *args, char *sub) {
  ...
  if ((chan = (ChannelList *)find_in_list((List **)&chan2, channel, 0)) &&
...
int timer_idlekick(void *args, char *sub) {
  ...
  if (channel && (tmp = lookup_channel(channel, from_server, CHAN_NOUNLINK)) &&
      tmp->have_op && tmp->max_idle && tmp->check_idle) {
    NickList *nick;
    for (nick = next_nicklist(tmp, NULL); nick;
         nick = next_nicklist(tmp, nick)) {
      if (!my_stricmp(nick->nick, get_server_nickname(from_server)))
        continue;
      if ((nick_isop(nick) || nick_isvoice(nick)) &&
          !get_cset_int_var(tmp->csets, KICK_OPS_CSET))
        continue;
      if ((user = nick->userlist) &&
          check_channel_match(user->channels, channel))
        continue;
      if (now - nick->idle_time >= tmp->max_idle) {
        if (kick_count <= get_int_var(MAX_IDLEKICKS_VAR)) {
          ...
          send_to_server("KICK %s %s :\002%s\002: (Idle Channel User)", channel,
                         nick->nick, version);
          add_timer(0, empty_string, 60 * 1000, 1, timer_unban,
                    m_sprintf("%d %s", from_server, p), NULL,
                    current_window->refnum, "idle-kick");
          new_free(&p);
        } else
          break;
        kick_count++;
  ...
  if (tmp && tmp->max_idle && tmp->check_idle)
    add_timer(0, empty_string, get_int_var(IDLE_CHECK_VAR) * 1000, 1,
              timer_idlekick, channel, NULL, current_window->refnum,
              "idle-kick");
  else
    new_free(&channel);
  new_free(&sub);
...
BUILT_IN_COMMAND(addidle) {
  ...
  if ((p = next_arg(args, &args))) {
    ...
    if (!(new = (ChannelList *)find_in_list((List **)&idlechan_list, channel,
                                            0))) {
    ...
    if (!new->check_idle) {
      bitchsay("Idle checking turned %s for %s", on_off(new->check_idle),
               channel);
      if (tmp)
        tmp->check_idle = tmp->max_idle = 0;
      new->max_idle = 0;
    } else {
      bitchsay("Idle checking turned %s for %s %d mins",
               on_off(tmp->check_idle), channel, (int)(tmp->max_idle / 60));
      if (tmp) {
...
BUILT_IN_COMMAND(showidle) {
  ...
  for (nick = ntmp; nick; nick = nick->next) {
    if (!count && do_hook(SHOWIDLE_HEADER_LIST, "%s %lu", tmp->channel,
                          (unsigned long)tmp->max_idle))
      put_it("%s",
             convert_output_format(
                 "$G %W$a%n: Idle check for %W$0%n Max Idle Time %K[%W$1- %K]",
                 "%s %s", tmp->channel, convert_time(tmp->max_idle)));
    ltime = now - nick->idle_time;
#ifdef WANT_USERLIST
    if (do_hook(SHOWIDLE_LIST, "%s %s %d %lu", nick->nick, nick->host,
                find_user_level(nick->nick, nick->host, tmp->channel),
                (unsigned long)ltime))
#else
    if (do_hook(SHOWIDLE_LIST, "%s %s %d %lu", nick->nick, nick->host, 0,
                (unsigned long)ltime))
#endif
      put_it("%s", convert_output_format("$[20]0 Idle%W: %K[%n$1- %K]", "%s %s",
                                         nick->nick, convert_time(ltime)));
    count++;
  }
  if (count)
  ...
  if ((tmp = prepare_command(&server, channel, NEED_OP)) && tmp->max_idle) {
    ...
    for (nick = next_nicklist(tmp, NULL); nick;
      ...
      if (!my_stricmp(nick->nick, get_server_nickname(from_server)))
        continue;
      if (nick->userlist &&
          check_channel_match(nick->userlist->channels, tmp->channel))
        continue;
      if (nick_isop(nick))
        continue;
      if (now - nick->idle_time >= tmp->max_idle) {
        if (kick_count <= get_int_var(MAX_IDLEKICKS_VAR))
          my_send_to_server(server,
                            "KICK %s %s :\002%s\002: (Idle Channel User)",
                            tmp->channel, nick->nick, version);
        else
          bitchsay(" found idle user %-12s channel %s", nick->nick,
                   tmp->channel);
        kick_count++;
      }
    }
...
BUILT_IN_COMMAND(channel_stats) {
  ...
  if (args && *args) {
    ...
    if (!my_strnicmp(channel, "-ALL", strlen(channel))) {
      ...
      if (do_hook(CHANNEL_STATS_LIST,
                  "%s %s %s %lu %lu %lu %lu %u %u %u %u %u %u %u %u %u %u %u "
                  "%u %u %u %u %u %u %u %u %u %u %u %u %d %s",
                  channel, empty_string, empty_string,
                  nick_mem + chan_mem + ban_mem, nick_mem, chan_mem, ban_mem,
                  stats_ops, stats_dops, stats_bans, stats_unbans, stats_topics,
                  stats_kicks, stats_pubs, stats_parts, stats_signoffs,
                  stats_joins, total_bans, max_bans, stats_sops, stats_sdops,
                  stats_sbans, stats_sunbans, usershere, usersaway, chanops,
                  chanunop, total_nicks, max_nicks, numircops, max_hops,
                  ircops ? ircops : "")) {
        put_it("%s",
               convert_output_format("$G %CInformation for channels %K: %W$0",
                                     "%s", channel));
        put_it("%s", convert_output_format(
                         "     MEM usage%K:%w Total%K:%w %c$0 bytes   "
                         "%K[%cNicks $1 b Chan $2 b Bans $3 b%K]",
                         "%d %d %d %d", (int)(nick_mem + chan_mem + ban_mem),
                         (int)nick_mem, (int)chan_mem, (int)ban_mem));
        put_it("%s",
               convert_output_format(
                   "Ops        %K[%W$[-5]0%K]%w  De-Ops     %K[%W$[-5]1%K]%w  "
                   "Bans       %K[%W$[-5]2%K]%w  Unbans     %K[%W$[-5]3%K]%w",
                   "%u %u %u %u", stats_ops, stats_dops, stats_bans,
                   stats_unbans));
        put_it("%s",
               convert_output_format(
                   "Topics     %K[%W$[-5]0%K]%w  Kicks      %K[%W$[-5]1%K]%w  "
                   "Publics    %K[%W$[-5]2%K]%w  Parts      %K[%W$[-5]3%K]%w",
                   "%u %u %u %u", stats_topics, stats_kicks, stats_pubs,
                   stats_parts));
        put_it("%s",
               convert_output_format(
                   "Signoffs   %K[%W$[-5]0%K]%w  Joins      %K[%W$[-5]1%K]%w  "
                   "TotalBans  %K[%W$[-5]2%K]%w  MaxBans    %K[%W$[-5]3%K]%w",
                   "%u %u %u %u", stats_signoffs, stats_joins, total_bans,
                   max_bans));
        put_it("%s",
               convert_output_format(
                   "ServOps    %K[%W$[-5]0%K]%w  ServDeop   %K[%W$[-5]1%K]%w  "
                   "ServBans   %K[%W$[-5]2%K]%w  ServUB     %K[%W$[-5]3%K]%w",
                   "%u %u %u %u", stats_sops, stats_sdops, stats_sbans,
                   stats_sunbans));
        put_it("%s",
               convert_output_format(
                   "Users Here %K[%W$[-5]0%K]%w  Users Away %K[%W$[-5]1%K]%w  "
                   "Opped      %K[%W$[-5]2%K]%w  Unopped    %K[%W$[-5]3%K]%w",
                   "%u %u %u %u", usershere, usersaway, chanops, chanunop));
        put_it("%s", convert_output_format(
                         "TotalNicks %K[%W$[-5]0%K]%w  MaxNicks   "
                         "%K[%W$[-5]1%K]%w  ServerHops %K[%W$[-5]2%K]%w",
                         "%d %d %d", total_nicks, max_nicks, max_hops));
        put_it("%s",
               convert_output_format("IRCops     %K[%W$[3]0%K]%w$1-", "%d %s",
                                     numircops, ircops ? ircops : ""));
      }
      new_free(&ircops);
      new_free(&channel);
      return;
  ...
  if (do_hook(CHANNEL_STATS_LIST,
              "%s %s %s %ld %ld %ld %ld %u %u %u %u %u %u %u %u %u %u %u %u %u "
              "%u %u %u %u %u %u %u %u %u %u %d %s",
              new->channel, my_ctime(new->channel_create.tv_sec),
              convert_time(time_since(&new->join_time)),
              nick_mem + sizeof(ChannelList) + ban_mem, nick_mem,
              (unsigned long)sizeof(ChannelList), ban_mem, new->stats_ops,
              new->stats_dops, new->stats_bans, new->stats_unbans,
              new->stats_topics, new->stats_kicks, new->stats_pubs,
              new->stats_parts, new->stats_signoffs, new->stats_joins,
              new->totalbans, new->maxbans, new->stats_sops, new->stats_sdops,
              new->stats_sbans, new->stats_sunbans, usershere, usersaway,
              chanops, chanunop, new->totalnicks, new->maxnicks, numircops,
              max_hops, ircops ? ircops : "")) {
    put_it("%s", convert_output_format("$G %CInformation for channel %K: %W$0",
                                       "%s", new->channel));
    put_it("%s", convert_output_format(
                     "$G %CChannel created %K: %W$4-%n in memory %W$0 $1 $2 $3",
                     "%s %s", convert_time(time_since(&new->join_time)),
                     my_ctime(new->channel_create.tv_sec)));
    put_it("%s",
           convert_output_format(
               "     MEM usage%K:%w Total%K:%w %c$0 bytes   %K[%cNicks $1 b "
               "Chan $2 b Bans $3 b%K]",
               "%d %d %d %d", (int)(nick_mem + sizeof(ChannelList) + ban_mem),
               (int)nick_mem, (int)sizeof(ChannelList), (int)ban_mem));
    put_it("%s",
           convert_output_format(
               "Ops        %K[%W$[-5]0%K]%w  De-Ops     %K[%W$[-5]1%K]%w  Bans "
               "      %K[%W$[-5]2%K]%w  Unbans     %K[%W$[-5]3%K]%w",
               "%u %u %u %u", new->stats_ops, new->stats_dops, new->stats_bans,
               new->stats_unbans));
    put_it("%s",
           convert_output_format(
               "Topics     %K[%W$[-5]0%K]%w  Kicks      %K[%W$[-5]1%K]%w  "
               "Publics    %K[%W$[-5]2%K]%w  Parts      %K[%W$[-5]3%K]%w",
               "%u %u %u %u", new->stats_topics, new->stats_kicks,
               new->stats_pubs, new->stats_parts));
    put_it("%s",
           convert_output_format(
               "Signoffs   %K[%W$[-5]0%K]%w  Joins      %K[%W$[-5]1%K]%w  "
               "TotalBans  %K[%W$[-5]2%K]%w  MaxBans    %K[%W$[-5]3%K]%w",
               "%u %u %u %u", new->stats_signoffs, new->stats_joins,
               new->totalbans, new->maxbans));
    put_it("%s",
           convert_output_format(
               "ServOps    %K[%W$[-5]0%K]%w  ServDeop   %K[%W$[-5]1%K]%w  "
               "ServBans   %K[%W$[-5]2%K]%w  ServUB     %K[%W$[-5]3%K]%w",
               "%u %u %u %u", new->stats_sops, new->stats_sdops,
               new->stats_sbans, new->stats_sunbans));
    put_it("%s",
           convert_output_format(
               "Users Here %K[%W$[-5]0%K]%w  Users Away %K[%W$[-5]1%K]%w  "
               "Opped      %K[%W$[-5]2%K]%w  Unopped    %K[%W$[-5]3%K]%w",
               "%u %u %u %u", usershere, usersaway, chanops, chanunop));
    put_it("%s", convert_output_format(
                     "TotalNicks %K[%W$[-5]0%K]%w  MaxNicks   %K[%W$[-5]1%K]%w "
                     " ServerHops %K[%W$[-5]2%K]%w",
                     "%d %d %d", new->totalnicks, new->maxnicks, max_hops));
    put_it("%s", convert_output_format("IRCops     %K[%W$[3]0%K]%w$1-", "%d %s",
                                       numircops, ircops ? ircops : ""));

    put_it("%s", convert_output_format(
                     "  %CThere is %R$0%C limit and limit checking is %R$1-",
                     "%s %s", new->limit ? ltoa(new->limit) : "no",
                     new->tog_limit ? "Enabled" : "Disabled"));
    put_it("%s",
           convert_output_format("  %CIdle user check is %K[%R$0-%K]", "%s",
                                 new->check_idle ? "Enabled" : "Disabled"));
    /*put_it("%s", convert_output_format("$G End of channel stats for $0", "%s",
     * new->channel));*/
...
void update_stats(int what, NickList *nick, ChannelList *chan, int splitter) {
  ...
  case JOINLIST: {
    ...
    if (!splitter && chan->have_op &&
        is_other_flood(chan, nick, JOIN_FLOOD, &t) &&
        get_cset_int_var(chan->csets, KICK_ON_JOINFLOOD_CSET) &&
        !nick->sent_kick++) {
      char *t1, *host, *banstr;
      ...
      *host++ = 0;
      ...
      send_to_server(
          "KICK %s %s :\002Join flood\002 (%d joins in %dsecs of %dsecs)",
          chan->channel, nick->nick,
          get_cset_int_var(chan->csets, KICK_ON_JOINFLOOD_CSET), t,
          get_cset_int_var(chan->csets, JOINFLOOD_TIME_CSET));
      if (get_int_var(AUTO_UNBAN_VAR))
        add_timer(0, empty_string, get_int_var(AUTO_UNBAN_VAR) * 1000, 1,
                  timer_unban,
                  m_sprintf("%d %s %s", from_server, chan->channel, banstr),
                  NULL, current_window->refnum, "join-flood");
    }
    break;
  }
  ...
  case MODEDEOPLIST:
    ...
    if (chan->have_op && is_other_flood(chan, nick, DEOP_FLOOD, &t)) {
      if (get_cset_int_var(chan->csets, DEOP_ON_DEOPFLOOD_CSET) <
          get_cset_int_var(chan->csets, KICK_ON_DEOPFLOOD_CSET))
        send_to_server("MODE %s -o %s", chan->channel, nick->nick);
      else if (!nick->sent_kick++)
        send_to_server(
            "KICK %s %s :\002De-op flood\002 (%d de-ops in %dsecs of %dsecs)",
            chan->channel, nick->nick,
            get_cset_int_var(chan->csets, KICK_ON_DEOPFLOOD_CSET), t,
            get_cset_int_var(chan->csets, DEOPFLOOD_TIME_CSET));
    }
    break;
  case MODEBANLIST:
    if (splitter)
...
#else
char *stripansicodes(const char *line) { return line; }
char *mircansi(unsigned char *line) { return line; }
#endif

int check_split(char *nick __attribute__((unused)), char *reason) {
...
void BX_userage(char *command, char *use) {
  if (command) {
    if (do_hook(USAGE_LIST, "%s %s", command,
                use ? use : "No Help Available for this command"))
      put_it("%s", convert_output_format(
                       fget_string_var(FORMAT_USAGE_FSET), "%s %s", command,
                       convert_output_format(
                           use ? use : "%WNo Help available for this command",
                           NULL, NULL)));
  } else
...
void check_auto_away(time_t idlet) {
  ...
  if (awaymsg)
    malloc_sprintf(&msg, "%s: [%d mins]", convert_output_format(awaymsg, NULL),
                   idle_mins);
  else
...
void putlog(int type __attribute__((unused)), const char *chname, const char *format, ...) {
#ifdef PUBLIC_ACCESS
  return;
#else
  va_list va;
...
static int redirect_msg(char *to, enum REDIR_TYPES what, char *str,
                        int showansi) {
  char *new_str;
  if (showansi)
    new_str = str;
  ...
  case PRIVMSG:
    if (*to == '=') {
      ...
      if (is_channel(to))
        put_it("%s", convert_output_format(
                         fget_string_var(FORMAT_SEND_PUBLIC_FSET),
                         "%s %s %s %s", update_clock(GET_TIME), to,
                         get_server_nickname(from_server), new_str));
      else
        put_it("%s", convert_output_format(
                         fget_string_var(FORMAT_SEND_MSG_FSET), "%s %s %s %s",
                         update_clock(GET_TIME), to,
                         get_server_nickname(from_server), new_str));
  ...
  case KBOOT:
    ...
    /* FALLTHROUGH */
  case KICK:
    send_to_server("KICK %s %s :%s", get_current_channel_by_refnum(0), to,
                   new_str);
    break;
  case TOPIC:
  ...
  case NOTICE:
    put_it("%s",
           convert_output_format(fget_string_var(FORMAT_SEND_NOTICE_FSET),
                                 "%s %s %s %s", update_clock(GET_TIME), to,
                                 get_server_nickname(from_server), new_str));
...
BUILT_IN_COMMAND(do_dirlasttype) {
  ...
  if (!my_strnicmp(command, "RELCR", 5)) {
    ...
    for (count = size - 1; count != -1; count--) {
      if (t[count].last_msg)
        put_it("%2d %s", count,
               convert_output_format(fget_string_var(FORMAT_RELM_FSET),
                                     "%s %s %s %s %s", t[count].time,
                                     t[count].from, t[count].uh, t[count].to,
                                     t[count].last_msg));
    }
    return;
  }
  ...
  while (args && *args) {
    ...
    if (!my_strnicmp(comm, "-list", strlen(comm))) {
      for (count = size - 1; count != -1; count--) {
        ...
        case 2:
          put_it("%2d %s", count,
                 convert_output_format(form, "%s %s %s", t[count].time,
                                       t[count].to, t[count].last_msg));
          break;
        case 3:
          put_it("%2d %s", count,
                 convert_output_format(form, "%s %s %s", t[count].time,
                                       t[count].from, t[count].last_msg));
          break;
        case 4:
          put_it("%2d %s", count,
                 convert_output_format(form, "%s %s %s %s", t[count].time,
                                       t[count].from, t[count].to,
                                       t[count].last_msg));
          break;
        case 5:
          put_it("%2d %s", count,
                 convert_output_format(form, "%s %s %s %s %s", t[count].time,
                                       t[count].from, t[count].uh, t[count].to,
                                       t[count].last_msg));
        }
      }
  ...
  if (channel || what == WALLOP) {
    ...
    switch (numargs) {
    case 2:
      malloc_strcpy(&p, convert_output_format(form, "%s %s %s", t[count].time,
                                              t[count].to, t[count].last_msg));
      break;
    case 3:
      malloc_strcpy(&p,
      ...
      break;
    case 4:
      malloc_strcpy(&p, convert_output_format(form, "%s %s %s %s",
                                              t[count].time, t[count].from,
                                              t[count].to, t[count].last_msg));
      break;
    case 5:
      malloc_strcpy(&p,
                    convert_output_format(form, "%s %s %s %s %s", t[count].time,
                                          t[count].from, t[count].uh,
                                          t[count].to, t[count].last_msg));
    }
...
BUILT_IN_COMMAND(ChanWallOp) {
  ...
  if (get_current_channel_by_refnum(0)) {
    ...
    if (((ver = get_server_version(current_window->server)) ==
         Server2_8hybrid6))
      enable_all = 1;
    else if (((ver = get_server_version(current_window->server)) <
              Server_u2_8)) {
    ...
    snprintf(buffer, sizeof(buffer), "[\002BX-Wall\002/\002%s\002] %s", channel,
             args);
    if (ver >= Server_u2_10 || enable_all) {
      send_to_server(enable_all ? "NOTICE @%s :%s" : "WALLCHOPS %s :%s",
                     channel, buffer);
      put_it("%s", convert_output_format(
                       fget_string_var(FORMAT_BWALL_FSET), "%s %s %s %s %s",
                       update_clock(GET_TIME), channel, "*", "*", args));
...
void log_toggle(int flag, ChannelList *chan __attribute__((unused))) {
  char *logfile;

  if (((logfile = get_string_var(MSGLOGFILE_VAR)) == NULL) ||
      !get_string_var(CTOOLZ_DIR_VAR)) {
...
char *get_kill_reason(const char *target, const char *nick) {
  return random_text(DEFAULT_BITCHX_KILL_FILE, target, nick,
                     get_string_var(DEFAULT_REASON_VAR));
}
  ...
  if ((chan = lookup_channel(rptr->channel, rptr->server, 0))) {
    if ((nick = find_nicklist_in_channellist(rptr->nick, chan, 0))) {
      if (rptr->re_he.h_addr_list[0].s_addr) {
        ...
        do_nslookup(rptr->host, rptr->nick, rptr->user, rptr->channel,
                    rptr->server, auto_nslookup, NULL);
#ifdef PANA_DEBUG
      else
...
static void print_ns_succeed(struct reslist *rptr) {
  ...
  if (do_hook(NSLOOKUP_LIST, "%s %s %s %s %s", h,
              rptr->re_he.h_name ? rptr->re_he.h_name : empty_string,
              (char *)inet_ntoa(rptr->re_he.h_addr_list[0]), n, u)) {
    int i;
    *buffer = 0;
    if (rptr->nick && rptr->user)
      bitchsay("[%s!%s@%s]: %s", n, u, h,
               rptr->re_he.h_name ? rptr->re_he.h_name : "invalid hostname");
    else
      bitchsay("%s is %s (%s)", h,
               rptr->re_he.h_name ? rptr->re_he.h_name : "invalid hostname",
...
void print_ns_fail(struct reslist *rptr) {
  if (rptr->command) {
    char buffer[BIG_BUFFER_SIZE];
    if (rptr->nick && rptr->user)
      snprintf(buffer, sizeof(buffer), "%s!%s@%s ", rptr->nick, rptr->user,
               rptr->host);
    else
  ...
  if (do_hook(NSLOOKUP_LIST, "%s %s %s", rptr->host,
              rptr->nick ? rptr->nick : empty_string,
              rptr->user ? rptr->user : empty_string)) {
    if (rptr->nick && rptr->user)
      bitchsay("nslookup of %s!%s@%s failed.", rptr->nick, rptr->user,
               rptr->host);
    else
...
static int do_query_name(struct resinfo *, char *, register struct reslist *,
                         char *, char *, char *, char *, int, void (*func)(),
                         char *);
static int do_query_number(struct resinfo *, char *, register struct reslist *,
                           char *, char *, char *, char *, int, void (*func)(),
                           char *);
...
void ar_get_windows_dns(void) {
  ...
  if (ret == NO_ERROR) {
    ...
    _res.nsaddr_list[0].sin_addr.s_addr =
        inet_addr(buf->DnsServerList.IpAddress.String);
...
int ar_open(void) {
  ...
  if (_res.options & RES_USEVC) {
     ...
     */
    while (
        connect(ar_resfd, (struct sockaddr *)sip++, sizeof(struct sockaddr))) {
      (void)close(ar_resfd);
      ar_resfd = -1;
      if (i >= _res.nscount)
        break;
    ...
    if (ar_resfd > -1)
      (void)setsockopt(ar_resfd, SOL_SOCKET, SO_BROADCAST, (char *)&on,
                       sizeof(on));
  }
 ...
 */
static struct reslist *ar_make_request(register struct resinfo *resi,
                                       char *nick, char *user, char *h,
                                       char *chan, int server, void (*func)(),
                                       char *command) {
 ...
 * being resolved.
 */
long ar_timeout(time_t now, char *info, int size,
 ...
 */
static int ar_query_name(char *name, int class, int type,
                         struct reslist *rptr) {
  ...
  r = res_mkquery(QUERY, name, class, type, NULL, 0, NULL, buf, sizeof(buf));
  if (r <= 0) {
    h_errno = NO_RECOVERY;
    return r;
  }
 ...
 */
int ar_gethostbyname(char *name, char *info, int size, char *nick, char *user,
                     char *h, char *chan, int server, void (*func)(),
                     char *command) {
  char host[HOSTLEN + 1];
  ...
  return do_query_name(&resi, host, NULL, nick, user, h, chan, server, func,
                       command);
}

static int do_query_name(struct resinfo *resi, char *name,
                         register struct reslist *rptr, char *nick, char *user,
                         char *h, char *chan, int server, void (*func)(),
                         char *command) {
  char hname[HOSTLEN];
  int len;
 ...
 */
int ar_gethostbyaddr(char *addr, char *info, int size, char *nick, char *user,
                     char *h, char *chan, int server, void (*func)(),
                     char *command) {
  struct resinfo resi = {0};
  ...
  return do_query_number(&resi, addr, NULL, nick, user, h, chan, server, func,
                         command);
}
 ...
 */
static int do_query_number(struct resinfo *resi, char *numb,
                           register struct reslist *rptr, char *nick,
                           char *user, char *h, char *chan, int server,
...
static int ar_resend_query(struct reslist *rptr) {
  ...
  case T_PTR:
    ...
    return do_query_number(NULL, (char *)&rptr->re_addr, rptr, NULL, NULL, NULL,
                           NULL, -1, NULL, NULL);
  case T_A:
    ar_reinfo.re_resends++;
    return do_query_name(NULL, rptr->re_name, rptr, NULL, NULL, NULL, NULL, -1,
                         NULL, NULL);
  default:
    break;
  }
 ...
 */
static int ar_procanswer(struct reslist *rptr, HEADER *hptr, unsigned char *buf,
                         unsigned char *eob) {
  unsigned char *cp;
  char **alias;
  ...
  while (hptr->ancount-- > 0 && cp < eob) {
    ...
    case T_A:
      ...
      *adr = 0;
      ...
      break;
    case T_PTR:
      if ((n = dn_expand(buf, eob, cp, ar_hostbuf, sizeof(ar_hostbuf) - 1)) <
          0) {
        cp += n;
        continue;
      }
 ...
 */
struct hostent *ar_answer(char *reip, int size,
                          void (*func)(struct reslist *)) {
  ...
  if ((rptr->re_type == T_PTR) && (_res.options & RES_CHECKPTR)) {
     ...
     */
    ar_reinfo.re_na_look++;
    (void)strncpy(rptr->re_name, rptr->re_he.h_name, sizeof(rptr->re_name) - 1);
    rptr->re_he.h_name = NULL;
    rptr->re_retries = MAX_RETRIES; /*_res.retry + 2;*/
    rptr->re_sends = 1;
    rptr->re_resend = 1;
  ...
  if (reip && rptr->re_rinfo.ri_ptr && size)
    memcpy(reip, rptr->re_rinfo.ri_ptr, MIN(rptr->re_rinfo.ri_size, size));
...
void set_nslookupfd(fd_set *rd __attribute__((unused))) {
#ifdef WANT_NSLOOKUP
#ifdef THREAD
...
long print_nslookup(fd_set *rd __attribute__((unused))) {
#ifdef WANT_NSLOOKUP
#ifdef THREAD
  dns_check(rd);
#else
...
char *do_nslookup(char *host __attribute__((unused)), char *nick __attribute__((unused)), char *user __attribute__((unused)), char *chan __attribute__((unused)), int server __attribute__((unused)),
                  void (*func)(void) __attribute__((unused)), char *command __attribute__((unused))) {
#ifdef WANT_NSLOOKUP
#ifdef THREAD
  ...
  if (isdigit((unsigned char)*(host + strlen(host) - 1))) {
    ...
    ar_gethostbyaddr((char *)&temp1.s_addr, (char *)&ar_seq, sizeof(ar_seq),
                     nick, user, host, chan, server, func, command);
  } else {
    ar_seq++;
    ar_gethostbyname(host, (char *)&ar_seq, sizeof(ar_seq), nick, user, host,
                     chan, server, func, command);
  }
#endif /* THREAD */
#endif /* WANT_NSLOOKUP */
  return NULL;
...
void userhost_nsl(UserhostItem *stuff, char *nick, char *args) {
  ...
  do_nslookup(stuff->host, stuff->nick, stuff->user, NULL, from_server, NULL,
              (nsl && *nsl) ? nsl : NULL);
}
#endif
...
BUILT_IN_COMMAND(nslookup) {
  ...
  while ((host = next_arg(args, &args))) {
    ...
    if (!host || !*host)
      break;
    if (!strchr(host, '.'))
      userhostbase(host, userhost_nsl, 1, "%s%s%s", host,
                   cmd ? space : empty_string, cmd ? cmd : empty_string);
    else
...
char *cluster(char *hostname) {
  ...
  *result = 0;
  ...
  if (*host && isdigit((unsigned char)*(host + strlen(host) - 1))) {
    ...
    tmp = host;
    while ((size_t)(tmp - host) < strlen(host)) {
    ...
    *tmp = '\0';
    ...
    if (my_stricmp(tmp, "com") && my_stricmp(tmp, "edu") &&
        my_stricmp(tmp, "net") &&
        (stristr(host, "com") || stristr(host, "edu")))
      num = 2;
...
} sock_manager = {0, 0, -1, {{0, 0, 0, NULL, 0, 0, NULL, NULL, NULL, NULL, NULL, 0, NULL}}};
...
char *checkmenu(MenuList *check, int menuid) {
  ...
  tmpml = check;
  while (tmpml != NULL) {
    if (tmpml->menutype == GUISUBMENU || tmpml->menutype == GUISHARED ||
        tmpml->menutype == GUIBRKSUBMENU) {
...
void scan_gui(fd_set *rd) {
  ...
  if (FD_ISSET(guiipc[0], rd)) {
    ...
    case EVREFRESH:
      ...
      break;
    case EVSTRACK:
      if (newscrollerpos != lastscrollerpos ||
        ...
        newpos =
            (int)(((bleah - newscrollerpos) / bleah) *
                  (scrollwin->display_buffer_size - scrollwin->display_size)) +
            scrollwin->display_size;
...
void read_clonelist(int s) {
  ...
  case -1: {
    do_hook(SOCKET_LIST, "%d %s %d", s, sock_manager.sockets[s].server,
            sock_manager.sockets[s].port);
    close_socketread(s);
    break;
  }
  case 0:
  ...
  default: {
    ...
    if ((buffer[strlen(buffer) - 1] == '\r') ||
      ...
      buffer[strlen(buffer) - 1] = 0;
    if (*buffer)
      do_hook(SOCKET_LIST, "%d %s %d %s", s, sock_manager.sockets[s].server,
              sock_manager.sockets[s].port, buffer);
  }
  }
}
...
void read_clonenotify(int s) {
  ...
  do_hook(SOCKET_NOTIFY_LIST, "%d %s %d %d", s, sock_manager.sockets[s].server,
          sock_manager.sockets[s].port, (unsigned int)flags);
}
...
int BX_get_max_fd(void) { return sock_manager.max_fd + 1; }

int BX_add_socketread(int s, int port, unsigned long flags, char *server,
                      void (*func_read)(int), void (*func_write)(int)) {
...
int check_dcc_socket(int s) {
  if (sock_manager.count && sock_manager.sockets[s].is_read &&
      sock_manager.sockets[s].info) {
    if (((dcc_struct_type *)sock_manager.sockets[s].info)->struct_type ==
        DCC_STRUCT_TYPE)
      return 1;
  }
  return 0;
...
void scan_sockets(fd_set *rd, fd_set *wr) {
  ...
  for (i = 0; i < sock_manager.max_fd + 1; i++) {
    if (sock_manager.sockets[i].is_read &&
        FD_ISSET(sock_manager.sockets[i].is_read, rd))
      (sock_manager.sockets[i].func_read)(sock_manager.sockets[i].is_read);
    if (sock_manager.sockets[i].func_write &&
        sock_manager.sockets[i].is_write &&
        FD_ISSET(sock_manager.sockets[i].is_write, wr))
...
static void handle_socket_connect(int rc) {
  ...
  if ((getpeername(rc, (struct sockaddr *)&addr, &address_len)) != -1) {
    ...
    if (addr.sf_family == AF_INET) {
      ...
      if (getnameinfo((struct sockaddr *)&addr, sizeof(struct sockaddr_foobar),
                      hostname, 128, NULL, 0, 0))
        hostname =
            (char *)inet_ntop(AF_INET6, (void *)&(addr.sf_addr6), buf, 128);
    }
#endif
    put_it("Hostname %s port %d is running (%s)", hostname, htons(addr.sf_port),
...
static int scan(char *remote_host, int low_port, int high_port) {
  ...
  for (port = low_port; port <= high_port; port++) {
    if ((rc = connect_by_number(remote_host, &port, SERVICE_CLIENT,
                                PROTOCOL_TCP, 1)) < 0)
      continue;
  ...
  if (args && *args) {
    ...
    if (strchr(remote_host, '.') || strchr(remote_host, ':')) {
      if (resolv(remote_host)) {
        bitchsay("Scanning %s's tcp ports %d through %d", remote_host, low_port,
                 high_port);
        scan(remote_host, low_port, high_port);
      } else
        bitchsay("No such host %s", remote_host);
    } else
      userhostbase(remote_host, userhost_scanport, 1, "%s %d %d", remote_host,
                   low_port, high_port);
  }
  return;
}
...
void userhost_ignore(UserhostItem *uhi, char *nick1, char *args) {
  ...
  if (uhi && uhi->nick && strcmp(uhi->user, "<UNKNOWN>") &&
      !my_stricmp(uhi->nick, nick1)) {
    nick = uhi->nick;
    user = uhi->user;
    host = uhi->host;
  ...
  if (!arg || !*arg || !my_stricmp(arg, "+HOST"))
    snprintf(ignorebuf, sizeof(ignorebuf), "*!*@%s ALL -CRAP -PUBLIC",
             cluster(host));
  else if (!my_stricmp(arg, "+USER"))
    snprintf(ignorebuf, sizeof(ignorebuf), "*%s@%s ALL -CRAP -PUBLIC", user,
             cluster(host));
    ...
    while (igptr != NULL) {
      igtmp = igptr->next;
      if (wild_match(igptr->nick, ignorebuf) || wild_match(nick, igptr->nick)) {
...
BUILT_IN_COMMAND(reset) { refresh_screen(0, NULL); }
...
BUILT_IN_COMMAND(cycle) {
  ...
  if (!(chan = prepare_command(&server, to, NO_OP)))
    return;
  my_send_to_server(server, "PART %s\nJOIN %s%s%s", chan->channel,
                    chan->channel, chan->key ? space : empty_string,
                    chan->key ? chan->key : empty_string);
}

int do_newuser(char *command __attribute__((unused)), char *args, char *subargs) {
...
BUILT_IN_COMMAND(do_ig) {
  ...
  while ((nick = next_arg(args, &args))) {
    if (!nick || !*nick)
      goto bad_ignore;
    if (*nick == '-' || *nick == '+') {
      if (!my_stricmp(nick, "-USER") || !my_stricmp(nick, "+HOST") ||
          !my_stricmp(nick, "+USER") || !my_stricmp(nick, "-HOST"))
    ...
    if (need_time)
      userhostbase(nick, userhost_ignore, 1, "%s %d", ignore_type,
                   get_int_var(IGNORE_TIME_VAR));
    else
...
void put_user(const NickList *nick, const char *channel) {
  ...
  put_it("%s", convert_output_format(
                   users_format, "%s %s %s %s %d %c",
#ifdef WANT_USERLIST
                   nick->userlist   ? convert_flags(nick->userlist->flags)
                   : nick->shitlist ? ltoa(nick->shitlist->level)
                                    : "n/a",
#else
                   "n/a",
#endif
                   channel, nick->nick, nick->host, nick->serverhops,
                   nick_isop(nick)      ? '@'
                   : nick_isvoice(nick) ? 'v'
                                        : ' '));
}
...
BUILT_IN_COMMAND(users) {
  ...
  NickList *nicks;
  NickList *sortl = NULL;
  char *to = NULL, *spec = "*!*@*", *temp1 = NULL;
  char modebuf[BIG_BUFFER_SIZE + 1];
  char msgbuf[BIG_BUFFER_SIZE + 1];
  int count = 0, ops = 0, msg = 0, num_kicks = 0, hook = 0, not = 0,
      server = from_server, sorted = NICKSORT_NORMAL;
  ...
  *modebuf = 0;
  ...
  if (!spec || !*spec)
    spec = "*!*@*";
  if (!(chan = prepare_command(&server, to, NO_OP))) {
    bitchsay(to ? "Not on that channel %s" : "No such channel %s",
             to ? to : empty_string);
    return;
  }
  ...
  switch (msg) {
  case 6:
    if (do_hook(STAT_HEADER_LIST, "%s %s %s %s %s", "Nick", "dops", "kicks",
                "nicks", "publics"))
  ...
  case 0: {
    ...
    if ((f = fget_string_var(FORMAT_USERS_TITLE_FSET)))
      put_it("%s", convert_output_format(f, "%s %s", update_clock(GET_TIME),
                                         chan->channel));
  }
  default:
    break;
  ...
  for (nicks = sortl; nicks; nicks = nicks->next) {
    ...
    if (((!not&&wild_match(spec, modebuf)) ||
         (not&&!wild_match(spec, modebuf))) &&
        (!ops || ((ops == 1) && nick_isop(nicks)) ||
      ...
      case 4: /* kill */
      {
        if (!isme(nicks->nick))
          my_send_to_server(
              server, "KILL %s :%s (%i", nicks->nick,
              args && *args ? args
                            : get_kill_reason(nicks->nick,
      ...
      case 5: /* mass kick */
      {
        ...
        if ((get_int_var(NUM_KICKS_VAR) &&
             (num_kicks == get_int_var(NUM_KICKS_VAR))) ||
            strlen(msgbuf) >= 450) {
          my_send_to_server(server, "KICK %s %s :%s", chan->channel, msgbuf,
                            (args && *args) ? args : "-=punt=-");
          *msgbuf = 0;
          num_kicks = 0;
        }
      ...
      case 6: {
        if (!isme(nicks->nick)) {
          if (do_hook(STAT_LIST, "%s %d %d %d %d", nicks->nick, nicks->dopcount,
                      nicks->kickcount, nicks->nickcount, nicks->floodcount))
            put_it("%-10s  %4d   %4d   %4d     %4d", nicks->nick,
                   nicks->dopcount, nicks->kickcount, nicks->nickcount,
                   nicks->floodcount);
        }
        break;
      }
      case 7:
        if (do_hook(USERS_IP_LIST, "%s %s %s", nicks->nick, nicks->host,
                    nicks->ip ? nicks->ip : "Unknown"))
          put_it("%s!%s = %s", nicks->nick, nicks->host,
                 nicks->ip ? nicks->ip : "Unknown");
        break;
      case 1:
      case 2: {
        ...
        if (strlen(msgbuf) + strlen(args) >= 490) {
          put_it("%s",
                 convert_output_format(
                     fget_string_var((msg == 1) ? FORMAT_SEND_MSG_FSET
                                                : FORMAT_SEND_NOTICE_FSET),
                     "%s %s %s %s", update_clock(GET_TIME), msgbuf,
                     get_server_nickname(from_server), args));
          my_send_to_server(server, "%s %s :%s",
                            (msg == 1) ? "PRIVMSG" : "NOTICE", msgbuf, args);
          *msgbuf = 0;
        }
        break;
      }
      default: {
        if (!count &&
            do_hook(USERS_HEADER_LIST, "%s %s %s %s %s %s %s", "Level", "aop",
                    "prot", "Channel", "Nick", "+o", "UserHost") &&
            fget_string_var(FORMAT_USERS_HEADER_FSET))
          put_it("%s", convert_output_format(
                           fget_string_var(FORMAT_USERS_HEADER_FSET), "%s",
                           chan->channel));

        if ((hook = do_hook(USERS_LIST, "%lu %s %s %s %d %c",
                            (unsigned long)(nicks->userlist   ? nicks->userlist->flags
                            : nicks->shitlist ? (unsigned long)nicks->shitlist->level
                                              : 0UL),
                            chan->channel, nicks->nick, nicks->host,
                            nicks->serverhops,
                            nick_isop(nicks)      ? '@'
                            : nick_isvoice(nicks) ? 'v'
                                                  : ' '))) {
  ...
  if (count && *msgbuf) {
    ...
    case 2: {
      put_it("%s", convert_output_format(
                       ...
                       "%s %s %s %s", update_clock(GET_TIME), msgbuf,
                       get_server_nickname(from_server), args));
      my_send_to_server(server, "%s %s :%s", (msg == 1) ? "PRIVMSG" : "NOTICE",
                        msgbuf, args);
      break;
    }
    case 5: {
...
#define COF_BUFFER_FREE(p)                                                     \
  (cof_buffer[cparse_recurse] + cof_buffer_sz[cparse_recurse] - (p))
  ...
  if (cparse_recurse >= MAX_RECURSE) {
    yell(
        "cparse_recurse() recursed too many times!  this should never happen!");
    return empty_string;
  }
  cparse_recurse++;
  ...
  while (*tmpc) {
    ...
    if (*tmpc == '%') {
      ...
      if (isdigit((unsigned char)*tmpc)) {
        ...
        strlcpy(s, make_timestamp(do_timestamp, timestamp_str),
                COF_BUFFER_FREE(s));
        while (*s)
          s++;
        tmpc++;
...
#ifdef RECURSE_CPARSE
        subformat =
            convert_output_format_raw((const char *)new_str, NULL, VA_NULL);
#else
        subformat = new_str;
#endif
        /* Ensure sufficient buffer space */
        if ((size_t)COF_BUFFER_FREE(s) < strlen(subformat) + 1) {
          size_t s_pos = s - cof_buffer[cparse_recurse];

          cof_buffer_sz[cparse_recurse] += BIG_BUFFER_SIZE + strlen(subformat);
          RESIZE(cof_buffer[cparse_recurse], char,
                 cof_buffer_sz[cparse_recurse]);
...
#ifdef GUI
/* This cut-down version of convert_output_format is used by the GUI menu
 * functions. */
...
void add_last_type(LastMsg *array, int size, const char *from, const char *uh,
...
int check_last_type(LastMsg *array, int size, char *from, char *uh) {
  int i;
  for (i = 0; i < size - 1; i++) {
    if (array[i].from && array[i].uh && !my_stricmp(from, array[i].from) &&
        !my_stricmp(uh, array[i].uh))
      return 1;
  }
  return 0;
}
...
ChannelList *BX_prepare_command(int *active_server, char *channel, int flags) {
  ...
  *active_server = server;
  ...
  need_op =
      flags == NEED_OP || (flags == PC_TOPIC && (chan->mode & MODE_TOPIC));
...
void check_server_connect(int server __attribute__((unused))) {
  ...
  if (server == -2)
    return;
  if ((server == -1) ||
      (!get_server_in_timed(server) && get_server_lastmsg(server) + 50 < now)) {
    if (!timer_callback_exists(timed_server)) {
      add_timer(0, empty_string, 10 * 1000, 1, timed_server,
                m_strdup(ltoa(server)), NULL, current_window->refnum,
                "reconnect");
