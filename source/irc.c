...
int irc_port = IRC_PORT,                      /* port of ircd */
    ...
    background = 0, do_check_pid = 0, do_ignore_ajoin = 0,
#ifdef HAVE_LIBSSL
    do_use_ssl = 0,
#endif
    run_level = 0,

    foreground = 1, reconnect = 0, /* reconnecting to old process */
    use_nat_address = 0,           /* use NAT address */
    term_initialized = 0;

char zero[] = "0", one[] = "1", space[] = " ", space_plus[] = " +",
     space_minus[] = " -", dot[] = ".", star[] = "*", comma[] = ",",
     empty_string[] = "", on[] = "ON", off[] = "OFF";
...
int inbound_line_mangler = 0, logfile_line_mangler = 0,
    operlog_line_mangler = 0, outbound_line_mangler = 0;
                ...
                *args_str = NULL,             /* list of command line args */
                    *last_notify_nick = NULL, /* last detected nickname */
                        *auto_str = NULL,     /* auto response str */
                            *new_script =
                                NULL, /* rephacement for .bitchxrc and .ircrc */
...
time_t idle_time = 0, now = 0, start_time;
...
static const char *const switch_help =
    ...
    "  The [server list] is a whitespace-separated list of server "
    "descriptions.  Each\n"
    "    server description is of the form "
    "hostname[:port[:password[:nick[:network]]]].\n"
    ...
    "   -c <channel>\tjoins <channel> on startup. don\'t forget to escape the "
    "# using \\\n"
    "   -b\t\tload " BITCHXRC_NAME " or " IRCRC_NAME
...
char *BX_update_clock(int flag) {
  ...
  if (flag == RESET_TIME || time_val.tm_min != min ||
...
void reset_clock(Window *win, char *unused __attribute__((unused)), int unused1 __attribute__((unused))) {
  update_clock(RESET_TIME);
  update_all_status(win, NULL, 0);
}

/* sig_refresh_screen: the signal-callable version of refresh_screen */
static SIGNAL_HANDLER(sig_refresh_screen) { refresh_screen(0, NULL); }

/* irc_exit: cleans up and leaves */
static SIGNAL_HANDLER(irc_exit_old) { irc_exit(1, NULL, NULL); }
...
static SIGNAL_HANDLER(nothing) { /* nothing to do! */ }
...
void BX_irc_exit(int really_quit, char *reason, char *format, ...) {
  ...
  if (really_quit) {
    put_it("%s", convert_output_format("$G Signon time  :    $0-", "%s",
                                       my_ctime(start_time)));
    put_it("%s", convert_output_format("$G Signoff time :    $0-", "%s",
                                       my_ctime(now)));
    put_it("%s", convert_output_format("$G Total uptime :   $0-", "%s",
...
#ifdef TDEBUG
  putlog(LOG_ALL, "*", "Error logged. %s at (%d)  %s", cx_file, cx_line,
         cx_function ? cx_function : empty_string);
#endif
  printf("\n\r\n\rIRCII has been terminated by a [%s]\n\r",
         sys_siglist[unused]);
  printf("Please email " BUG_EMAIL "\n\r");
  printf("with as much detail as possible about what you were doing when it "
         "happened.\n\r");
  printf("Please include the version of IRCII (%s) and type of system in the "
         "report.\n\r",
         irc_version);
  fflush(stdout);
 ...
 */
static void quit_response(char *dummy __attribute__((unused)), char *ptr) {
  int len;
...
void irc_quit(char key __attribute__((unused)), char *ptr __attribute__((unused))) {
  static int in_it = 0;

  if (in_it)
    return;
  in_it = 1;
  add_wait_prompt("Do you really want to quit? ", quit_response, empty_string,
                  WAIT_PROMPT_LINE, 1);
  in_it = 0;
}
...
static SIGNAL_HANDLER(sig_detach) { detachcmd(NULL, NULL, NULL, NULL); }

void set_detach_on_hup(Window *dummy __attribute__((unused)), char *unused __attribute__((unused)), int value) {
  if (value)
...
static char *parse_args(char *argv[], int argc, char **envp __attribute__((unused))) {
  int ac;
  int add_servers = 0;
#if !defined(WINNT)
  struct passwd *entry;
  ...
  *nickname = 0;
   ...
   */
  if (!*username && (ptr = getenv("IRCUSER")))
    strlcpy(username, ptr, sizeof username);
  else if (!*username && (ptr = getenv("USER")))
    strlcpy(username, ptr, sizeof username);
  else if (!*username) {
#ifdef IDENT_FAKE
    char *p = NULL, *q = NULL;
  ...
  if (!LocalHostName &&
      ((ptr = getenv("IRC_HOST")) || (ptr = getenv("IRCHOST"))))
...
void BX_io(const char *what) {
  ...
  struct timeval my_now, my_timer, timeout;
  int hold_over, rc;
  fd_set rd, wd;
  ...
  if (x_debug & DEBUG_WAITS) {
    if (level != old_level) {
      yell("Moving from io level [%d] to level [%d] from [%s]", old_level,
           level, what);
      old_level = level;
    }
  }
  ...
  if (level && (level - last_warn == 5)) {
    last_warn = level;
    yell("io's nesting level is [%d],  [%s]<-[%s]<-[%s]<-[%s]<-[%s]<-[%s]",
         level, what, caller[level - 1], caller[level - 2], caller[level - 3],
         caller[level - 4], caller[level - 5]);
    if ((level % 30) == 0)
...
#ifdef WANT_LLOOK
  if (get_int_var(LLOOK_VAR) && from_server > -1 &&
      !get_server_linklook(from_server)) {
    if (now - get_server_linklook_time(from_server) >
        get_int_var(LLOOK_DELAY_VAR)) {
...
int main(int argc, char *argv[], char *envp[]) {
  ...
  *cx_function = 0;
  ...
  if (!dumb_mode) {
    ...
    if (background) {
      use_input = 0;
      main_screen->fdin = main_screen->fdout =
    ...
    if (background) {
      ...
      term_reset();
      fprintf(stderr,
              "try recompiling the client with WANT_DETACH defined\r\n");
      exit(0);
      use_input = 0;
#endif
    }
