...
#define BUILT_IN_KEYBINDING(x)                                                 \
  void x(char key __attribute__((unused)),                                     \
         char *string __attribute__((unused)))

#define HOLDLAST current_screen->current_window->screen_hold

Display *get_screen_hold(Window *win) { return win->screen_hold; }

static int safe_puts(register char *str, int len, int echo __attribute__((unused))) {
  int i = 0;
  while (*str && i < len) {
    term_putchar(*str);
    str++;
 ...
 */
char *BX_get_input(void) { return (&(MIN_CHAR)); }
...
extern char *BX_get_input_prompt(void) { return input_prompt; }
 ...
 */
void BX_set_input_prompt(Window *win __attribute__((unused)), char *prompt, int unused __attribute__((unused))) {
  if (prompt)
...
BUILT_IN_KEYBINDING(input_forward_word) {
  ...
  while ((my_isspace(THIS_CHAR) || ispunct((unsigned char)THIS_CHAR)) &&
         (THIS_CHAR))
    THIS_POS++;
  while (!(ispunct((unsigned char)THIS_CHAR) || my_isspace(THIS_CHAR)) &&
         (THIS_CHAR))
    THIS_POS++;
...
BUILT_IN_KEYBINDING(input_backward_word) {
  cursor_to_input();
  while ((THIS_POS > MIN_POS) &&
         (my_isspace(PREV_CHAR) || ispunct((unsigned char)PREV_CHAR)))
    THIS_POS--;
  while ((THIS_POS > MIN_POS) &&
         !(ispunct((unsigned char)PREV_CHAR) || my_isspace(PREV_CHAR)))
    THIS_POS--;
...
BUILT_IN_KEYBINDING(input_delete_next_word) {
  ...
  cursor_to_input();
  pos = THIS_POS;
  while ((my_isspace(INPUT_BUFFER[pos]) ||
          ispunct((unsigned char)INPUT_BUFFER[pos])) &&
         INPUT_BUFFER[pos])
    pos++;
  while (!(ispunct((unsigned char)INPUT_BUFFER[pos]) ||
           my_isspace(INPUT_BUFFER[pos])) &&
         INPUT_BUFFER[pos])
    pos++;
  c = INPUT_BUFFER[pos];
...
BUILT_IN_KEYBINDING(input_transpose_characters) {
  ...
  if (last_input_screen->buffer_pos > MIN_POS) {
    ...
    if (THIS_CHAR)
      pos = THIS_POS;
    else if (strlen(get_input()) > (size_t)MIN_POS + 2) {
      pos = THIS_POS - 1;
      end_of_line = 1;
    } else
      return;
...
BUILT_IN_KEYBINDING(refresh_inputline) { update_input(UPDATE_ALL); }
...
/* BIND functions: */
BUILT_IN_KEYBINDING(forward_character) { input_move_cursor(RIGHT); }

BUILT_IN_KEYBINDING(backward_character) { input_move_cursor(LEFT); }

BUILT_IN_KEYBINDING(backward_history) { get_history(PREV); }

BUILT_IN_KEYBINDING(forward_history) { get_history(NEXT); }
  ...
  if ((cmd && *cmd == *cmdchar && got_space) || !cmd) {
    ...
    if (nick) {
      ...
      if (fget_string_var(FORMAT_NICK_MSG_FSET))
        malloc_strcpy(&tmp, stripansicodes(convert_output_format(
                                fget_string_var(FORMAT_NICK_MSG_FSET),
                                "%s%s %s %s", cmdchar,
                                nick->type ? nick->type
                                : cmd      ? cmd
                                           : "msg",
                                nick->nick, line ? line : empty_string)));
      else
        malloc_sprintf(&tmp, "%s%s %s %s", cmdchar,
                       nick->type ? nick->type
                       : cmd      ? cmd
                                  : "msg",
                       nick->nick, line ? line : empty_string);
...
void add_autonick_input(char *nick, char *line) {
  ...
  if ((do_hook(REPLY_AR_LIST, "%s", nick))) {
    if (fget_string_var(FORMAT_NICK_AUTO_FSET))
      malloc_strcpy(&tmp1, stripansicodes(convert_output_format(
                               fget_string_var(FORMAT_NICK_AUTO_FSET), "%s %s",
                               nick, line ? line : empty_string)));
    else
...
BUILT_IN_KEYBINDING(send_line) {
  ...
  if (last_input_screen->promptlist &&
    ...
    if (line && (*line != *cmdchar) && get_int_var(NICK_COMPLETION_VAR) &&
        !current_window->query_nick) {
      char auto_comp_char;
      char *p;
      ...
      if ((p = strchr(tmp, auto_comp_char)) &&
          do_hook(NICK_COMP_LIST, "%s", line)) {
        ChannelList *chan;
        NickList *nick;
        char *channel;
        *p++ = 0;
        if (*tmp && *p &&
            ((size_t)strlen(tmp) >= (size_t)get_int_var(NICK_COMPLETION_LEN_VAR) &&
          ...
          if (nick) {
            if (fget_string_var(FORMAT_NICK_COMP_FSET))
              malloc_strcpy(&tmp, stripansicodes(convert_output_format(
                                      fget_string_var(FORMAT_NICK_COMP_FSET),
                                      "%s %s", nick->nick, p)));
            else
              malloc_sprintf(&tmp, "%s\002%c\002%s", nick->nick, auto_comp_char,
                             p);
          } else
...
#define METAX(x)                                                               \
  BUILT_IN_KEYBINDING(meta##x##_char) { last_input_screen->meta_hit = (x); }
METAX(39)
METAX(38) METAX(37) METAX(36) METAX(35) METAX(34) METAX(33) METAX(32) METAX(31)
    METAX(30) METAX(29) METAX(28) METAX(27) METAX(26) METAX(25) METAX(24)
        METAX(23) METAX(22) METAX(21) METAX(20) METAX(19) METAX(18) METAX(17)
            METAX(16) METAX(15) METAX(14) METAX(13) METAX(12) METAX(11)
                METAX(10) METAX(9) METAX(8) METAX(7) METAX(6) METAX(5) METAX(3)
                    METAX(2) METAX(1)
...
BUILT_IN_KEYBINDING(quote_char) { last_input_screen->quote_hit = 1; }
 ...
 */
BUILT_IN_KEYBINDING(insert_bold) { input_add_character(BOLD_TOG, string); }

BUILT_IN_KEYBINDING(insert_reverse) { input_add_character(REV_TOG, string); }

BUILT_IN_KEYBINDING(insert_underline) { input_add_character(UND_TOG, string); }

BUILT_IN_KEYBINDING(highlight_off) { input_add_character(ALL_OFF, string); }

BUILT_IN_KEYBINDING(insert_blink) { input_add_character(BLINK_TOG, string); }

BUILT_IN_KEYBINDING(insert_altcharset) { input_add_character(ALT_TOG, string); }
...
/*
 * input_clear_screen: the CLEAR_SCREEN function for BIND.  Clears the screen
 * and starts it if it is held
 */
...
static void handle_swap(int windownum) {
  ...
  set_channel_window(current_window,
                     get_current_channel_by_refnum(current_window->refnum),
...
#ifdef WANT_CHELP
BUILT_IN_KEYBINDING(w_help) { chelp(NULL, "WINDOW", NULL, NULL); }
#endif
...
BUILT_IN_KEYBINDING(wholeft) { show_wholeft(NULL); }
...
char *BX_getchannick(char *oldnick, char *nick) {
  ...
  if (channel) {
    ...
    if (oldnick) {
      ...
      for (cnick = next_nicklist(chan, NULL); cnick;
      ...
      /* we have a new nick */
        ...
        for (; cnick; cnick = next_nicklist(chan, cnick)) {
          ...
          if (!my_strnicmp(cnick->nick, nick, strlen(nick)) ||
              my_strnstr(cnick->nick, nick, strlen(nick)))
#else
          if (!my_strnicmp(cnick->nick, nick, strlen(nick)))
#endif
          {
...
NickTab *BX_getnextnick(int which, char *input_nick, char *oldnick,
                        char *nick) {
  ChannelList *chan;
  NickList *cnick = NULL;
  ...
  if ((chan = prepare_command(&server, NULL, PC_SILENT))) {
    ...
    if (input_nick) {
      ...
      /* we have a new nick */
         ...
         */
        for (; cnick; cnick = next_nicklist(chan, cnick)) {
          if (!my_strnicmp(cnick->nick, input_nick, strlen(input_nick)) ||
              !strcasecmp(cnick->nick, input_nick))
            break;
        }
      }
    } else if (in_completion == STATE_CNICK) {
...
struct _name_type name_type[] = {
    {"MP3", 3},  {"DCC", 3}, {"EXEC", 4}, {"LS", 2}, {"LOAD", 4}, {"SERVER", 6},
    {"CDCC", 4}, {"MSG", 1}, {"KB", 2},   {"CD", 2}, {"", 0}};
...
char *get_completions(enum completion type, char *possible, int *count,
                      char **suggested) {
  char *booya = NULL;
  char *path = NULL;
  char *path2, *freeme;
  ...
  case SERVER_COMPLETION: {
    for (i = 0; i < server_list_size(); i++) {
      if (possible &&
          my_strnicmp(possible, get_server_name(i), strlen(possible)))
        continue;
  ...
  case CHAN_COMPLETION: {
    ...
    if (possible && (*possible == '#' || *possible == '&')) {
      ...
      for (chan = get_server_channels(current_window->server); chan;
           chan = chan->next) {
        if ((len == 1) ||
            (len >= 2 && !my_strnicmp(possible, chan->channel, len))) {
          (*count)++;
          m_s3cat(&booya, space, chan->channel);
        }
      }
...
#elif defined(WINNT) /* Might need something for WinNT itself                  \
  ...
  case DCC_COMPLETION: {
    ...
    if (!possible)
      path =
          m_sprintf("%s%s%s", dl, strrchr(dl, '/') ? empty_string : "/", "*");
    else {
      if (*possible == '~' && (*(possible + 1) != '/')) {
        if ((booya = get_home_dir(possible, count)))
          return booya;
 ...
 * array elements.
 */
void set_input_best(enum completion type, char *inp, char *old, int count,
                    char **completes) {
  int i, c, j = 0, match = 0;
  ...
  if (count == 2) {
#if !defined(__EMX__) && !defined(WINNT)
    if (type == CHAN_COMPLETION || type == NICK_COMPLETION ||
  ...
  if (old && *old) {
    if (strchr(completes[0], ' '))
      snprintf(buffer, sizeof buffer, "%s%s\"%s\"", inp, space,
               i ? completes[0] : old);
    else
      snprintf(buffer, sizeof buffer, "%s%s%s", inp, space,
               i ? completes[0] : old);
    ...
    if (count == 1) {
      ...
      if (type == TABKEY_COMPLETION)
        snprintf(buffer, sizeof buffer, "%s %s%s%s ",
                 (p && *p == '/') ? p : "/m", get,
                 (p && (*p != '/')) ? space : empty_string,
                 (p && (*p != '/')) ? p : empty_string);
      else {
        if (wcount == 1 && got_space)
          snprintf(buffer, sizeof buffer, "%s %s ", get_input(), get);
        else
          snprintf(buffer, sizeof buffer, "%s%s%s ", p ? p : get,
                   p ? space : empty_string, p ? get : empty_string);
      }
      if ((type == CDCC_COMPLETION || type == LOAD_COMPLETION ||
           type == FILE_COMPLETION || type == DCC_COMPLETION) ||
          ((type == EXEC_COMPLETION) && (get[strlen(get) - 1] == '/')))
        chop(buffer, 1);
      set_input(buffer);
      if (strchr(get, ' '))
        THIS_POS = strlen(INPUT_BUFFER) - 1;
      ...
      case EXEC_COMPLETION: {
        ...
        *path = 0;
        ...
        while (n && *n) {
          ...
          if (++count == 4) {
            put_it("%s",
                   convert_output_format(fget_string_var(FORMAT_COMPLETE_FSET),
                                         "%s", buffer));
            count = 0;
            *buffer = 0;
          }
