...
char *BX_next_unit(char *str, const char *args, int *arg_flag, int stage) {
  ...
  char *ptr2,              /* used to point matching brackets */
      ...
      *lastc,              /* used to denote end of token-set */
          *tmp = NULL, op; /* the op were working on */
...
#define SETUP_FLOAT_OPERATION(STAGE) SETUP(dvalue1, dvalue2, atof, STAGE)

/* This does a setup for an integer operation. */
#define SETUP_INTEGER_OPERATION(STAGE) SETUP(value1, value2, my_atol, STAGE)
...
<hop> ive heard of middle enddian which is  CDAB
      ...
      if (*str) {
        /* array qualifier */
        int size = strlen(str) + (result1 ? strlen(result1) : 0) +
                   (ptr ? strlen(ptr) : 0) + 2;
 ...
 */
char *BX_expand_alias(const char *string, const char *args, int *args_flag,
                      char **more_text) {
  char *buffer = NULL, *ptr, *stuff = NULL, *free_stuff, *quote_str = NULL;
  char quote_temp[2];
  char ch;
  int is_quote = 0;
  int unescape = 1;
...
char *call_structure(char *name, const char *args, int *args_flag, char *rest,
                     char *rest1) {
 ...
 */
char *BX_alias_special_char(char **buffer, char *ptr, const char *args,
                            char *quote_em, int *args_flag) {
  char *tmp, *tmp2, pad_char = 0;
    ...
    if ((tmp = MatchingBracket(ptr, '[', ']'))) {
      *tmp = 0;
      if (*ptr == '$') {
        ...
        slen = strlen(str);
        if (slen && !isdigit((unsigned char)str[slen - 1]))
          pad_char = str[slen - 1];
   ...
   */
  case LEFT_PAREN: {
    char *sub_buffer = NULL, *tmp2 = NULL, *tmpsav = NULL, *ph = ptr + 1;

    if ((ptr = MatchingBracket(ph, '(', ')')) || (ptr = strchr(ph, ')')))
      *ptr++ = 0;
    else
  ...
  case '\'': {
    ...
    add_wait_prompt(tmp, do_alias_string, NULL,
                    (c == DOUBLE_QUOTE) ? WAIT_PROMPT_LINE : WAIT_PROMPT_KEY,
                    1);
    while (!alias_string)
      io("Input Prompt");
 ...
 */
static void TruncateAndQuote(char **buff, const char *add, int length,
                             const char *quote_em, char pad_char) {
  /*
   * Semantics:
  ...
  if (length) {
    ...
    strformat(buffer, add, length,
              pad_char ? pad_char : get_int_var(PAD_CHAR_VAR));
    add = buffer;
  }
  if (quote_em && add) {
