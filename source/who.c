...
static void who_queue_list(void) {
  ...
  while (item) {
    yell("[%d] [%d] [%s] [%s] [%s]", count, item->who_mask,
 ...
 * parse.c
 */
BUILT_IN_COMMAND(whocmd) { whobase(args, NULL, NULL, NULL); }

/*
 * whobase: What does all the work.
 */
void BX_whobase(char *args, void (*line)(WhoEntry *, char *, char **),
                void (*end)(WhoEntry *, char *, char **), char *format, ...) {
...
void whoreply(char *from, char **ArgList) {
  ...
  if (!new_w && !who_whine) {
    ...
    if (do_hook(WHO_LIST, "%s %s %s %s %s %s %s", channel, nick, stat, user,
                host, server, name))
      put_it("%s", convert_output_format(fget_string_var(FORMAT_WHO_FSET),
                                         "%s %s %s %s %s %s %s", channel, nick,
  ...
  do {
    ...
    *buf_data = 0;
    ...
    if (*stat == 'S') /* this only true for the header WHOREPLY */
    {
      ...
      channel = "Channel";
      snprintf(buffer, sizeof buffer, "%s %s %s %s %s %s %s", channel, nick,
               stat, user, host, server, name);
      set_display_target(channel, LOG_CRAP);
      if (new_w->who_stuff)
        ; /* munch it */
        ...
        put_it("%s",
               convert_output_format(fget_string_var(FORMAT_WHO_FSET),
                                     "%s %s %s %s %s %s %s", channel, nick,
                                     stat, user, host, server, name));
      reset_display_target();
      return;
    ...
    if (ok) {
      ...
      set_display_target(channel, LOG_CRAP);
      add_to_channel(channel, nick, from_server, opped, voice, buf_data, server,
                     stat, 0, my_atol(name));
      if (new_w->who_stuff)
        parse_line(NULL, new_w->who_stuff, buffer, 0, 0, 1);
      else if (!in_join_list(channel, from_server) &&
               do_hook(WHO_LIST, "%s", buffer)) {
      ...
      if (get_int_var(AUTO_NSLOOKUP_VAR))
        do_nslookup(host, nick, user, channel, from_server, auto_nslookup,
                    NULL);
#endif
      reset_display_target();
    }
    ...
    if (new_w->end) {
      ...
      if (new_w->who_end)
        parse_line(NULL, new_w->who_end, buffer, 0, 0, 1);
               ...
               do_hook(current_numeric, "%s", buffer))
 ...
 * actually an ISON.
 */
void ison_returned(char *from __attribute__((unused)), char **ArgList) {
 ...
 */
BUILT_IN_COMMAND(userhostcmd) { userhostbase(args, NULL, 1, NULL); }

BUILT_IN_COMMAND(useripcmd) { userhostbase(args, NULL, 0, NULL); }

BUILT_IN_COMMAND(usripcmd) { userhostbase(args, NULL, 2, NULL); }

void BX_userhostbase(char *args, void (*line)(UserhostItem *, char *, char *),
                     int userhost, char *format, ...) {
  ...
  *buffer = 0;
  while ((nick = next_arg(args, &args)) != NULL) {
    if (check_nickname(nick)) {
      ...
      if (!total) {
        say("%s -cmd with no nicks specified", userhost == 1   ? "USERHOST"
                                               : userhost == 0 ? "USERIP"
                                                               : "USRIP");
        return;
      }
 ...
 */
void userhost_returned(char *from __attribute__((unused)), char **ArgList) {
  ...
  while (ptr && *ptr) {
    ...
    if (ArgList && *ArgList &&
      ...
      *item.host++ = 0;
       ...
       */
      else if (do_hook(current_numeric, "%s %s %s %s %s", item.nick,
                       item.oper ? "+" : "-", item.away ? "-" : "+", item.user,
                       item.host))
        put_it("%s %s is %s@%s%s%s", numeric_banner(), item.nick, item.user,
               item.host, item.oper ? " (Is an IRC operator)" : empty_string,
...
void userhost_cmd_returned(UserhostItem *stuff, char *nick __attribute__((unused)), char *text) {
