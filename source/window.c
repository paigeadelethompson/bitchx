...
static Window *window_next(Window *window, char **args, char *usage __attribute__((unused)));
static Window *window_previous(Window *window, char **args, char *usage __attribute__((unused)));
...
void BX_delete_window(Window *window) {
   ...
   */
  if (!dead && window->screen &&
      window->screen->last_window_refnum == window->refnum)
   ...
   */
  {
    ...
    if (window->display_buffer_size != 0)
      ircpanic("display_buffer size is %d. should be 0",
               window->display_buffer_size);
  }
...
static void swap_window(Window *v_window, Window *window) {
  ...
  window->display_size =
      v_window->display_size + v_window->double_status - window->double_status;
...
void BX_move_window(Window *window, int offset) {
  ...
  last = NULL;
  for (pos = 0, tmp = window->screen->window_list; pos != win_pos;
       tmp = tmp->next, pos++)
    last = tmp;
  if (!last)
    window->screen->window_list = window;
...
void BX_recalculate_windows(Screen *screen) {
  ...
  if (screen && !screen->current_window) {
    int display_size =
        current_term->li - 2 - screen->window_list->double_status;
...
void BX_swap_last_window(char key __attribute__((unused)), char *ptr __attribute__((unused))) {
...
void BX_swap_next_window(char key __attribute__((unused)), char *ptr __attribute__((unused))) {
...
void BX_swap_previous_window(char key __attribute__((unused)), char *ptr __attribute__((unused))) {
...
char *BX_get_status_by_refnum(unsigned refnum, unsigned line) {
  ...
  if ((the_window = get_window_by_refnum(refnum))) {
    if ((int)line > the_window->double_status)
      return empty_string;
 ...
 */
void BX_next_window(char key __attribute__((unused)), char *ptr __attribute__((unused))) {
  Window *w;
 ...
 * window
 */
void BX_previous_window(char key __attribute__((unused)), char *ptr __attribute__((unused))) {
  Window *w;
 ...
 */
void BX_update_all_status(Window *win __attribute__((unused)), char *unused __attribute__((unused)), int unused1 __attribute__((unused))) {
  Window *window = NULL;
...
Window *BX_get_window_target_by_desc(char *name) {
  ...
  while ((traverse_all_windows(&tmp))) {
    if (is_channel(name) && tmp->server != -1) {
      ...
      if ((chan =
               (ChannelList *)find_in_list((List **)&chan2, (char *)name, 0)))
 ...
 */
int BX_is_current_channel(const char *channel, int server __attribute__((unused)), int delete) {
  Window *tmp = NULL;
  int found = 0;

  while ((traverse_all_windows(&tmp))) {
    if (tmp->current_channel && !my_stricmp(channel, tmp->current_channel) &&
 ...
 */
const char *BX_set_current_channel_by_refnum(unsigned int refnum,
                                             char *channel) {
  Window *tmp;
  char *oldc;
  ...
  do_hook(SWITCH_CHANNELS_LIST, "%d %s %s", refnum, oldc ? oldc : zero,
...
void BX_window_check_servers(int unused __attribute__((unused))) {
  Window *tmp;
 ...
 */
void BX_save_display_target(const char **saved_from,
...
/* restore_message_from: restores a previously saved who_from variable */
void BX_restore_display_target(const char *saved_from,
                               unsigned long saved_level) {
  who_from = saved_from;
  ...
  if (who_from) {
    ...
    while (traverse_all_windows(&tmp)) {
      ...
      if ((tmp->window_level & LOG_DEBUG) == LOG_DEBUG)
        continue;
      if (tmp->current_channel && wordinlist(who_from, tmp->current_channel)) {
        if (tmp->server == from_server) {
          target_window = tmp;
          return;
        }
      }
      if (tmp->bind_channel && wordinlist(who_from, tmp->bind_channel)) {
        if (tmp->server == from_server) {
          target_window = tmp;
          return;
        }
      ...
      if (tmp->query_nick) {
        ...
        if ((who_level == LOG_DCC || who_level == LOG_CTCP ||
             who_level == LOG_ACTION) &&
            *tmp->query_nick == '=' && wordinlist(who_from, tmp->query_nick)) {
          target_window = tmp;
          return;
        }
      }
    ...
    while (traverse_all_windows(&tmp)) {
      if (tmp->nicks && from_server == tmp->server) {
        if (find_in_list((List **)&(tmp->nicks), (char *)who_from,
                         !USE_WILDCARDS)) {
          target_window = tmp;
          return;
        }
    ...
    if (is_channel((char *)who_from) && from_server > -1) {
      ...
      if ((chan = (ChannelList *)find_in_list((List **)&chan2, (char *)who_from,
                                              !USE_WILDCARDS))) {
        tmp = NULL;
        while ((traverse_all_windows(&tmp))) {
          ...
          if (tmp->server != from_server)
            continue;
          if (((unsigned int)chan->refnum == tmp->refnum) && (tmp->server == chan->server)) {
            target_window = tmp;
            return;
          }
        }
...
void BX_reset_display_target(void) { set_display_target(NULL, LOG_CRAP); }
 ...
 * value
 */
void BX_set_scroll_lines(Window *win __attribute__((unused)), char *unused __attribute__((unused)), int size) {
  if (size == 0) {
    return;
 ...
 */
void BX_set_continued_lines(Window *win __attribute__((unused)), char *value, int unused __attribute__((unused))) {
  if (value &&
      ((int)strlen(stripansicodes(value)) > (current_term->TI_cols / 2)))
    value[current_term->TI_cols / 2] = '\0';
}

void BX_free_formats(Window *window) { remove_wsets_for_window(window); }

/* current_refnum: returns the reference number for the current window */
unsigned int BX_current_refnum(void) { return current_window->refnum; }
 ...
 */
void BX_set_scrollback_size(Window *w __attribute__((unused)), char *unused __attribute__((unused)), int size) {
  Window *window = NULL;
...
static void list_a_window(Window *window, int len) {
  ...
  say(WIN_FORM, ltoa(window->refnum), 12, 12,
      get_server_nickname(window->server), len, len,
      window->name ? window->name : "<None>", cnw, cnw,
      window->current_channel ? window->current_channel : "<None>",
      window->query_nick ? window->query_nick : "<None>",
      get_server_itsname(window->server) ? get_server_itsname(window->server)
                                         : "<None>",
 ...
 */
static Window *window_add(Window *window, char **args, char *usage __attribute__((unused))) {
  char *ptr;
  NickList *new;
 ...
 * the current window.
 */
static Window *window_back(Window *window, char **args __attribute__((unused)), char *usage __attribute__((unused))) {
  Window *tmp;
 ...
 */
static Window *window_balance(Window *window, char **args __attribute__((unused)), char *usage __attribute__((unused))) {
  if (window->screen)
 ...
 */
static Window *window_beep_always(Window *window, char **args, char *usage __attribute__((unused))) {
 ...
 */
static Window *window_bind(Window *window, char **args, char *usage __attribute__((unused))) {
  char *arg;
  Window *w = NULL;
 ...
 */
Window *window_channel(Window *window, char **args, char *usage __attribute__((unused))) {
  char *arg;
    ...
    while ((ch = next_in_comma_list(arg, &arg))) {
      ...
      if (is_bound(channel, window->server))
        say("Channel %s is already bound elsewhere", channel);
               ...
               is_on_channel(channel, window->server,
...
void win_create(int var, int test) {
  if (get_int_var(var) &&
      (test || current_window->current_channel || current_window->query_nick)) {
    char *args = NULL;
    switch (var) {
 ...
 */
static Window *window_create(Window *window, char **args __attribute__((unused)), char *usage __attribute__((unused))) {
#ifdef WINDOW_CREATE
  Window *tmp;
 ...
 */
static Window *window_delete(Window *window, char **args __attribute__((unused)), char *usage __attribute__((unused))) {
#ifdef WINDOW_CREATE
  if (window->screen)
 ...
 */
static Window *window_describe(Window *window, char **args __attribute__((unused)), char *usage __attribute__((unused))) {
  if (window->name)
  ...
  if (window->screen) {
    ...
    say("\tScreen: <None>");
  say("\tGeometry Info: [%d %d %d %d %d %d]", window->top, window->bottom,
      window->held_displayed, window->display_size, window->cursor,
  ...
  say("\tQuery User: %s %s", window->query_nick ? window->query_nick : "<None>",
      window->query_cmd ? window->query_cmd : empty_string);

  if (window->prompt)
    say("\tPrompt: %s", window->prompt);

  say("\tSecond status line is %s", onoff[window->double_status]);
  say("\tSplit line is %s triple is %s", onoff[window->status_split],
      onoff[window->status_lines]);
 ...
 */
static Window *window_discon(Window *window, char **args __attribute__((unused)), char *usage __attribute__((unused))) {
  if (window->server != -1)
 ...
 */
static Window *window_double(Window *window, char **args, char *usage __attribute__((unused))) {
 ...
 */
static Window *window_split(Window *window, char **args, char *usage __attribute__((unused))) {
...
static Window *window_triple(Window *window, char **args, char *usage __attribute__((unused))) {
  int booya = window->status_lines;
 ...
 */
static Window *window_echo(Window *window, char **args, char *usage __attribute__((unused))) {
  const char *to_echo;
 ...
 */
static Window *window_fixed(Window *window, char **args, char *usage __attribute__((unused))) {
 ...
 */
static Window *window_goto(Window *window, char **args, char *usage __attribute__((unused))) {
 ...
 */
static Window *window_grow(Window *window, char **args, char *usage __attribute__((unused))) {
 ...
 */
static Window *window_hide(Window *window, char **args __attribute__((unused)), char *usage __attribute__((unused))) {
  hide_window(window);
  return current_window;
}
 ...
 */
static Window *window_hide_others(Window *window, char **args __attribute__((unused)), char *usage __attribute__((unused))) {
  Window *tmp, *next;
 ...
 */
static Window *window_hold_mode(Window *window, char **args, char *usage __attribute__((unused))) {
 ...
 */
static Window *window_kill(Window *window, char **args __attribute__((unused)), char *usage __attribute__((unused))) {
 ...
 */
static Window *window_kill_others(Window *window, char **args __attribute__((unused)), char *usage __attribute__((unused))) {
  Window *tmp, *next;
 ...
 */
static Window *window_killswap(Window *window, char **args __attribute__((unused)), char *usage __attribute__((unused))) {
  if (invisible_list) {
    swap_window(window, invisible_list);
    delete_window(window);
  } else
 ...
 */
static Window *window_last(Window *window, char **args __attribute__((unused)), char *usage __attribute__((unused))) {
  set_screens_current_window(window->screen, NULL);
  return current_window;
}
 ...
 */
static Window *window_lastlog(Window *window, char **args, char *usage __attribute__((unused))) {
  char *arg = next_arg(*args, args);
 ...
 */
static Window *window_lastlog_level(Window *window, char **args, char *usage __attribute__((unused))) {
  char *arg = next_arg(*args, args);
  ;
 ...
 */
static Window *window_level(Window *window, char **args, char *usage __attribute__((unused))) {
  char *arg;
  int add = 0;
  int newlevel = 0;
 ...
 */
Window *window_list(Window *window, char **args __attribute__((unused)), char *usage __attribute__((unused))) {
  Window *tmp = NULL;
  int len = 6;
  int cnw = get_int_var(CHANNEL_NAME_WIDTH_VAR);
  ...
  say(WIN_FORM, "Ref", 12, 12, "Nick", len, len, "Name", cnw, cnw, "Channel",
      "Query", "Server", "Level", empty_string);
 ...
 */
static Window *window_log(Window *window, char **args, char *usage __attribute__((unused))) {
 ...
 */
static Window *window_logfile(Window *window, char **args, char *usage __attribute__((unused))) {
  char *arg = next_arg(*args, args);
...
static Window *window_move(Window *window, char **args, char *usage __attribute__((unused))) {
  move_window(window, get_number("MOVE", args, NULL));
  return window;
}

static Window *window_name(Window *window, char **args, char *usage __attribute__((unused))) {
  char *arg;
...
static Window *window_new(Window *window, char **args __attribute__((unused)), char *usage __attribute__((unused))) {
  Window *tmp;
...
static Window *window_new_hide(Window *window, char **args __attribute__((unused)), char *usage __attribute__((unused))) {
  new_window(NULL);
  return window;
}

static Window *window_next(Window *window, char **args __attribute__((unused)), char *usage __attribute__((unused))) {
  Window *tmp;
  Window *next = NULL;
...
static Window *window_noserv(Window *window, char **args __attribute__((unused)), char *usage __attribute__((unused))) {
  window->server = -1;
  return window;
}

static Window *window_notify(Window *window, char **args __attribute__((unused)), char *usage __attribute__((unused))) {
...
static Window *window_notify_level(Window *window, char **args, char *usage __attribute__((unused))) {
  char *arg;
...
static Window *window_number(Window *window, char **args, char *usage __attribute__((unused))) {
  Window *tmp;
  char *arg;
  int i;
 ...
 */
static Window *window_pop(Window *window, char **args __attribute__((unused)), char *usage __attribute__((unused))) {
  int refnum;
  WindowStack *tmp;
  Window *win = NULL;
...
static Window *window_previous(Window *window, char **args __attribute__((unused)), char *usage __attribute__((unused))) {
  Window *tmp;
...
static Window *window_prompt(Window *window, char **args, char *usage __attribute__((unused))) {
  char *arg;
...
static Window *window_push(Window *window, char **args __attribute__((unused)), char *usage __attribute__((unused))) {
  WindowStack *new;
...
Window *window_query(Window *window, char **args, char *usage __attribute__((unused))) {
  NickList *tmp;
  char *nick = NULL;
  char *host = NULL;
  char *cmd = NULL;
  ...
  if ((nick = window->query_nick)) {
    ...
    while ((ptr = next_in_comma_list(nick, &nick))) {
      ...
      if ((tmp = (NickList *)remove_from_list((List **)&window->nicks, ptr))) {
        new_free(&tmp->nick);
        new_free(&tmp->host);
        new_free((char **)&tmp);
      }
...
static Window *window_refresh(Window *window, char **args __attribute__((unused)), char *usage __attribute__((unused))) {
...
static Window *window_refnum(Window *window, char **args, char *usage __attribute__((unused))) {
  Window *tmp;
...
static Window *window_remove(Window *window, char **args, char *usage __attribute__((unused))) {
  char *arg;
  ...
  if ((arg = next_arg(*args, args))) {
    ...
    while (arg) {
      ...
      if ((new =
               (NickList *)remove_from_list((List **)&(window->nicks), arg))) {
...
static Window *window_show(Window *window, char **args, char *usage __attribute__((unused))) {
  Window *tmp;
...
static Window *window_scratch(Window *window, char **args, char *usage __attribute__((unused))) {
  int scratch = 0;
...
Window *window_scroll(Window *window, char **args, char *usage __attribute__((unused))) {
  int scroll = 0;
...
static Window *window_scrollback(Window *window, char **args, char *usage __attribute__((unused))) {
...
static Window *window_skip(Window *window, char **args, char *usage __attribute__((unused))) {
...
static Window *window_show_all(Window *window, char **args __attribute__((unused)), char *usage __attribute__((unused))) {
  while (invisible_list)
    show_window(invisible_list);
  return window;
}

static Window *window_shrink(Window *window, char **args, char *usage __attribute__((unused))) {
  resize_window(RESIZE_REL, window, -get_number("SHRINK", args, NULL));
  return window;
}

static Window *window_size(Window *window, char **args, char *usage __attribute__((unused))) {
  char *ptr = *args;
  int number;
...
static Window *window_stack(Window *window, char **args __attribute__((unused)), char *usage __attribute__((unused))) {
  WindowStack *last, *tmp, *crap;
  Window *win = NULL;
  int len = 4;

  while ((traverse_all_windows(&win))) {
    if (win->name && (strlen(win->name) > (size_t)len))
      len = strlen(win->name);
  }
...
static Window *window_status_special(Window *window, char **args, char *usage __attribute__((unused))) {
  char *arg;
...
static Window *window_swap(Window *window, char **args, char *usage __attribute__((unused))) {
  Window *tmp;
...
static Window *window_unbind(Window *window, char **args, char *usage __attribute__((unused))) {
  char *arg, *channel;
...
static Window *window_set(Window *window, char **args, char *usage __attribute__((unused))) {
  wset_variable(NULL, *args, NULL, NULL);
  *args = NULL;
  return window;
}

static Window *window_update(Window *window, char **args __attribute__((unused)), char *usage __attribute__((unused))) {
...
static Window *window_menu(Window *window, char **args __attribute__((unused)), char *usage __attribute__((unused))) {
#ifdef GUI
  ...
  if (my_stricmp(addthismenu, "-delete") &&
      !(menutoadd = (MenuStruct *)findmenu(addthismenu))) {
    say("Menu not found!");
    return window;
  }
...
#ifdef GUI
static Window *window_setpos(Window *window, char **args, char *usage __attribute__((unused))) {
  int x = 0, y = 0, cx = 0, cy = 0, min = 0, max = 0, restore = 0, activate = 0,
      bottom = 0, top = 0, size = 0, position = 0, t = 0;
  char *tmp;
  ...
  gui_setwindowpos(window->screen, x, y, cx, cy, top, bottom, min, max, restore,
                   activate, size, position);
  return window;
}

static Window *window_font(Window *window, char **args, char *usage __attribute__((unused))) {
  char *tmp;
...
static Window *window_nicklist(Window *window, char **args, char *usage __attribute__((unused))) {
  char *tmp;
...
static window_ops options[] = {
    {"ADD", window_add,
     "[nick|#channel|nick1,nick2,...]\n- Adds [nick|#channel|nick1,nick2,...] "
     "to the nicks to display in a window"},
    ...
    {"CHANNEL", window_channel,
     "[#channel|#chan1,#chan2,...]\n- Attempts to join a channel in current "
     "window"},
    ...
    {"KILLSWAP", window_killswap,
     "\n- Swaps the current window with the last one hidden and kills the "
     "swapped out window"},
    ...
    {"NOTIFY", window_notify,
     "[on|off|toggle]\n- Turns notification on/off/toggle for the current "
     "window."},
    ...
    {"POP", window_pop,
     "\n- Pops the top window off the window stack. If hidden it's made "
     "visible"},
    ...
    {"PUSH", window_push,
     "[refnum]\n- Places the current window or specified window on the window "
     "stack"},
    ...
    {"REMOVE", window_remove,
     "[nick|#channel|nick1,nick2,...]\n- Removes nick|#channel from this "
     "window's display"},
    {"SERVER", window_server,
     "[servername[:[<port>][:[<password>][:[<nickname>]]]]]\n- Attempts to "
     "connect current window with a new server"},
    {"SET", window_set, "[text]\n- Displays or sets window specific set's"},
#ifdef GUI
    {"SETWINDOWPOS", window_setpos,
    ...
    {"SPLIT", window_split,
     "[on|off|toggle]\n- Places a third status line at the top of the screen "
     "if possible on/off/toggle"},
    {"STACK", window_stack,
     "\n- Shows the current window stack which have been added using /window "
     "push"},
    {"STATUS_SPECIAL", window_status_special,
     "[text]\n- A special window format"},
    {"SWAP", window_swap,
     "[refnum]\n- Swap the current window with the specified hidden [refnum] "
     "window"},
    {"TRIPLE", window_triple,
     "[on|off|toggle]\n- Turns on/off/toggle a triple status bar"},
    {"UNBIND", window_unbind,
     "[#channel]\n- Removes a channel bound to a window. If not specified then "
     "the current channel is used"},
    {"UPDATE", window_update, "\n- Updates the window status on all windows"},
    {NULL, NULL, NULL}};

static Window *window_help(Window *window, char **args, char *usage __attribute__((unused))) {
  int i, c = 0, done = 0;
  char *cmd;
  ...
  *buffer = 0;
  ...
  if (!cmd) {
    for (i = 0; options[i].command; i++) {
      ...
      if (++c == 5) {
        put_it("%s",
               convert_output_format("$G $[13]0 $[13]1 $[13]2 $[13]3 $[13]4",
                                     "%s", buffer));
        *buffer = 0;
        c = 0;
      }
    ...
    userage("WINDOW help",
            "%R[%ncommand%R]%n to get help on specific commands");
  } else {
    for (i = 0; options[i].command; i++) {
      if (!my_stricmp(options[i].command, cmd)) {
        snprintf(buffer, sizeof buffer, "WINDOW %s", cmd);
        userage(buffer,
                options[i].usage ? options[i].usage : " - No help available");
        done++;
      }
    }
    if (!done)
      put_it("%s",
             convert_output_format("$G WINDOW - No such command", NULL, NULL));
  }
  return window;
}
...
#ifdef WANT_DLL
    if (dll_window &&
        (dll = (WindowDll *)find_in_list((List **)&dll_window, arg, 0))) {
...
void BX_scrollback_backwards_lines(int scroll_lines) {
  ...
  position =
      (int)(bleah -
            ((((float)(window->distance_from_display - window->display_size)) /
              (float)(window->display_buffer_size - window->display_size)) *
             bleah));
 ...
 *
 * Returns the number of lines forwards or backwards that the window should
 * scroll in one increment.
 */
...
void BX_scrollback_forwards(char dumb __attribute__((unused)), char *dumber __attribute__((unused))) {
  scrollback_forwards_lines(window_scrollback_count(current_window));
}

void BX_scrollback_backwards(char dumb __attribute__((unused)), char *dumber __attribute__((unused))) {
  scrollback_backwards_lines(window_scrollback_count(current_window));
}

void BX_scrollback_end(char dumb __attribute__((unused)), char *dumber __attribute__((unused))) {
  Window *window = current_window;
...
void BX_scrollback_start(char dumb __attribute__((unused)), char *dumber __attribute__((unused))) {
  Window *window = current_window;
...
void BX_unstop_all_windows(char dumb __attribute__((unused)), char *dumber __attribute__((unused))) {
  Window *tmp = NULL;
 ...
 */
void BX_reset_line_cnt(Window *win, char *unused __attribute__((unused)), int value) {
  win->hold_mode = value;
...
void BX_toggle_stop_screen(char unused __attribute__((unused)), char *not_used __attribute__((unused))) {
...
void BX_make_window_current(Window *window) {
  ...
  do_hook(WINDOW_FOCUS_LIST, "%d %d %d %d", old_screen, old_win, new_screen,
          new_win);
}
...
int get_current_winref(void) { return current_window->refnum; }
