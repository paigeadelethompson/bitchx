...
static BuiltIns built_in[] = {{'.', alias_sent_nick},
                              {',', alias_recv_nick},
                              {':', alias_joined_nick},
                              {';', alias_public_nick},
                              {'`', alias_uptime},
...
#define BUILT_IN_FUNCTION(x, y)                                                \
  static char *x(char *fn __attribute__((unused)), char *y)
...
#define NUMBER_OF_FUNCTIONS                                                    \
  (sizeof(built_in_functions) / sizeof(BuiltInFunctions)) - 2
...
static int func_exist(char *name) {
  ...
  find_fixed_array_item(built_in_functions, sizeof(BuiltInFunctions),
                        NUMBER_OF_FUNCTIONS + 1, tmp, &cnt, &pos);
  if (cnt < 0)
    return 1;
...
void add_tcl_alias(Tcl_Interp *tcl_interp, void *func1, void *func2) {
  ...
  while (built_in_functions[i].func) {
    ...
    Tcl_CreateCommand(tcl_interp, lower(str),
                      func1 ? func1 : built_in_functions[i].func, NULL, NULL);
    i++;
  }
  i = 0;
  while (built_in[i].func) {
    snprintf(str, sizeof str, "_%c", built_in[i].name);
    Tcl_CreateCommand(tcl_interp, str, func2 ? func2 : built_in[i].func, NULL,
                      NULL);
    i++;
  }
}
...
static inline void move_link_to_top(NewBuiltInFunctions *tmp,
                                    NewBuiltInFunctions *prev,
                                    HashEntry *location) {
  if (prev) {
...
static void init_functions(void) {
  ...
  for (i = 0; i <= (int)NUMBER_OF_FUNCTIONS; i++) {
...
BUILT_IN_COMMAND(debugfunc) {
  ...
  for (i = 0; i < FUNCTION_HASHSIZE; i++) {
    if (!(ptr = (NewBuiltInFunctions *)functions[i].list))
      continue;
    while (ptr) {
      put_it("DEBUG_FUNC[%d]: %s  %d links  %d hits", i, ptr->name,
             functions[i].links, functions[i].hits);
      ptr = ptr->next;
    }
  }
}
...
char **get_builtins(char *name, int *cnt) {
  ...
  *cnt = 0;
  ...
  while (built_in_functions[i].func && i <= (int)NUMBER_OF_FUNCTIONS) {
...
static char *alias_nick(void) {
  return m_strdup((current_window->server != -1
                       ? get_server_nickname(current_window->server)
                       : empty_string));
}
char *alias_away(void) { return m_strdup(get_server_away(from_server)); }
static char *alias_sent_nick(void) {
  return m_strdup(get_server_sent_nick(from_server));
}
static char *alias_recv_nick(void) {
  return m_strdup(get_server_recv_nick(from_server));
}
static char *alias_msg_body(void) {
  return m_strdup(get_server_sent_body(from_server));
}
static char *alias_joined_nick(void) {
...
static char *alias_oper(void) {
  return m_strdup(get_server_operator(from_server)
                      ? get_string_var(STATUS_OPER_VAR)
                      : empty_string);
}
...
static char *alias_idle(void) {
  return m_sprintf("%ld", (long)(now - idle_time));
}
static char *alias_show_userhost(void) {
  return m_strdup(get_server_userhost(from_server));
}
static char *alias_current_numeric(void) {
  return m_sprintf("%03d", -current_numeric);
}
static char *alias_hookname(void) {
  return m_sprintf("%s", *hook_name ? hook_name : empty_string);
}
static char *alias_thingansi(void) { return m_strdup(numeric_banner()); }
static char *alias_uptime(void) {
  return m_sprintf("%s", convert_time(now - start_time));
}
...
static char *alias_serverlag(void) {
  return m_sprintf("%d", get_server_lag(from_server));
}
static char *alias_currentwindow(void) {
  return m_sprintf("%d", current_window ? current_window->refnum : 0);
}
static char *alias_serverlistsize(void) {
  return m_sprintf("%d", server_list_size());
}
...
static char *alias_server(void) {
  return m_strdup(
      (parsing_server_index != -1)   ? get_server_itsname(parsing_server_index)
      : (get_window_server(0) != -1) ? get_server_itsname(get_window_server(0))
                                     : empty_string);
}
...
static char *alias_current_network(void) {
  return m_strdup(
      (parsing_server_index != -1)   ? get_server_network(parsing_server_index)
      : (get_window_server(0) != -1) ? get_server_network(get_window_server(0))
                                     : empty_string);
}
...
static char *alias_serverport(void) {
  return m_sprintf("%d", (parsing_server_index != -1)
                         ...
                         : (get_window_server(0) != -1)
                             ? get_server_port(get_window_server(0))
                             : 0);
}
...
static char *alias_modes(void) {
  char *tmp;
  return m_strdup((tmp = get_current_channel_by_refnum(0))
                      ? get_channel_mode(tmp, from_server)
                      : empty_string);
}
...
#define RETURN_IF_EMPTY(x)                                                     \
  if (empty(x))                                                                \
  RETURN_EMPTY
#define GET_INT_ARG(x, y)                                                      \
  {                                                                            \
    RETURN_IF_EMPTY(y);                                                        \
    x = atol(safe_new_next_arg(y, &y));                                        \
  }
#define GET_FLOAT_ARG(x, y)                                                    \
  {                                                                            \
    RETURN_IF_EMPTY(y);                                                        \
    x = atof(safe_new_next_arg(y, &y));                                        \
  }
#define GET_STR_ARG(x, y)                                                      \
  {                                                                            \
    RETURN_IF_EMPTY(y);                                                        \
    x = new_next_arg(y, &y);                                                   \
    RETURN_IF_EMPTY(x);                                                        \
...
#undef BUILT_IN_FUNCTION
#define BUILT_IN_FUNCTION(x, y)                                                \
  static char *x(char *fn __attribute__((unused)), char *y __attribute__((unused)))
...
BUILT_IN_FUNCTION(function_left, input) {
  ...
  if (strlen(input) > (size_t)count)
    input[count] = 0;

  RETURN_STR(input);
}

char *function_getkey(char *n __attribute__((unused)), char *input) {
  char *temp;
  RETURN_IF_EMPTY(input);
 ...
 */
BUILT_IN_FUNCTION(function_srand, word __attribute__((unused))) {
 ...
 * 	    wrote this comment.
 */
BUILT_IN_FUNCTION(function_time, input __attribute__((unused))) { RETURN_INT(time(NULL)); }
 ...
 */
BUILT_IN_FUNCTION(function_ischannel, input) { RETURN_INT(is_channel(input)); }
...
BUILT_IN_FUNCTION(function_toupper, input) { return (upper(m_strdup(input))); }

BUILT_IN_FUNCTION(function_tolower, input) { return (lower(m_strdup(input))); }

BUILT_IN_FUNCTION(function_curpos, input __attribute__((unused))) {
...
BUILT_IN_FUNCTION(function_pid, input __attribute__((unused))) { RETURN_INT(getpid()); }

BUILT_IN_FUNCTION(function_ppid, input __attribute__((unused))) { RETURN_INT(getppid()); }
...
BUILT_IN_FUNCTION(function_idle, input __attribute__((unused))) { return alias_idle(); }
...
BUILT_IN_FUNCTION(function_remw, text) {
  ...
  while (str && ((str != text && !isspace((unsigned char)str[-1])) ||
...
BUILT_IN_FUNCTION(function_common, word) {
  char *left = NULL, **leftw = NULL, *right = NULL, **rightw = NULL,
       *booya = NULL;
  int leftc, lefti;
  int rightc, righti;
...
BUILT_IN_FUNCTION(function_copattern, word) {
  char *booya = NULL, *pattern = NULL, *firstl = NULL, *firstlist = NULL,
       *firstel = NULL, *secondl = NULL, *secondlist = NULL, *secondel = NULL;
...
char *function_shift(char *n __attribute__((unused)), char *word) {
  char *value = NULL;
  char *var = NULL;
  char *booya = NULL;
  int old_display = window_display;
...
char *function_unshift(char *n __attribute__((unused)), char *word) {
  char *value = NULL;
  char *var = NULL;
  char *booya = NULL;
  int old_display = window_display;
...
char *function_push(char *n __attribute__((unused)), char *word) {
  char *value = NULL;
  char *var = NULL;
  int old_display = window_display;
...
char *function_pop(char *n __attribute__((unused)), char *word) {
  char *value = NULL;
  char *var = NULL;
  char *pointer = NULL;
  int old_display = window_display;
...
BUILT_IN_FUNCTION(function_which, word) {
  ...
  if ((file1 = path_search(
           file1, (word && *word) ? word : get_string_var(LOAD_PATH_VAR)))) {
    RETURN_STR(file1);
  } else {
    new_free(&file1);
    RETURN_EMPTY;
...
BUILT_IN_FUNCTION(function_translate, words) {
  register char *oldc;
  char *newc, *text, delim;
  register char *ptr;
  int size_old, size_new, x;
...
BUILT_IN_FUNCTION(function_server_version, word) {
  ...
  if (version == Server2_8)
    RETURN_STR("2.8");
  else if (version == Server2_9)
    RETURN_STR("2.9");
  else if (version == Server2_10)
    RETURN_STR("2.10");
  else if (version == Server2_8ts4)
    RETURN_STR("2.8ts4");
  else if (version == Server2_8hybrid)
    RETURN_STR("2.8hybrid5");
  else if (version == Server2_8hybrid6)
    RETURN_STR("2.8hybrid6");
  else if (version == Server2_8comstud)
    RETURN_STR("2.8comstud");

  else if (version == Server_u2_8)
    RETURN_STR("u2.8");
  else if (version == Server_u2_9)
    RETURN_STR("u2.9");
  else if (version == Server_u2_10)
    RETURN_STR("u2.10");
  else if (version == Server_u3_0)
    RETURN_STR("u3.0");
...
BUILT_IN_FUNCTION(function_truncate, words) {
  int num = 0;
  double value = 0;
  char buffer[BIG_BUFFER_SIZE], format[BIG_BUFFER_SIZE];
...
BUILT_IN_FUNCTION(function_tdiff2, input) {
  ...
  snprintf(result_str, sizeof result_str, "%s%s%s%s", days_str, hours_str,
 ...
 */
BUILT_IN_FUNCTION(function_utime, input __attribute__((unused))) {
  struct timeval tp;
  get_time(&tp);
  return m_sprintf("%ld %ld", (unsigned long)tp.tv_sec,
...
BUILT_IN_FUNCTION(function_lastserver, input __attribute__((unused))) { RETURN_INT(last_server); }
...
BUILT_IN_FUNCTION(function_numwords, input) { RETURN_INT(word_count(input)); }

BUILT_IN_FUNCTION(function_strlen, input) { RETURN_INT(strlen(input)); }
...
BUILT_IN_FUNCTION(function_info, words __attribute__((unused))) {
  /*
          char *which;
...
BUILT_IN_FUNCTION(function_geom, words __attribute__((unused))) {
  return m_sprintf("%d %d", current_window->screen->co,
                   current_window->screen->li);
}
...
BUILT_IN_FUNCTION(function_uptime, input __attribute__((unused))) {
  ...
  snprintf(result, sizeof result, "%ldd %ldh %ldm %lds", days, hours, minutes,
           seconds);
  RETURN_STR(result);
}
...
BUILT_IN_FUNCTION(function_checkshit, input) {
  ...
  for (tmp = shitlist_list; tmp; tmp = tmp->next) {
    if (wild_match(tmp->filter, uh) &&
        check_channel_match(tmp->channels, channel)) {
      matched = 1;
      break;
    }
  }
  ...
  if (tmp && matched)
    return m_sprintf("%d %s %s %s", tmp->level, tmp->channels, tmp->filter,
                     tmp->reason);
  RETURN_EMPTY;
}
...
BUILT_IN_FUNCTION(function_get_info, input) {
  ...
  int size = -1;
  GET_STR_ARG(nick, input);
  for (tmp = next_userlist(NULL, &size, &needed); tmp;
       tmp = next_userlist(tmp, &size, &needed)) {
...
BUILT_IN_FUNCTION(function_set_info, input) {
  ...
  int done = 0;
  GET_STR_ARG(nick, input);
  for (tmp = next_userlist(NULL, &size, &location); tmp;
       tmp = next_userlist(tmp, &size, &location)) {
...
BUILT_IN_FUNCTION(function_checkuser, input) {
  ...
  if ((tmp = find_bestmatch("*", uh, channel, NULL)))
    return m_sprintf("%lu %s %s %s", tmp->flags, tmp->host, tmp->channels,
                     tmp->password ? tmp->password : empty_string);
#endif
  RETURN_EMPTY;
}
...
BUILT_IN_FUNCTION(function_epic, words __attribute__((unused))) { RETURN_INT(1); }

BUILT_IN_FUNCTION(function_runlevel, words __attribute__((unused))) {
...
BUILT_IN_FUNCTION(function_lastnotice, words) {
  ...
  malloc_sprintf(&str, "%s %s %s %s %s", last_notice[count].time,
                 last_notice[count].from, last_notice[count].uh,
                 last_notice[count].to, last_notice[count].last_msg);
  return str;
}
...
BUILT_IN_FUNCTION(function_lastmessage, words) {
  ...
  malloc_sprintf(&str, "%s %s %s %s %s", last_msg[count].time,
                 last_msg[count].from, last_msg[count].uh, last_msg[count].to,
                 last_msg[count].last_msg);
  return str;
}

char *function_addtabkey(char *n __attribute__((unused)), char *words) {
  char *arrayname = NULL;
  char *nick = NULL;
...
char *function_gettabkey(char *n __attribute__((unused)), char *words) {
  char *arrayname = NULL;
  NickTab *nick = NULL;
  int direction;
...
static int num_sort_it(const void *one, const void *two) {
  ...
  while (*oneptr && *twoptr) {
    while (*oneptr && *twoptr && !isdigit((unsigned char)*oneptr) &&
           !isdigit((unsigned char)*twoptr)) {
      if (*oneptr != *twoptr)
...
BUILT_IN_FUNCTION(function_glob, word) {
  ...
  char *path, *path2 = NULL, *retval = NULL;
  int numglobs, i;
  glob_t globbers;
...
BUILT_IN_FUNCTION(function_isuser, words) {
  ...
  if ((Shit = nickinshit(nick, uhost)) &&
      check_channel_match(Shit->channels, channel))
    return m_sprintf("SHIT %s %d %s", Shit->filter, Shit->level,
                     Shit->channels);
#endif
  RETURN_EMPTY;
}
...
BUILT_IN_FUNCTION(function_isban, word) {
  ...
  if ((Ban = (ShitList *)find_in_list((List **)&shitlist_list, ban, 0)) &&
      check_channel_match(channel, Ban->channels))
    RETURN_INT(1);
  RETURN_INT(0);
}
...
BUILT_IN_FUNCTION(function_openserver, word) {
  ...
  if (port > 0) {
    ...
    if ((socket_num = connect_by_number(servern, &this_sucks, SERVICE_CLIENT,
                                        PROTOCOL_TCP, 1)) < 0)
      RETURN_INT(-1);
    if ((add_socketread(socket_num, port, 0, servern,
                        !notify_mode ? read_clonelist : read_clonenotify,
                        NULL)) < 0) {
...
char *function_cparse(char *n __attribute__((unused)), char *word) {
  char *format = NULL;
  char *blah;
  GET_STR_ARG(format, word);
  in_cparse++;
  blah = convert_output_format(format, word && *word ? "%s" : NULL,
                               word && *word ? word : NULL);
  in_cparse--;
  RETURN_STR(blah);
}
...
BUILT_IN_FUNCTION(function_gethost, word) {
  ...
  if (from_server != -1) {
    for (chan = get_server_channels(from_server); chan; chan = chan->next) {
      for (nick = next_nicklist(chan, NULL); nick;
           nick = next_nicklist(chan, nick)) {
...
BUILT_IN_FUNCTION(function_irclib, input __attribute__((unused))) { RETURN_STR(irc_lib); }

BUILT_IN_FUNCTION(function_country, input) { RETURN_STR(country(input)); }
...
BUILT_IN_FUNCTION(function_uname, input) {
  ...
  *tmp = 0;
  for (i = 0, len = strlen(input); i < len; i++) {
    if ((size_t)(ptr - tmp) >= size)
      break;

    if (input[i] == '%') {
      switch (input[++i]) {
      case 'm':
        strlcpy(ptr, un.machine, size);
        break;
      case 'n':
        strlcpy(ptr, un.nodename, size);
        break;
      case 'r':
        strlcpy(ptr, un.release, size);
        break;
      case 's':
        strlcpy(ptr, un.sysname, size);
        break;
      case 'v':
        strlcpy(ptr, un.version, size);
        break;
      case 'a':
        snprintf(ptr, size, "%s %s %s %s %s", un.sysname, un.nodename,
...
BUILT_IN_FUNCTION(function_winrefs, args __attribute__((unused))) {
  Window *w = NULL;
  char *retval = NULL;
...
#define MASK1(x, y)                                                            \
  snprintf(stuff, sizeof stuff, x, y);                                         \
  break;
#define MASK2(x, y, z)                                                         \
  snprintf(stuff, sizeof stuff, x, y, z);                                      \
  break;
#define MASK3(x, y, z, a)                                                      \
  snprintf(stuff, sizeof stuff, x, y, z, a);                                   \
  break;
#define MASK4(x, y, z, a, b)                                                   \
  snprintf(stuff, sizeof stuff, x, y, z, a, b);                                \
  break;
#define MASK5(x, y, z, a, b, c)                                                \
  snprintf(stuff, sizeof stuff, x, y, z, a, b, c);                             \
  break;
  ...
  if (ip == 0)
    ...
    case 3:
      MASK3("*!*%s@*%s%s", USER, DOT, domain)
    case 4:
      MASK2("*!*@*%s%s", DOT, domain)
    case 5:
    ...
    case 8:
      MASK4("%s!*%s@*%s%s", nick, USER, DOT, domain)
    case 9:
      MASK3("%s!*@*%s%s", nick, DOT, domain)
    case 10:
      mask_digits(&host);
      MASK3("*!*@%s%s%s", host, DOT, domain)
    case 11:
    ...
    switch (which) {
    case 0:
      MASK3("*!%s@%s.%s", user, domain, host)
    case 1:
      MASK3("*!*%s@%s.%s", USER, domain, host)
    case 2:
      MASK2("*!*@%s.%s", domain, host)
    case 3:
      MASK2("*!*%s@%s.*", USER, domain)
    case 4:
      MASK1("*!*@%s.*", domain)
    case 5:
      MASK4("%s!%s@%s.%s", nick, user, domain, host)
    case 6:
      MASK4("%s!*%s@%s.%s", nick, USER, domain, host)
    case 7:
      MASK3("%s!*@%s.%s", nick, domain, host)
    case 8:
      MASK3("%s!*%s@%s.*", nick, USER, domain)
    case 9:
      MASK2("%s!*@%s.*", nick, domain)
    case 10:
      MASK1("*!*@%s.*", domain)
    case 11:
      MASK2("*!*%s@%s.*", USER, domain)
...
BUILT_IN_FUNCTION(function_remws, word) {
  char *left = NULL, **lhs = NULL, *right = NULL, **rhs = NULL, *booya = NULL;
  int leftc, lefti, rightc, righti;
  int found = 0;
  ...
  *right++ = 0;
  ...
  for (righti = 0; righti < rightc; righti++) {
    found = 0;
    for (lefti = 0; lefti < leftc; lefti++) {
      if (rhs[righti] && lhs[lefti] && !my_stricmp(lhs[lefti], rhs[righti])) {
        found = 1;
        break;
      }
    }
...
BUILT_IN_FUNCTION(function_getuid, input __attribute__((unused))) { RETURN_INT(getuid()); }

BUILT_IN_FUNCTION(function_getgid, input __attribute__((unused))) { RETURN_INT(getgid()); }

BUILT_IN_FUNCTION(function_getlogin, input __attribute__((unused))) {
#ifdef HAVE_GETLOGIN
  RETURN_STR(getlogin());
#else
...
BUILT_IN_FUNCTION(function_getpgrp, input) { RETURN_INT(getpgrp()); }
...
BUILT_IN_FUNCTION(function_iscurchan, input) {
  ...
  while (traverse_all_windows(&w)) {
     ...
     */
    if (w->current_channel && !my_stricmp(chan, w->current_channel) &&
  ...
  if (*str == '#' && *(str + 1)) {
    ...
    for (i = 0; i < get_max_fd() + 1; i++) {
      if (check_dcc_socket(i) && (s = get_socket(i))) {
        n1 = (DCC_int *)s->info;
        if (n1 && (n1->dccnum == (unsigned)dcc_item)) {
          found++;
          break;
        }
      }
...
BUILT_IN_FUNCTION(function_winitem, word) {
  ...
  return m_sprintf(
      "%u %s %d %s %s %s %s %d %d %d %s %d %d %d %d %s \"%s\" \"%s\" %s %s %s",
#else
  return m_sprintf(
      "%u %s %d %s %s %s %s %d %d %d %s %s %s %s %s %s \"%s\" \"%s\" %s %s %s",
#endif
      window->refnum, window->name ? window->name : none, window->server,
      ...
      (window->screen && window->screen->menu) ? window->screen->menu
                                               : none, /* oirc menu name */
      window->visible, window->screen ? window->screen->co : 0,
      window->screen ? window->screen->li : 0,
#ifdef GUI
      fontinfo, winx, winy, wincx, wincy,
#else
      none, none, none, none,
      none, /* font windowx windowy window-width window-height */
#endif
      window->logfile ? window->logfile : none, flags,
      window->nicks ? "n" : empty_string, none, none,
...
BUILT_IN_FUNCTION(function_lastclickline, input) {
  ...
  *retinfo = 0;
  ...
  case 3:
    if (newclicklinedata[lastclickcol] != ' ') {
      ...
      while (newclicklinedata[e] != ' ' && e < strlen(newclicklinedata))
        e++;
      if (newclicklinedata[i] == '[' || newclicklinedata[i] == '(' ||
          newclicklinedata[i] == '<')
        i++;
      if (newclicklinedata[e - 1] == ']' || newclicklinedata[e - 1] == ')' ||
          newclicklinedata[e - 1] == '>')
        e--;
      for (c = i; c < e; c++)
  ...
  case 4:
    ...
    again:
      if (newclicklinedata[i] == '#' || newclicklinedata[i] == '@' ||
          newclicklinedata[i] == '+' || newclicklinedata[i] == '%' ||
          newclicklinedata[i] == '*') {
        i++;
        goto again;
      }
...
BUILT_IN_FUNCTION(function_screensize, input) {
  ...
  if (!my_stricmp(input, "cx"))
    ...
    snprintf(retbuffer, sizeof retbuffer, "%d %d", gui_screen_width(),
             gui_screen_height());
...
BUILT_IN_FUNCTION(function_getopt, input) {
  static char switches[INPUT_BUFFER_SIZE + 1] = "",
                                           ...
                                           last_input[INPUT_BUFFER_SIZE + 1] =
                                               "",
                                           *sptr = switches, *aptr = args;
  char *optopt_var, *optarg_var, *optstr, *optptr, *tmp;
  char extra_args[INPUT_BUFFER_SIZE + 1] = "";
  int old_window_display = window_display, arg_flag = 0;
...
BUILT_IN_FUNCTION(function_ttyname, input) { RETURN_STR(ttyname(0)); }
...
BUILT_IN_FUNCTION(function_isdisplaying, input) { RETURN_INT(window_display); }
