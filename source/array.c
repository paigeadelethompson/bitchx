...
/*
        ...
        RETURNS: 0 (zero) if it successfully sets and existing item in the array
                ...
                -1 if it was unable to find the array specified (item_number >
0) -2 if it was unable to find the item in an existing array (item_number was
too large)
...
static an_array array_info = {NULL, NULL, 0L};
...
extern void delete_array(char *name) {
  ...
  if (array_info.size > 1) {
    ...
    array_info.size--;
    for (ptr = &array_info.item[item], cnt = item; cnt < array_info.size;
         cnt++, ptr++, array++) {
      *ptr = *(ptr + 1);
      *array = *(array + 1);
    }
  ...
  if (name && (array = get_array(name))) {
    ...
    if (*input) {
      for (index = 0; index < array->size; index++) {
        if ((current_match = wild_match(input, array->item[index])) >
            best_match) {
          match = index;
...
BUILT_IN_FUNCTION(function_rmatchitem) {
  ...
  if (name && (array = get_array(name))) {
    ...
    if (*input) {
      for (index = 0; index < array->size; index++) {
        if ((current_match = wild_match(array->item[index], input)) >
            best_match) {
          match = index;
          best_match = current_match;
        }
...
BUILT_IN_FUNCTION(function_getarrays) {
  ...
  for (index = 0; index < array_info.size; index++)
    if (!input || !*input ||
        wild_match(input, array_info.item[array_info.index[index]]))
      m_s3cat(&result, space, array_info.item[array_info.index[index]]);
 ...
 */
BUILT_IN_FUNCTION(function_numarrays){RETURN_INT(array_info.size)}
    ...
    if (itemstr) {
      ...
      if (item >= 0 && item < array->size) {
        ...
        if (array->size == 1)
          ...
          new_free(&array->item[item]);
          array->size--;
          for (strptr = &(array->item[item]), cnt = item; cnt < array->size;
               cnt++, strptr++)
            *strptr = *(strptr + 1);
