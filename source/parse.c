...
#define do_output(x)                                                           \
  put_it("%s%s%s",                                                             \
         get_int_var(TIMESTAMP_VAR) ? update_clock(GET_TIME) : empty_string,   \
         get_int_var(TIMESTAMP_VAR) ? space : empty_string, x)
...
int annoy_kicks(int list_type __attribute__((unused)), char *to, char *from, char *ptr,
                NickList *nick) {
  int kick_em = 0;
  ChannelList *chan;
  ...
  if (nick && (nick->userlist && nick->userlist->flags))
    return 0;
  if (!check_channel_match(get_string_var(PROTECT_CHANNELS_VAR), to) ||
      !are_you_opped(to))
    return 0;
  ...
  if (get_cset_int_var(chan->csets, ANNOY_KICK_CSET) && !nick_isop(nick)) {
    ...
    if (annoy_hl(ptr, BOLD_TOG))
      malloc_sprintf(&buffer, "KICK %s %s :%s", to, from,
                     "autokick for " BOLD_TOG_STR "bold" BOLD_TOG_STR);
    else if (strchr(ptr, BELL_CHAR))
      malloc_sprintf(&buffer, "KICK %s %s :%s", to, from,
                     "autokick for beeping");
    else if (annoy_hl(ptr, COLOR_CHAR))
      malloc_sprintf(&buffer, "KICK %s %s :%s", to, from,
                     "autokick for " UND_TOG_STR "mirc color" UND_TOG_STR);
    else if (annoy_hl(ptr, UND_TOG))
      malloc_sprintf(&buffer, "KICK %s %s :%s", to, from,
                     "autokick for " UND_TOG_STR "underline" UND_TOG_STR);
    else if (annoy_hl(ptr, REV_TOG))
      malloc_sprintf(&buffer, "KICK %s %s :%s", to, from,
                     "autokick for " REV_TOG_STR "inverse" REV_TOG_STR);
    else if (annoy_hl(ptr, BLINK_TOG))
      malloc_sprintf(&buffer, "KICK %s %s :%s", to, from,
                     "autokick for " BLINK_TOG_STR "flashing" BLINK_TOG_STR);
    else if (annoy_caps(ptr))
      malloc_sprintf(&buffer, "KICK %s %s :%s", to, from,
                     "autokick for CAPS LOCK");
      ...
      *p++ = '\0';
      ...
      if (get_int_var(AUTO_UNBAN_VAR))
        add_timer(
            0, empty_string, get_int_var(AUTO_UNBAN_VAR) * 1000, 1, timer_unban,
            m_sprintf("%d %s *!*%s", from_server, to, cluster(FromUserHost)),
            NULL, current_window->refnum, "auto-unban");
      new_free(&host);
      kick_em = 1;
    }
    if (buffer) {
      ...
      if (stristr(ptr, word->string)) {
        if (wild_match(word->channel, to)) {
          if (!ops && (nick_isop(nick) || nick_isvoice(nick)))
            break;
          send_to_server("KICK %s %s :%s %s", to, from,
                         "\002BitchX BWK\002: ", word->string);
          kick_em = 1;
          break;
        }
      }
...
int BX_is_channel(const char *to) {
  return to && (*to == MULTI_CHANNEL || *to == STRING_CHANNEL ||
                *to == ID_CHANNEL || *to == LOCAL_CHANNEL);
}
...
static void p_topic(char *from, char **ArgList) {
  ...
  if ((tmp = lookup_channel(ArgList[0], from_server, CHAN_NOUNLINK))) {
    ...
    if (tmp->topic_lock) {
      ...
      malloc_strcpy(&tmp->topic, ArgList[1]);
    add_last_type(&last_topic[0], 1, from, FromUserHost, tmp->channel,
                  ArgList[1]);
    do_logchannel(LOG_TOPIC, tmp, "%s %s %s", from, ArgList[0],
                  ArgList[1] ? ArgList[1] : empty_string);
  }
  if (tmp && check_ignore(from, FromUserHost, tmp->channel, IGNORE_TOPICS,
                          NULL) != IGNORED) {
      ...
      if (ArgList[1] && *ArgList[1]) {
        if (fget_string_var(FORMAT_TOPIC_CHANGE_HEADER_FSET))
          put_it("%s", convert_output_format(
                           fget_string_var(FORMAT_TOPIC_CHANGE_HEADER_FSET),
                           "%s %s %s %s", update_clock(GET_TIME), from,
                           ArgList[0], ArgList[1]));
        put_it("%s",
               convert_output_format(fget_string_var(FORMAT_TOPIC_CHANGE_FSET),
                                     "%s %s %s %s", update_clock(GET_TIME),
                                     from, ArgList[0], ArgList[1]));
      } else
        put_it("%s", convert_output_format(
                         fget_string_var(FORMAT_TOPIC_UNSET_FSET), "%s %s %s",
                         update_clock(GET_TIME), from, ArgList[0]));
    }
    reset_display_target();
  }
  logmsg(LOG_TOPIC, from, 0, "%s %s", ArgList[0],
         ArgList[1] ? ArgList[1] : empty_string);
#ifdef GUI
  xterm_settitle();
#endif
...
static void p_wallops(char *from, char **ArgList) {
  ...
  if (from_server || check_flooding(from, WALLOP_FLOOD, line, NULL)) {
    ...
    int level;
    if (check_ignore(from, FromUserHost, NULL, IGNORE_WALLOPS, NULL) == IGNORED)
      return;
    set_display_target(from, LOG_WALLOP);
    level = set_lastlog_msg_level(LOG_WALLOP);
    autorep = check_auto_reply(line);
    add_last_type(&last_wall[0], MAX_LAST_MSG, from, NULL,
                  from_server ? "S" : "*", line);
    if (do_hook(WALLOP_LIST, "%s %c %s", from, from_server ? 'S' : '*', line))
      put_it("%s",
             convert_output_format(
                 fget_string_var(from_server ? FORMAT_WALLOP_FSET
                                             : (autorep ? FORMAT_WALL_AR_FSET
                                                        : FORMAT_WALL_FSET)),
                 "%s %s %s %s", update_clock(GET_TIME), from,
                 from_server ? "!" : "*", line));
...
static void p_privmsg(char *from, char **Args) {
  ...
  if (flood_type == PUBLIC_FLOOD) {
    ...
    if (is_other_flood(channel, tmpnick, PUBLIC_FLOOD, &blah)) {
      flooding = 1;
      flood_prot(tmpnick->nick, FromUserHost, flood_type,
                 get_cset_int_var(channel->csets, PUBFLOOD_IGNORE_TIME_CSET),
                 channel->channel);
    }
  } else
    flooding = !check_flooding(from, flood_type, ptr, NULL);

  if (list_type == PUBLIC_LIST || list_type == PUBLIC_OTHER_LIST ||
      list_type == PUBLIC_MSG_LIST) {
  ...
  if (!flooding) {
    ...
    case PUBLIC_MSG_LIST:
      if (do_hook(list_type, "%s %s %s", from, to, ptr)) {
        ...
        put_it("%s", convert_output_format(
                         fget_string_var(ar_true ? FORMAT_PUBLIC_MSG_AR_FSET
                                                 : FORMAT_PUBLIC_MSG_FSET),
                         "%s %s %s %s %s", update_clock(GET_TIME), from,
                         FromUserHost, to, ptr));
        do_beep = 1;
      }
      break;
    ...
    case MSG_GROUP_LIST:
      if (do_hook(list_type, "%s %s %s", from, to, ptr)) {
        ...
        put_it("%s", convert_output_format(
                         fget_string_var(FORMAT_MSG_GROUP_FSET), "%s %s %s %s",
                         update_clock(GET_TIME), from, to, ptr));
        do_beep = 1;
      }
      break;
    ...
    case MSG_LIST:
      ...
      if (do_hook(list_type, "%s %s", from, ptr)) {
        if (get_server_away(from_server)) {
          ...
          do_beep = 1;
        put_it("%s", convert_output_format(
                         fget_string_var(FORMAT_MSG_FSET), "%s %s %s %s",
                         update_clock(GET_TIME), from, FromUserHost, ptr));
      ...
      if (get_server_away(from_server) && get_int_var(SEND_AWAY_MSG_VAR)) {
        if (!check_last_type(&last_msg[0], MAX_LAST_MSG, from, FromUserHost))
          my_send_to_server(
              from_server, "NOTICE %s :%s", from,
              stripansicodes(convert_output_format(
                  fget_string_var(FORMAT_SEND_AWAY_FSET), "%l %l %s", now,
                  get_server_awaytime(from_server),
                  get_int_var(MSGLOG_VAR) ? "On" : "Off")));
      }
      break;
    ...
    case PUBLIC_LIST:
      ...
      if (do_hook(list_type, "%s %s %s", from, to, ptr)) {
        logmsg(LOG_PUBLIC, from, 0, "%s %s", to, ptr);
        do_logchannel(
            LOG_PUBLIC, channel, "%s",
            convert_output_format(fget_string_var((list_type == PUBLIC_AR_LIST)
                                                      ? FORMAT_PUBLIC_AR_FSET
                                                      : FORMAT_PUBLIC_FSET),
                                  "%s %s %s %s", update_clock(GET_TIME), from,
                                  to, ptr));
        put_it("%s", convert_output_format(
                         fget_string_var((list_type == PUBLIC_AR_LIST)
                                             ? FORMAT_PUBLIC_AR_FSET
                                             : FORMAT_PUBLIC_FSET),
                         "%s %s %s %s", update_clock(GET_TIME), from, to, ptr));
        do_beep = 1;
      }
      break;
    ...
    case PUBLIC_OTHER_LIST:
      ...
      if (do_hook(list_type, "%s %s %s", from, to, ptr)) {
        logmsg(LOG_PUBLIC, from, 0, "%s %s", to, ptr);
        do_logchannel(LOG_PUBLIC, channel, "%s",
                      convert_output_format(
                          fget_string_var(list_type == PUBLIC_OTHER_AR_LIST
                                              ? FORMAT_PUBLIC_OTHER_AR_FSET
                                              : FORMAT_PUBLIC_OTHER_FSET),
                          "%s %s %s %s", update_clock(GET_TIME), from, to,
                          ptr));
        put_it("%s", convert_output_format(
                         fget_string_var(list_type == PUBLIC_OTHER_AR_LIST
                                             ? FORMAT_PUBLIC_OTHER_AR_FSET
...
static void p_quit(char *from, char **ArgList) {
  ...
  for (chan = walk_channels(from, 1, from_server); chan;
    ...
    if (ignore != IGNORED) {
      ...
      if (do_hook(CHANNEL_SIGNOFF_LIST, "%s %s %s", chan->channel, from,
                  reason))
        one_prints = 1;
    }
  }
  ...
  if (one_prints) {
    ...
    if ((ignore != IGNORED) && do_hook(SIGNOFF_LIST, "%s %s", from, reason) &&
        !netsplit)
      put_it("%s",
             convert_output_format(fget_string_var(FORMAT_CHANNEL_SIGNOFF_FSET),
                                   "%s %s %s %s %s", update_clock(GET_TIME),
                                   from, FromUserHost, chanlist, reason));
  }
...
static int sping_reply(char *from, char *sping_dest, int server) {
  ...
  reset_display_target();
  put_it("%s", convert_output_format("$G Server pong from %W$0%n $1 seconds",
                                     "%s %s", from, buff));
...
static void p_error(char *from __attribute__((unused)), char **ArgList) {
...
/*
 * This only handles negotiating the SASL capability with the PLAIN method. It
 * would be good to add DH-BLOWFISH, and later, full capability support.
 */
...
static void p_cap(char *from __attribute__((unused)), char **ArgList) {
  char *caps, *p;
  Server *srv;
  ...
  if (!strncmp(ArgList[1], "LS", 2)) {
    ...
    if (!is_server_connected(from_server) &&
        (srv = cap_server(from_server)) != NULL &&
...
static void p_away(char *from, char **ArgList) {
  ...
  if (message)
    put_it("%s", convert_output_format("$G $0- is away: $1-", "%s %s", from,
                                       message));
  else
    put_it("%s", convert_output_format("$G $0- is no longer away", "%s", from));
}

static void p_authenticate(char *from __attribute__((unused)), char **ArgList) {
...
void add_user_who(WhoEntry *w __attribute__((unused)), char *from __attribute__((unused)), char **ArgList) {
  char *userhost;
  int op = 0, voice = 0;
  ...
  add_to_channel(ArgList[0], ArgList[4], from_server, op, voice, userhost,
                 ArgList[3], ArgList[5], 0,
                 ArgList[6] ? my_atol(ArgList[6]) : 0);
#ifdef WANT_NSLOOKUP
  if (get_int_var(AUTO_NSLOOKUP_VAR))
    do_nslookup(ArgList[2], ArgList[4], ArgList[1], ArgList[0], from_server,
                auto_nslookup, NULL);
#endif
}

void add_user_end(WhoEntry *w __attribute__((unused)), char *from __attribute__((unused)), char **ArgList) {
  ...
  *extra = 0;
  ...
  if (!my_stricmp(from, get_server_nickname(from_server))) {
    ...
    if (!in_join_list(channel, from_server)) {
      ...
      if ((int)current_window->refnum !=
          (refnum = get_win_from_join_list(channel, from_server))) {
        switched = 1;
    ...
    chan = add_to_channel(channel, from, from_server, op, vo, FromUserHost,
                          NULL, NULL, whowas && irc_serv ? 1 : 0, 0);
    ...
    do_logchannel(LOG_JOIN, chan, "%s, %s %s %s", from, FromUserHost, channel,
                  extra);
    if (!irc_serv)
      check_channel_limit(chan);
  }
  ...
  if (!in_join_list(channel, from_server) && chan)
    tmpnick =
        check_auto(channel, find_nicklist_in_channellist(from, chan, 0), chan);
#endif
  flush_mode_all(chan);
  ...
  if (whowas) {
    if (irc_serv) {
      if ((do_hook(LLOOK_JOIN_LIST, "%s %s", irc_serv->name, irc_serv->link)))
        put_it("%s",
               convert_output_format(fget_string_var(FORMAT_NETJOIN_FSET),
                                     "%s %s %s %d", update_clock(GET_TIME),
                                     irc_serv->name, irc_serv->link, 0));
  ...
  if (check_ignore(from, FromUserHost, channel, IGNORE_JOINS, NULL) !=
          IGNORED &&
      chan) {
    if (do_hook(JOIN_LIST, "%s %s %s %s", from, channel,
                FromUserHost ? FromUserHost : "UnKnown", extra)) {
      if (chan && (tmpnick = find_nicklist_in_channellist(from, chan, 0))) {
        if (tmpnick->userlist)
          put_it("%s", convert_output_format(
                           fget_string_var(FORMAT_FRIEND_JOIN_FSET),
                           "%s %s %s %s %s %s", update_clock(GET_TIME), from,
                           FromUserHost ? FromUserHost : "UnKnown", channel,
                           tmpnick->userlist ? (tmpnick->userlist->comment
                                                    ? tmpnick->userlist->comment
                                                    : empty_string)
                                             : empty_string,
                           extra));
        else
          put_it("%s",
                 convert_output_format(
                     fget_string_var(FORMAT_JOIN_FSET), "%s %s %s %s %s",
                     update_clock(GET_TIME), from,
                     FromUserHost ? FromUserHost : "UnKnown", channel, extra));
      } else
        put_it("%s",
               convert_output_format(
                   fget_string_var(FORMAT_JOIN_FSET), "%s %s %s %s %s",
      ...
      if (!its_me && chan && chan->have_op) {
        if (get_cset_int_var(chan->csets, LAMELIST_CSET)) {
          if (lame_list && find_in_list((List **)&lame_list, from, 0)) {
            ...
            send_to_server("KICK %s %s :\002Lame Nick detected\002",
                           chan->channel, from);
            if (get_int_var(AUTO_UNBAN_VAR))
              add_timer(
                  0, empty_string, get_int_var(AUTO_UNBAN_VAR) * 1000, 1,
                  timer_unban,
                  m_sprintf("%d %s %s!*", from_server, chan->channel, from),
                  NULL, current_window->refnum, "auto-unban");
          }
        }
        if (get_cset_int_var(chan->csets, LAMEIDENT_CSET)) {
          ...
          static const char lame_chars[] =
              "\x01\x02\x03\x04\x05\x06\x07\x08\x0b\x0c\x0d\x0e\x0f\x10\x11\x12"
              "\x13\x14\x15\x16\x17\x18\x19\x1a";
          if (strpbrk(FromUserHost, lame_chars)) {
            char *host = strchr(FromUserHost, '@') + 1;
            send_to_server(
                "MODE %s +b *!*@%s\r\nKICK %s %s :\002Lame Ident detected\002",
                chan->channel, cluster(host), chan->channel, from);
          }
        }
      }
    }
...
void check_auto_join(int server __attribute__((unused)), char *from, char *channel, char *key __attribute__((unused))) {
  ChannelList *chan = NULL;
...
#ifdef WANT_USERLIST
    if (((get_cset_int_var(chan->csets, AUTO_REJOIN_CSET)) ||
         (!chan && get_int_var(AUTO_REJOIN_VAR))) &&
        (channel && ((u = lookup_userlevelc(from, FromUserHost, channel,
                                            NULL)) != NULL))) {
      if ((u->flags & ADD_BOT))
        goto got_request;
    } else
...
static void p_invite(char *from, char **ArgList) {
  if (check_ignore(from, FromUserHost, ArgList[1] ? ArgList[1] : NULL,
                   IGNORE_INVITES, NULL) == IGNORED) {
  ...
  if (ArgList[0] && ArgList[1]) {
    ...
    if (check_flooding(from, INVITE_FLOOD, ArgList[1], NULL) &&
        do_hook(INVITE_LIST, "%s %s %s", from, ArgList[1],
                ArgList[2] ? ArgList[2] : empty_string)) {
      char *s;
      put_it("%s", convert_output_format(fget_string_var(FORMAT_INVITE_FSET),
    ...
    add_last_type(&last_invite_channel[0], 1, from, FromUserHost, NULL,
                  ArgList[1]);
    reset_display_target();
  }
}

static void p_silence(char *from __attribute__((unused)), char **ArgList) {
  char *target = ArgList[0];
  char *mag = target++;

  if (do_hook(SILENCE_LIST, "%c %s", *mag, target))
    put_it("%s", convert_output_format(fget_string_var(FORMAT_SILENCE_FSET),
                                       "%s %c %s", update_clock(GET_TIME), *mag,
                                       target));
}
...
static void p_kill(char *from, char **ArgList) {
  ...
  set_input_prompt(current_window, get_string_var(INPUT_PROMPT_VAR), 0);
  if (serverkill) {
    say("Server [%s] has rejected you (probably due to a nick collision)",
        from);
    ...
    if (do_hook(DISCONNECT_LIST, "Killed by %s (%s)", from,
      ...
      put_it("%s", convert_output_format(
                       fget_string_var(FORMAT_KILL_FSET), "%s %s %s",
                       update_clock(GET_TIME), from,
...
static void p_ping(char *from __attribute__((unused)), char **ArgList) {
...
static void p_nick(char *from, char **ArgList) {
  ...
  if (one_prints) {
    ...
    if (do_hook(NICKNAME_LIST, "%s %s", from, line))
      put_it("%s", convert_output_format(
                       fget_string_var(
                           its_me ? FORMAT_NICKNAME_USER_FSET
                           : im_on_channel(what_channel(from, from_server),
                                           from_server)
                               ? FORMAT_NICKNAME_FSET
                               : FORMAT_NICKNAME_OTHER_FSET),
                       "%s %s %s %s", update_clock(GET_TIME), from, "-", line));
  }
...
static int check_mode_change(NickList *nick, char type_mode, char *from,
                             char *this_nick, char *channel) {
  time_t right_now = now;
  int found = 0;
  if (!nick->userlist && !isme(nick->nick)) {
    if ((!nick_isop(nick) && type_mode == '+') ||
        (nick_isop(nick) && type_mode == '-')) {
      switch (type_mode) {
      case '-':
...
static void check_bitch_mode(char *from, char *uh, char *channel, char *line,
                             ChannelList *chan) {
  NickList *nick;
  char *new_mode = NULL;
  char *n = NULL;
  ...
  if (!from || !chan ||
      (chan && (!get_cset_int_var(chan->csets, BITCH_CSET) || !chan->have_op)))
    return;
  ...
  if (!nick->userlist ||
      !check_channel_match(nick->userlist->channels, channel)) {
    char *p;
    ...
    if (found)
      put_it("%s", convert_output_format(
                       fget_string_var(FORMAT_BITCH_FSET), "%s %s %s %s %s %s",
                       update_clock(GET_TIME), from, uh, channel, new_mode, n));
  }
  reset_display_target();
}
...
static void p_mode(char *from, char **ArgList) {
  ...
  flag =
      check_ignore(from, FromUserHost, target,
                   (smode ? IGNORE_SMODES : IGNORE_MODES) | IGNORE_CRAP, NULL);
  ...
  if (target && line) {
    ...
    if (is_channel(target)) {
      ...
      if (flag != IGNORED &&
          do_hook(MODE_LIST, "%s %s %s", from, target, line)) {
        enum FSET_TYPES fset = smode ? FORMAT_SMODE_FSET : FORMAT_MODE_FSET;
        put_it("%s",
               convert_output_format(fget_string_var(fset), "%s %s %s %s %s",
                                     update_clock(GET_TIME), from, display_uh,
                                     target, line));
      }
      ...
      if (flag != IGNORED &&
        ...
        enum FSET_TYPES fset = my_stricmp(from, target)
...
static void strip_modes(char *from, char *channel, char *line) {
  ...
  if (is_channel(channel)) {
    for (pointer = mode; *pointer; pointer++) {
      ...
      case 'l':
        if (mag == '+')
          do_hook(MODE_STRIPPED_LIST, "%s %s %c%c %s", from, channel, mag, c,
                  next_arg(copy, &copy));
        else
  ...
  if (channel && target && chan) {
    ...
    if (!my_stricmp(target, get_server_nickname(from_server))) {
      ...
      do_logchannel(LOG_KICK_USER, chan, "%s %s, %s %s %s", from, FromUserHost,
                    target, channel, comment);
      if (rejoin)
        send_to_server("JOIN %s%s%s", channel, chankey ? space : empty_string,
                       chankey ? chankey : empty_string);
      new_free(&chankey);
      if (do_hook(KICK_LIST, "%s %s %s %s", target, from, channel,
                  comment ? comment : empty_string))
        put_it("%s",
               convert_output_format(fget_string_var(FORMAT_KICK_USER_FSET),
                                     "%s %s %s %s %s", update_clock(GET_TIME),
                                     from, channel, target, comment));
      remove_channel(channel);
      update_all_status(window ? window : current_window, NULL, 0);
      update_input(UPDATE_ALL);
      logmsg(LOG_KICK_USER, from, 0, "%s %s %s %s", FromUserHost, target,
             channel, comment);
      if (rejoin)
      ...
      if ((check_ignore(from, FromUserHost, channel, IGNORE_KICKS, NULL) !=
           IGNORED) &&
          do_hook(KICK_LIST, "%s %s %s %s", target, from, channel, comment))
        put_it("%s",
               convert_output_format(fget_string_var(FORMAT_KICK_FSET),
                                     "%s %s %s %s %s", update_clock(GET_TIME),
                                     from, channel, target, comment));
      /* if it's me that's doing the kick don't flood check */
      if (!itsme) {
        f_nick = find_nicklist_in_channellist(target, chan, 0);
        if (chan->have_op && from_nick &&
            is_other_flood(chan, from_nick, KICK_FLOOD, &t)) {
          if (get_cset_int_var(chan->csets, KICK_ON_KICKFLOOD_CSET) >
              get_cset_int_var(chan->csets, DEOP_ON_KICKFLOOD_CSET))
            send_to_server("MODE %s -o %s", chan->channel, from);
          else if (!from_nick->sent_kick++)
            send_to_server(
                "KICK %s %s :\002Mass kick detected - (%d kicks in "
                "%dsec%s)\002",
                chan->channel, from,
                get_cset_int_var(chan->csets, KICK_ON_KICKFLOOD_CSET), t,
                plural(t));
        }
#ifdef WANT_USERLIST
      ...
      logmsg(LOG_KICK, from, 0, "%s %s %s %s", FromUserHost, target, channel,
             comment);
      do_logchannel(LOG_KICK, chan, "%s %s %s %s %s", from, FromUserHost,
                    target, channel, comment);
    }
  }
  update_all_status(current_window, NULL, 0);
...
static void p_part(char *from, char **ArgList) {
  ...
  if ((tmpc = lookup_channel(channel, from_server, CHAN_NOUNLINK)))
    update_stats(LEAVELIST, find_nicklist_in_channellist(from, tmpc, 0), tmpc,
                 0);

  if ((check_ignore(from, FromUserHost, channel, IGNORE_PARTS, NULL) !=
       IGNORED) &&
      do_hook(LEAVE_LIST, "%s %s %s %s", from, channel, FromUserHost,
              ArgList[1] ? ArgList[1] : empty_string))
    put_it("%s", convert_output_format(fget_string_var(FORMAT_LEAVE_FSET),
                                       "%s %s %s %s %s", update_clock(GET_TIME),
                                       from, FromUserHost, channel,
                                       ArgList[1] ? ArgList[1] : empty_string));
  if (!my_stricmp(from, get_server_nickname(from_server))) {
    remove_channel(channel);
    ...
    logmsg(LOG_PART, from, 0, "%s %s", channel,
           ArgList[1] ? ArgList[1] : empty_string);
    do_logchannel(LOG_PART, tmpc, "%s %s %s", channel, from,
                  ArgList[1] ? ArgList[1] : empty_string);
  }
  update_all_status(current_window, NULL, 0);
  update_input(UPDATE_ALL);
...
static void rfc1459_odd(char *from, char *comm, char **ArgList) {
  PasteArgs(ArgList, 0);
  if (do_hook(ODD_SERVER_STUFF_LIST, "%s %s %s", from ? from : "*", comm,
              ArgList[0])) {
    if (from)
...
static void p_rpong(char *from, char **ArgList) {
  if (!ArgList[3]) {
    ...
    say("Pingtime %s - %s : %s ms (total delay: %ld s)", from, ArgList[1],
        ArgList[2], delay);
  }
}

protocol_command rfc1459[] = {{"ADMIN", NULL, NULL, 0, 0, 0},
...
BUILT_IN_COMMAND(debugmsg) {
  ...
  for (i = 0; rfc1459[i].command; i++) {
    put_it("DEBUG_MSG: %10s[%03lu] # %ld -> %ld bytes", rfc1459[i].command, i,
           rfc1459[i].count, rfc1459[i].bytes);
...
void parse_server(char *orig_line) {
  ...
  if (inbound_line_mangler) {
    ...
    if (mangle_line(line, inbound_line_mangler, len) > (size_t)len)
