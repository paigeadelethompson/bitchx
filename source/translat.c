 ...
 */
void enter_digraph(char key __attribute__((unused)), char *ptr __attribute__((unused))) {
...
unsigned char get_digraph(unsigned char c1) {
  int i = 0;
  unsigned char c, c2 = last_input_screen->digraph_first;

  last_input_screen->digraph_hit = 0;
  if (c1 > c2) /* Make sure we have the lowest one in c1. */
    c = c1, c1 = c2, c2 = c;
  while (dig_table_lo[i]) { /* Find digraph and return it. */
 ...
 */
void set_translation(Window *win __attribute__((unused)), char *tablename, int unused __attribute__((unused))) {
  /* Transparent / UTF-8: pass 8-bit bytes through untranslated.
   * The byte-wise tables would corrupt multibyte UTF-8 sequences. */
  if (!tablename || !*tablename || !my_stricmp(tablename, "UTF-8") ||
      !my_stricmp(tablename, "UTF8")) {
    translation = 0;
    return;
  }
  ...
  if (!(table = uzfopen(&filename, ".", 0))) {
    say("Cannot open character table definition \"%s\" !", tablename);
  ...
  while (fgets(buffer, 80, table)) {
    sscanf(buffer, "0x%x,0x%x,0x%x,0x%x,0x%x,0x%x,0x%x,0x%x", inputs + 0,
...
BUILT_IN_COMMAND(digraph) {
  ...
  if ((arg = next_arg(args, &args)) && (*arg == '-')) {
    ...
    if (my_strnicmp(arg, "add", len) == 0) {
       ...
       */
      if ((i = strlen((char *)dig_table_lo)) == DIG_TABLE_SIZE - 1)
        say("Sorry, digraph table full.");
      else {
        while ((c1 = my_getarg(&args)) && (c2 = my_getarg(&args)) &&
               (c3 = my_getarg(&args))) {
          ...
          if (get_digraph(c2) == 0) {
            ...
            dig_table_hi[i] = c2;
            i++;
            dig_table_lo[i] = dig_table_hi[i] = dig_table_di[i] =
                (unsigned char)0;
            digraph_changed = 1;
      ...
      if ((i = strlen((char *)dig_table_lo)) == 0)
        ...
        if ((c1 = my_getarg(&args)) && (c2 = my_getarg(&args))) {
          i = 0;
          if (c1 > c2)
            c3 = c1, c1 = c2, c2 = c3;
      ...
      /* Clear digraph table. */
      dig_table_lo[0] = dig_table_hi[0] = dig_table_di[0] = (unsigned char)0;
      digraph_changed = 1;
...
void save_digraphs(FILE *fp) {
  if (digraph_changed) {
    ...
    for (i = 0; dig_table_lo[i]; i++) {
      if (!(i % 5))
        fputs("\nDIGRAPH -ADD ", fp);
      fprintf(fp, "%d %d %d  ", dig_table_lo[i], dig_table_hi[i],
              dig_table_di[i]);
    }
    fputc('\n', fp);
  }
