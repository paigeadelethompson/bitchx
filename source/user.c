 ...
 */
static inline void move_link_to_top(UserList *tmp, UserList *prev,
                                    HashEntry *location) {
  if (prev) {
    UserList *old_list;
 ...
 */
static inline void remove_link_from_list(UserList *tmp, UserList *prev,
                                         HashEntry *location) {
  if (prev) {
...
static inline UserList *find_userlist_tier3(char *host, char *channel,
                                            int remove) {
...
static inline UserList *find_userlist_tier1(char *userhost, char *channel,
                                            unsigned long hvalue, int remove) {
  HashEntry *location;
...
static inline UserList *find_userlist_tier2(char *userhost,
                                            unsigned long hvalue, int remove) {
  HashEntry *location;
  register UserList *tmp, *prev = NULL;
...
UserList *find_bestmatch(const char *nick __attribute__((unused)), const char *userhost,
                         const char *channel, const char *passwd) {
  UserList *best = NULL;
  UserList *best_passwd = NULL;
  UserList *tmp;
  long hvalue = 0;
  int best_user_match = 0, best_chan_match = 0;
  int chan_match, user_match, passwd_match = 0;
  const char *check;

  /*check = clear_server_flags(userhost);*/
  check = userhost;
  if ((hvalue = hash_name(strrchr(check, '@'), USERHOST_HASHSIZE)) != -1) {
    for (tmp = (UserList *)UserListByHost_Table[hvalue].list; tmp;
         tmp = tmp->next) {
  ...
  if ((hvalue = hash_name(channel, USERCHAN_HASHSIZE)) != -1) {
    for (tmp = (UserList *)UserListByChannel_Table[hvalue].list; tmp;
         tmp = tmp->next) {
      user_match = wild_match(tmp->host, check);
...
BUILT_IN_COMMAND(debug_user) {
  ...
  for (i = 0; i < USERHOST_HASHSIZE; i++) {
    ...
    while (user) {
      tier1++;
      put_it("Tier1[%d] %d %d %s %s", i, location->links, location->hits,
             user->host, user->channels);
      user = user->next;
    }
  }
  for (i = 0; i < USERCHAN_HASHSIZE; i++) {
    ...
    while (user) {
      tier2++;
      put_it("Tier2[%d] %d %d %s %s", i, location->links, location->hits,
             user->host, user->channels);
      user = user->next;
    }
  }
    ...
    while (((UserList *)((HashEntry *)*location)[hvalue].list) == NULL) {
      ...
      if (hvalue == USERHOST_HASHSIZE && *size == USERHOST_HASHSIZE) {
        ...
        *size = USERCHAN_HASHSIZE;
        ...
        hvalue = (unsigned long)-1;
        break;
      }
    }
    if (hvalue != (unsigned long)-1 && ((HashEntry *)*location)[hvalue].list != NULL)
