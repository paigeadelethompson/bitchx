 ...
 */
void set_history_size(Window *win __attribute__((unused)), char *unused __attribute__((unused)), int size) {
  int i, cnt;
  History *ptr;
...
char *get_from_history(int which) { return (get_from_history_buffer(which)); }
  ...
  if (max) {
    if ((value = next_arg(args, &args)) != NULL) {
      if (!my_stricmp(value, "-CLEAR")) {
        for (tmp = command_history_head; command_history_head;
             tmp = command_history_head) {
          new_free(&tmp->stuff);
