...
Array_item *BX_array_lookup(Array *array, char *name,
                            int wild __attribute__((unused)), int delete) {
  int count, location;
...
Array_item *BX_find_array_item(Array *set, char *name, int *cnt, int *loc) {
  ...
  *cnt = 0;
  ...
  while ((min > 0) && (hash & mask) == (ARRAY_ITEM(set, min)->hash & mask))
    min--, lin_ints++;
  while ((max < set->max - 1) &&
         (hash & mask) == (ARRAY_ITEM(set, max)->hash & mask))
    max++, lin_ints++;
...
#define FIXED_ITEM(list, pos, size)                                            \
  (*(Array_item *)((char *)list + (pos * size)))
 ...
 */
void *BX_find_fixed_array_item(void *list, size_t size, int howmany,
                               const char *name, int *cnt, int *loc) {
  size_t len = strlen(name);
  int min = 0, max = howmany, old_pos = -1, pos, c;
  ...
  *cnt = 1;
  ...
  max = pos + 1;
  while ((max < howmany) &&
         !strncmp(name, FIXED_ITEM(list, max, size).name, len))
    (*cnt)++, max++;
