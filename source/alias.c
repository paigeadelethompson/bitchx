...
char *after_expando(char *start, int lvalue, int *call) {
  ...
  while (*str && (isalpha((unsigned char)*str) ||
                  isdigit((unsigned char)*str) || *str == '_' || *str == '.'))
    str++;
  ...
  while (*str == '[') {
    if (!(rest = MatchingBracket(str + 1, '[', ']'))) {
      if (!(rest = strchr(str, ']'))) {
        debugyell("Unmatched bracket in %s (%s)", lval[lvalue], start);
        return empty_string;
      }
    }
    str = rest + 1;
  ...
  if (!lvalue && *str == '(') {
    if (!(rest = MatchingBracket(str + 1, '(', ')'))) {
      if (!(rest = strchr(str, ')'))) {
        debugyell("Unmatched paren in %s (%s)", lval[lvalue], start);
        return empty_string;
      }
    }
    *call = 1;
...
BUILT_IN_COMMAND(aliascmd) {
  ...
  if (!my_strnicmp(name, "/S", 2)) {
    ...
    say("Var command hits/misses/passes/missed-by [%ld/%ld/%ld/%3.1f]",
        var_cache_hits, var_cache_misses, var_cache_passes,
        (var_cache_misses ? (double)(var_cache_missed_by / var_cache_misses)
                          : 0.0));
    say("Cmd command hits/misses/passes/missed-by [%ld/%ld/%ld/%3.1f]",
        cmd_cache_hits, cmd_cache_misses, cmd_cache_passes,
        (cmd_cache_misses ? (double)(cmd_cache_missed_by / cmd_cache_misses)
                          : 0.0));
    say("Ints(bin/lin)/Chars(bin/lin)/Lookups: [(%d/%d)/(%d/%d)] (%d/%d)",
        bin_ints, lin_ints, bin_chars, lin_chars, alist_searches,
        char_searches);
    show_alias_caches();
    return;
  }
      ...
      if (!ptr)
        ...
        *ptr++ = 0;
        while (*ptr && my_isspace(*ptr))
          ptr++;
        if (!*ptr)
          say("Missing alias body in %s %s", command, real_name);
    ...
    if (*args == LEFT_BRACE) {
      ...
      if (!ptr)
        ...
        *ptr++ = 0;
        ...
        if (*ptr)
          say("Junk [%s] after closing brace in %s %s", ptr, command,
              real_name);
...
void show_alias_caches(void) {
  ...
  for (i = 0; i < var_alias.cache_size; i++) {
    if (var_alias.cache[i])
      debugyell("VAR cache [%d]: [%s] [%s]", i, var_alias.cache[i]->name,
                var_alias.cache[i]->stuff);
    else
  ...
  for (i = 0; i < cmd_alias.cache_size; i++) {
    if (cmd_alias.cache[i])
      debugyell("CMD cache [%d]: [%s] [%s]", i, cmd_alias.cache[i]->name,
                cmd_alias.cache[i]->stuff);
    else
...
Alias *find_var_alias(char *name) {
  ...
  if (!item) {
    ...
    if ((item =
             (Alias *)find_array_item((Array *)&var_alias, name, &cnt, &loc)))
      var_cache_misses++;
    else
      var_cache_passes++;
  }
      ...
      if (!name || !strncmp(call_stack[cnt].alias.list[x]->name, name, len)) {
        if ((s = strchr(call_stack[cnt].alias.list[x]->name + len, '.'))) {
          ...
          if (!LastStructName || (DotLoc != LastDotLoc) ||
              strncmp(call_stack[cnt].alias.list[x]->name, LastStructName,
                      DotLoc)) {
            put_it("\t%*.*s\t<Structure>", DotLoc, DotLoc,
                   call_stack[cnt].alias.list[x]->name);
            LastStructName = call_stack[cnt].alias.list[x]->name;
            LastDotLoc = DotLoc;
          }
        } else {
          if (call_stack[cnt].alias.list[x]->stub)
            put_it("\t%s STUBBED TO %s", call_stack[cnt].alias.list[x]->name,
                   call_stack[cnt].alias.list[x]->stub);
          else
            put_it("\t%s\t%s", call_stack[cnt].alias.list[x]->name,
                   call_stack[cnt].alias.list[x]->stuff);
        }
      }
    }
  }
  ...
  for (cnt = 0; cnt < var_alias.max; cnt++) {
    if (!name || !strncmp(var_alias.list[cnt]->name, name, len)) {
      if ((s = strchr(var_alias.list[cnt]->name + len, '.'))) {
        ...
        if (!LastStructName || (DotLoc != LastDotLoc) ||
            strncmp(var_alias.list[cnt]->name, LastStructName, DotLoc)) {
          put_it("\t%*.*s\t<Structure>", DotLoc, DotLoc,
                 var_alias.list[cnt]->name);
          LastStructName = var_alias.list[cnt]->name;
          LastDotLoc = DotLoc;
        }
      } else {
        if (var_alias.list[cnt]->stub)
          put_it("\t%s STUBBED TO %s", var_alias.list[cnt]->name,
                 var_alias.list[cnt]->stub);
        else
          put_it("\t%s\t%s", var_alias.list[cnt]->name,
                 var_alias.list[cnt]->stuff);
      }
    }
  }
}
  ...
  for (cnt = 0; cnt < cmd_alias.max; cnt++) {
    if (!name || !strncmp(cmd_alias.list[cnt]->name, name, len)) {
      if ((s = strchr(cmd_alias.list[cnt]->name + len, '.'))) {
        ...
        if (!LastStructName || (DotLoc != LastDotLoc) ||
            strncmp(cmd_alias.list[cnt]->name, LastStructName, DotLoc)) {
          put_it("\t%*.*s\t<Structure>", DotLoc, DotLoc,
                 cmd_alias.list[cnt]->name);
          LastStructName = cmd_alias.list[cnt]->name;
          LastDotLoc = DotLoc;
        }
      } else {
        if (cmd_alias.list[cnt]->stub)
          put_it("\t%s STUBBED TO %s", cmd_alias.list[cnt]->name,
                 cmd_alias.list[cnt]->stub);
        else
          put_it("\t%s\t%s", cmd_alias.list[cnt]->name,
                 cmd_alias.list[cnt]->stuff);
      }
    }
  }
}
...
char **get_subarray_elements(char *root, int *howmany, int type) {
  ...
  *howmany = 0;
  ...
  for (cnt = 0; cnt < as->max; cnt++) {
    ...
    if ((len < len2) && ((cmp = streq(root, as->list[cnt]->name)) == len)) {
...
char *parse_line_alias_special(char *name, char *what, char *args, int d1,
                               int d2, void *arglist, int function) {
...
char *call_user_function(char *alias_name, char *args) {
  ...
  if (cnt < 0)
    result = parse_line_alias_special(alias_name, sub_buffer, args, 0, 1,
                                      arglist, 1);
...
/* XXX Ugh. */
void call_user_alias(char *alias_name, char *alias_stuff, char *args,
                     void *arglist) {
...
void set_current_command(char *line) { call_stack[wind_index].current = line; }

void unset_current_command(void) { call_stack[wind_index].current = NULL; }

void BX_lock_stack_frame(void) { call_stack[wind_index].locked = 1; }
...
#define GET_INT_ARG(x, y)                                                      \
  {                                                                            \
    RETURN_IF_EMPTY(y);                                                        \
    x = my_atol(safe_new_next_arg(y, &y));                                     \
  }
...
static char *glob_commands(const char *prefix, int *cnt) {
  ...
  *cnt = 0;
  ...
  for (var = find_command(prefix, cnt);
       var && var->name && !strncmp(prefix, var->name, prefix_len); var++) {
