...
static void show_timer(char *command) {
  ...
  get_time(&current);
  put_it("%s", convert_output_format(fget_string_var(FORMAT_TIMER_FSET),
                                     "%s %s %s %s", "Timer", "Seconds",
                                     "Events", "Command"));
  for (tmp = PendingTimers; tmp; tmp = tmp->next) {
    char buf[40];
    ...
    put_it("%s", convert_output_format(
                     fget_string_var(FORMAT_TIMER_FSET), "%s %s %d %s %s",
                     tmp->ref, buf, tmp->events,
                     tmp->callback ? "(internal callback)"
                                   : (tmp->command ? tmp->command : ""),
                     tmp->whom ? tmp->whom : empty_string));
  }
}
...
static TimerList *get_timer(char *ref) {
  ...
  for (tmp = PendingTimers; tmp; tmp = tmp->next) {
    if (!my_stricmp(tmp->ref, ref) ||
        (tmp->whom && !my_stricmp(tmp->whom, ref)))
      return tmp;
  }
...
char *function_timer(char *n __attribute__((unused)), char *args) {
  ...
  return m_sprintf("%s %d %d %.16g %d %0.3f %s%s", t->ref, t->server, t->window,
                   t->interval, t->events, time_left,
 ...
 */
char *BX_add_timer(int update, char *refnum_want, double when, long events,
                   int(callback)(void *, char *), char *what, char *subargs,
                   int winref, char *whom) {
