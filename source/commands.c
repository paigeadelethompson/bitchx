/*
 * commands.c: This is really a mishmash of function and such that deal with
 * IRCII commands (both normal and keybinding commands)
 *
...
char relay_help[] =
    "%R[%n-list|-kick|-wall|-wallop|-msg|-notice|-topic|-kboot|-ansi|-kill|-"
    "help%R]%n %Y<%n#|channel|nick%Y> <%nchannel|nick%Y>";
char scripting_command[] = "- Scripting command";

IrcCommand irc_command[] = {
    {"", empty_string, do_send_text, 0, NULL},
    {"#", NULL, comment, 0, NULL},
    {"4OP", NULL, do_4op, SERVERREQ,
     "%Y<%Cnick%Y>%n\n- Sets mode +oooo on %Y<%Cnick%Y>%n"},
    {":", NULL, comment, 0, NULL},
    {"ABORT", NULL, abortcmd, 0, "- Saves IRCII settings then exits IRC"},
    {"ABOUT", NULL, about, 0,
     "- Shows yet another ansi screen with greets to people who have "
     "contributed to the %PBitchX%n project"},
    {"ADDFORWARD", "AddForward", do_forward, 0,
     "%Y<%Bchannel%G|%Cnick%Y>%n\n - Forward all messages to "
     "%Y<%Bchannel%G|%Cnick%Y>%n"},
    {"ADDIDLE", "AddIdle", addidle, 0,
     "%Y<%Bchannel%Y>%n %R[%nseconds%R]%n\n- Adds %Y<%Bchannel%Y>%n as idle "
     "channel with %R[%nseconds%R]%n"},
    {"ADDLAMENICK", "AddLameNick", add_bad_nick, 0,
     "%Y<%Cnick%G|%Cnick%C nick nick%Y>%n\n - Adds %Y<%Cnick%Y>%n to your lame "
     "nicklist, bans nick!*@*"},
    {"ADDNOFLOOD", "AddNoFlood", add_no_flood, 0,
     "%Y<%Cnick%Y>%n\n - Adds %Cnick%n to your no flood list"},
#ifdef WANT_USERLIST
    {"ADDSHIT", "AddShit", add_shit, 0,
     "%Y<%Cnick%G|%Cnick%G!%nuser%Y@%nhostname%Y>%n %Y<%Bchannel%G|%Y*>%n "
     "%R[%nshitlevel%R]%n %R[%nreason%R]%n\n- Add "
     "%Y<%Cnick%G|%Cnick%G!%nuser%Y@%nhostname%Y>%n to your shitlist on "
     "%Y<%Bchannel%G|%Y*>%n with %R[%nshitlevel%R]%n and for %R[%nreason%R]%n"},
    {"ADDUSER", "AddUser", add_user, 0,
     "%Y<%Cnick%G|%Cnick%G!%nuser%Y@%nhostname%Y>%n %Y<%Bchannel%G|%Y*>%n "
     "%R[%nuserlevel%R]%n %R[%nauto-op%R]%n %R[%nprotection%R]%n "
     "%R[%npassword%R]%n\n - Adds "
     "%Y<%Cnick%G|%Cnick%G!%nuser%Y@%nhostname%Y>%n on %Y<%Bchannel%G|%Y*>%n "
     "with %R[%nuserlevel%R]%n %R[%nauto-op%R]%n and %R[%nprotection%R]%n "
     "levels to your userlist"},
#endif
    {"ADDWORD", NULL, add_ban_word, 0,
     "%Y<%Bchannel%G|%Y*>%n %Y<%nword%y(%ns%y)%Y>%n\n- Adds "
     "%Y<%nword%y(%ns%y)%Y>%n to wordlist, anyone saying it in "
     "%Y<%Bchannel%G|%Y*>%n will be kicked"},
    {"ADMIN", "ADMIN", send_comm, SERVERREQ,
     "%R[%nserver%R]%n\n- Shows the iRC Administration information on current "
     "server or %R[%nserver%R]%n"},
    {"AJOIN", "AJoin", auto_join, 0,
     "%Y<%Bchannel%y(%Bs%y)%Y>%n\n- Add %Y<%Bchannel%y(%Bs%y)%Y>%n to AJoin "
     "list%P;%n it joins %Y<%Bchannel%y(%Bs%y)%Y>%n on startup or change of "
     "server"},
    {"AJOINLIST", "AJoinList", auto_join, 0,
     "- Display list of ajoin channels"},
    {"ALIAS", zero, aliascmd, 0, scripting_command},
    {"ASSIGN", one, assigncmd, 0, scripting_command},
    {"AWAY", "Away", away, SERVERREQ,
     "%R[%nreason%R]%n\n- Sets you away on server if %R[%nreason%R]%n else "
     "sets you back"},
    {"AWAYLOG", "AwayLog", awaylog, 0,
     "%R[%nALL|MSGS|NOTICES|...%R]%n\n- Displays or changes what you want "
     "logged in your lastlog. This is a comma separated list"},
    ...
    {"BAN", NULL, ban, 0,
     "%Y<%Cnick%G|%Cnick%G!%nuser%y@%nhostname%Y>%n\n- Ban "
     "%Y<%Cnick%G|%Cnick%G!%nuser%y@%nhostname%Y>%n from current channel"},
    {"BANSTAT", NULL, banstat, 0,
     "%R[%Bchannel%R]%n\n- Show bans on current channel or %R[%Bchannel%R]%n"},
    {"BANTYPE", NULL, bantype, 0,
     "%W/%nbantype "
     "%Y<%nNormal%G|%nBetter%G|%nHost%G|%nDomain%G|%nScrew%G|%nIP%Y>%n\n- When "
     "a ban is done on a nick, it uses %Y<%nbantype%Y>%n"},
    {"BANWORDS", NULL, add_ban_word, 0,
     "%Y<%nchannel|*%Y>%n word(s)\n- Adds word or words to the banned words "
     "list for %Y<%nchannel%Y>"},
    {"BEEP", NULL, beepcmd, 0, "- Creates a beep noise"},
#ifdef WANT_CHELP
    {"BHELP", "BHELP", chelp, 0,
     "%Y<%nhelp%W|%nindex%W|%nother%Y>%n\n - BitchX help command"},
#endif
    {"BIND", NULL, bindcmd, 0, "- Command to bind a key to a function"},
    {"BK", "BK", kickban, SERVERREQ,
     "%Y<%Cnick%Y>%n %R[%nreason%R]%n\n- Deops, bans and kicks %Y<%Cnick%Y>%n "
     "for %R[%nreason%R]%n"},
    {"BKI", "BKI", kickban, SERVERREQ,
     "%Y<%Cnick%Y>%n %R[%nreason%R]%n\n- Deops, bans, kicks and ignores "
     "%Y<%Cnick%Y>%n for %R[%nreason%R]%n"},
    {"BLESS", NULL, blesscmd, 0, scripting_command},
    {"BREAK", NULL, breakcmd, 0, NULL},
    {"BYE", "QUIT", e_quit, 0, "- Quit Irc"},
    {"C", "MODE", send_mode, 0,
     "%Y<%nnick|channel%Y> <%nmode%Y>%n\n- Sets a mode for a channel or nick"},
    {"CALL", NULL, e_call, 0, scripting_command},
#ifndef BITCHX_LITE
    {"CBOOT", "Cboot", cboot, 0,
     "%Y<%nnick%Y>%n %R[%nreason%R]%n\n- Kicks a %Y<%nnick%Y>%n off the "
     "TwilightZone with %R[%nreason%R]"},
#endif
    {"CD", NULL, cd, 0,
     "%Y<%ndir%Y>%n\n - Changes current directory to %Y<%ndir%Y>%n or prints "
     "current directory"},
#ifdef WANT_CDCC
    {"CDCC", NULL, cdcc, 0,
     "%Y<%ncommand%Y>%n\n- Channel, DOffer, Echo, Load, List, MinSpeed, "
     "Notice, Offer, PList, Queue, ReSend, Save, Type Describe Note"},
#endif
#ifdef WANT_CD
    ...
    {"CHANSERV", "CHANSERV", send_comm, SERVERREQ,
     "- Network services command"},
    {"CHANST", NULL, channel_stats, SERVERREQ,
     "%Y<%n-ALL%Y> %R[%Bchannel%R]%n\n- Shows statistics on current channel or "
     "%R[%Bchannel%R]%n"},
    {"CHAT", "Chat", chat, SERVERREQ,
    ...
    {"CHGPASS", "ChgPass", change_user, 0,
     "%Y<%Cnick%Y>%n %Y<%nprotection-level%Y>%n"},
    {"CHGPROT", "ChgProt", change_user, 0,
     "%Y<%Cnick%Y>%n %Y<%nprotection-level%Y>%n"},
    {"CHGUH", "ChgUH", change_user, 0, "%Y<%Cnick%Y>%n %Y<%nuserhost%Y>%n"},
#endif
    {"CHOPS", "Chops", users, SERVERREQ,
     "%R[%Bchannel%R]%n\n- Shows, in a full format, all the nicks with op "
     "status"},
    ...
    {"CMSG", "Cmsg", cmsg, 0,
     "%Y<%Cnick%Y>%n %Y<%ntext%Y>%n\n- While in the TwilightZone, a private "
     "message will be sent to %Y<%Cnick%Y>%n with %Y<%ntext%Y>%n"},
#else
    {"CLONES", "Clones", check_clones, SERVERREQ, NULL},
#endif
#ifdef __EMXPM__
    {"CODEPAGE", "CODEPAGE", pmcodepage, 0,
     "OS/2 - Changes the current VIO Codepage"},
#endif
    {"CONNECT", "CONNECT", send_comm, SERVERREQ,
     "%Y<%nserver1%Y>%n %Y<%nport%Y>%n %R[%nserver2%R]%n\n%Y*%n Requires irc "
     "operator status"},
    ...
    {"CSET", "Cset", cset_variable, 0,
     "%R[%n*|channel|chan*%R]%n\n- changes variables that affect a channel or "
     "displays them"},
    {"CTCP", NULL, ctcp, SERVERREQ,
     "%Y<%Cnick%Y>%n %Y<%nrequest%Y>%n\n- CTCP sends %Y<%Cnick%Y>%n with "
     "%Y<%nrequest%Y>%n"},
#ifndef BITCHX_LITE
    ...
    {"CYCLE", NULL, cycle, SERVERREQ,
     "%R[%Bchannel%R]%n\n- Leaves current channel or %R[%Bchannel%R]%n and "
     "immediately rejoins"},
    ...
    {"DC", NULL, init_dcc_chat, SERVERREQ,
     "%Y<%Cnick%Y>%n\n- Starts a DCC CHAT to %Y<%Cnick%Y>%n"},
    {"DCA", "Dca", dcx, 0, "- Kills all dcc connections"},
    {"DCC", NULL, dcc, 0,
     "Displays a active DCC list. For more info /dcc help"},
    {"DCG", "Dcg", dcx, 0,
...
#endif
    {"DEHOP", "h", dodeop, SERVERREQ,
     "%Y<%C%nnick(s)%Y>%n\n- Removes halfops from %Y<%Cnick%y(%Cs%y)%Y>%n"},
    {"DEOP", "o", dodeop, SERVERREQ,
     "%Y<%C%nnick(s)%Y>%n\n- Deops %Y<%Cnick%y(%Cs%y)%Y>%n"},
    {"DEOPER", NULL, deop, SERVERREQ,
    ...
    {"DESCRIBE", NULL, describe, SERVERREQ,
     ...
     "%Y<%Cnick%G|%Bchannel%Y>%n with %Y<%naction%Y>%n"},
#ifdef ALLOW_DETACH
    {"DETACH", NULL, detachcmd, 0, NULL},
#endif
    {"DEVOICE", "v", dodeop, SERVERREQ,
    ...
    {"DNS", "NSlookup", nslookup, 0,
     "%YDNS<%nnick|hostname%y>%n\n- Attempts to nslookup on nick or hostname"},
    ...
    {"DUMP", "Dump", dumpcmd, 0,
     "%Y<%ntype%Y>%n\n- Dumps %Y<%ntype%Y>%n to "
     "screen\n%Y<%ntype%Y>%n:\n%YAlias%n  %YAll%n  %YBind%n  %YChstats%n  "
    ...
    {"EVALSERVER", NULL, evalserver, 0,
     "<refnum> <command> sends to <refnum> server command"},
#if !defined(WINNT)
    {"EXEC", NULL, execcmd, 0,
     "%Y<%ncommand%Y>%n\n- Executes %Y<%ncommand%Y>%n with the shell set from "
     "%PSHELL%n"},
#endif
...
#endif
    {"FINGER", "FINGER", ctcp_simple, 0,
     "%Y<%Cnick%Y>%n\n- Sends a CTCP FINGER query to %Y<%Cnick%Y>%n"},
    {"FK", "FK", masskick, SERVERREQ,
     "%Y<%Cnick%G!%nuser%Y@%nhostname%Y>%n%R[%nreason%R]%n\nFinds clienTs "
     "matching %Y<%Cnick%G!%nuser%Y@%nhostname%Y>%n and immediately kicks them "
     "from current channel for %R[%nreason%R]%n"},
    ...
    {"FPORTS", NULL, findports, 0,
     "%Y<%nhostname%Y>%n %R[%Y<%nlowport%Y>%n %Y<%nhighport%Y>%R]%n\n- "
     "Attempts to find ports on %Y<%nhostname%Y>%n on %R[%Y<%nlowport%Y>%n "
     "%Y<%n %Y<%nhighport%Y>%R]%n"},
...
#ifdef WANT_FTP
    {"FTP", "Ftp", init_ftp, 0,
     "%Y<%nhostname%Y> %R[%nlogin%R] [%npassword%R] [%n-p port%R]%n\n- Open a "
     "ftp connection to a host"},
#endif
    {"FUCK", "Fuck", kickban, SERVERREQ,
     "%Y<%Cnick%Y>%n %R[%nreason%R]%n\n- Deops, bans and kicks %Y<%Cnick%Y>%n "
     "for %R[%nreason%R]%n"},
    {"FUCKEM", NULL, fuckem, SERVERREQ, NULL},
    {"GONE", "Gone", away, SERVERREQ,
     "%R[%nreason%R]%n\n- Sets you away on server if %R[%nreason%R]%n else "
     "sets you back. Does not announce to your channels."},
    {"HASH", "HASH", send_comm, SERVERREQ, "Server command"},
    {"HELP", NULL, help, 0, NULL},
    {"HELPOP", "HELPOP", send_comm, SERVERREQ, NULL},
    {"HELPSERV", "HELPSERV", send_comm, SERVERREQ,
     "- Network services command"},
    ...
    {"HOSTNAME", "HOSTNAME", e_hostname, 0,
     "%Y<%nhostname%Y>%n\n- Shows list of possible hostnames with option to "
     "change it on virtual hosts"},
    {"I", "INVITE", do_invite, SERVERREQ, "- See %YINVITE%n"},
    {"IF", "IF", ifcmd, 0, scripting_command},
    {"IG", "Ig", do_ig, 0,
     "+%G|%n-%Y<%Cnick%Y>%n\n- Ignores ALL except crap and public of nick!host "
     "matching %Y<%Cnick%Y>%n"},
    {"IGH", "IgH", do_ig, 0,
     "+%G|%n-%Y<%Cnick%Y>%n\n- Ignores ALL except crap and public of hostname "
     "matching %Y<%Cnick%Y>%n"},
    {"IGHT", "IgHt", do_ig, 0,
     "+%G|%n-%Y<%Cnick%Y>%n\n- Ignores ALL except crap and public of hostname "
     "matching %Y<%Cnick%Y>%n and expires this on a timer"},
    ...
    {"INVITE", "INVITE", do_invite, SERVERREQ,
     "%Y<%Cnick%Y>%n %R[%Bchannel%R]%n\n- Invites %Y<%Cnick%Y>%n to current "
     "channel or %R[%Bchannel%R]%n"},
    ...
    {"J", "JOIN", e_channel, SERVERREQ,
     "%Y<%nchannel%Y> %R[%nkey%R]%n\n- Joins a %Y<%nchannel%Y>"},
    {"JNW", "Jnw", jnw, SERVERREQ,
     "%Y<%nchannel%Y> %R[%nkey%R]%n %R[%n-hide%R]%n\n- Joins a "
     "%Y<%nchannel%Y>%n in a new window"},
    {"JOIN", "JOIN", e_channel, SERVERREQ, NULL},
    {"K", NULL, dokick, SERVERREQ,
     "%Y<%Cnick%Y>%n %R[%nreason%R]%n\n- Kicks %Y<%Cnick%Y>%n for "
     "%R[%nreason%R]%n on current channel"},
    {"KB", "KB", kickban, SERVERREQ,
     "%Y<%Cnick%Y>%n %R[%nreason%R]%n\n- Deops, kicks and bans %Y<%Cnick%Y>%n "
     "for %R[%nreason%R]%n"},
    {"KBI", "KBI", kickban, SERVERREQ,
     "%Y<%Cnick%Y>%n %R[%nreason%R]%n\n- Deops, kicks and bans %Y<%Cnick%Y>%n "
     "for %R[%nreason%R]%n"},
    {"KICK", "KICK", send_kick, SERVERREQ,
     "%Y<%Bchannel%G|%Y*>%n %Y<%Cnick%Y>%n %R[%nreason%R]%n"},
    {"KICKIDLE", "KickIdle", kickidle, SERVERREQ,
     "%Y<%Bchannel%Y>%n\n- Kicks all idle people on %Y<%Bchannel%Y>%n"},
    {"KILL", "KILL", send_kill, SERVERREQ,
     "%Y<%Cnick%Y>%n %R[%nreason%R]%n\n%Y*%n Requires irc operator status\n- "
     "Kills %Y<%Cnick%Y>%n for %R[%nreason%R]%n"},
    {"KLINE", "KLINE", send_kline, SERVERREQ,
     "%Y<%nuser%y@%nhost%Y> <%nreason%Y>%n\n%Y*%n Requires irc operator "
    ...
    {"LAMENICKLIST", NULL, add_bad_nick, 0,
     "- Lists nicks in your lame nick list"},
    {"LASTLOG", NULL, lastlog, 0,
     "-file filename%G|%n-clear%G|%n-max #%G|%n-liternal "
     "pat%G|%n-beep%G|%nlevel"},
    {"LEAVE", "PART", send_comm, SERVERREQ,
     "%Y<%Bchannel%Y>%n\n- Leaves current channel or %Y<%Bchannel%Y>%n"},
    {"LINKS", "LINKS", send_comm, SERVERREQ,
...
#endif
    {"LK", "LameKick", LameKick, SERVERREQ,
     "%R[%nreason%R]%n\n- Kicks all non +o people on current channel with "
     "%R[%nreason%R]%n"},
    {"LKW", "Lkw", lkw, SERVERREQ,
     "%R[%Bchannel%R]%n\n- Leave the current channel, killing the window in "
     "current channel or%R[%Bchannel%R]%n as well"},
    {"LLOOK", NULL, linklook, SERVERREQ,
     "%Y*%n Requires set %YLLOOK%n %RON%n\n- Lists all the servers which are "
     "current split from the IRC network"},
    ...
    {"MDVOICE", "MDVoice", massdeop, SERVERREQ,
     "- Mass de-voices current channel"},
    {"ME", NULL, me, SERVERREQ,
     "<action>\n- Sends an action to current channel"},
    {"MEMOSERV", "MEMOSERV", send_comm, SERVERREQ,
     "- Network services command"},
#ifdef GUI
    ...
    {"MKNU", NULL, mknu, SERVERREQ,
     "- Mass kick bans non-userlist users on current channel"},
    {"MODE", "MODE", send_channel_com, SERVERREQ,
     "%Y<%n#channel%W|%nnick%Y>%B +-%Y<%nmode%Y>%n"},
    {"MODELOCK", "ModeLock", mode_lock, 0,
     "%Y<%Bchannel%Y>%n +%G|%n-%Y<%ninstampkl #%Y>%n\n- Locks "
     "%Y<%Bchannel%Y>%n with +%G|%n-%Y<%nmodes%Y>%n"},
    ...
    {"MSG", "PRIVMSG", e_privmsg, 0,
     "%Y<%C[=]nick%Y>%n %Y<%ntext%Y>%n\n- Send %Y<%Cnick%Y>%n a message with "
     "%Y<%ntext%Y>%n"},
    {"MTOPIC", "MTopic", do_mtopic, 0, NULL},
    {"MUB", NULL, unban, SERVERREQ, "- Mass unbans current channel"},
    {"MULT", NULL, multkick, SERVERREQ,
     "%Y<%nnick(s)%Y>%n %R[%nreason%R]%n\n- Kicks all %Y<%nnick(s)%Y>%n with "
     "%R[%nreason%R]"},
    {"MVOICE", "MVoice", massop, SERVERREQ,
     "%R[%Cnick%G|%Bchannel%G|%Cnick%Y!%Cuser%Y@%nhostname%Y]%n\n- Mass voice "
     "nicks matching "
     "%R[%Cnick%G|%Bchannel%G|%Cnick%Y!%Cuser%Y@%nhostname%Y]%n"},
    {"N", "NAMES", do_mynames, 0, NULL},
    {"NAMES", "NAMES", funny_stuff, SERVERREQ,
     "%R[%Bchannel%R]%n\n- Shows names on current channel or "
     "%R[%Bchannel%R]%n"},
    {"NEWNICK", NULL, newnick, 0, NULL},
    {"NEWUSER", NULL, newuser, 0, NULL},
    {"NICK", "NICK", e_nick, 0, "-Changes nick to specified nickname"},
    {"NICKSERV", "NICKSERV", send_comm, SERVERREQ,
     "- Network services command"},
    {"NOCHAT", "NoChat", chat, 0,
     "%Y<%nnick%Y>%n\n- Kills chat reqest from %Y<%nnick%Y>"},
    {"NOPS", "Nops", users, 0,
     "%R[%Bchannel%R]%n\n- Shows, in a full format, all the nicks without ops "
     "in %R[%Bchannel%R]%n or channel"},
    {"NOTE", "NOTE", send_comm, SERVERREQ, NULL},
    {"NOTICE", "NOTICE", e_privmsg, 0,
     "%Y<%Cnick%G|%Bchannel%Y> %Y<%ntext%Y>%n\n- Sends a notice to "
     "%Y<%Cnick%G|%Bchannel%Y> with %Y<%ntext%Y>%n"},
    {"NOTIFY", NULL, notify, 0,
     "%Y<%Cnick|+|-nick%Y>%n\n- Adds/displays/removes %Y<%Cnick%Y>%n to notify "
     "list"},
    {"NSLOOKUP", "NSLookup", nslookup, 0,
    ...
    {"NWHOWAS", NULL, whowas, 0,
     "- Displays internal whowas info for all channels. This information "
     "expires after 20 minutes for users on internal list, 10 minutes for "
     "others"},
    {"OBITS", NULL, obits, 0,
     "- Displays obituaries for some BitchX friends who have left us"},
    {"OFFERS", "Offers", do_offers, 0, NULL},
    {"ON", NULL, oncmd, 0, scripting_command},
    {"OOPS", NULL, do_oops, SERVERREQ,
     "%Y<%Cnick%Y>%n\n- Sends a oops message to last recipient of a message "
     "and sends the correct message to %Y<%Cnick%Y>%n"},
    {"OP", "o", doop, SERVERREQ, "%Y<%Cnick%Y>%n\n- Gives %Y<%Cnick%Y>%n +o"},
    {"OPER", "OPER", oper, SERVERREQ,
     "%Y*%n Requires irc operator status\n%Y<%Cnick%Y>%n %R[%npassword%R]%n"},
    {"OPERSERV", "OPERSERV", send_comm, SERVERREQ,
     "- Network services command"},
    {"OPERWALL", "OPERWALL", e_wall, 0, NULL},
    {"ORIGNICK", "OrigNick", orig_nick, 0,
     "%Y<%Cnick%Y>%n\n- Attempts to regain a nick that someone else has taken "
     "from you."},
    {"OSTAT", NULL, serv_stat, 0,
     "-Displays some internal statistics gathered about the server"},
#ifdef WANT_OPERVIEW
    {"OV", NULL, ov_window, 0,
     "%R[%non|off%R]%n %R[%-hide|+hide|hide%R]%n\n- turns on/off oper view "
     "window as well as creating hidden window"},
#endif
    ...
    {"RANDOMNICK", NULL, randomnick, 0,
     "%Y<%Cnick%Y>%n\n- Changes your nick to a random nick. If nick is "
     "specified is is used as a prefix"},
    ...
    {"REDIRECT", NULL, redirect, 0,
     "%Y<%Cnick%G|%Bchannel%Y> <command>\n- Redirects <command> to "
     "%Y<%Cnick%G|%Bchannel%Y>"},
    {"REHASH", "REHASH", send_comm, SERVERREQ,
     "%Y*%n Requires irc operator status\n- Rehashs ircd.conf for new "
     "configuration"},
    {"REINIT", NULL, init_vars, 0, "- Reinitializes internal variables"},
    {"REINITSTAT", NULL, init_window_vars, 0,
    ...
    {"REPEAT", NULL, repeatcmd, 0,
     "%Y<%ntimes%Y>%n %Y<%ncommand%Y>%n\n- Repeats %Y<%ntimes%Y>%n "
     "%Y<%ncommand%Y>%n"},
    {"REQUEST", NULL, ctcp, 0,
     "%Y<%Cnick%G|%Bchannel%Y>%n %Y<%nrequest%Y>%n\n- Sends CTCP "
     "%Y<%nrequest%Y>%n to %Y<%Cnick%G|%Bchannel%Y>%n"},
    {"RESET", NULL, reset, 0, "- Fixes flashed terminals"},
    {"RESTART", "RESTART", send_comm, SERVERREQ,
     "%Y*%n Requires irc operator status\n- Restarts server"},
    {"RETURN", NULL, returncmd, 0, NULL},
    {"REVISIONS", NULL, show_revisions, 0,
     "- Shows CVS revisions of source files."},
    /* nuxx requested this command */
    {"RMAP", "MAP", send_2comm, SERVERREQ,
     "- Sends out a /map command to the server%n"},
    ...
    {"SAY", empty_string, do_send_text, 0,
     "-%Y<%ntype%Y>%n Says whatever you write"},
    {"SC", "NAMES", do_mynames, 0, NULL},
    {"SCAN", "scan", do_scan, 0,
     "%R[%Bchannel%R]%n\n- Scans %R[%Bchannel%R]%n or current channel for all "
     "nicks"},
    {"SCANB", "ScanB", do_scan, 0,
     "%R[%Bchannel%R]%n\n- Scans %R[%Bchannel%R]%n or current channel for "
     "bots"},
    {"SCANF", "ScanF", do_scan, 0,
     "%R[%Bchannel%R]%n\n- Scans %R[%Bchannel%R]%n or current channel for "
     "friends"},
    ...
    {"SHOWIDLE", "ShowIdle", showidle, 0,
     "%Y<-sort %R[none|time|host|nick%R]%Y> %R[%Bchannel%R]%n\n- Shows idle "
     "people on current channel or %R[%Bchannel%R]%n"},
    {"SHOWLOCK", "ShowLock", mode_lock, 0,
     "- Show the mode lock on the channel"},
    {"SHOWSPLIT", NULL, linklook, 0, "- Shows split servers"},
    {"SHOWWORDKICK", NULL, show_word_kick, 0,
     "- Shows the internal banned word list"},
    {"SILENCE", "SILENCE", send_comm, SERVERREQ,
     "%R[%n+nick|-nick%R]%n\n- Server-side ignore"},
    {"SLEEP", NULL, sleepcmd, 0, scripting_command},
    {"SPAM", NULL, spam, 0, NULL},
    {"SPING", "Sping", sping, 0,
     "%Y<%nserver|.|-clear%Y>%n\n- Checks how lagged you are to "
     "%Y<%nserver%Y>%n"},
    {"SQUERY", "SQUERY", send_2comm, SERVERREQ,
     "%Y<%nservice%Y> <%ntext%Y>%n\n - Sends %Y<%ntext%Y>%n to "
     "%Y<%nservice%Y>%n"},
    {"SQUIT", "SQUIT", send_2comm, SERVERREQ,
     "%Y<%nserver1%Y>%n %R[%nserver2%R]%n\n%Y*%n Requires irc operator "
     "status\n- Disconnects %Y<%nserver1%Y>%n from current server or "
     "%R[%nserver2%R]%n"},
    ...
    {"SUMMON", "SUMMON", send_comm, SERVERREQ,
     "%Y<%nuser%Y> %R[%nserver%R [%nchannel%R]]%n\n- calls %Y<%nuser%Y>%n on "
     "%R[%nserver%R]%n to %R[%nchannel%R]%n"},
    {"SV", "Sv", show_version, 0,
     "%R[%nnick|#channel%R]%n\nShow Client Version information"},
    {"SWALLOP", "SWALLOPS", e_wall, 0, NULL},
#ifdef WANT_OPERVIEW
    {"SWATCH", NULL, s_watch, 0,
    ...
    {"TKB", "TKB", kickban, 0,
     "%Y<%Cnick%Y>%n %R[%ntime%R]%n\n- Deops, kicks, and bans %Y<%Cnick%Y>%n "
     "for %R[%ntime%R]%n"},
    {"TKLINE", "TKLINE", send_kline, 0,
     "%Y<%nuser%y@%nhost%Y> <%nreason%Y>%n\n%Y*%n Requires irc operator "
     "status\n- Adds a temporary K-line"},
    {"TLOCK", "TLock", topic_lock, 0,
     "%Y<%Bchannel%Y>%n %R[%non%G|%noff%R]%n\n- [Un]Locks %Y<%Bchannel%Y>%n "
     "with the current topic"},
    {"TOGGLE", NULL, do_toggle, 0,
     "- Various Client toggles which can be change"},
    {"TOPIC", "TOPIC", e_topic, 0,
     "- Change or show the current or specified channel topic"},
    {"TRACE", "TRACE", do_trace, 0,
     "<argument> <server>\n- Without a specified server it shows the current "
     "connections on local server\n- Arguments: -s -u -o   trace for servers, "
     "users, ircops"},
    ...
    {"UNFORWARD", "NoForward", do_forward, 0,
     "%Y<%Cnick%G|%Bchannel%Y>\n- Remove %Y<%Cnick%G|%Bchannel%Y> from forward "
     "list"},
    ...
    {"UNKLINE", "UNKLINE", send_kline, 0,
     "<host|host,host2> remove klines for host(s)"},
    ...
    {"UNUSER", "UnUser", add_user, 0,
     "%Y<%nnick%W|%nnick!user@hostname%Y> <%n#channel%W|%n*%Y>%n"},
#endif
    {"UNVOICE", "v", dodeop, 0, NULL},
    {"UNWORDKICK", "UnWordKick", add_ban_word, 0,
     "%Y<%n#channel%Y> <%nword%Y>%n"},
    ...
    {"WHICH", "WHICH", BX_load, 0,
     "- Shows which script would be loaded without loading it"},
    {"WHILE", "WHILE", whilecmd, 0, scripting_command},
    {"WHO", "Who", whocmd, SERVERREQ, NULL},
    {"WHOIS", "WHOIS", whois, SERVERREQ, NULL},
    {"WHOKILL", "WhoKill", whokill, SERVERREQ,
     "%Y<%Cnick%G!%nuser%Y@%nhostname%Y>%n %R[%nreason%R]%n\n%Y*%n Requires "
     "irc operator status\n- Kills multiple clients matching the filter "
     "%Y<%Cnick%G!%nuser%Y@%nhostname%Y>%n with %R[%n%W:%nreason%R]%n"},
...
BUILT_IN_COMMAND(obits) {
  charset_ibmpc();
put_it("%s",
          convert_output_format("%P" "?" "?" "?" "?" "?" "---%p?%P--%p" "?" "?" "%P-%p" "?" "?" "?" "?" "---%K?%p--?%K?"
                                "-%p" "?" "?" "?" "%K" "?" "?" "?" "?-%p?-%K" "?" "?" "---%K --  -%n",
                                NULL, NULL));
  put_it("%s", convert_output_format("%P|%n ", NULL, NULL));
  put_it("%s", convert_output_format("%P?%n  From its humble beginnings to the "
                                     "latest release, BitchX has always been",
                                     NULL, NULL));
  put_it("%s", convert_output_format("%P?%n  about community. Over the years, "
                                     "a handful of people have voluntarily ",
                                     NULL, NULL));
  put_it("%s", convert_output_format("%P|%n  contributed a great deal of time "
                                     "and energy to the project by writing ",
                                     NULL, NULL));
  put_it("%s",
         convert_output_format("%p:%n  scripts, providing support, "
                               "troubleshooting, patches, enhancements - or ",
                               NULL, NULL));
  put_it("%s", convert_output_format("%P|%n  simply making BitchX a fun thing "
                                     "to be involved with. With such a long ",
                                     NULL, NULL));
  put_it("%s", convert_output_format("%p'%n  history, we have had our losses. "
                                     "Several of the friends that we came to ",
                                     NULL, NULL));
  put_it("%s",
         convert_output_format("%p|%n  regard as a kind of extended family "
                               "left us before their time, and we would",
                               NULL, NULL));
  put_it("%s", convert_output_format("%P?%n  like to remember them here.", NULL,
                                     NULL));
  put_it("%s", convert_output_format("%p?%n ", NULL, NULL));
  put_it("%s", convert_output_format("%p:%n                            %P?%n ",
                                     NULL, NULL));
  put_it("%s",
         convert_output_format("%K|%n                            %P?%n Jason "
                               "Higham, known to his friends on IRC as ",
                               NULL, NULL));
  put_it("%s", convert_output_format(
                   "%p|%n                            %P?%n %Wssshooter%n, left "
                   "us on July 11th, 2004. He is ",
                   NULL, NULL));
  put_it("%s",
         convert_output_format("                             %P?%n remembered "
                               "fondly by his family and friends.  ",
                               NULL, NULL));
  put_it("%s", convert_output_format("%K|%n  %P?%n", NULL, NULL));
  put_it("%s", convert_output_format(
                   "   %P?%n On the 18th of July, 2004, our dearly missed",
                   NULL, NULL));
  put_it("%s", convert_output_format(
                   "   %P?%n friend Miles Wilson (also known as %Wsellfone%n)",
                   NULL, NULL));
  put_it("%s", convert_output_format("   %P?%n passed away in his sleep.", NULL,
                                     NULL));
  put_it("%s", convert_output_format("                     %P?%n", NULL, NULL));
  put_it("%s", convert_output_format("                     %P?%n Amy Elizabeth "
                                     "Haskew died on June 16, 2009 at",
                                     NULL, NULL));
  put_it("%s", convert_output_format("                     %P?%n 34.  Goodbye "
                                     "%Wturtlex%n, you'll always be",
                                     NULL, NULL));
  put_it("%s", convert_output_format("                     %P?%n remembered as "
                                     "someone very kind, and wise beyond",
                                     NULL, NULL));
  put_it("%s", convert_output_format("                     %P?%n your years.",
                                     NULL, NULL));
  put_it("%s", convert_output_format("      %P?%n", NULL, NULL));
  put_it("%s", convert_output_format("      %P?%n RIP Marc Casillo aka "
                                     "%Wfrash%n [25/Nov/1970 - 28/Sep/2009] ",
                                     NULL, NULL));
  put_it("%s",
         convert_output_format("                         %P?%n", NULL, NULL));
  put_it("%s",
         convert_output_format("                         %P?%n Matthew "
                               "Luberto, known to his many friends online",
                               NULL, NULL));
  put_it("%s",
         convert_output_format("                         %P?%n as %Wvoid%n, "
                               "passed away in his sleep on February 16, ",
                               NULL, NULL));
  put_it("%s", convert_output_format("                         %P?%n 2013 in "
                                     "his 38th year.  We'll miss you, void.",
                                     NULL, NULL));
  put_it(" ");
#if defined(LATIN1)
  charset_lat1();
...
const IrcCommand *BX_find_command(const char *com, int *cnt) {
  ...
  retval = (IrcCommand *)find_fixed_array_item(
      (void *)irc_command, sizeof(IrcCommand), NUMBER_OF_COMMANDS + 1, com, cnt,
      &loc);
  return retval;
}
...
BUILT_IN_COMMAND(do_forward) {
  ...
  if (command && (!my_stricmp(command, "NOFORWARD") ||
                  !my_stricmp(command, "UNFORWARD"))) {
    if (forwardnick) {
...
BUILT_IN_COMMAND(blesscmd) { bless_local_stack(); }
...
BUILT_IN_COMMAND(do_oops) {
  ...
  if (sent_nick && sent_body && to && *to) {
    ...
    if (window_display && do_hook(SEND_MSG_LIST, "%s %s", to, sent_body))
      put_it("%s", convert_output_format(fget_string_var(FORMAT_SEND_MSG_FSET),
                                         "%s %s %s %s", update_clock(GET_TIME),
                                         to, get_server_nickname(from_server),
                                         sent_body));
  }
}
...
BUILT_IN_COMMAND(reconnect_cmd) {
  ...
  if (do_hook(DISCONNECT_LIST, "Reconnecting to server"))
    put_it("%s",
           convert_output_format("$G Reconnecting to server %K[%W$1%K]",
                                 "%s %d", update_clock(GET_TIME), from_server));
...
BUILT_IN_COMMAND(my_clear) {
  char *arg;
  int all = 0, scrollback = 0, unhold = 0;
...
BUILT_IN_COMMAND(do_invite) {
  ...
  if (args && *args) {
    while (1) {
      ...
      if (args && *args) {
        if (!is_channel(args) ||
            !(chan = prepare_command(&server, args, NO_OP)))
          return;
      ...
      if (!chan)
        return;
      my_send_to_server(server, "INVITE %s %s%s%s", inick, chan->channel,
                        chan->key ? space : empty_string,
                        chan->key ? chan->key : empty_string);
    }
  }
  return;
}
...
BUILT_IN_COMMAND(do_getout) {
  ...
  if (!all) {
    ...
    if (!(chan = prepare_command(
              &server, channel ? make_channel(channel) : channel, NO_OP)))
      return;
    my_send_to_server(server, "PART %s", chan->channel);
    if ((tmp = (AJoinList *)remove_from_list((List **)&ajoin_list,
                                             chan->channel))) {
      if (!tmp->ajoin_list) {
        new_free(&tmp->name);
        new_free(&tmp->key);
...
BUILT_IN_COMMAND(do_4op) {
  ...
  if (!nick)
    return;
  my_send_to_server(server, "MODE %s +oooo %s %s %s %s", chan->channel, nick,
                    nick, nick, nick);
}
...
int nick_in_scan(NickList *nick, enum SCAN_TYPE scan_type, char *mask) {
  ...
  case SCAN_CHANOPS:
    if (!nick_isop(nick) && !nick_ishalfop(nick))
      return 0;
    break;
...
BUILT_IN_COMMAND(do_scan) {
  ...
  put_it("%s", convert_output_format(
                   s, "%s %s %d %d %s", update_clock(GET_TIME), chan->channel,
  ...
  for (nick = snick; nick; nick = nick->next) {
    ...
    malloc_strcpy(&nick_buffer,
                  convert_output_format(nick_format, "%s", nick->nick));
    malloc_strcat(&buffer, convert_output_format(user_format, "%c %s", nick_sym,
                                                 nick_buffer));
    ...
    if (count++ >= (cols - 1)) {
      if (fget_string_var(FORMAT_NAMES_BANNER_FSET))
        put_it("%s%s",
               convert_output_format(fget_string_var(FORMAT_NAMES_BANNER_FSET),
                                     NULL, NULL),
               buffer);
      else
        put_it("%s", buffer);
  ...
  if (fget_string_var(FORMAT_NAMES_FOOTER_FSET))
    put_it("%s",
           convert_output_format(fget_string_var(FORMAT_NAMES_FOOTER_FSET),
                                 "%s %s %d %d %s", update_clock(GET_TIME),
                                 chan->channel, n_inscan, n_total,
...
BUILT_IN_COMMAND(pingcmd) {
  ...
  switch (ping_type) {
  case 1:
    snprintf(buffer, 100, "%s PING %lu %lu", to, (unsigned long)tp.tv_sec,
             (unsigned long)tp.tv_usec);
    break;
  case 2:
    snprintf(buffer, 100, "%s ECHO %lu %lu", to, (unsigned long)tp.tv_sec,
             (unsigned long)tp.tv_usec);
    break;
  default:
...
BUILT_IN_COMMAND(do_offers) {
  ...
  if ((type = in_ctcp()) == -1)
    echocmd(NULL, "*** You may noT use the CTCP command in an ON CTCP_REPLY!",
            empty_string, NULL);
  else {
...
BUILT_IN_COMMAND(ctcp) {
  ...
  if ((to = next_arg(args, &args)) != NULL) {
    ...
    if ((type = in_ctcp()) == -1)
      ...
      put_it("%s", convert_output_format(fget_string_var(FORMAT_SEND_CTCP_FSET),
                                         "%s %s %s %s", update_clock(GET_TIME),
                                         to, stag ? stag : "VERSION",
                                         args ? args : empty_string));
      add_last_type(&last_sent_ctcp[0], 1, NULL, NULL, to,
                    stag ? stag : "VERSION");
    }
  }
}
...
BUILT_IN_COMMAND(funny_stuff) {
  char *arg, *stuff;
  int min = 0, max = 0, flags = 0, ircu = 0;
...
BUILT_IN_COMMAND(echocmd) {
  ...
  int old_und = 0, old_rev = 0, old_bold = 0, old_color = 0, old_blink = 0,
      old_ansi = 0;
  int xtended = 0;
    ...
    while (more && args && (*args == '-' || *args == '/')) {
      ...
      case 'L': {
        ...
        if (toupper(flag_arg[2]) == 'I') /* LINE */
        {
          ...
          if (!target_window) {
            debugyell("%s: -LINE only works if -WIN is specified first",
                      command);
...
BUILT_IN_COMMAND(oper) {
  ...
  if (!(password = next_arg(args, &args))) {
    add_wait_prompt("Operator Password:", oper_password_received, nick,
                    WAIT_PROMPT_LINE, 0);
    return;
  }
...
AJoinList *add_to_ajoin_list(char *channel, char *args, int type) {
  ...
  if (!(new = (AJoinList *)list_lookup((List **)&ajoin_list, channel, 0,
                                       !REMOVE_FROM_LIST))) {
    char *key, *group;
...
BUILT_IN_COMMAND(auto_join) {
  ...
  if (command && *command && !my_stricmp(command, "AJoinList")) {
    ...
    for (new = ajoin_list; new; new = new->next) {
      ...
      if (!count)
        put_it("AJoin List");
      put_it("%20s server/group: %s%s%s", new->name,
             new->group ? new->group : "<none>",
             new->key ? " key: " : empty_string,
             new->key ? new->key : empty_string);
      count++;
    }
    if (count)
  ...
  if (command && *command && !my_stricmp(command, "UNAJOIN")) {
    if (channel && (new = (AJoinList *)remove_from_list(
                        (List **)&ajoin_list, make_channel(channel)))) {
      if (new->ajoin_list) {
  ...
  if (is_server_connected(
          from_server) /* && get_server_channels(from_server)*/) {
    bitchsay("Auto-Joining %s%s%s", new->name, new->key ? space : empty_string,
             new->key ? new->key : empty_string);
    if (!new->group || is_server_valid(new->group, from_server)) {
      ...
      send_to_server("JOIN %s %s", new->name,
                     new->key ? new->key : empty_string);
    }
  }
}
...
BUILT_IN_COMMAND(e_channel) {
  ...
  if ((chan = next_arg(args, &args)) != NULL) {
    if (!my_strnicmp(chan, "-i", 2)) {
      ...
      if (!is_server_connected(from_server))
      ...
      else if (is_on_channel(buffer, from_server,
        ...
        if (is_bound_anywhere(buffer) &&
          ...
          if (do_hook(CHANNEL_SWITCH_LIST, "%s",
        ...
        if (get_int_var(JOIN_NEW_WINDOW_VAR) &&
            (current_window->current_channel || current_window->query_nick))
          win_create(JOIN_NEW_WINDOW_VAR, 0);
        send_to_server("%s %s%s%s", command, buffer,
                       args ? space : empty_string, args ? args : empty_string);
...
BUILT_IN_COMMAND(comment) { /* nothing to do... */ }
...
BUILT_IN_COMMAND(e_nick) {
  ...
  if (!(nick = next_arg(args, &args))) {
    ...
    if (get_pending_nickname(get_window_server(0)))
      say("A nickname change to %s is pending.",
          get_pending_nickname(get_window_server(0)));
...
void add_address(Virtuals **vhost_list, int norev, struct sockaddr *sa) {
  ...
  if (!strcmp(result, "127.0.0.1") || !strcmp(result, "0.0.0.0") ||
      !strcmp(result, "::1") || !strcmp(result, "0::1") ||
      !strcmp(result, "::") || !strcmp(result, "0::0") ||
      !strncasecmp(result, "fe8", 3) || !strncasecmp(result, "fe9", 3) ||
      !strncasecmp(result, "fea", 3) || !strncasecmp(result, "feb", 3)) {
    return;
  }
...
BUILT_IN_COMMAND(e_hostname) {
  ...
  if (arg && *arg && *arg != '#' && !is_number(arg)) {
    ...
    for (i = 1; virtuals; i++) {
      ...
      if (i == 1)
        put_it("%s", convert_output_format("$G Current hostnames available",
                                           NULL, NULL));

      put_it("%s", convert_output_format("%K[%W$[3]0%K] %B$1", "%d %s", i,
                                         new->hostname));
      if (i == switch_to)
  ...
  if (newhost) {
    ...
    if ((hp = gethostbyname(LocalHostName)))
      memcpy((void *)&LocalHostAddr.sf_addr, hp->h_addr,
...
BUILT_IN_COMMAND(whois) {
  ...
  if (!my_stricmp(command, "WHOWAS")) {
    ...
    send_to_server(
        "WHOWAS %s %s",
        (word_one && *word_one) ? word_one : get_server_nickname(from_server),
  ...
  if (!(nick = next_arg(args, &args))) /* whois command */
  {
    ...
    send_to_server("WHOIS %s%s%s", nick, (args && *args) ? space : empty_string,
                   (args && *args) ? args : empty_string);
}
...
BUILT_IN_COMMAND(back) {
  ...
  if (get_server_awaytime(from_server)) {
    ...
    if (get_int_var(SEND_AWAY_MSG_VAR) && !silent) {
      bitchsay("You were /away for %i hours %i minutes and %i seconds. "
               "[\002BX\002-MsgLog %s]",
               hours, minutes, seconds, on_off(get_int_var(MSGLOG_VAR)));
  ...
  if (get_int_var(MSGLOG_VAR)) {
    char tmp[100];
    snprintf(tmp, sizeof(tmp), " read /away msgs (%d msg%s) log [Y/n]? ",
             get_int_var(MSGCOUNT_VAR), plural(get_int_var(MSGCOUNT_VAR)));
...
static void real_quit(char *dummy, char *ptr) {
  ...
  if (ptr && *ptr) {
    if (*ptr == 'Y' || *ptr == 'y') {
      irc_exit(1, dummy, "%s",
               convert_output_format(fget_string_var(FORMAT_SIGNOFF_FSET),
                                     "%s %s %s %s", update_clock(GET_TIME),
                                     get_server_nickname(get_window_server(0)),
                                     m_sprintf("%s@%s", username, hostname),
                                     dummy));
    }
  }
...
BUILT_IN_COMMAND(e_quit) {
  ...
  if (args && *args)
    ...
    strlcpy(Reason, get_signoffreason(get_server_nickname(from_server)),
            sizeof(Reason));
  if (!*Reason)
  ...
  if (active_dcc)
    add_wait_prompt("Active DCC\'s. Really Quit [y/N] ? ", real_quit, Reason,
                    WAIT_PROMPT_LINE, 1);
  else {
    from_server = old_server;
    irc_exit(1, Reason, "%s",
...
BUILT_IN_COMMAND(e_wall) {
  ...
  if (!get_server_umode(from_server, 'w'))
    put_it("!! %s", args);
  add_last_type(&last_sent_wall[0], 1, get_server_nickname(from_server), NULL,
                "*", args);
  reset_display_target();
}
...
BUILT_IN_COMMAND(do_trace) {
  ...
  while (args && *args) {
    ...
    if (flags && *flags == '-') {
      ...
      case 'O':
        set_server_trace_flag(from_server,
                              get_server_trace_flag(from_server) | TRACE_OPER);
        break;
      case 'U':
        set_server_trace_flag(from_server,
                              get_server_trace_flag(from_server) | TRACE_USER);
        break;
      case 'S':
        set_server_trace_flag(from_server, get_server_trace_flag(from_server) |
                                               TRACE_SERVER);
      default:
        break;
      }
  ...
  if (get_server_trace_flag(from_server) & TRACE_OPER)
    bitchsay("Tracing server %s%sfor Operators", serv ? serv : empty_string,
             serv ? space : empty_string);
  if (get_server_trace_flag(from_server) & TRACE_USER)
    bitchsay("Tracing server %s%sfor Users", serv ? serv : empty_string,
             serv ? space : empty_string);
  if (get_server_trace_flag(from_server) & TRACE_SERVER)
    bitchsay("Tracing server %s%sfor servers", serv ? serv : empty_string,
             serv ? space : empty_string);
  send_to_server("%s%s%s", command, serv ? space : empty_string,
                 serv ? serv : empty_string);
}
...
BUILT_IN_COMMAND(do_stats) {
  ...
  if (flags) {
    switch (*flags) {
    case 'o':
      set_server_stat_flag(from_server,
                           get_server_stat_flag(from_server) | STATS_OLINE);
      str = "%GType   Host                      Nick";
      break;
    case 'y':
      set_server_stat_flag(from_server,
                           get_server_stat_flag(from_server) | STATS_YLINE);
      str = "%GType   Class  PingF  ConnF  MaxLi      SendQ";
      break;
    case 'i':
      set_server_stat_flag(from_server,
                           get_server_stat_flag(from_server) | STATS_ILINE);
      str = "%GType   Host                      Host                       "
            "Port Class";
      break;
    case 'h':
      set_server_stat_flag(from_server,
                           get_server_stat_flag(from_server) | STATS_HLINE);
      str = "%GType    Server Mask     Server Name";
      break;
    case 'l':
      set_server_stat_flag(from_server,
                           get_server_stat_flag(from_server) | STATS_LINK);
      str = "%Glinknick|server               sendq smsgs sbytes rmsgs rbytes "
            "timeopen";
      break;
    case 'c':
      set_server_stat_flag(from_server,
                           get_server_stat_flag(from_server) | STATS_CLASS);
      str = "%GType   Host                      Host                       "
            "Port Class";
      break;
    case 'u':
      set_server_stat_flag(from_server,
                           get_server_stat_flag(from_server) | STATS_UPTIME);
      break;
    case 'k':
      set_server_stat_flag(from_server,
                           get_server_stat_flag(from_server) | STATS_TKLINE);
      str = "%GType   Host                      UserName    Reason";
      break;
    case 'K':
      set_server_stat_flag(from_server,
                           get_server_stat_flag(from_server) | STATS_KLINE);
      str = "%GType   Host                      UserName    Reason";
      break;
    case 'm':
      set_server_stat_flag(from_server,
                           get_server_stat_flag(from_server) | STATS_MLINE);
    default:
      break;
    }
    new_flag = flags;
...
BUILT_IN_COMMAND(send_kick) {
  ...
  reset_display_target();
  if (!(chan = prepare_command(&server, !strcmp(channel, "*") ? NULL : channel,
                               NEED_OP)))
    return;

  comment = args ? args : get_kick_reason(kickee, get_server_nickname(server));
  my_send_to_server(server, "%s %s %s :%s", command, chan->channel, kickee,
                    comment);
}
...
void redirect_text(int to_server, const char *nick_list, const char *text,
                   unsigned flags) {
...
static int try_send_special_target(char *target, const char *text,
                                   unsigned stxt_flags) {
  /*
  ...
  case '=':
    if (text && *text) {
      ...
      if (!is_number(target + 1) && !dcc_activechat(target + 1) &&
          !dcc_activebot(target + 1) && !dcc_activeraw(target + 1)) {
      ...
      add_last_type(&last_sent_dcc[0], MAX_LAST_MSG, NULL, NULL, target + 1,
                    text);
      from_server = old_server;
      new_free(&line);
    }
...
void BX_send_text(const char *nick_list, const char *text, unsigned flags) {
  ...
  int i, done_forward = 0, is_current = 0, old_window_display = window_display;
  char *current_nick, *next_nick, *free_nick, *line;

  struct target_type target[4] = {
      {NULL, SEND_MSG_LIST, "PRIVMSG", LOG_MSG, NULL, NULL, NULL},
      {NULL, SEND_PUBLIC_LIST, "PRIVMSG", LOG_PUBLIC, NULL, NULL, NULL},
      {NULL, SEND_NOTICE_LIST, "NOTICE", LOG_NOTICE, NULL, NULL, NULL},
      {NULL, SEND_NOTICE_LIST, "NOTICE", LOG_NOTICE, NULL, NULL, NULL}};
  ...
  target[1].format_encrypted =
      fget_string_var(FORMAT_SEND_ENCRYPTED_PUBLIC_FSET);
  target[2].output = fget_string_var(FORMAT_SEND_NOTICE_FSET);
  target[2].format_encrypted =
      fget_string_var(FORMAT_SEND_ENCRYPTED_NOTICE_FSET);
  target[3].output = fget_string_var(FORMAT_SEND_NOTICE_FSET);
  target[3].other_output = fget_string_var(FORMAT_SEND_NOTICE_FSET);
  target[3].format_encrypted =
      fget_string_var(FORMAT_SEND_ENCRYPTED_NOTICE_FSET);
  ...
  while ((current_nick = next_nick)) {
    ...
    if ((key = is_crypted(current_nick))) {
      ...
      line = sed_encrypt_msg(text, key);
      if (!(flags & STXT_QUIET) &&
          do_hook(target[i].hook_type, "%s %s", current_nick, text))
        put_it("%s",
               convert_output_format(target[i].format_encrypted, "%s %s %s %s",
                                     update_clock(GET_TIME), current_nick,
                                     get_server_nickname(from_server), text));
  ...
  for (i = 0; i < 4; i++) {
    ...
    if (i == 1 || i == 3) {
      ...
      if ((channel = get_current_channel_by_refnum(0))) {
        ...
        if ((chan = lookup_channel(channel, from_server, 0))) {
          nick = find_nicklist_in_channellist(get_server_nickname(from_server),
                                              chan, 0);
    ...
    if (!(flags & STXT_QUIET) &&
        (blah_hook =
             do_hook(target[i].hook_type, "%s %s", target[i].nick_list, text)))
      {}
    ...
    if (blah_hook || chan)
      s = convert_output_format(
          is_current ? target[i].output : target[i].other_output, "%s %s %s %s",
          update_clock(GET_TIME), target[i].nick_list,
          get_server_nickname(from_server), text);
    if (blah_hook)
      put_it("%s", s);
    ...
    if (i == 0) {
      ...
      add_last_type(&last_sent_msg[0], MAX_LAST_MSG, NULL, NULL,
                    target[i].nick_list, text);
    } else if ((i == 2) || (i == 3))
      add_last_type(&last_sent_notice[0], MAX_LAST_MSG,
                    get_server_nickname(from_server), NULL, target[i].nick_list,
                    text);
  ...
  if (!(flags & STXT_QUIET) && get_server_away(from_server) &&
...
#endif
        if (!my_stricmp(cmd, "SAY") || !my_stricmp(cmd, "PRIVMSG") ||
            !my_stricmp(cmd, "MSG"))
    ...
    if (*com == '/' || strchr(cmdchars, *com)) {
      *firstcmdchar = *cmdchars;
      ...
      if ((alias_cnt == 1) && (cmd_cnt == 0) && (dll_cnt <= 0)) {
        snprintf(buffer, sizeof buffer, "%s%s %s", firstcmdchar, *aliases,
                 rest);
        set_input(buffer);
...
#ifdef WANT_DLL
                  || ((dll_cnt == 1) && (alias_cnt == 1) &&
                      !strcmp(dll->name, aliases[0]))
#endif
                      )) {
        snprintf(buffer, sizeof buffer, "%s%s%s %s", firstcmdchar,
        ...
        *buffer = 0;
        if (command) {
          *buffer = 0;
          ...
          if (c)
            put_it("%s", convert_output_format("$G $[15]0 $[15]1 $[15]2 $[15]3",
                                               "%s", buffer));
#ifdef WANT_DLL
          if (dll_commands)
...
BUILT_IN_COMMAND(e_call) { dump_call_stack(); }
 ...
 *  that come up?
 */
void BX_parse_line(const char *name, char *org_line, const char *args,
                   int hist_flag, int append_flag, int handle_local) {
  char *line = NULL, *stuff, *s;
  int args_flag = 0, die = 0;
  ...
  if (!*org_line)
    ...
    do {
      while (*line == '{') {
        ...
        if (((internal_debug & DEBUG_CMDALIAS) ||
             (internal_debug & DEBUG_HOOK)) &&
            alias_debug && !in_debug_yell)
...
int BX_parse_command(char *line, int hist_flag, char *sub_args) {
  ...
  const char *com;
  char *this_cmd = NULL;
  int args_flag = 0, add_to_hist, cmdchar_used = 0;
  int noisy = 1;
  ...
  if (hist_flag && !cmdchar_used && !get_int_var(COMMAND_MODE_VAR)) {
    ...
    char *rest, *alias = NULL, *alias_name = NULL, *cline = NULL;
    const IrcCommand *command = NULL;
    void *arglist = NULL;
    int cmd_cnt, alias_cnt = 0;
#ifdef WANT_DLL
    int dll_cnt;
    IrcCommandDll *dll = NULL;
#endif
        ...
        if ((dll && (dll_cnt < 0)) || ((alias_cnt == 0) && (dll_cnt == 1))) {
          ...
          *cx_function = 0;
          ...
          if (rest && *rest && !my_stricmp(rest, "-help")) {
            ...
            bitchsay("%s: You are not connected to a server. Use /SERVER to "
                     "connect.",
                     com);
  ...
  if (old_display_var != (unsigned int)get_int_var(DISPLAY_VAR))
...
struct load_info {
  ...
  int line;
  int start_line;
} load_level[MAX_LOAD_DEPTH] = {{NULL, 0, 0}};
  ...
  while (args && *args && (filename = next_arg(args, &args))) {
    ...
    if (!end_strcmp(filename, ".tcl", 4)) {
      ...
      if (Tcl_EvalFile(tcl_interp, filename) != TCL_OK)
        error("Unable to load filename %s [%s]", filename,
    ...
    /* *_NOT_* attached, so don't "fix" it */
    {
      ...
      for (;; load_level[load_depth].line++) {
        ...
        while ((start[len - 1] == '\n') &&
               (len >= 2 && start[len - 2] == '\\') &&
               (len < 3 || start[len - 3] != '\\') && (len < MAX_LINE_LEN) &&
        ...
        while (start && *start) {
          ...
          while ((ptr = strpbrk(optr, "{};/")) && ptr != optr &&
                 ptr[-1] == '\\')
            optr = ptr + 1;
          ...
          if (no_semicolon)
            ...
            if (!paste_level) {
              parse_line(NULL, current_row,
                         flag                             ? args
                         : get_int_var(INPUT_ALIASES_VAR) ? empty_string
                                                          : NULL,
                         0, 0, 1);
            ...
            *ptr = c;
            ...
            case '/': {
              ...
              /* We're not in a comment... should we start one? */
                ...
                if ((ptr[1] == '*') &&
                    (!get_int_var(COMMENT_BREAKAGE_VAR) || ptr == start)) {
            ...
            case '}': {
              if (in_comment)
                break;
              if (!paste_level)
                error("Unexpected } in %s, line %d", expanded,
                      load_level[load_depth].line);
              else {
                --paste_level;
      ...
      if (in_comment)
        error("File %s ended with an unterminated comment in line %d", expanded,
              comment_line);
      if (current_row) {
        if (paste_level)
          error("Unexpected EOF in %s trying to match '[' at line %d", expanded,
                paste_line);
        else
          parse_line(NULL, current_row,
                     flag                             ? args
...
BUILT_IN_COMMAND(sendlinecmd) {
  ...
  window_display = 1;
  if (args && *args)
    parse_line(NULL, args, get_int_var(INPUT_ALIASES_VAR) ? empty_string : NULL,
               1, 0, 1);
  update_input(UPDATE_ALL);
  window_display = display;
  from_server = server;
...
BUILT_IN_COMMAND(describe) {
  ...
  if (target && args && *args) {
    ...
    if (do_hook(SEND_ACTION_LIST, "%s %s", target, message))
      put_it("%s", convert_output_format(
                       fget_string_var(FORMAT_SEND_ACTION_OTHER_FSET),
                       "%s %s %s %s", update_clock(GET_TIME),
                       get_server_nickname(from_server), target, message));
    reset_display_target();
  }
}
...
BUILT_IN_COMMAND(me) {
  ...
  if (args && *args) {
    ...
    } else if (!(target = get_target_by_refnum(0)))
      target = get_current_channel_by_refnum(0);
    if (target) {
      char *message;
      message = args;
      ...
      if (do_hook(SEND_ACTION_LIST, "%s %s", target, message)) {
        if (strchr("&#", *target))
          put_it("%s", convert_output_format(
                           fget_string_var(FORMAT_SEND_ACTION_FSET),
                           "%s %s %s %s", update_clock(GET_TIME),
                           get_server_nickname(from_server), target, message));
        else
          put_it("%s", convert_output_format(
...
BUILT_IN_COMMAND(beepcmd) { term_beep(); }
...
BUILT_IN_COMMAND(pretend_cmd) { parse_server(args); }
...
BUILT_IN_COMMAND(e_pause) {
  ...
  if (!(sec = next_arg(args, &args))) {
    ...
    add_wait_prompt(empty_string, e_pause_cb, NULL, WAIT_PROMPT_DUMMY, 0);
    e_pause_cb_throw++;
   ...
   * with the arguments.
   */
  add_timer(0, empty_string, milliseconds, 1, e_pause_timer, NULL, NULL,
            get_current_winref(), "pause");
...
BUILT_IN_COMMAND(tcl_load) { tcl_command(NULL, NULL, NULL, NULL); }
...
BUILT_IN_COMMAND(help) {
  ...
  if (args && *args) {
    ...
    if (cntdll < 0 && cmddll) {
      userage(cmddll->name,
              cmddll->help ? cmddll->help : "No Help available Yet");
      return;
    }
#endif
  }
...
#endif
  {
    ...
    tcnt = cnt + cntdll;
    put_it("%s",
           convert_output_format("$G Found %K[%W$0%K]%n matches for %K[%W$1%K]",
                                 "%d %s", tcnt, comm));
  }
  *buffer = 0;
  c = 0;
  for (i = 0; i < cnt; i++) {
    ...
    if (++c == 5) {
      put_it("%s", convert_output_format(
                       "$G $[13]0 $[13]1 $[13]2 $[13]3 $[13]4", "%s", buffer));
      *buffer = 0;
      c = 0;
    }
  }
...
#endif
  if (c)
    put_it("%s", convert_output_format("$G $[13]0 $[13]1 $[13]2 $[13]3 $[13]4",
                                       "%s", buffer));
  userage("help", "%R[%ncommand%R]%n or /command -help %n to get help on "
                  "specific commands");
}
