  ...
  if (*args == '{') {
    ...
    if (args && *args && (cmd = next_arg(args, &args)) &&
      ...
      while (1) {
        ...
        ptr =
            parse_inline(newexp, subargs ? subargs : empty_string, &args_used);
...
BUILT_IN_COMMAND(whilecmd) {
  char *exp = NULL, *ptr = NULL, *body = NULL, *newexp = NULL;
...
BUILT_IN_COMMAND(foreach) {
  char *struc = NULL, *ptr, *body = NULL, *var = NULL;
  char **sublist;
  int total;
  int i;
  int slen;
...
BUILT_IN_COMMAND(fe) {
  char *list = NULL, *templist = NULL, *placeholder, *sa, *vars, *varmem = NULL,
       *var[255], *word = NULL, *todo = NULL, fec_buffer[2] = {0};
  int ind, y, args_flag;
  int old_display;
