...
cmd_t C_msg[] = {
    /*	{ "die",	msg_die,	 ADD_KILL, "kill a client. Needs
       Userlevel KILL" },*/
    {NULL, NULL, -1, NULL}};

cmd_t C_ctcp[] = {{NULL, NULL, -1, NULL}};

cmd_t C_notice[] = {{NULL, NULL, -1, NULL}};
...
static int CompareKeyListField(Tcl_Interp *, char *, char *, char **, int *,
                               int *);

static int SplitAndFindField(Tcl_Interp *, char *, char *, fieldInfo_t *);
 ...
 *-----------------------------------------------------------------------------
 */
static int CompareKeyListField(tcl_interp, fieldName, field, valuePtr,
                               valueSizePtr, bracedPtr)
...
int *bracedPtr;
{
  ...
  if (nextPtr[0] != '\0') {
    Tcl_AppendResult(tcl_interp, "invalid keyed list format: ",
                     "trailing data following value in field: \"", elementPtr,
                     "\"", (char *)NULL);
    return TCL_ERROR;
  }
  *valuePtr = elementPtr;
...
fieldInfo_t *fieldInfoPtr;
{
  ...
  for (idx = 0; idx < fieldInfoPtr->argc; idx++) {
    result = CompareKeyListField(tcl_interp, fieldName, fieldInfoPtr->argv[idx],
                                 &fieldInfoPtr->valuePtr,
                                 &fieldInfoPtr->valueSize, &braced);
    if (result != TCL_BREAK)
 ...
 *-----------------------------------------------------------------------------
 */
int Tcl_GetKeyedListKeys(tcl_interp, subFieldName, keyedList, keysArgcPtr,
                         keysArgvPtr)
Tcl_Interp *tcl_interp;
char *subFieldName;
char *keyedList;
...
char ***keysArgvPtr;
{
  ...
  while (*scanPtr != '\0') {
    ...
    if (result != TCL_OK)
      goto errorExit;
    result = TclFindElement(tcl_interp, fieldPtr, &keyPtr, &dummyPtr, &keySize,
                            NULL);
    if (result != TCL_OK)
      goto errorExit;
  ...
  while (*scanPtr != '\0') {
    TclFindElement(tcl_interp, scanPtr, &fieldPtr, &scanPtr, &fieldSize, NULL);
 ...
 *-----------------------------------------------------------------------------
 */
char *Tcl_DeleteKeyedListField(Tcl_Interp *tcl_interp, char *fieldName,
                               char *keyedList) {
  char *nameSeparPtr;
  char *newList;
  int idx;
 ...
 *----------------------------------------------------------------------------
 */
int Tcl_KeyldelCmd(ClientData clientData, Tcl_Interp *tcl_interp, int argc,
                   char **argv) {
  char *keyedList, *newList;
  char *varPtr;

  if (argc != 3) {
    Tcl_AppendResult(tcl_interp, tclXWrongArgs, argv[0], " listvar key",
                     (char *)NULL);
    return TCL_ERROR;
  }
 ...
 *-----------------------------------------------------------------------------
 */
int Tcl_KeylgetCmd(ClientData clientData, Tcl_Interp *tcl_interp, int argc,
                   char **argv) {
  char *keyedList;
  char *fieldValue;
  char **fieldValuePtr;
  ...
  result = Tcl_GetKeyedListField(tcl_interp, argv[2], keyedList, fieldValuePtr);
  if (result == TCL_ERROR)
    return TCL_ERROR;
 ...
 *-----------------------------------------------------------------------------
 */
int Tcl_KeylkeysCmd(ClientData clientData, Tcl_Interp *tcl_interp, int argc,
                    char **argv) {
 ...
 *-----------------------------------------------------------------------------
 */
int Tcl_KeylsetCmd(ClientData clientData, Tcl_Interp *tcl_interp, int argc,
                   char **argv) {
  ...
  for (idx = 2; idx < argc; idx += 2) {
    prevList = newList;
    newList =
        Tcl_SetKeyedListField(tcl_interp, argv[idx], argv[idx + 1], prevList);
...
int Tcl_LmatchCmd(ClientData notUsed, Tcl_Interp *tcl_interp, int argc,
                  char **argv) {
#define EXACT 0
#define GLOB 1
#define REGEXP 2
  ...
  if (Tcl_SplitList(tcl_interp, argv[argc - 2], &listArgc, &listArgv) !=
      TCL_OK) {
    return TCL_ERROR;
  }
  if (listArgc == 0) {
...
int Tcl_RelativeExpr(Tcl_Interp *tcl_interp, char *cstringExpr, long stringLen,
                     long *exprResultPtr) {
  ...
  if (!(STRNEQU(cstringExpr, "end", 3) || STRNEQU(cstringExpr, "len", 3))) {
 ...
 *-----------------------------------------------------------------------------
 */
int Tcl_LvarpopCmd(ClientData clientData, Tcl_Interp *tcl_interp, int argc,
                   char **argv) {
  int listArgc, idx;
  long listIdx;
  char **listArgv;
 ...
 *-----------------------------------------------------------------------------
 */
int Tcl_LemptyCmd(ClientData clientData, Tcl_Interp *tcl_interp, int argc,
                  char **argv) {
  char *scanPtr;
...
void init_public_var(Tcl_Interp *intp) {
  ...
  if (current_window && current_window->current_channel)
    Tcl_SetVar(intp, "curchan", current_window->current_channel,
               TCL_GLOBAL_ONLY);
  if (from_server > -1) {
    Tcl_SetVar(intp, "server", get_server_name(from_server), TCL_GLOBAL_ONLY);
    Tcl_SetVar(intp, "botnick", get_server_nickname(from_server),
               TCL_GLOBAL_ONLY);
...
/* add a timer */
char *tcl_add_timer(TimerList **stack, long elapse, char *cmd,
  ...
  while (mark != NULL) {
    ...
    if ((left = BX_time_diff(now1, mark->time)) <= 0) {
      ...
      if (Tcl_SplitList(tcl_interp, mark->command, &argc, &argv) != TCL_OK)
        putlog(LOG_CRAP, "*", "(Timer) Error for '%s': %s", mark->command,
...
void check_timers(void) { do_check_timers(&tcl_Pending_timers); }

void check_utimers(void) { do_check_timers(&tcl_Pending_utimers); }
...
void tclTimerTimeout(struct timeval *wake_time __attribute__((unused))) {}

int check_tcl_dcc(char *cmd, char *nick __attribute__((unused)), char *host __attribute__((unused)), int idx) {
  int x, atr = 0;
