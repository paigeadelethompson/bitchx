...
static char *levels[] = {
    "CRAP",     "PUBLIC",   "MSGS",     "NOTICES",  "WALLS",    "WALLOPS",
    "NOTES",    "OPNOTES",  "SNOTES",   "ACTIONS",  "DCC",      "CTCP",
    "USERLOG1", "USERLOG2", "USERLOG3", "USERLOG4", "USERLOG5", "BEEP",
    "TCL",      "SEND_MSG", "KILL",     "MODEUSER", "MODECHAN", "KICK",
    "KICKUSER", "PARTS",    "INVITES",  "JOIN",     "TOPIC",    "HELP",
    "NOTIFY",   "DEBUG"};
...
char *bits_to_lastlog_level(unsigned long level) {
  ...
  if (level == LOG_ALL)
    ...
    for (*buffer = i = 0, p = 1; i < (int)NUMBER_OF_LEVELS; i++, p <<= 1) {
      if (level & p) {
        if (*buffer)
      ...
      if ((len = strlen(str)) != 0) {
        if (my_strnicmp(str, "ALL", len) == 0)
          ...
          if (*str == '-') {
            ...
            neg = 0;
          for (i = 0, p = 1; i < (int)NUMBER_OF_LEVELS; i++, p <<= 1) {
 ...
 */
void set_lastlog_level(Window *win __attribute__((unused)), char *str, int unused __attribute__((unused))) {
 ...
 */
void set_msglog_level(Window *win __attribute__((unused)), char *str, int unused __attribute__((unused))) {
  msglog_level = parse_lastlog_level(str, 1);
  set_string_var(MSGLOG_LEVEL_VAR, bits_to_lastlog_level(msglog_level));
}

void set_new_server_lastlog_level(Window *win __attribute__((unused)), char *str, int unused __attribute__((unused))) {
 ...
 */
void set_lastlog_size(Window *win_unused __attribute__((unused)), char *unused __attribute__((unused)), int size) {
  int i, diff;
  Window *win = NULL;
...
BUILT_IN_COMMAND(lastlog) {
  int cnt, from = 0, p, i, level = 0, msg_level, len, mask = 0, header = 1,
           lines = 0, reverse = 0, time_log = 1, remove = 0;
  ...
  while ((arg = new_next_arg(args, &args)) != NULL) {
    if (*arg == '-') {
      ...
      if (!(len = strlen(arg))) {
         ...
         */
        for (i = 0, p = 1; i < (int)NUMBER_OF_LEVELS; i++, p *= 2) {
  ...
  for (i = 0; (i < cnt) && start_pos;
       start_pos = (reverse ? start_pos->next : start_pos->prev)) {
    if (!mask || (mask & start_pos->level)) {
      i++;
      if (!match || wild_match(blah, start_pos->msg)) {
        char *s = !get_int_var(LASTLOG_ANSI_VAR)
                      ? stripansicodes(start_pos->msg)
                      : start_pos->msg;

        if (!fp) {
          put_it("%s", !time_log ? s
                                 : convert_output_format(
                                       fget_string_var(FORMAT_LASTLOG_FSET),
                                       "%l %s", start_pos->time, s));
          grab_http("*", "*", start_pos->msg);
        } else {
          if (time_log) {
            s = convert_output_format(fget_string_var(FORMAT_LASTLOG_FSET),
                                      "%l %s", start_pos->time, s);
            chop(s, 3);
          }
          fprintf(fp, "%s\n", s);
...
Lastlog *get_lastlog_current_head(Window *win) { return win->lastlog_head; }
...
unsigned long real_notify_level(void) { return (notify_level); }

unsigned long real_lastlog_level(void) { return (lastlog_level); }

void set_notify_level(Window *win __attribute__((unused)), char *str, int unused __attribute__((unused))) {
...
int logmsg(unsigned long log_type, const char *from, int flag,
           const char *format, ...) {
#ifdef PUBLIC_ACCESS
  return 0;
#else
  ...
  case 0:
    ...
    if (msglog_level & log_type && format) {
      ...
      if (!do_hook(MSGLOG_LIST, "%s %s %s %s", timestr, type, from,
                   msglog_buffer))
        break;
      if (!logptr)
        return 0;
      if (!(format = fget_string_var(FORMAT_MSGLOG_FSET)))
        format = "[$[10]0] [$1] - $2-";
      lines = split_up_line(
          stripansicodes(convert_output_format(format, "%s %s %s %s", type,
                                               timestr, from, msglog_buffer)),
          80);
      for (; *lines; lines++) {
  ...
  case 1:
    ...
    break;
  case 2:
    if (!do_hook(MSGLOG_LIST, "%s %s %s %s", timestr, "Off", empty_string,
                 empty_string))
      return 1;
    if (!logptr)
      return 1;
...
void set_beep_on_msg(Window *win __attribute__((unused)), char *str, int unused __attribute__((unused))) {
...
BUILT_IN_COMMAND(awaylog) {
  if (args && *args) {
    ...
    set_string_var(MSGLOG_LEVEL_VAR, bits_to_lastlog_level(msglog_level));
    put_it("%s", convert_output_format("$G Away logging set to: $0-", "%s",
                                       get_string_var(MSGLOG_LEVEL_VAR)));
  } else
    put_it("%s", convert_output_format("$G Away logging currently: $0-", "%s",
                                       bits_to_lastlog_level(msglog_level)));
}
...
BUILT_IN_FUNCTION(function_line) {
  ...
  if (do_level)
    return m_sprintf("%s %s", start_pos->msg, levels[start_pos->level]);
  else
