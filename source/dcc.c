...
static const size_t n_builtin_dcc_types =
    sizeof builtin_dcc_types / sizeof builtin_dcc_types[0];
static struct dcc_type **dcc_types = NULL;
static size_t n_dcc_types = 0;

static char DCC_current_transfer_buffer[BIG_BUFFER_SIZE + 2];
...
static void update_transfer(void);
...
static const DCC_commands dcc_commands[] = {
    ...
    {"SEND", BX_dcc_filesend,
     "[-b block] [-e passwd] [-p port] [nick|nick1,nick2] filename(s)\n- Send "
     "list of files to nick(s)"},
    {"RESEND", BX_dcc_resend,
     "[-b block] [-e passwd] [-p port] [nick|nick1,nick2] filename(s)\n- Send "
     "list of files to nick(s)"},
    {"TSEND", BX_dcc_filesend,
     "[-b block] [-e passwd] [-p port] [nick|nick1,nick2] filename(s)\n- Send "
     "list of files to nick(s)"},
    {"TRESEND", BX_dcc_resend,
     "[-b block] [-e passwd] [-p port] [nick|nick1,nick2] filename(s)\n- Send "
     "list of files to nick(s)"},
...
/* get_dcc_type()
 *
 * Look up the given DCC name and return the corresponding DCC type, or -1 if
...
static int get_dcc_type(const char *name) {
  ...
  for (i = 1; (size_t)i < n_dcc_types; i++) {
...
void send_ctcp_booster(char *nick, char *type, char *file,
                       unsigned long address, unsigned short portnum,
                       unsigned long filesize) {
  if (filesize)
    send_ctcp(CTCP_PRIVMSG, nick, CTCP_DCC, "%s %s %lu %u %lu", type, file,
              address, portnum, filesize);
  else
    send_ctcp(CTCP_PRIVMSG, nick, CTCP_DCC, "%s %s %lu %u", type, file, address,
              portnum);
}
...
void send_reject_ctcp(char *nick, char *type, char *name) {
   ...
   */
  userhostbase(nick, output_reject_ctcp, 1, "%s %s %s", nick, type,
               name ? name : "<any>");
}
...
void BX_erase_dcc_info(int s, int reject, char *format, ...) {
  ...
  if (check_dcc_socket(s) && (n = (DCC_int *)get_socketinfo(s))) {
    if (dcc_types[s1->flags & DCC_TYPES]->dcc_ops->close)
      (*dcc_types[s1->flags & DCC_TYPES]->dcc_ops->close)(s, n->remote.s_addr,
                                                          n->remport);
    if (reject && ((now - last_reject) < 2))
      send_reject_ctcp(s1->server,
                       (((s1->flags & DCC_TYPES) == DCC_FILEOFFER) ? "GET"
                        : ((s1->flags & DCC_TYPES) == DCC_FILEREAD)
                            ? "SEND"
 ...
 */
static int dcc_match(const SocketList *s, const char *nick, const char *desc,
                     const char *other, int type, int active, int num) {
  const DCC_int *n = s->info;

  if (type != -1 && type != (int)(s->flags & DCC_TYPES))
    return 0;
  if (num != -1 && num != (int)n->dccnum)
    return 0;
 ...
 */
static DCC_List *find_dcc_pending(const char *nick, const char *desc,
                                  const char *othername, int type, int remove,
                                  int num) {
  SocketList *s;
  DCC_List *new_i;
...
static void refileread_send_start(int s, DCC_int *n) {
  ...
  put_it("%s", convert_output_format(
                   "$G %RDCC %YTelling uplink we want to start at%n: $0", "%s",
                   ulongcomma(n->transfer_orders.byteoffset)));
...
DCC_int *BX_dcc_create(char *nick, char *filename, char *passwd,
                       unsigned long filesize, int port, int type,
                       unsigned long flags, void (*func)(int)) {
  ...
  if (((new_i = find_dcc_pending(nick, filename, NULL, type, 1, -1)) &&
       new_i->sock.flags & DCC_OFFER) ||
      (flags & DCC_OFFER)) {
    SocketList *s = NULL;
    int new_s;
    ...
    if ((new_s = connect_by_number(inet_ntoa(new->remote), &new->remport,
                                   SERVICE_CLIENT, PROTOCOL_TCP, 0)) < 0
#ifdef HAVE_LIBSSL
      ...
      SSL_show_errors();
#endif
      erase_dcc_info(s->is_read, 1, "%s",
                     convert_output_format(
                         "$G %RDCC%n Unable to create connection: $0-", "%s",
    ...
    flags |= DCC_ACTIVE;
    add_socketread(new_s, new->remport, flags | type, nick,
                   new_i->sock.func_read, NULL);
    ...
    if ((getpeername(new_s, (struct sockaddr *)&remaddr, &rl)) != -1) {
      ...
      if (type != DCC_RAW) {
        ...
        if (type == DCC_FILEOFFER)
          blah = do_hook(DCC_CONNECT_LIST, "%s %s %s %d %ld %s", nick,
                         dcc_types[type]->name, inet_ntoa(remaddr.sin_addr),
                         ntohs(remaddr.sin_port), (unsigned long)new->filesize,
                         new->filename);
        else
          blah = do_hook(DCC_CONNECT_LIST, "%s %s %s %d", nick,
                         dcc_types[type]->name, inet_ntoa(remaddr.sin_addr),
                         ntohs(remaddr.sin_port));
        if (blah && !dcc_quiet)
          put_it("%s", convert_output_format(
                           fget_string_var(FORMAT_DCC_CONNECT_FSET),
                           "%s %s %s %s %s %d", update_clock(GET_TIME),
                           dcc_types[type]->name, nick,
                           new->userhost ? new->userhost : "u@h",
                           inet_ntoa(remaddr.sin_addr),
                           (int)ntohs(remaddr.sin_port)));
      }
    }
    if (dcc_types[type]->dcc_ops->open)
      (*dcc_types[type]->dcc_ops->open)(new_s, type, new->remote.s_addr,
                                        ntohs(remaddr.sin_port));
    ...
    flags |= DCC_WAIT;
    if ((s = connect_by_number(NULL, &portnum, SERVICE_SERVER, PROTOCOL_TCP,
                               1)) < 0) {
...
#endif
    send_ctcp_booster(nick, Type, file, ntohl(myip.sf_addr.s_addr), portnum,
                      filesize);
    if (!doing_multi && !dcc_quiet) {
      if (filesize) {
        if (do_hook(DCC_OFFER_LIST, "%s %s %s %lu", nick, Type, file, filesize))
          put_it("%s", convert_output_format(
                           fget_string_var(FORMAT_SEND_DCC_CHAT_FSET),
...
static void start_dcc_chat(int s) {
...
#ifndef BITCHX_LITE
  if (type == DCC_BOTMODE)
    func = dcc_bot_socketread;
#endif
  if ((add_socketread(new_s, ntohs(remaddr.sin_port), flags, nick, func,
                      NULL)) < 0) {
    erase_dcc_info(s, 0, "%s",
                   convert_output_format(
                       "$G %RDCC error: accept() failed. punt!!", NULL, NULL));
    close_socketread(s);
    return;
  }
  flags &= ~DCC_WAIT;
  ...
  if (do_hook(DCC_CONNECT_LIST, "%s %s %s %d", nick, dcc_types[type]->name,
    ...
    put_it("%s",
           convert_output_format(
               fget_string_var(FORMAT_DCC_CONNECT_FSET), "%s %s %s %s %s %d",
               update_clock(GET_TIME), dcc_types[type]->name, nick,
               n->userhost ? n->userhost : "u@h", inet_ntoa(remaddr.sin_addr),
               ntohs(remaddr.sin_port)));
  ...
  if (dcc_types[type]->dcc_ops->open)
    (*dcc_types[type]->dcc_ops->open)(new_s, type, n->remote.s_addr,
                                      ntohs(remaddr.sin_port));
  reset_display_target();
}
...
void BX_dcc_chat_socketread(int s) {
  ...
  if (dcc_types[type]->dcc_ops->input)
    bytesread =
        (*dcc_types[type]->dcc_ops->input)(s, type, bufptr, 1, BIG_BUFFER_SIZE);
  else
#ifdef HAVE_LIBSSL
  ...
  case -1: {
    ...
    if (do_hook(DCC_LOST_LIST, "%s %s %s", nick, dcc_types[type]->name,
                real_tmp))
      put_it("%s",
             convert_output_format(fget_string_var(FORMAT_DCC_ERROR_FSET),
  ...
  default: {
    ...
    *uhost = 0;
    if (n->userhost)
      ...
      strmopencat(uhost, 99, "Unknown@",
                  "unknown" /*inet_ntoa(remaddr.sin_addr)*/, NULL);
    FromUserHost = uhost;
    ...
    if (*tmp == DEFAULT_BOTCHAR) {
      ...
      if ((flags & DCC_BOTCHAT)) {
        if (my_strnicmp(pc, "chat", 4)) {
          if (check_tcl_dcc(pc, nick, n->userhost ? n->userhost : empty_string,
                            s))
            break;
        }
      } else if (!my_strnicmp(pc, "chat", 4) &&
                 check_tcl_dcc(pc, nick,
                               n->userhost ? n->userhost : empty_string, s))
        break;
    ...
    *equal_nickname = 0;
    ...
    if (strbegins(tmp, DCC_CTCP_MESSAGE) ||
        strbegins(tmp, CTCP_DELIM_STR "ACTION")) {
      char *tmp2 = LOCAL_COPY(tmp);
      if (*tmp2 != CTCP_DELIM_CHAR)
        tmp2 += strlen(DCC_CTCP_MESSAGE);
      strlcpy(tmp,
              do_ctcp(equal_nickname, get_server_nickname(from_server),
                      stripansicodes(tmp2)),
              sizeof tmp);
      if (!*tmp)
        break;
      ...
      strlcpy(tmp,
              do_notice_ctcp(equal_nickname, get_server_nickname(from_server),
                             stripansicodes(tmp2)),
              sizeof tmp);
      if (!*tmp)
        break;
...
static void dcc_bot_socketread(int s) {
  ...
  if (dcc_types[type]->dcc_ops->input)
    bytesread =
        (*dcc_types[type]->dcc_ops->input)(type, s, bufptr, 1, BIG_BUFFER_SIZE);
  else
#ifdef HAVE_LIBSSL
    bytesread = dgets(bufptr, s, 1, BIG_BUFFER_SIZE, sl->ssl_fd);
#else
...
static void new_dcc_message_transmit(char *user, char *text, char *text_display,
                                     int type, unsigned stxt_flags,
                                     int check_host) {
  SocketList *s = NULL;
  DCC_int *n = NULL;
  ...
  *tmp = 0;
  ...
  case DCC_RAW:
    if (check_host) {
      if (!(host = next_arg(text, &text))) {
        put_it("%s",
               convert_output_format("$G %RDCC%n No host specified for DCC RAW",
                                     NULL, NULL));
        return;
      }
    }
  ...
  if (!s || !(s->flags & DCC_ACTIVE)) {
    put_it("%s", convert_output_format(
                     "$G %RDCC No active $0:$1 connection for $2", "%s %s %s",
                     dcc_types[type]->name, host ? host : "(null)", user));
    return;
  }
  n = (DCC_int *)s->info;
  ...
  if (!(stxt_flags & STXT_QUIET) && type != DCC_RAW) {
    if (do_hook(list, "%s %s", user, text_display ? text_display : text))
      put_it("%s", convert_output_format(
                       fget_string_var(FORMAT_SEND_DCC_CHAT_FSET), "%c %s %s",
                       thing, user, text_display ? text_display : text));
  }
}

extern void dcc_chat_transmit(char *user, char *text, char *orig,
                              unsigned stxt_flags) {
  int fd;
  ...
  if ((fd = atol(user))) {
    ...
    if (!check_dcc_socket(fd)) {
      put_it("%s", convert_output_format(
                       "$G %RDCC%n Descriptor [$0] is not an open DCC RAW",
                       "%d", fd));
      return;
    }
    s = get_socket(fd);
...
extern void dcc_bot_transmit(char *user, char *text, unsigned stxt_flags) {
  ...
  new_dcc_message_transmit(user, text, NULL, DCC_BOTMODE,
                           stxt_flags | STXT_QUIET, 1);
  reset_display_target();
}
...
static int parse_offer_params(const struct dcc_offer *offer,
                              unsigned long *p_addr, unsigned short *p_port,
                              unsigned long *p_filesize) {
  ...
  *p_port = (unsigned short)strtoul(offer->port, NULL, 10);
  ...
  if (*p_addr == 0 || *p_port < 1024) {
    ...
    put_it("%s",
           convert_output_format("$G %RDCC%n Handshake ignored because of "
                                 "privileged port or zero address [$0:$1]",
                                 "%s %d", inet_ntoa(in), *p_port));
    return 0;
  }
...
static int check_collision(char *nick, const char *description, int type) {
  ...
  if (s) {
    if ((s->flags & DCC_ACTIVE)) {
      /* collision. */
      put_it("%s",
             convert_output_format("$G %RDCC%n Received DCC $0 request from $1 "
                                   "while previous session active",
                                   "%s %s", type, nick));
      return 0;
    }
    if ((s->flags & DCC_WAIT)) {
      if (type == DCC_CHAT) {
        if (do_hook(DCC_CONNECT_LIST, "%s CHAT %s", nick,
                    "already requested connecting"))
          put_it("%s", convert_output_format(
                           fget_string_var(FORMAT_DCC_CONNECT_FSET),
                           "%s %s %s %s %s %d", update_clock(GET_TIME), "CHAT",
                           nick, space, "already requested connecting...", 0));
        dcc_chat(NULL, nick);
        return 0;
      }
      send_ctcp(CTCP_NOTICE, nick, CTCP_DCC,
                "DCC %s collision occurred while connecting to %s (%s)", type,
                nickname, description);
      erase_dcc_info(s->is_read, 1, "%s",
                     convert_output_format("$G %RDCC%n $0 collision for $1:$2",
                                           "%s %s %s", type, nick,
                                           description));
      close_socketread(s->is_read);
      return 0;
...
static DCC_List *add_dcc_pending(DCC_int *dcc, int flags,
                                 void (*func_read)(int)) {
...
/* show_dcc_offer()
 * Wrapper to show the user using the default format a non-file (eg. CHAT) DCC
 * offer.
 */
static void show_dcc_offer(const DCC_int *n, const char *dcc_name) {
  if (!dcc_quiet) {
    put_it("%s",
           convert_output_format(fget_string_var(FORMAT_DCC_REQUEST_FSET),
                                 "%s %s %s %s %s %s %d", update_clock(GET_TIME),
                                 dcc_name, n->filename, n->user, n->userhost,
...
/* show_dcc_fileoffer()
 * Wrapper to show the user using the default format a file (eg. SEND) DCC
 * offer.
 */
static void show_dcc_fileoffer(const DCC_int *n, const char *dcc_name) {
  if (!dcc_quiet) {
    ...
    put_it("%s",
           convert_output_format(fget_string_var(FORMAT_DCC_REQUEST_FSET),
                                 "%s %s \"%s\" %s %s %s %d %s %s",
                                 update_clock(GET_TIME), dcc_name, n->filename,
                                 n->user, n->userhost, inet_ntoa(n->remote),
                                 n->remport, _GMKs(n->filesize), buf));
  }

  if (!n->filesize)
    put_it("%s",
           convert_output_format(
               "$G %RDCC%n Warning: Offered file has zero size", NULL, NULL));
...
static void dcc_chat_offer(const struct dcc_offer *offer, int server) {
  ...
  if (do_hook(DCC_REQUEST_LIST, "%s %s %s 0", n->user, dcc_types[Ctype]->name,
              n->filename)) {
  ...
  if (autoget)
    dcc_create(offer->nick, n->filename, NULL, n->filesize, 0, Ctype, DCC_OFFER,
               dcc_chat_socketread);
}
...
static int do_autoget(const char *nick, unsigned long filesize) {
  if (!get_int_var(DCC_AUTOGET_VAR) &&
    ...
    return 0;
  if (filesize / 1024 > (unsigned long)get_int_var(DCC_MAX_AUTOGET_SIZE_VAR))
    return 0;
  if (!filesize)
    return 0;
  return 1;
...
static void dcc_send_offer(const struct dcc_offer *offer, int server) {
  ...
  if (do_hook(DCC_REQUEST_LIST, "%s %s %s %lu", n->user, dcc_types[Ctype]->name,
  ...
  if (!dcc_overwrite_var && stat(fullname, &resume_sb) == 0) {
...
#ifdef MIRC_BROKEN_DCC_RESUME
      else if (resume_sb.st_size < n->filesize &&
               get_int_var(DCC_AUTORESUME_VAR)) {
        put_it("%s",
               convert_output_format(
                   "$G %RDCC%n Warning: File $0 exists: trying to autoresume",
                   "%s", fullname));
        autoresume = 1;
      }
#endif
    ...
    if (!autoget) {
      ...
      if (resume_sb.st_size < n->filesize)
        put_it("%s", convert_output_format(
                         "$G %RDCC%n Warning: File $0 exists: use /DCC RENAME "
                         "or /DCC RESUME if you don't want to overwrite",
                         "%s", fullname));
      else
#endif
        put_it("%s", convert_output_format(
  ...
  if (autoget && fullname) {
    ...
    if (!dcc_quiet) {
      put_it("%s",
             convert_output_format(
                 "$G %RDCC%n Auto-accepting $0 of file %C$2-%n from %K[%C$1%K]",
                 "%s %s %s", dcc_type_name(Ctype, tdcc), offer->nick,
                 n->filename));
    }
    if ((n->file = open(fullname, O_WRONLY | O_CREAT | O_BINARY, 0644)) > 0) {
      if ((new = dcc_create(offer->nick, n->filename, NULL, n->filesize, 0,
                            Ctype, DCC_OFFER | tdcc, start_dcc_get)))
        new->blocksize = get_int_var(DCC_BLOCK_SIZE_VAR);
    } else
      put_it("%s", convert_output_format("$G %RDCC%n Unable to open $0-", "%s",
                                         fullname));
  }
}
...
static void dcc_resend_offer(const struct dcc_offer *offer, int server) {
  ...
  if (autoget && fullname) {
    ...
    if ((n->file = open(fullname, O_WRONLY | O_CREAT | O_BINARY | O_APPEND,
                        0644)) > 0) {
      if ((new = dcc_create(offer->nick, n->filename, NULL, n->filesize, 0,
                            Ctype, DCC_OFFER | tdcc, start_dcc_get)))
        new->blocksize = get_int_var(DCC_BLOCK_SIZE_VAR);
...
static int check_dcc_init(const struct dcc_offer *offer) {
  ...
  if (i >= 0 && dcc_types[i]->dcc_ops->init) {
    ...
    if (parse_offer_params(offer, &address, &port, &filesize))
      return dcc_types[i]->dcc_ops->init(
          offer->type, offer->nick, offer->userhost, offer->description,
          offer->size, offer->extra, address, port);
  }
  return 0;
}
...
void handle_dcc_offer(struct dcc_offer *offer) {
  ...
  if (!my_stricmp(offer->type, "CHAT"))
...
#endif
  else if (!my_stricmp(offer->type, "SEND") ||
           !my_stricmp(offer->type, "TSEND"))
    dcc_send_offer(offer, from_server);
  else if (!my_stricmp(offer->type, "RESEND") ||
           !my_stricmp(offer->type, "TRESEND"))
     ...
     * borrowed from epic.
     */
    dcc_getfile_resume_demanded(offer->nick, offer->description, offer->address,
                                offer->port);
  else if (!my_stricmp(offer->type, "ACCEPT"))
    dcc_getfile_resume_start(offer->nick, offer->description, offer->address,
                             offer->port);
#endif
  else
    put_it("%s", convert_output_format(
                     "$G %RDCC%n Unknown DCC $0 ($1) received from $2",
                     "%s %s %s", offer->type, offer->description, offer->nick));
...
void process_dcc(char *args) {
  ...
  if (dcc_dllcommands) {
    ...
    if ((dcc_comm = (DCC_dllcommands *)find_in_list((List **)&dcc_dllcommands,
                                                    command, 0))) {
  ...
  put_it("%s", convert_output_format("$G Unknown %RDCC%n command: $0", "%s",
                                     command));
  reset_display_target();
}
...
void dcc_chat(char *command, char *args) {
  ...
  if (!user) {
    put_it("%s", convert_output_format(
                     "$G %RDCC%n You must supply a nickname for DCC chat", NULL,
                     NULL));
    return;
  }
  ...
  while ((nick = next_in_comma_list(user, &user))) {
    ...
    if (isme(nick))
      continue;
    if ((s = find_dcc(nick, "chat", NULL, bot ? DCC_BOTMODE : DCC_CHAT, 1, -1,
                      -1))) {
      if ((s->flags & DCC_ACTIVE) || (s->flags & DCC_WAIT))
        put_it("%s",
               convert_output_format(
                   "$G %RDCC%n A previous DCC chat to $0 exists", "%s", nick));
      new = (DCC_int *)s->info;
      if (s->flags & DCC_WAIT) {
        send_ctcp_booster(
            nick, dcc_types[DCC_CHAT]->name, "chat",
            htonl(get_int_var(DCC_USE_GATEWAY_ADDR_VAR)
                      ? get_server_uh_addr(from_server).sf_addr.s_addr
                      : get_server_local_addr(from_server).sf_addr.s_addr),
            htons(s->port), new->filesize);
        add_sockettimeout(s->is_read, 120, NULL);
      }
      continue;
    }
    if ((new = dcc_create(nick, "chat", passwd, 0, port ? atol(port) : 0,
                          bot ? DCC_BOTMODE : DCC_CHAT, DCC_TWOCLIENTS | flags,
                          start_dcc_chat))) {
...
void close_dcc_file(int snum) {
  ...
  set_display_target(NULL, LOG_DCC);
  if (do_hook(DCC_LOST_LIST, "%s %s %s %s TRANSFER COMPLETE", s->server,
              type_name, strip_path(n->filename), lame_ultrix))
    put_it("%s",
           convert_output_format(
               fget_string_var(FORMAT_DCC_LOST_FSET), "%s %s %s %s %s %s %s %s",
...
void BX_dcc_send_socketread(int snum) {
  ...
  if (!(s->flags & DCC_TDCC) && n->readwaiting) {
    /* n->readwaiting is set when we have sent a block and are waiting for an
     * acknowledgement. */
    int numbytes = 0;
    u_32int_t bytes = 0;
    ...
    if ((ioctl(snum, FIONREAD, &numbytes)) == -1) {
      erase_dcc_info(
          snum, 1,
          convert_output_format("$G %RDCC%n Remote $0 closed dcc send", "%s",
                                s->server));
      close_socketread(snum);
      return;
    }
    if (numbytes) {
      if (read(snum, &bytes, sizeof bytes) < (int)sizeof bytes) {
        erase_dcc_info(
            snum, 1,
            convert_output_format("$G %RDCC%n Remote closed dcc send", NULL));
        close_socketread(snum);
      }
    ...
    if (num != bytesread) {
      if (num == -1) {
        if (errno == EWOULDBLOCK || errno == ENOBUFS)
          ...
          erase_dcc_info(
              snum, 1,
              convert_output_format("$G %RDCC%n Remote $0 closed dcc send",
                                    "%s", s->server));
          close_socketread(snum);
          return;
        }
...
void start_dcc_send(int s) {
  ...
  set_display_target(NULL, LOG_DCC);
  if ((add_socketread(new_s, ntohs(remaddr.sin_port), flags, nick,
                      dcc_send_socketread, NULL)) < 0) {
    erase_dcc_info(s, 1, "%s",
                   convert_output_format(
                       "$G %RDCC error: accept() failed. punt!!", NULL, NULL));
    close_socketread(s);
    return;
  ...
  if ((n->file = open(n->filename, O_RDONLY | O_BINARY)) == -1) {
    erase_dcc_info(new_s, 1, "%s",
                   convert_output_format(
                       "$G %RDCC%n Unable to open $0: $1-", "%s %s",
                       n->filename, errno ? strerror(errno) : "Unknown Host"));
  ...
  if (type == DCC_REFILEOFFER) {
    ...
    if (byte_order_test() != received.byteorder) {
       ...
       */
      n->transfer_orders.packet_id = ((received.packet_id & 0x00ff) << 8) |
    ...
    if (n->transfer_orders.packet_id != DCC_PACKETID) {
      put_it("%s", convert_output_format("$G %RDCC%n reget packet is invalid!!",
                                         NULL, NULL));
      memset(&n->transfer_orders, 0, sizeof(struct transfer_struct));
    } else
      put_it("%s",
             convert_output_format("$G %RDCC%n reget starting at $0", "%s",
                                   ulongcomma(n->transfer_orders.byteoffset)));
  }
  ...
  if (do_hook(DCC_CONNECT_LIST, "%s %s %s %d %ld %s", nick, type_name,
              inet_ntoa(remaddr.sin_addr), ntohs(remaddr.sin_port),
              (unsigned long)n->filesize, n->filename) &&
      !dcc_quiet)
    put_it("%s",
           convert_output_format(
               fget_string_var(FORMAT_DCC_CONNECT_FSET), "%s %s %s %s %s %d %d",
               update_clock(GET_TIME), type_name, nick,
               n->userhost ? n->userhost : "u@h", inet_ntoa(remaddr.sin_addr),
               ntohs(remaddr.sin_port), n->transfer_orders.byteoffset));
  get_time(&n->starttime);
  get_time(&n->lasttime);
  close_socketread(s);
...
void real_file_send(char *nick, char *filename, char *passwd, char *port,
                    int tdcc, unsigned long type, int blocksize) {
  DCC_int *new = NULL;
  SocketList *s = NULL;
  ...
  if (*filename == '/')
    ...
    if (!(fullname = expand_twiddle(filename))) {
      put_it("%s", convert_output_format("$G %RDCC%n Unable to access $0", "%s",
                                         filename));
      return;
    }
  ...
  if (strchr(FileBuf, '*')) {
    ...
    while ((dir = readdir(dp))) {
      ...
      if ((s = find_dcc(nick, filebuf, NULL, type, 1, -1, -1))) {
        if ((s->flags & DCC_ACTIVE) || (s->flags & DCC_WAIT))
          put_it("%s", convert_output_format(
                           "$G %RDCC%n A previous DCC send to $0 exists", "%s",
                           nick));
        new = (DCC_int *)s->info;
        if (s->flags & DCC_WAIT) {
          send_ctcp_booster(
              nick, dcc_types[type]->name, filebuf,
              htonl(get_server_local_addr(from_server).sf_addr.s_addr),
              htons(s->port), new->filesize);
          add_sockettimeout(s->is_read, 120, NULL);
        }
        continue;
...
if ((new = dcc_create(
                 nick, filebuf, passwd, st.st_size, port ? atol(port) : (long)filesize,
                 type, (tdcc ? DCC_TDCC : 0) | DCC_TWOCLIENTS, start_dcc_send)))
        new->blocksize = blocksize;
    ...
    if (!dcc_quiet) {
      ...
      if (count)
        put_it("%s", convert_output_format(
                         "$G %RDCC%n Sent DCC SEND request to $0 for $1 $2-",
                         "%s %s %s", nick, buff, filename));
      else
        put_it("%s",
               convert_output_format("$G %RDCC%n No Files found matching $*",
                                     "%s", filename));
    }
    return;
  }
  if (access(FileBuf, R_OK)) {
    put_it("%s", convert_output_format("$G %RDCC%n No such file $0 exists",
                                       "%s", FileBuf));
    return;
  }
  stat(FileBuf, &st);
  if (S_ISDIR(st.st_mode)) {
    put_it("%s", convert_output_format("$G %RDCC%n $0 is a directory", "%s",
                                       FileBuf));
    return;
  }
  if ((s = find_dcc(nick, FileBuf, NULL, type, 1, -1, -1))) {
    if ((s->flags & DCC_ACTIVE) || (s->flags & DCC_WAIT))
      put_it("%s",
             convert_output_format(
                 "$G %RDCC%n A previous DCC send to $0 exists", "%s", nick));
    new = (DCC_int *)s->info;
    if (s->flags & DCC_WAIT) {
      send_ctcp_booster(
          nick, dcc_types[type]->name, FileBuf,
...
void BX_dcc_filesend(char *command, char *args) {
  ...
  if (!user) {
    put_it("%s", convert_output_format(
                     "$G %RDCC%n You must supply a nickname for DCC send", NULL,
                     NULL));
    return;
  }
  if (!blocksize || blocksize > MAX_DCC_BLOCK_SIZE)
  ...
  while ((nick = next_in_comma_list(user, &user))) {
    ...
    while ((filename = new_next_arg(new_args, &new_args))) {
      if (!filename || !*filename)
        break;
      real_file_send(nick, filename, passwd, port, tdcc, DCC_FILEOFFER,
                     blocksize);
    }
    doing_multi++;
  }
...
void BX_dcc_resend(char *command, char *args) {
  ...
  if (!user) {
    put_it("%s", convert_output_format(
                     "$G %RDCC%n You must supply a nickname for DCC resend",
                     NULL, NULL));
    return;
  }
  if (!blocksize || blocksize > MAX_DCC_BLOCK_SIZE)
  ...
  while ((nick = next_in_comma_list(user, &user))) {
    ...
    while ((filename = new_next_arg(new_args, &new_args))) {
      if (!filename || !*filename)
        break;
      real_file_send(nick, filename, passwd, port, tdcc, DCC_REFILEOFFER,
                     blocksize);
    }
    doing_multi++;
  }
...
void start_dcc_get(int snum) {
  ...
  switch (bytes_read) {
  case -1:
    erase_dcc_info(snum, 1, "%s",
                   convert_output_format(
                       "$G %RDCC get to $0 lost: Remote peer closed connection",
                       "%s", s->server));
    close_socketread(snum);
    return;
  case 0:
  ...
  if (n->filesize) {
    if (n->bytes_read + n->transfer_orders.byteoffset > n->filesize) {
      put_it("%s", convert_output_format("$G %RDCC%n Warning: incoming file is "
                                         "larger than the handshake said",
                                         NULL, NULL));
      put_it("%s",
             convert_output_format(
                 "$G %RDCC%n Warning: GET: closing connection", NULL, NULL));
...
void real_get_file(char *user, char *filename, char *port, char *passwd,
                   int tdcc, int blocksize) {
  DCC_int *new = NULL;
  SocketList *s = NULL;
  DCC_List *s1 = NULL;
  char *nick;
  ...
  while ((nick = next_in_comma_list(user, &user))) {
    ...
    if ((s1 = find_dcc_pending(nick, filename, NULL, DCC_FILEREAD, 0, -1))) {
      ...
      if ((s = find_dcc(nick, new->filename, NULL, DCC_FILEREAD, 1, -1, -1))) {
        put_it("%s",
               convert_output_format(
                   "$G %RDCC%n A previous DCC GET from $0 exists", "%s", nick));
        continue;
      }
    } else
      continue;
    if ((new = dcc_create(nick, filename, passwd, 0, port ? atol(port) : 0,
                          DCC_FILEREAD,
                          (tdcc ? DCC_TDCC : 0) | DCC_TWOCLIENTS | DCC_OFFER,
                          start_dcc_get))) {
      char *tmp = NULL;
      char *fullname = NULL;
      new->blocksize = blocksize;
      if (get_string_var(DCC_DLDIR_VAR))
        malloc_sprintf(&tmp, "%s/%s", get_string_var(DCC_DLDIR_VAR),
                       new->filename);
      else
      ...
      if ((new->file = open(fullname, O_WRONLY | O_TRUNC | O_CREAT | O_BINARY,
                            0644)) == -1) {
        erase_dcc_info(s->is_read, 1, "%s",
                       convert_output_format(
                           "$G %RDCC%n Unable to open $0: $1-", "%s %s",
                           fullname, errno ? strerror(errno) : "Unknown"));
        close_socketread(s->is_read);
      }
      new_free(&fullname);
      new_free(&tmp);
...
void dcc_getfile(char *command, char *args) {
  ...
  if (!user) {
    put_it("%s", convert_output_format(
                     "$G %RDCC%n You must supply a nickname for DCC get", NULL,
                     NULL));
    return;
  }
  if (!blocksize || blocksize > MAX_DCC_BLOCK_SIZE)
...
void real_reget_file(char *user, char *filename, char *port, char *passwd,
                     int tdcc, int blocksize) {
  DCC_int *new = NULL;
  SocketList *s = NULL;
  char *nick;
  ...
  while ((nick = next_in_comma_list(user, &user))) {
    ...
    if ((s1 = find_dcc_pending(nick, filename, NULL, DCC_REFILEREAD, 0, -1))) {
      new = (DCC_int *)s1->sock.info;
      if ((s = find_dcc(nick, new->filename, NULL, DCC_REFILEREAD, 1, -1,
                        -1))) {
        put_it("%s", convert_output_format(
                         "$G %RDCC%n A previous DCC REGET from $0 exists", "%s",
                         nick));
        continue;
      }
    } else
    ...
    if (!(fullname = expand_twiddle(tmp)))
      malloc_strcpy(&fullname, tmp);
    if ((new->file = open(fullname, O_WRONLY | O_CREAT | O_BINARY, 0644)) !=
        -1) {
      if ((new = dcc_create(nick, new->filename, passwd, 0,
                            port ? atol(port) : 0, DCC_REFILEREAD,
                            (tdcc ? DCC_TDCC : 0) | DCC_TWOCLIENTS | DCC_OFFER,
                            start_dcc_get)))
        new->blocksize = blocksize;
    }
...
void dcc_regetfile(char *command, char *args) {
  ...
  if (!user) {
    put_it("%s", convert_output_format(
                     "$G %RDCC%n You must supply a nickname for DCC reget",
                     NULL, NULL));
    return;
  }
  if (!blocksize || blocksize > MAX_DCC_BLOCK_SIZE)
...
static const char *get_stat_format(double pcomplete) {
  ...
  static const char *const bar_format[] = {
      ...
      STAT_FORMAT_PREFIX "%K����������%n" STAT_FORMAT_SUFFIX,           /* 10 */
      STAT_FORMAT_PREFIX "%K����������%n" STAT_FORMAT_SUFFIX,           /* 20 */
      STAT_FORMAT_PREFIX "%K��۰������%n" STAT_FORMAT_SUFFIX,           /* 30 */
      STAT_FORMAT_PREFIX "%K���%1%K�%0%K������%n" STAT_FORMAT_SUFFIX,   /* 40 */
      STAT_FORMAT_PREFIX "%K���%1%K��%0%K�����%n" STAT_FORMAT_SUFFIX,   /* 50 */
      STAT_FORMAT_PREFIX "%K���%1%K���%0%K����%n" STAT_FORMAT_SUFFIX,   /* 60 */
      STAT_FORMAT_PREFIX "%K���%1%K����%0%K���%n" STAT_FORMAT_SUFFIX,   /* 70 */
  ...
  if (!get_int_var(DCC_BAR_TYPE_VAR) && idx >= 0 &&
      idx < (int)(sizeof bar_format / sizeof bar_format[0])) {
    return bar_format[idx];
  }
  return STAT_FORMAT_PREFIX "$[10]4" STAT_FORMAT_SUFFIX;
}

void dcc_glist(char *command __attribute__((unused)), char *args __attribute__((unused))) {
#define DCC_FORMAT_STAT_PENDING STAT_FORMAT_PREFIX "$[25]4 $[7]5 $6-"
#define DCC_FORMAT_STAT                                                        \
  STAT_FORMAT_PREFIX "$[7]4 $[-4]5 $[-3]6 $[-3]7 $[-3]8  $[7]9 $10"
  int i;
  DCC_int *n = NULL;
  SocketList *s;
  int type;
  ...
  reset_display_target();
#if !defined(WINNT) && !defined(__EMX__) &&                                    \
    (defined(LATIN1) || defined(CHARSET_CUSTOM))
  charset_ibmpc();
#endif
  if (do_hook(DCC_HEADER_LIST, "%s %s %s %s %s %s %s", "Dnum", "Type", "Nick",
              "Status", "K/s", "File", "Encrypt")) {
#ifdef ONLY_STD_CHARS
    put_it("%s",
           convert_output_format(
    ...
    put_it(
        "%s",
        convert_output_format(
            "%K��%n�%W�%n�%K�����%n�%W�%n�%K���������%n�%W�%n�%K���������������"
            "%n�%W�%n�%K�����%n�%W�%n�%K�����%n�%W�%n�%K�������������������",
            NULL, NULL));
#endif
  }
  for (c = pending_dcc; c; c = c->next, count++) {
    ...
    if (do_hook(DCC_STAT_LIST, "%d %s %s %s %s %s %s", n->dccnum, type_name,
                c->sock.server, dcc_get_state(s), "N/A", strip_path(filename),
                n->encrypt ? "E" : empty_string)) {
      put_it("%s", convert_output_format(DCC_FORMAT_STAT_PENDING,
                                         "%d %s %s %s %s %s %s", n->dccnum,
                                         type_name, n->encrypt ? "E" : "�",
  ...
  for (i = 0; i < get_max_fd() + 1; i++, count++) {
    ...
    if (!(s->flags & DCC_ACTIVE) || (type == DCC_CHAT) || (type == DCC_RAW) ||
        (type == DCC_BOTMODE) || (type == DCC_FTPOPEN)) {
      if (do_hook(DCC_STAT_LIST, "%d %s %s %s %s %s %s", n->dccnum, type_name,
                  s->server, dcc_get_state(s), "N/A", strip_path(filename),
                  n->encrypt ? "E" : empty_string))
        put_it("%s", convert_output_format(
                         DCC_FORMAT_STAT, "%d %s %s %s %s %s %s %s", n->dccnum,
                         type_name, n->encrypt ? "E" : "�", s->server,
    ...
    /* Active file DCCs use the STATF / STATF1 lines */
      ...
      if (do_hook(DCC_STATF_LIST, "%d %s %s %s %s %s %s", n->dccnum, type_name,
                  s->server, dcc_get_state(s), kilobytes, strip_path(filename),
                  n->encrypt ? "E" : empty_string)) {
      ...
      if (do_hook(DCC_STATF1_LIST, "%4.1f %lu %lu %d %d", pcomplete * 100.0,
                  (unsigned long)bytes, (unsigned long)n->filesize, minutes,
                  seconds)) {
        ...
        snprintf(stats, sizeof stats, "%4.1f%% (%lu of %lu bytes)",
                 pcomplete * 100.0, (unsigned long)bytes,
                 (unsigned long)n->filesize);
        snprintf(spec, sizeof spec,
                 BOLD_TOG_STR "[" REV_TOG_STR "%*.*s" REV_TOG_STR
                              "%*.*s]" BOLD_TOG_STR " ETA " BOLD_TOG_STR
                              "%02d:%02d",
                 size, size, space, BAR_LENGTH - size, BAR_LENGTH - size, space,
                 minutes, seconds);
...
#endif
  if (count)
    do_hook(DCC_POST_LIST, "%s %s %s %s %s %s %s", "DCCnum", "Type", "Nick",
            "Status", "K/s", "File", "Encrypt");
  else
...
int check_dcc_list(char *name) {
  ...
  for (i = 0; i < get_max_fd() + 1; i++) {
    if (check_dcc_socket(i)) {
      SocketList *s;
      s = get_socket(i);
      if (s->server && !my_stricmp(name, s->server) &&
          !(s->flags & DCC_ACTIVE)) {
...
void update_transfer(void) {
  ...
  *transfer_buffer = 0;
  for (i = 0; i < get_max_fd() + 1; i++) {
    ...
    if ((s->flags & DCC_OFFER) || (s->flags & DCC_WAIT) || (flags == DCC_RAW) ||
        (flags == DCC_RAW_LISTEN) || (flags == DCC_CHAT) ||
        (flags == DCC_BOTMODE) || (flags == DCC_FTPOPEN))
      continue;
    n = (DCC_int *)s->info;
    ...
    if (bytes >= 0) {
      ...
      strmopencat(transfer_buffer, BIG_BUFFER_SIZE, ltoa((int)perc), "% ",
                  NULL);
#if 0
  ...
  if (count) {
    ...
    if (fget_string_var(FORMAT_DCC_FSET)) {
      strlcpy(DCC_current_transfer_buffer,
              convert_output_format(fget_string_var(FORMAT_DCC_FSET), "%s",
                                    transfer_buffer),
...
void dcc_show_active(char *command __attribute__((unused)), char *args __attribute__((unused))) {
  put_it("%s",
         convert_output_format(
             "$G %RDCC%n  DCC Active = \002$0\002, Limit = \002$1\002", "%d %d",
             get_active_count(), get_int_var(DCC_SEND_LIMIT_VAR)));
}

void dcc_set_quiet(char *command __attribute__((unused)), char *args __attribute__((unused))) {
  dcc_quiet ^= 1;
  put_it("%s", convert_output_format("$G %RDCC%n  DCC Quiet = \002$0\002", "%s",
                                     on_off(dcc_quiet)));
}

void dcc_set_paths(char *command __attribute__((unused)), char *args __attribute__((unused))) {
  dcc_paths ^= 1;
  put_it("%s", convert_output_format("$G %RDCC%n  DCC paths is now \002$0\002",
                                     "%s", on_off(dcc_paths)));
}

void dcc_tog_rename(char *command __attribute__((unused)), char *args __attribute__((unused))) {
  int arename = get_int_var(DCC_AUTORENAME_VAR);
  arename ^= 1;
  set_int_var(DCC_AUTORENAME_VAR, arename);
  put_it("%s",
         convert_output_format("$G %RDCC%n  DCC auto rename is now \002$0\002",
                               "%s", on_off(get_int_var(DCC_AUTORENAME_VAR))));
}

void dcc_tog_resume(char *command __attribute__((unused)), char *args __attribute__((unused))) {
  int arename = get_int_var(DCC_AUTORESUME_VAR);
  arename ^= 1;
  set_int_var(DCC_AUTORESUME_VAR, arename);
  put_it("%s",
         convert_output_format("$G %RDCC%n  DCC auto resume is now \002$0\002",
                               "%s", on_off(get_int_var(DCC_AUTORESUME_VAR))));
}

void dcc_overwrite_toggle(char *command __attribute__((unused)), char *args __attribute__((unused))) {
  dcc_overwrite_var ^= 1;
  put_it("%s", convert_output_format("  DCC overwrite is now \002$0\002", "%s",
                                     on_off(dcc_overwrite_var)));
}

void dcc_tog_auto(char *command __attribute__((unused)), char *args __attribute__((unused))) {
  int dcc_auto = get_int_var(DCC_AUTOGET_VAR);
  dcc_auto ^= 1;
  set_int_var(DCC_AUTOGET_VAR, dcc_auto);
  put_it("%s", convert_output_format("  DCC autoget is now \002$0\002", "%s",
                                     on_off(dcc_auto)));
}

void dcc_stats(char *command __attribute__((unused)), char *unused __attribute__((unused))) {
  char max_rate_in[20];
  char min_rate_in[20];
  ...
  sprintf(min_rate_in, "%6.2f",
          ((dcc_min_rate_in != DBL_MAX) ? dcc_min_rate_in / 1024.0 : 0.0));
  sprintf(max_rate_out, "%6.2f", dcc_max_rate_out / 1024.0);
  sprintf(min_rate_out, "%6.2f",
          ((dcc_min_rate_out != DBL_MAX) ? dcc_min_rate_out / 1024.0 : 0.0));
  if (do_hook(
          DCC_TRANSFER_STAT_LIST, "%lu %s %s %lu %s %s %lu %u %u %s %s %s %s",
          (unsigned long)dcc_bytes_in, max_rate_in, min_rate_in,
          (unsigned long)dcc_bytes_out, max_rate_out, min_rate_out,
          (unsigned long)(send_count_stat + get_count_stat), get_active_count(),
          get_int_var(DCC_SEND_LIMIT_VAR), on_off(get_int_var(DCC_AUTOGET_VAR)),
          on_off(dcc_paths), on_off(dcc_quiet), on_off(dcc_overwrite_var))) {
    char in[50], out[50];
    ...
    put_it("%s",
           convert_output_format("       %G========================%K[%Cdcc "
                                 "transfer stats%K]%G=======================",
                                 NULL));
    ...
    put_it("%s",
           convert_output_format(
               "       %G|%g|-%K[%Cx%cferd %Ci%cn%K]%g-|-%K[%Cx%cferd "
               "%Co%cut%K]%g-|-%K[%Ct%cotal "
               "%Cf%ciles%K]%g-|-%K[%Ca%cctive%K]%g-|-[%Cl%cimit%K]%g-|%G|",
               NULL));
    put_it("%s",
           convert_output_format(
               "       %G|%g| %W$[-10]0 %g|  %W$[-10]1 %g|    %W$[-10]2 %g| "
               "%W$[-8]3 %g| %W$[-7]4 %g|%G|",
    ...
    put_it("%s",
           convert_output_format(
               "       %g|----%K[%Ci%cn %Cs%ctats%K]%g---|---%K[%Co%cut "
               "%Cs%ctats%K]%g---|----------%K[%Ct%coggles%K]%g----------|",
               NULL));
    put_it("%s", convert_output_format(
                     "       %g| %Cm%nax: %W$[-6]0%n%Rkb/s %g| %Cm%nax: "
                     "%W$[-6]1%n%Rkb/s %g|   %Ca%nutoget: %W$[-3]2%n   "
                     "%Cp%naths: %W$[-3]3 %g|",
                     "%s %s %s %s", max_rate_in, max_rate_out,
                     on_off(get_int_var(DCC_AUTOGET_VAR)), on_off(dcc_paths)));
    put_it("%s",
           convert_output_format("       %g| %Cm%nin: %W$[-6]0%n%Rkb/s %g| "
                                 "%Cm%nin: %W$[-6]1%n%Rkb/s %g| %Co%nverwrite: "
                                 "%W$[-3]2%n   %Cq%nuiet: %W$[-3]3 %g|",
    ...
    put_it("%s",
           convert_output_format(
               "       %G�%g��%K[%Cx%cferd %Ci%cn%K]%g��-%K[%Cx%cferd "
               "%Co%cut%K]%gķ�%K[%Ct%cotal "
               "%Cf%ciles%K]%g���%K[%Ca%cctive%K]%gķ�[%Cl%cimit%K]%gķ%G�",
               NULL));
    put_it("%s",
           convert_output_format(
               "       %G�%g� %W$[-10]0 %g�  %W$[-10]1 %g�    %W$[-10]2 %g� "
    ...
    put_it("%s",
           convert_output_format(
               "       %g�����%K[%Ci%cn %Cs%ctats%K]%g�������%K[%Co%cut "
               "%Cs%ctats%K]%g��ķ����������%K[%Ct%coggles%K]%g���������ķ",
               NULL));
    put_it("%s", convert_output_format(
                     "       %g� %Cm%nax: %W$[-6]0%n%Rkb/s %g� %Cm%nax: "
                     "%W$[-6]1%n%Rkb/s %g�   %Ca%nutoget: %W$[-3]2%n   "
...
BUILT_IN_COMMAND(chat) {
  ...
  if (args && *args) {
    ...
    if ((new =
             dcc_create(last_chat_req, "chat", NULL, 0, 0, DCC_CHAT,
                        DCC_TWOCLIENTS | DCC_OFFER | flags, start_dcc_chat))) {
      char *equal_user;
      new->blocksize = get_int_var(DCC_BLOCK_SIZE_VAR);
...
void dcc_exempt(char *command __attribute__((unused)), char *args) {
  int remove;
  List *nptr = NULL;
  char *nick;
  if (!args || !*args) {
    int count = 0;
    for (nptr = next_namelist(dcc_no_flood, NULL, DCC_HASHSIZE); nptr;
         nptr = next_namelist(dcc_no_flood, nptr, DCC_HASHSIZE)) {
      if (count == 0)
        put_it("%s", convert_output_format(
                         "$G %RDCC%n autoget list/no flood list", NULL, NULL));
      put_it("%s", nptr->name);
      count++;
    }
    if (count == 0)
  ...
  while (nick && *nick) {
    ...
    if (remove && nptr) {
      ...
      put_it("%s", convert_output_format(
                       "$G %RDCC%n No such nick on the exempt list %K[%W$0%K]",
                       "%s", nick));
    nick = next_arg(args, &args);
  }
}
 ...
 */
static void output_reject_ctcp(UserhostItem *stuff, char *nick __attribute__((unused)), char *args) {
  ...
  if (nickname_received && *nickname_received)
    send_ctcp(CTCP_NOTICE, nickname_received, CTCP_DCC, "REJECT %s %s", type,
              description);
}
...
extern void dcc_reject(char *from, char *type, char *args) {
  ...
  description = next_arg(args, &args);
  if ((s = find_dcc(from, description, NULL, CType, 0, -1, -1)) ||
      (s1 = find_dcc_pending(from, description, NULL, CType, 1, -1))) {
    DCC_int *n;
    if (s1)
      s = &s1->sock;
    n = (DCC_int *)s->info;
...
BUILT_IN_COMMAND(dcx) {
  ...
  for (i = 0; i < get_max_fd() + 1; i++) {
    ...
    if (all || (type == flag)) {
      ...
      erase_dcc_info(i, 1, "%s",
                     convert_output_format("$G %RDCC%n closing dcc $0 to $1",
                                           "%s %s", dcc_types[flag]->name,
                                           s->server));
      close_socketread(i);
    }
  }
...
void cancel_dcc_auto(int i, unsigned long flags, SocketList *s) {
  if (!last_notify || strcmp(s->server, last_notify)) {
    send_to_server("NOTICE %s :DCC %s Auto Closed", s->server,
                   dcc_types[flags]->name);
    malloc_strcpy(&last_notify, s->server);
  }
  erase_dcc_info(
      i, 1, "%s",
      convert_output_format("$G %RDCC%n Auto-closing idle dcc $0 to $1",
                            "%s %s", dcc_types[flags]->name, s->server));
  close_socketread(i);
}
...
void dcc_check_idle(void) {
  ...
  for (i = 0; i < get_max_fd() + 1; i++) {
    time_t client_idle, this_idle_time;
    ...
    case DCC_FILEOFFER:
    case DCC_REFILEOFFER:
      if (cdcc_minspeed && minidlecheck &&
          (((now - n->starttime.tv_sec) % minidlecheck) == 0)) {
        unsigned long sent = n->bytes_sent / 1024;
        double this_speed = 0.0;
        char lame_ultrix1[20];
        char lame_ultrix[20];
        this_speed =
            (double)((double)sent / (double)(now - n->starttime.tv_sec));
        if (this_speed < (float)cdcc_minspeed) {
          sprintf(lame_ultrix, "%2.4g",
                  (double)(sent / (now - n->starttime.tv_sec)));
          sprintf(lame_ultrix1, "%2.4g", (double)cdcc_minspeed);
          if (!last_notify || strcmp(s->server, last_notify)) {
            send_to_server("NOTICE %s :CDCC Slow dcc %s Auto Closed. Require "
                           "%sKB/s got %sKB/s",
                           s->server, dcc_types[flags]->name, lame_ultrix1,
                           lame_ultrix);
            malloc_strcpy(&last_notify, s->server);
          }
          erase_dcc_info(
              i, 1, "%s",
              convert_output_format("$G %RDCC%n Auto-closing Slow dcc $0 to $1 "
                                    "require $2KB/s got $3KB/s",
                                    "%s %s %s %s", dcc_types[flags]->name,
                                    s->server, lame_ultrix1, lame_ultrix));
          close_socketread(i);
        }
        break;
      }
...
void dcc_close(char *command __attribute__((unused)), char *args) {
  char *type;
  char *file;
  char *nick;
  int any_type = 0;
  ...
  while ((s = find_dcc(nick, file, NULL, i, 0, -1, num))) {
    ...
    n = (DCC_int *)s->info;
    count++;
    if (do_hook(DCC_LOST_LIST, "%s %s %s USER ABORTED CONNECTION", s->server,
                dcc_types[s->flags & DCC_TYPES]->name,
                n->filename ? n->filename : "<any>"))
      say("DCC %s:%s to %s closed", dcc_types[s->flags & DCC_TYPES]->name,
          file ? file : "<any>", s->server);
  ...
  while ((s1 = find_dcc_pending(nick, file, NULL, i, 1, num))) {
    ...
    send_reject_ctcp(s->server,
                     (((s->flags & DCC_TYPES) == DCC_FILEOFFER) ? "GET"
                      : ((s->flags & DCC_TYPES) == DCC_FILEREAD)
                          ? "SEND"
                          : dcc_types[s->flags & DCC_TYPES]->name),
                     n->filename);
  ...
  if (!count)
    put_it("%s", convert_output_format("$G %RDCC%n No DCC $0:$1 to $2 found",
                                       "%s %s %s", (i == -1 ? "<any>" : type),
                                       (file ? file : "<any>"),
                                       (num != -1) ? ltoa(num)
                                       : nick      ? nick
...
void dcc_closeall(char *command __attribute__((unused)), char *args __attribute__((unused))) { close_all_dcc(); }

extern int doing_notice;

#ifdef MIRC_BROKEN_DCC_RESUME

void dcc_getfile_resume_demanded(char *nick, char *filename, char *port,
                                 char *offset) {
  SocketList *s;
  DCC_int *n;
  ...
  if (!(s = find_dcc(nick, filename, port, DCC_FILEOFFER, 1, -1, -1))) {
    put_it("%s",
           convert_output_format(
               "$G %RDCC%n warning in dcc_getfile_resume_demanded", NULL));
    return;
  }
  n = (DCC_int *)s->info;
  if (!offset)
  ...
  send_ctcp(CTCP_PRIVMSG, nick, CTCP_DCC, "ACCEPT %s %s %s", filename, port,
            offset);
...
void dcc_getfile_resume_start(char *nick, char *description, char *port,
                              char *offset __attribute__((unused))) {
  SocketList *s;
  DCC_int *n;
  ...
  if ((s = find_dcc(nick, description, NULL, DCC_FILEREAD, 1, 1, -1))) {
    put_it("%s", convert_output_format(
                     "$G %RDCC%n warning in dcc_getfile_resume_start", NULL));
    return;
  }
  ...
  if (!pending || !(pending->sock.flags & DCC_RESUME_REQ)) {
    put_it("%s",
           convert_output_format(
               "$G %RDCC%n Ignoring unsolicited ACCEPT from $0", "%s", nick));
    return;
  }

  if (!(n = dcc_create(nick, description, NULL, 0, port ? atol(port) : 0,
                       DCC_FILEREAD, DCC_TWOCLIENTS | DCC_OFFER,
                       start_dcc_get)))
    return;
  ...
  if (!(n->file = open(fullname, O_WRONLY | O_APPEND | O_BINARY, 0644)) ||
      (fstat(n->file, &sb) != 0) || (sb.st_size >= n->filesize)) {
    int snum;
    ...
    if (s) {
      ...
      erase_dcc_info(
          snum, 1, "%s",
          convert_output_format(
              "$G %RDCC%n Unable to open $0: $1-", "%s %s", n->filename,
              errno ? strerror(errno) : "incoming file smaller than existing"));
      close_socketread(snum);
    }
  } else
    put_it("%s", convert_output_format("$G %RDCC%n RESUME of $0 at $1", "%s %l",
                                       n->filename, sb.st_size));
  new_free(&fullname);
  new_free(&tmp);
}

void dcc_resume(char *command __attribute__((unused)), char *args) {
  char *user, *nick;
  char *filename = NULL;
  char *fullname = NULL;
  char *tmp = NULL;
  ...
  while ((nick = next_in_comma_list(user, &user))) {
    ...
    if ((s = find_dcc(nick, filename, NULL, DCC_FILEREAD, 1, 1, -1))) {
      put_it("%s",
             convert_output_format(
                 "$G %RDCC%n DCC send already active. Unable to RESUME", NULL));
      continue;
    }
    ...
    if (stat(fullname, &sb) == -1) {
      ...
      put_it("%s", convert_output_format("$G %RDCC%n Cannot use DCC RESUME if "
                                         "the file doesn't exist [$0|$1-]",
                                         "%s %s", fullname, strerror(errno)));
      continue;
    }
    ...
    doing_privmsg = doing_notice = in_ctcp_flag = 0;
    send_ctcp(CTCP_PRIVMSG, nick, CTCP_DCC, "RESUME %s %d %ld", n->filename,
              n->remport, sb.st_size);
    doing_privmsg = old_dp;
    doing_notice = old_dn;
    in_ctcp_flag = old_dc;
  }
...
void dcc_help1(char *command __attribute__((unused)), char *args) {
  char *comm;
  int i, c;
  char buffer[BIG_BUFFER_SIZE + 1];
  ...
  if (args && *args) {
    ...
    if ((dcc_comm = (DCC_dllcommands *)find_in_list((List **)&dcc_dllcommands,
                                                    comm, 0))) {
      put_it("%s",
             convert_output_format("$G Usage: %W/%R$0%n $1 %K-%n $2-",
                                   "DCC %s %s", dcc_comm->name,
                                   dcc_comm->help ? dcc_comm->help
                                                  : "No help available yet"));
      return;
    }
    for (i = 0; dcc_commands[i].name != NULL; i++) {
      if (!strncmp(comm, dcc_commands[i].name, strlen(comm))) {
        put_it("%s", convert_output_format("$G Usage: %W/%R$0%n $1 %K-%n $2-",
                                           "DCC %s %s", dcc_commands[i].name,
                                           dcc_commands[i].help
                                               ? dcc_commands[i].help
                                               : "No help available yet"));
        return;
...
void read_ftp_file(int snum) {
  ...
  if ((ioctl(snum, FIONREAD, &len) == -1)) {
    put_it("%s", convert_output_format("$G %gFTP%n file error [$0-]", "%s",
                                       strerror(errno)));
    erase_dcc_info(snum, 0, NULL);
    close_socketread(snum);
    return;
  ...
  case 230: {
    ...
    if (n->othername) {
      ...
      if (do_hook(FTP_LIST, "%d Changing directory to %s", comm, n->othername))
        put_it("%s", convert_output_format("%gFTP%n Changing directory to $0-",
                                           "%s", n->othername));
    }
    new_free(&n->encrypt);
    new_free(&n->userhost);
...
void open_ftpget(SocketList *s, char *args) {
  ...
  bufptr = tmp;
  while (1) {
    if (dgets(bufptr, s->is_read, 1, BIG_BUFFER_SIZE, NULL) == -1 &&
        dgets_errno > 0)
      goto error_ftp;
    if (*bufptr == '5')
      goto error_ftp;
  ...
  add_socketread(s1, 0, DCC_FTPGET | DCC_ACTIVE, s->server, read_ftp_file,
                 NULL);
  new = new_malloc(sizeof(DCC_int));
  {
    char *t, *expand;
  ...
  if (do_hook(FTP_LIST, "%s %s", "FTP Attempting to get", filename))
    put_it("%s", convert_output_format("$G %gFTP%n Attempting to get $0", "%s",
                                       filename));
  return;
error_ftp:
  close(data);
  ...
  if (s) {
    ...
    if (s->flags & DCC_WAIT) {
      if (do_hook(FTP_LIST, "%s", "FTP connection is busy"))
        put_it("%s", convert_output_format("$G %gFTP%n connection is busy.",
                                           NULL, NULL));
      return 1;
    }
    ...
    if (command && *command) {
      if (!my_strnicmp(command, "ls", 2) || !my_strnicmp(command, "dir", 3)) {
        ...
        close(sock);
        if (s1 != -1) {
          add_socketread(s1, 0, DCC_FTPCOMMAND | DCC_ACTIVE, s->server,
                         start_ftp, NULL);
          new = new_malloc(sizeof(DCC_int));
          new->struct_type = DCC_STRUCT_TYPE;
          get_time(&new->starttime);
    ...
    if (do_hook(FTP_LIST, "%s", "FTP is not connected"))
      put_it("%s",
             convert_output_format("$G %gFTP%n is not connected.", NULL, NULL));
    return 0;
  }
  return 1;
error_port:
...
void dcc_ftpopen(char *command __attribute__((unused)), char *args) {
  char u[] = "anonymous";
  char p[] = "- bxuser@";
  char *host = NULL, *user = NULL, *pass = NULL, *dir = NULL, *t_host = NULL;
  ...
  if (!(t_host = next_arg(args, &args))) {
    put_it("%s /dcc ftp hostname user passwd [-p port] [-b blocksize]",
           convert_output_format("$G %RDCC%n", NULL, NULL));
    return;
  }
  ...
  if ((s = find_dcc(host, "ftpopen", NULL, DCC_FTPOPEN, 0, -1, -1))) {
    put_it("%s", convert_output_format(
                     "$G %GFTP%n A previous DCC FTP to $0 exists", "%s", host));
    new_free(&host);
    return;
  }

#if defined(WINNT) || defined(__EMX__)
  if ((snum = connect_by_number(host, &port, SERVICE_CLIENT, PROTOCOL_TCP, 0)) <
      0)
#else
  if ((snum = connect_by_number(host, &port, SERVICE_CLIENT, PROTOCOL_TCP, 1)) <
      0)
#endif
  {
    put_it("%s", convert_output_format("$G %gFTP%n command failed connect",
                                       NULL, NULL));
    new_free(&host);
    return;
  } else {
...
void dcc_rename(char *command __attribute__((unused)), char *args) {
  DCC_List *tmp;
  DCC_int *n;
  char *nick, *filename;
  ...
  if (!nick || !filename || !*filename) {
    put_it("%s",
           convert_output_format("$G /DCC rename nick filename", NULL, NULL));
    return;
  }
...
/* add_dcc_bind()
 *
 * Bind a new set of DCC operations to a DCC name.  This can either be a
 * completely new DCC type, or a new set of operations for a builtin DCC type.
 * This fails if the given DCC name is already bound by another module.
 */
int BX_add_dcc_bind(char *name, char *module, const struct dcc_ops *dcc_ops) {
  ...
  if (i < 0) {
    ...
    for (i = 0; i < (int)n_dcc_types; i++) {
      if (!dcc_types[i])
        break;
    }

    if (i & ~DCC_TYPES) {
      yell("Failed to add DCC binding [%s] for module [%s], too many DCC "
           "bindings.",
           name, module);
      return 0;
    }

    if (i >= (int)n_dcc_types) {
      n_dcc_types = i + 1;
    ...
    if (dcc_types[i]->module) {
      yell("Failed to add DCC binding [%s] for module [%s], already bound by "
           "module [%s].",
...
int BX_remove_dcc_bind(char *name __attribute__((unused)), int type) {
  if (type >= (int)n_dcc_types || !dcc_types[type] || !dcc_types[type]->module)
    return 0;

  new_free(&dcc_types[type]->module);
  dcc_types[type]->dcc_ops = &null_ops;
  if (type >= (int)n_builtin_dcc_types) {
...
int BX_remove_all_dcc_binds(const char *module) {
  ...
  for (i = 0; i < (int)n_dcc_types; i++) {
    if (dcc_types[i] && dcc_types[i]->module &&
        !strcasecmp(dcc_types[i]->module, module))
...
char *get_dcc_info(SocketList *s, DCC_int *n, int i) {
  return m_sprintf("%s %s %s %ld %lu %lu %lu %s #%d %d",
                   ...
                   s->server, dcc_get_state(s), (long)n->starttime.tv_sec,
                   (unsigned long)n->transfer_orders.byteoffset,
                   (unsigned long)n->bytes_sent, (unsigned long)n->bytes_read,
                   n->filename, i, n->server);
}

void dcc_raw_transmit(char *user __attribute__((unused)), char *text __attribute__((unused)), unsigned stxt_flags __attribute__((unused))) { return; }
...
char *dcc_raw_connect(char *host __attribute__((unused)), u_short port __attribute__((unused))) { return m_strdup("-1"); }

char *dcc_raw_listen(int port __attribute__((unused))) {
#if 0
	SocketList	*Client;
	char		*PortName;
