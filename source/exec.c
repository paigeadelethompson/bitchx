...
char *sys_siglist[] = {"ZERO",    "SIGINT",  "SIGKILL", "SIGPIPE", "SIGFPE",
                       "SIGHUP",  "SIGTERM", "SIGSEGV", "SIGTSTP", "SIGQUIT",
                       "SIGTRAP", "SIGILL",  "SIGEMT",  "SIGALRM", "SIGBUS",
                       "SIGLOST", "SIGSTOP", "SIGABRT", "SIGUSR1", "SIGUSR2",
                       "SIGCHLD", "SIGTTOU", "SIGTTIN", "SIGCONT"};
...
BUILT_IN_COMMAND(execcmd) {
  ...
  char *who = NULL, *logical = NULL, *redirect = NULL, *flag;
  unsigned refnum = 0;
  int sig, len, i, refnum_flag = 0, logical_flag = 0;
  int direct = 0;
  Process *proc;
  char *stdoutc = NULL, *stderrc = NULL, *stderrpc = NULL, *stdoutpc = NULL,
       *endc = NULL;
  ...
  if (in_cparse) {
    ...
    yell("with what version of BitchX as well the scripts loaded when this "
         "happened");
    return;
  }
  /*
  ...
  if (!*args) {
    ...
    for (i = 0; i < process_list_size; i++) {
      ...
      if (proc->logical)
        say("\t%d (%s) (pid %d): %s", i, proc->logical, proc->pid, proc->name);
      else
  ...
  if (*args == '%') {
    ...
    if (refnum_flag) {
      proc->refnum = refnum;
      if (refnum)
        say("Output from process %d (%s) now going to this window", i,
            proc->name);
      else
        say("Output from process %d (%s) not going to any window", i,
            proc->name);
    }
    ...
    if (logical_flag) {
      if (is_logical_unique(logical)) {
        ...
        say("Process %d (%s) is now called %s", i, proc->name, proc->logical);
      } else
...
void do_processes(fd_set *rd) {
  ...
  for (i = 0; i < process_list_size; i++) {
    ...
    if (proc->p_stdout != -1 && FD_ISSET(proc->p_stdout, rd))
      handle_filedesc(proc, &proc->p_stdout, EXEC_PROMPT_LIST, EXEC_LIST);

    if (proc->p_stderr != -1 && FD_ISSET(proc->p_stderr, rd))
      handle_filedesc(proc, &proc->p_stderr, EXEC_PROMPT_LIST,
                      EXEC_ERRORS_LIST);
 ...
 */
static void handle_filedesc(Process *proc, int *fd, int hook_nonl,
                            int hook_nl) {
  char exec_buffer[IO_BUFFER_SIZE + 1];
  int len;

  switch ((len = dgets(exec_buffer, *fd, 0, IO_BUFFER_SIZE,
                       NULL))) /* No buffering! */
  {
  ...
  case 0: /* We didnt get a full line */
  {
    if (hook_nl == EXEC_LIST && proc->stdoutpc)
      ...
      do_hook(hook_nonl, "%s %s", proc->logical, exec_buffer);
    else
      do_hook(hook_nonl, "%d %s", proc->index, exec_buffer);
  ...
  default: /* We got a full line */
  {
    ...
    if (proc->redirect)
      redirect_text(proc->server, proc->who, exec_buffer,
                    !strcmp(proc->redirect, "NOTICE") ? STXT_NOTICE : 0);
    ...
    if (hook_nl == EXEC_LIST && proc->stdoutc)
      ...
      if ((do_hook(hook_nl, "%s %s", proc->logical, exec_buffer)))
        if (!proc->redirect)
          put_it("%s", exec_buffer);
    } else {
      if ((do_hook(hook_nl, "%d %s", proc->index, exec_buffer)))
        if (!proc->redirect)
          put_it("%s", exec_buffer);
    }
...
static void cleanup_dead_processes(void) {
  ...
  for (i = 0; i < process_list_size; i++) {
    ...
    if (dead->logical && *dead->logical)
      flag = do_hook(EXEC_EXIT_LIST, "%s %d %d", dead->logical, dead->termsig,
                     dead->retcode);
    else
      flag = do_hook(EXEC_EXIT_LIST, "%d %d %d", dead->index, dead->termsig,
                     dead->retcode);
    ...
    if (flag) {
      if (get_int_var(NOTIFY_ON_TERMINATION_VAR)) {
        if (dead->termsig > 0 && dead->termsig < NSIG)
          say("Process %d (%s) terminated with signal %s (%d)", dead->index,
              dead->name,
#if defined(USING_STRSIGNAL)
          ...
          say("Process %d (%s) terminated with return code %d", dead->index,
              dead->name, dead->retcode);
      }
    }
