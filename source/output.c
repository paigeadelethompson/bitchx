 ...
 * swoops into effect
 */
void refresh_screen(unsigned char dumb __attribute__((unused)), char *dumber __attribute__((unused))) {
...
void say(const char *format, ...) {
  if (window_display && format) {
    ...
    snprintf(putbuf, LARGE_BIG_BUFFER_SIZE, "%s ",
             thing_ansi ? thing_ansi : three_stars);
    len = strlen(putbuf);
...
void BX_bitchsay(const char *format, ...) {
  ...
  if (window_display && format) {
    va_list args;
    va_start(args, format);
    sprintf(putbuf, "%s \002%s\002: ", thing_ansi ? thing_ansi : three_stars,
            version);
    len = strlen(putbuf);
...
char *ov_server(int server) { return ov_format(get_server_itsname(server)); }
...
void serversay(const char *from, const char *format, ...) {
  ...
  if (window_display && format) {
    ...
    out = m_sprintf("%s %s",
                    convert_output_format(get_string_var(SERVER_PROMPT_VAR),
                                          "%s", ov_format(from)),
                    putbuf);
    put_echo(out);
