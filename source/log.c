...
void do_log(int flag, char *logfile, FILE **fp) {
  ...
  if (flag) {
    if (*fp)
      ...
      if ((*fp = fopen(logfile, get_int_var(APPEND_LOG_VAR) ? "a" : "w")) !=
          NULL) {
...
void logger(Window *win __attribute__((unused)), char *unused __attribute__((unused)), int flag) {
  char *logfile;
 ...
 */
void set_log_file(Window *win __attribute__((unused)), char *filename, int unused __attribute__((unused))) {
  char *expanded;
 ...
 */
void BX_add_to_log(FILE *fp, time_t t __attribute__((unused)), const char *line, int mangler) {
