...
char *check_channel_type(char *channel) {
  ...
  snprintf(new_channel_format, sizeof new_channel_format, "[%.6s] %s", channel,
           channel + 6);
...
void add_userhost_to_channel(char *channel, char *nick, int server,
                             char *userhost) {
  NickList *new;
  ChannelList *chan;
...
ChannelList *BX_add_channel(char *channel, int server, int refnum) {
  ...
  if ((new = lookup_channel(channel, server, CHAN_NOUNLINK)) != NULL) {
    ...
    if (!(whowaschan = check_whowas_chan_buffer(channel, refnum, 1))) {
      ...
      malloc_strcpy(&(new->channel), channel);
      add_to_list((List **)&new->csets,
                  (List *)create_csets_for_channel(channel));
    } else {
    ...
    if ((!new->window || !(tmp = get_window_by_refnum(new->window->refnum))) &&
        current_window) {
  ...
  if (!is_current_channel(channel, server, 0)) {
    ...
    while ((traverse_all_windows(&tmp))) {
      if (tmp->server != from_server)
        continue;
      if (tmp->name &&
          (!strcmp(tmp->name, "oper_view") || !strcmp(tmp->name, "debug")))
        continue;
    ...
    set_current_channel_by_refnum(new->window ? new->window->refnum : 0,
                                  channel);
  }
  update_all_windows();
  return new;
 ...
 */
ChannelList *BX_add_to_channel(char *channel, char *nick, int server, int oper,
                               int voice, char *userhost, char *server1,
                               char *away, int serv_split, int server_hops) {
  NickList *new = NULL;
    ...
    if ((chan = lookup_channel(channel, server, CHAN_NOUNLINK))) {
      ...
      if (!(new = find_nicklist_in_channellist(nick, chan, 0))) {
        ...
        if (!(whowas = check_whowas_buffer(nick, userhost, channel))) {
          ...
          new->idle_time = new->kicktime = new->doptime = new->nicktime =
          ...
          new->sent_reop = new->sent_deop = new->bancount = new->nickcount =
              new->dopcount = new->kickcount = new->floodcount = new->ip_count =
 ...
 */
char *BX_do_compress_modes(ChannelList *chan, int server __attribute__((unused)), char *channel __attribute__((unused)),
                           char *modes) {
  int add = 0, isbanned = 0, isopped = 0, isvoiced = 0, mod = -1;
  char *tmp = NULL, *rest = NULL, nmodes[16], nargs[100];
...
#ifdef WANT_USERLIST
    for (tmp = next_nicklist(channel, NULL); tmp;
         tmp = next_nicklist(channel, tmp))
...
static void apply_channel_modes(char *from, char *mode_str,
  ...
  for (; *mode_str; mode_str++) {
    ...
    case 'h':
      ...
      update_stats(add ? MODEHOPLIST : MODEDEHOPLIST, ThisNick, channel,
                   splitter);
      break;
    case 'o': {
      ...
      ThisNick = find_nicklist_in_channellist(from, channel, 0);
      update_stats(add ? MODEOPLIST : MODEDEOPLIST, ThisNick, channel,
                   splitter);
      if ((ThisNick = find_nicklist_in_channellist(person, channel, 0))) {
        if (add)
    ...
    case 'b':
      ...
      ThisNick = find_nicklist_in_channellist(from, channel, 0);
      update_stats(add ? MODEBANLIST : MODEUNBANLIST, ThisNick, channel,
                   splitter);
      if (add) {
        ThisNick = find_nicklist_in_channellist(person, channel, 0);
        if (!(new = (BanList *)find_in_list((List **)&channel->bans, person,
                                            0)) ||
            my_stricmp(person, new->ban)) {
        ...
        new->sent_unban = 0;
        if (!new->setby)
          malloc_strcpy(&new->setby,
                        from ? from : get_server_name(from_server));
        new->time = now;
#ifdef WANT_USERLIST
        if (!its_me && channel->have_op)
        ...
        if ((new = (BanList *)remove_from_list((List **)&channel->bans,
                                               person))) {
          new_free(&new->setby);
          new_free(&new->ban);
          new_free((char **)&new);
    ...
    case 'e':
      ...
      ThisNick = find_nicklist_in_channellist(from, channel, 0);
      update_stats(add ? MODEEBANLIST : MODEUNEBANLIST, ThisNick, channel,
                   splitter);
      if (add) {
        ThisNick = find_nicklist_in_channellist(person, channel, 0);
        if (!(new = (BanList *)find_in_list((List **)&channel->exemptbans,
                                            person, 0)) ||
            my_stricmp(person, new->ban)) {
          new = (BanList *)new_malloc(sizeof(BanList));
          malloc_strcpy(&new->ban, person);
        ...
        if ((new = (BanList *)remove_from_list((List **)&channel->exemptbans,
                                               person))) {
          new_free(&new->setby);
          new_free(&new->ban);
          new_free((char **)&new);
 ...
 */
void update_channel_mode(char *from, char *channel, int server, char *mode,
                         ChannelList *tmp) {
  if (!tmp && channel) {
 ...
 * happens.
 */
void BX_remove_from_channel(char *channel, char *nick, int server, int netsplit,
                            char *reason) {
  ChannelList *chan;
  NickList *tmp = NULL;
    ...
    if (server1 && !check_split_server(server1)) {
      ...
      if (do_hook(LLOOK_SPLIT_LIST, "%s %s", server2, server1)) {
        ...
        put_it("%s", convert_output_format(
                         fget_string_var(FORMAT_NETSPLIT_FSET), "%s %s %s",
                         update_clock(GET_TIME), server1, server2));
        bitchsay("%s \002%s\002 to see who left \002%s\002 to change to [%s]",
                 t ? "Press" : "", t ? t : "/wholeft", s ? s : "/server",
                 server1);
      }
...
static void handle_nickflood(char *old_nick __attribute__((unused)), char *new_nick,
                             register NickList *nick,
                             register ChannelList *chan, int flood_time) {
  time_t floodtime = now - nick->bantime;

  if ((!nick->bancount) || (nick->bancount && (floodtime > 3))) {
    if (!nick->sent_kick++)
      send_to_server(
          "KICK %s %s :\002Niq flood (%d nicks in %dsecs of %dsecs)\002",
          chan->channel, new_nick,
          get_cset_int_var(chan->csets, KICK_ON_NICKFLOOD_CSET), flood_time,
          get_cset_int_var(chan->csets, NICKFLOOD_TIME_CSET));
  }
}
...
void BX_rename_nick(char *old_nick, char *new_nick, int server) {
  ...
  for (chan = get_server_channels(server); chan; chan = chan->next) {
    if (chan->server == server) {
      if ((tmp = find_nicklist_in_channellist(old_nick, chan,
                                              REMOVE_FROM_LIST))) {
        tmp->stat_nicks++;
        if (chan->have_op && !isme(new_nick)) {
          if (is_other_flood(chan, tmp, NICK_FLOOD, &t))
            ...
            if (find_in_list((List **)&lame_list, new_nick, 0)) {
              send_to_server("MODE %s -o+b %s %s!*@*", chan->channel, new_nick,
                             new_nick);
...
int BX_is_on_channel(char *channel, int server, char *nick) {
  ...
  if (nick && (chan = lookup_channel(channel, server, CHAN_NOUNLINK)) &&
      chan->connected)
...
static void show_channel(ChannelList *chan) {
  ...
  *buffer = 0;
  ...
  say("\t%s %c%s (%s) (Win: %d): %s", chan->channel, s ? '+' : ' ',
      s ? s : "<none>", get_server_name(chan->server),
      chan->window ? (int)chan->window->refnum : -1, buffer);
}
...
void switch_channels(char key __attribute__((unused)), char *ptr __attribute__((unused))) {
...
void reconnect_all_channels(int server) {
  ...
  if (channel)
    my_send_to_server(server, "JOIN %s %s", channel,
...
char *what_channel(char *nick, int server __attribute__((unused))) {
  ChannelList *tmp;

  if (current_window->current_channel &&
      is_on_channel(current_window->current_channel, current_window->server,
                    nick))
...
extern void clear_mode_list(int server __attribute__((unused))) {
  struct modelist *curr, *next, *prev = NULL;
...
void unset_window_current_channel(Window *window) {
  ...
  for (tmp = get_server_channels(window->server); tmp; tmp = tmp->next) {
    if (tmp->window == window && window->current_channel &&
        !my_stricmp(tmp->channel, window->current_channel)) {
  ...
  for (i = 0; i < server_list_size(); i++) {
    for (chan = get_server_channels(i); chan; chan = chan->next) {
      ...
      if (chan->totalnicks + chan->csets->set_auto_limit != chan->limit)
        my_send_to_server(i, "MODE %s +l %d", chan->channel,
                          chan->totalnicks + chan->csets->set_auto_limit);
    }
  }
}
