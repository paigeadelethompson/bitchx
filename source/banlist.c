...
void add_mode(ChannelList *chan, char *mode, int plus, char *nick, char *reason,
              int max_modes) {
  char buffer[BIG_BUFFER_SIZE + 1];
...
BUILT_IN_COMMAND(LameKick) {
  ...
  if ((chan = prepare_command(&from_server, channel, NEED_OP))) {
    ...
    const size_t cmd_len = strlen(chan->channel) +
                           strlen("KICK   :<\002BX\002-LK> ") + strlen(reason);

    for (tmp = next_nicklist(chan, NULL); tmp; tmp = next_nicklist(chan, tmp)) {
      int level = 0;
      if (tmp->userlist)
        level = ((tmp->userlist->flags | 0xff) & PROT_ALL);
      if (!nick_isop(tmp) && !nick_isvoice(tmp) &&
          ((tmp->userlist && !level) || !tmp->userlist)) {
        m_s3cat(&targets, ",", tmp->nick);
        count++;
        total++;
        if (((get_int_var(NUM_KICKS_VAR)) &&
             (count >= get_int_var(NUM_KICKS_VAR))) ||
            (strlen(targets) + cmd_len) >=
                (MAX_PROTOCOL_SIZE - (NICKNAME_LEN + 5))) {
          my_send_to_server(from_server, "KICK %s %s :<\002BX\002-LK> %s",
                            chan->channel, targets, reason);
          new_free(&targets);
          count = 0;
        }
      }
    ...
    if (targets)
      my_send_to_server(from_server, "KICK %s %s :<\002BX\002-LK> %s",
                        chan->channel, targets, reason);
    new_free(&targets);
    say("Sent the Server all the Lamer Kicks, Sit back and Watch %d kicks!",
        total);
  }
...
char *ban_it(char *nick, char *user, char *host, char *ip) {
  ...
  case 4: /* Domain	*/
    tmp = strrchr(host, '.');
    if (tmp) {
      snprintf(banstr, sizeof banstr, "*!*@*%s", tmp);
    } else {
      snprintf(banstr, sizeof banstr, "*!*@%s", host);
    }
    break;
...
void userhost_unban(UserhostItem *uhi, char *nick, char *args) {
  ...
  for (bans = chan->bans; bans; bans = bans->next) {
    if (!bans->sent_unban && (wild_match(bans->ban, host) ||
                              (ip_str && wild_match(bans->ban, ip_str)))) {
      add_mode(chan, "b", 0, bans->ban, NULL, get_int_var(NUM_BANMODES_VAR));
      bans->sent_unban++;
      count++;
    }
...
void userhost_ban(UserhostItem *stuff, char *nick1, char *args) {
  ...
  if (!stuff || !stuff->nick || !strcmp(stuff->user, "<UNKNOWN>") ||
    ...
    if (channel && (whowas = check_whowas_nick_buffer(nick1, channel))) {
      ...
      *host++ = 0;
      ...
      bitchsay("No match for the %s of %s on %s", fuck ? "Fuck" : "Ban", nick1,
               channel);
      return;
    }
  } else {
  ...
  send_to_server("MODE %s %s %s %s", channel, on_chan ? ob : b,
                 on_chan ? nick : empty_string,
                 ban_it(nick, user, host, n ? n->ip : NULL));
  if (fuck) {
...
BUILT_IN_COMMAND(multkick) {
  ...
  while (temp && *temp) {
    ...
    my_send_to_server(server, "KICK %s %s :\002%s\002", chan->channel, nick,
                      this_reason);
  }
}
...
BUILT_IN_COMMAND(massdeop) {
  ...
  for (nicks = next_nicklist(chan, NULL); nicks;
       nicks = next_nicklist(chan, nicks)) {
    snprintf(buffer, sizeof(buffer), "%s!%s", nicks->nick, nicks->host);
    if ((all || (!isvoice && nick_isop(nicks)) ||
         (isvoice && nick_isvoice(nicks))) &&
  ...
  from_server = old_server;
  if (!count)
    say("No matches for %s of %s on %s", command ? command : "massdeop", spec,
        chan->channel);
}
...
BUILT_IN_COMMAND(doop) {
  ChannelList *chan;
  char *to, *temp = NULL;
  int maxmodes = get_int_var(NUM_OPMODES_VAR), old_server = from_server;
...
BUILT_IN_COMMAND(dodeop) {
  ChannelList *chan;
  char *to, *temp = NULL;
  int maxmodes = get_int_var(NUM_OPMODES_VAR), server;
...
BUILT_IN_COMMAND(massop) {
  ...
  flush_mode_all(chan);
  if (!count)
    say("No matches for %s of %s on %s", command ? command : "massop", spec,
        chan->channel);
}
...
BUILT_IN_COMMAND(masskick) {
  ...
  if (masskick_list) {
    ...
    const size_t cmd_len =
        strlen(chan->channel) + strlen("KICK   :\002\002") + strlen(reason);

    bitchsay("Performing (%s) Mass Kick on %s",
             all   ? "opz/non-opz"
             : ops ? "ops"
                   : "non-opz",
             chan->channel);
...
BUILT_IN_COMMAND(mknu) {
  ...
  for (nicks = next_nicklist(chan, NULL); nicks;
    ...
    count++;
    my_send_to_server(
        server, "KICK %s %s :(non-users) \002%s\002", chan->channel,
        nicks->nick,
        rest ? rest
...
BUILT_IN_COMMAND(masskickban) {
  ...
  for (nicks = next_nicklist(chan, NULL); nicks;
    ...
    *buffer = '\0';
    ...
    if ((all || !nick_isop(nicks) ||
         get_cset_int_var(chan->csets, KICK_OPS_CSET)) &&
        !isme(nicks->nick) && wild_match(tempbuf, buffer)) {
      count++;
      my_send_to_server(
          server, "KICK %s %s :(%s) \002%s\002", chan->channel, nicks->nick,
          spec,
          rest ? rest
               : get_kick_reason(nicks->nick, get_server_nickname(server)));
    }
...
BUILT_IN_COMMAND(massban) {
  ...
  for (nicks = next_nicklist(chan, NULL); nicks;
    ...
    if ((all || !nick_isop(nicks) ||
      ...
      char *temp = LOCAL_COPY(nicks->host), *q = clear_server_flags(temp),
           *p = strchr(temp, '@');
      ShitList *new;
  ...
  if (massban_list) {
    ...
    for (tmp = massban_list; tmp; tmp = tmp->next) {
      ...
      if (i > maxmodes) {
        modestr[i] = '\0';
        my_send_to_server(server, "MODE %s +%s %s", chan->channel, modestr,
                          buffer);
        i = 0;
        new_free(&buffer);
      }
  ...
  if (count) {
    ...
    for (bans = chan->bans; bans; bans = bans->next) {
      ...
      if (count && (count % get_int_var(NUM_BANMODES_VAR) == 0)) {
        my_send_to_server(server, "MODE %s -%s %s", chan->channel,
                          strfill('b', num), banstring);
        new_free(&banstring);
        num = 0;
      }
    }
...
BUILT_IN_COMMAND(dokick) {
  ...
  if (!(chan = prepare_command(&server, (to && !strcmp(to, "*")) ? NULL : to,
                               NEED_OP)))
    return;
...
BUILT_IN_COMMAND(kickban) {
    ...
    for (nicks = next_nicklist(chan, NULL); nicks;
      ...
      *host++ = 0;
      if (kick_first)
        ...
        my_send_to_server(
            server, "MODE %s -o+b %s %s\r\nKICK %s %s :%s", chan->channel,
            nicks->nick, ban_it(nicks->nick, user, host, nicks->ip),
      ...
      if (time >= 0)
        add_timer(0, empty_string, time * 1000.0, 1, timer_unban,
                  m_sprintf("%d %s %s", from_server, chan->channel,
                            ban_it(nicks->nick, user, host, nicks->ip)),
                  NULL, -1, "timed-unban");
      else if (command && !my_stricmp(command, "FUCK")) {
        char *temp = NULL;
        malloc_sprintf(&temp, "%s!*%s@%s %s 3 Auto-Shit", nicks->nick, user,
                       host, chan->channel);
#ifdef WANT_USERLIST
  ...
  if (!count && !isme(spec))
    userhostbase(spec, userhost_ban, 1, "%s %s", chan->channel,
                 command ? (!strcmp(command, "FUCK") ? "FUCK"
                            : set_ignore             ? "BKI"
                                                     : empty_string)
                         : empty_string);
  reset_display_target();
...
BUILT_IN_COMMAND(ban) {
  ...
  for (nicks = next_nicklist(chan, NULL); nicks;
    ...
    if (!my_stricmp(spec, nicks->nick)) {
      char *t = LOCAL_COPY(nicks->host), *user = clear_server_flags(t),
           *host = strchr(user, '@');

      *host++ = 0;
      my_send_to_server(server, "MODE %s -o+b %s %s", chan->channel,
                        nicks->nick,
                        ban_it(nicks->nick, user, host, nicks->ip));
      found++;
    }
...
BUILT_IN_COMMAND(banstat) {
  ...
  if ((chan = prepare_command(&server, channel, NO_OP))) {
    ...
    if (chan->bans) {
      if ((do_hook(BANS_HEADER_LIST, "%s %s %s %s %s", "#", "Channel", "Ban",
                   "SetBy", "Seconds")))
        put_it("%s", convert_output_format(
                         fget_string_var(FORMAT_BANS_HEADER_FSET), NULL));
      for (tmpc = chan->bans; tmpc; tmpc = tmpc->next, count++) {
        if (check &&
            (!wild_match(check, tmpc->ban) || !wild_match(tmpc->ban, check)))
          continue;
        if (do_hook(BANS_LIST, "%d %s %s %s %lu", count, chan->channel,
                    tmpc->ban,
                    tmpc->setby ? tmpc->setby : get_server_name(from_server),
                    (unsigned long)tmpc->time))
          put_it("%s",
                 convert_output_format(
                     fget_string_var(FORMAT_BANS_FSET), "%d %s %s %s %l", count,
                     chan->channel, tmpc->ban,
                     tmpc->setby ? tmpc->setby : get_server_name(from_server),
                     (unsigned long)tmpc->time));
      }
      if (do_hook(BANS_FOOTER_LIST, "%d %s", count, chan->channel))
        put_it("%s",
               convert_output_format(fget_string_var(FORMAT_BANS_FOOTER_FSET),
                                     "%d %s", count, chan->channel));
    }
    if (chan->exemptbans) {
      count = 1;
      if ((do_hook(EBANS_HEADER_LIST, "%s %s %s %s %s", "#", "Channel", "ExBan",
                   "SetBy", "Seconds")))
        put_it("%s", convert_output_format(
                         fget_string_var(FORMAT_EBANS_HEADER_FSET), NULL));
      for (tmpc = chan->exemptbans; tmpc; tmpc = tmpc->next, count++) {
        if (check &&
            (!wild_match(check, tmpc->ban) || !wild_match(tmpc->ban, check)))
          continue;
        if (do_hook(EBANS_LIST, "%d %s %s %s %lu", count, chan->channel,
                    tmpc->ban,
                    tmpc->setby ? tmpc->setby : get_server_name(from_server),
                    (unsigned long)tmpc->time))
          put_it("%s",
                 convert_output_format(
                     fget_string_var(FORMAT_EBANS_FSET), "%d %s %s %s %l",
                     count, chan->channel, tmpc->ban,
                     tmpc->setby ? tmpc->setby : get_server_name(from_server),
                     (unsigned long)tmpc->time));
      }
      if (do_hook(EBANS_FOOTER_LIST, "%d %s", count, chan->channel))
        put_it("%s",
               convert_output_format(fget_string_var(FORMAT_EBANS_FOOTER_FSET),
                                     "%d %s", count, chan->channel));
    }
    new_free(&check);
    new_free(&channel);
...
void remove_bans(char *stuff, char *line) {
  ...
  if (stuff && (chan = prepare_command(&server, stuff, NEED_OP))) {
    ...
    for (tmpc = chan->bans; tmpc; tmpc = tmpc->next, count++)
      if (!tmpc->sent_unban && (matchmcommand(line, count))) {
        ...
        if (num % get_int_var(NUM_BANMODES_VAR) == 0) {
          my_send_to_server(server, "MODE %s -%s %s", stuff, strfill('b', num),
                            banstring);
          new_free(&banstring);
          num = 0;
        }
    ...
    if (!done) {
      for (tmpc = chan->exemptbans; tmpc; tmpc = tmpc->next, count++)
        if (!tmpc->sent_unban && (matchmcommand(line, count))) {
          ...
          tmpc->sent_unban++;
          if (num % get_int_var(NUM_BANMODES_VAR) == 0) {
            my_send_to_server(server, "MODE %s -%s %s", stuff,
                              strfill('e', num), banstring);
            new_free(&banstring);
            num = 0;
          }
          done++;
    ...
    for (tmpc = chan->bans; tmpc; tmpc = next) {
      next = tmpc->next;
      if (tmpc->sent_unban) {
        if ((tmpc = (BanList *)remove_from_list((List **)&chan->bans,
                                                tmpc->ban))) {
          new_free(&tmpc->ban);
          new_free(&tmpc->setby);
          new_free((char **)&tmpc);
    ...
    for (tmpc = chan->exemptbans; tmpc; tmpc = next) {
      next = tmpc->next;
      if (tmpc->sent_unban) {
        if ((tmpc = (BanList *)remove_from_list((List **)&chan->exemptbans,
                                                tmpc->ban))) {
          new_free(&tmpc->ban);
          new_free(&tmpc->setby);
          new_free((char **)&tmpc);
  ...
  if ((chan = prepare_command(&server, NULL, NEED_OP))) {
    ...
    if ((do_hook(BANS_HEADER_LIST, "%s %s %s %s %s", "#", "Channel", "Ban",
      ...
      put_it("%s", convert_output_format(
                       fget_string_var(FORMAT_BANS_HEADER_FSET), NULL));
    for (tmpc = chan->bans; tmpc; tmpc = tmpc->next, count++)
      if (do_hook(BANS_LIST, "%d %s %s %s %lu", count, chan->channel, tmpc->ban,
                  tmpc->setby ? tmpc->setby : get_server_name(from_server),
                  (unsigned long)tmpc->time))
        put_it("%s",
               convert_output_format(
      ...
      if (do_hook(BANS_LIST, "%d %s %s %s %lu", count, chan->channel, tmpc->ban,
        ...
        put_it("%s",
               convert_output_format(
                   ...
                   tmpc->setby ? tmpc->setby : get_server_name(from_server),
                   (unsigned long)tmpc->time));
    add_wait_prompt("Which ban to delete (-2, 2-5, ...) ? ", remove_bans,
                    chan->channel, WAIT_PROMPT_LINE, 1);
  }
}

static const char *bantypes[] = {
    "*Unknown*",      "\002N\002ormal", "\002B\002etter", "\002H\002ost",
    "\002D\002omain", "\002U\002ser",   "\002S\002crew",  "\002I\002P"};
