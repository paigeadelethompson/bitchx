...
HookFunc hook_functions[] = {{"ACTION", NULL, 3, 0, 0},
                             {"BANS", NULL, 4, 0, 0},
 ...
 * by "nick".
 */
static void add_numeric_hook(int numeric, char *nick, char *stuff, Noise noisy,
                             int not, int sernum, int flexible) {
  NumericList *entry;
  Hook *new;
...
#ifdef WANT_DLL
static void add_numeric_dll_hook(int numeric, Noise noise, int serial,
                                 char *nick, char *package,
...
void add_dll_hook(int which, Noise noise, char *nick, char *package,
 ...
 */
static void add_hook(int which, char *nick, char *stuff, Noise noisy, int not,
                     int sernum, int flexible) {
  Hook *new;

  if (which < 0) {
    add_numeric_hook(-which, nick, stuff, noisy, not, sernum, flexible);
    return;
  }

  if (!(new =
            hook_remove_from_list(&hook_functions[which].list, nick, sernum))) {
    new = (Hook *)new_malloc(sizeof(Hook));
    new->nick = NULL;
    new->stuff = NULL;
  }
...
/* * * * * * REMOVING A HOOK * * * * * * * */
static void remove_numeric_hook(int numeric, char *nick, int sernum,
                                int quiet) {
  NumericList *hook;
  Hook *tmp, *next;
  ...
  if ((hook = find_numeric_list(numeric))) {
    if (nick) {
      if ((tmp = hook_remove_from_list(&hook->list, nick, sernum))) {
        if (!quiet) {
          say("%c%s%c removed from %d list", (tmp->flexible ? '\'' : '"'), nick,
...
static void remove_hook(int which, char *nick, int sernum, int quiet) {
  ...
  if (nick) {
    if ((tmp = hook_remove_from_list(&hook_functions[which].list, nick,
                                     sernum))) {
      if (!quiet)
        say("%c%s%c removed from %s list", (tmp->flexible ? '\'' : '"'), nick,
            (tmp->flexible ? '\'' : '"'), hook_functions[which].name);
...
static void show_hook(Hook *list, char *name) {
  ...
  if (hooks)
    put_it("%s",
           convert_output_format(hooks, "%s %s %c %s %c %s %s %d",
                                 list->filename[0] ? list->filename : "*", name,
...
int BX_do_hook(int which, char *format, ...) {
  ...
  /* Named list */
  else if ((unsigned)which < HOOK_COUNT) {
    /*
  ...
  {
    int currser = 0, oldser = INT_MIN, currmatch = 0, oldmatch = 0;
    Hook *bestmatch = NULL;
  ...
  hook_next:
     ...
     */
    if (tmp->noisy > QUIET)
      say("%s activated by %c%s%c", name, tmp->flexible ? '\'' : '"', buffer,
          tmp->flexible ? '\'' : '"');

    if ((tmp->debug & HF_DEBUG) && (internal_debug & DEBUG_HOOK) &&
        !in_debug_yell) {
...
BUILT_IN_COMMAND(oncmd) {
  ...
  Noise noisy = NORMAL;
  int not = 0, sernum = 0, remove = 0, which = INVALID_HOOKNUM;
  int flex = 0;
  char type;
  int first;
  ...
  if ((func = next_arg(args, &args)) != NULL) {
    ...
    if ((nick = new_new_next_arg(args, &args, &type))) {
       ...
       */
      if (which < 0)
        say("On %3.3u from %c%s%c do %s [%s] <%d>", -which, type, nick, type,
            (not? "nothing" : exp), noise_level[noisy], sernum);
      else
        say("On %s from %c%s%c do %s [%s] <%d>", hook_functions[which].name,
            type, nick, type, (not? "nothing" : exp), noise_level[noisy],
            sernum);
...
void do_stack_on(int type, char *args) {
  ...
  if (type == STACK_PUSH) {
    ...
    if ((which < 0 && nhook) || (which >= 0 && hook_functions[which].list))
...
void hook_add_to_list(Hook **list, Hook *item) {
  ...
  for (tmp = *list; tmp; last = tmp, tmp = tmp->next) {
    if (tmp->sernum < item->sernum)
      continue;
    else if ((tmp->sernum == item->sernum) &&
             my_stricmp(tmp->nick, item->nick) < 0)
      continue;
    else
      break;
  }
