...
char *strflags[] = {"VOICE", "OPS",   "BAN",   "UNBAN", "INVITE", "DCC",
                    "TCL",   "I_OPS", "FLOOD", "BOT",   NULL};
#define NUMBER_OF_FLAGS (sizeof(strflags) / sizeof(char *))

char *protflags[] = {"REOP", "DEOP",  "KICK", "PBAN", "PINVITE",
                     "UOPS", "UPROT", "CTCP", NULL};
...
void add_userhost_to_userlist(char *nick, char *uhost, char *channels,
                              char *passwd, unsigned int flags) {
  UserList *uptr;
...
void prepare_adduser(UserhostItem *stuff, char *nick, char *args) {
  ...
  char *channels = NULL, *passwd = NULL, *p = NULL, *uh, *e_host, *host;
  ...
  if (thetype == USERLIST_ADD) {
    ...
    if (!(uptr = lookup_userlevelc(stuff->nick, e_host, channels, NULL))) {
      ...
      send_to_server(
          "NOTICE %s :You have been added to my Userlist as *!%s with [%s]",
          nick, host, convert_flags_to_str(flags));
      send_to_server("NOTICE %s :You are allowed on channels [%s]", nick,
                     channels);
      if (passwd)
        send_to_server("NOTICE %s :Your password is [%s]", nick, passwd);
      put_it("%s",
             convert_output_format(
                 "$G Added [*!$0] on $1. Password is [$2] and flags are [$3-]",
                 "%s %s %s %s", host, channels, passwd ? passwd : "None",
                 convert_flags_to_str(flags)));
    } else if (passwd) {
      send_to_server("NOTICE %s :Your password is %s[%s]", nick,
                     uptr->password ? "changed to " : "", passwd);
    ...
    if (!(remove_userhost_from_userlist(host, channels)))
      ...
      put_it("%s", convert_output_format("$G Removed [$0] on $1 from userlist",
                                         "%s %s", host, channels));
  }
  new_free(&e_host);
  new_free(&host);
    ...
    if ((bang = strchr(nick, '!')) && (at = strchr(nick, '@'))) {
      ...
      *bang++ = 0;
      ...
      if (type == USERLIST_ADD)
        add_userhost_to_userlist(p, newuh ? newuh : bang, channels, e_pass,
                                 convert_str_to_flags(capabilities));
      else
        remove_userhost_from_userlist(newuh ? newuh : bang, channels);
      bitchsay("%s %s!%s %s Userlist",
               type == USERLIST_ADD ? "Adding" : "Deleting", p,
               newuh ? newuh : bang, type == USERLIST_ADD ? "to" : "from");
      new_free(&newuh);
    } else {
      char format[80];
...
BUILT_IN_COMMAND(add_shit) {
  ...
  if (args && *args) {
    ...
    if (!bang && !atsign && (cn = check_nickname(m_strdup(nick)))) {
      userhostbase(cn, prepare_addshit, 1, "%d %d %s %s", type, level, channels,
                   reason);
      new_free(&cn);
    } else {
      /*
      ...
      for (nick = next_nicklist(chan, NULL); nick;
        ...
        if (wild_match(added->filter, tmp)) {
          if (type) {
            ...
            BanList *b = NULL;
            nick->shitlist = NULL;
            if ((b = ban_is_on_channel(tmp, chan)) &&
                !eban_is_on_channel(tmp, chan))
              add_mode(chan, "b", 0, b->ban, NULL,
                       get_int_var(NUM_BANMODES_VAR));
          }
        }
      }
      flush_mode_all(chan);
...
void add_to_a_list(char *thestring, int thetype, char *nick, char *channels,
                   char *reason, int shitlevel) {
  ...
  case SHITLIST_REMOVE: {
    ...
    while ((sremove = (ShitList *)removewild_from_list((List **)&shitlist_list,
                                                       s_str))) {
      shit_count--;
      scount++;
...
BUILT_IN_COMMAND(showuserlist) {
  ...
  for (tmp = next_userlist(NULL, &size, &location); tmp;
    ...
    if (!first++ &&
        (hook = do_hook(USERLIST_HEADER_LIST, "%s %s %s %s %s", "Level", "Nick",
                        "Password", "Host", "Channels")))
      put_it("%s", convert_output_format(
                       fget_string_var(FORMAT_USERLIST_HEADER_FSET), NULL));
    if (do_hook(USERLIST_LIST, "%lu %s %s %s %s", tmp->flags, tmp->nick,
                tmp->host, tmp->channels,
                tmp->password ? tmp->password : empty_string))
      put_it("%s", convert_output_format(
                       fget_string_var(FORMAT_USERLIST_FSET), "%s %s %s %s %s",
                       convert_flags(tmp->flags), tmp->nick,
                       tmp->password ? tmp->password : "<none>", tmp->host,
                       tmp->channels));
  }
  if (first && hook && do_hook(USERLIST_FOOTER_LIST, "%s", "End of userlist"))
    put_it("%s",
           convert_output_format(fget_string_var(FORMAT_USERLIST_FOOTER_FSET),
                                 "%s %d", update_clock(GET_TIME), first));
}
...
BUILT_IN_COMMAND(showshitlist) {
  ...
  while (tmp) {
    if (!first++ && (hook = do_hook(SHITLIST_HEADER_LIST, "%s %s %s %s %s %s",
                                    "Lvl", "Nick", "Channels", "Reason",
                                    empty_string, empty_string)))
      put_it("%s", convert_output_format(
                       fget_string_var(FORMAT_SHITLIST_HEADER_FSET), NULL));
    if (do_hook(SHITLIST_LIST, "%d %s %s %s %s %s", tmp->level, tmp->filter,
                tmp->channels, tmp->reason ? tmp->reason : "<none>",
                empty_string, empty_string)) {
      put_it("%s", convert_output_format(
                       fget_string_var(FORMAT_SHITLIST_FSET), "%d %s %s %s",
                       tmp->level, tmp->filter, tmp->channels,
                       tmp->reason ? tmp->reason : "<No Reason"));
    }
    tmp = tmp->next;
  }
  if (first && hook && do_hook(SHITLIST_FOOTER_LIST, "%s", "End of shitlist"))
    put_it("%s",
           convert_output_format(fget_string_var(FORMAT_SHITLIST_FOOTER_FSET),
                                 "%s %d", update_clock(GET_TIME), shit_count));
}
...
BUILT_IN_COMMAND(set_user_info) {
  ...
  if (command) {
    ...
    for (tmp = next_userlist(NULL, &size, &location); tmp;
      ...
      if (tmp->comment) {
        put_it("%s",
               convert_output_format("$[10]0 $[25]1 %K[%C$2-%K]", "%s %s %s",
                                     tmp->nick, tmp->host, tmp->comment));
        count++;
      }
    }
    if (count)
 ...
 */
UserList *lookup_userlevelc(const char *nick, const char *userhost,
                            const char *channel, const char *passwd) {
...
ShitList *nickinshit(char *niq, char *uh) {
  ...
  while (thisptr) {
    if (!strcmp(thisptr->filter, theuh) ||
        (/*wild_match(theuh, thisptr->filter) || */ wild_match(thisptr->filter,
                                                               theuh)))
      return (thisptr);
...
int real_check_auto(void *arg, char *sub __attribute__((unused))) {
...
int delay_check_auto(char *channel) {
  ...
  for (possible = next_nicklist(chan, NULL); possible;
       possible = next_nicklist(chan, possible)) {
    if ((possible->shitlist || possible->userlist) &&
        (!(possible->sent_reop < 4) || !(possible->sent_deop < 4))) {
      malloc_sprintf(&buffer, "%s %s %s %d", channel, possible->nick,
                     possible->host, from_server);
      add_timer(0, empty_string, 3 * 1000, 1, real_check_auto, buffer, NULL, -1,
                "check-auto");
    }
  }
  add_timer(0, empty_string, 5 * 1000, 1, delay_flush_all,
            m_sprintf("%s %d", channel, from_server), NULL, -1, "check-auto");
  return 0;
}
...
int delay_opz(void *arg, char *sub) {
  ...
  if (chan && is_on_channel(channel, this_server, from) && chan->have_op) {
    ...
    if (nick && ((!nick_isop(nick) && *mode == 'o') ||
                 (!nick_isvoice(nick) && *mode == 'v'))) {
      my_send_to_server(this_server, "MODE %s +%s %s", channel, mode, from);
      if (get_int_var(SEND_OP_MSG_VAR))
        my_send_to_server(this_server,
                          "NOTICE %s :You have been delay Auto-%s'd", from,
                          *mode == 'o' ? "op" : "voice");
    }
  }
  new_free(&p);
...
int delay_kick(void *arg, char *sub) {
  ...
  if (protected) {
    ...
    if ((chan = prepare_command(&server, channel, PC_SILENT)))
      my_send_to_server(this_server,
                        "KICK %s %s :\002%s\002 Kick/ban me will ya", channel,
                        from, _VERSION_);
    new_free(&protected);
  }
  new_free(&p);
...
void run_user_flag(char *name, char *what, NickList *user, NickList *kicker) {
  ...
  *buffer = 0;
  strmopencat(buffer, BIG_BUFFER_SIZE * 5, user->nick, space, user->host, space,
              user->ip, space, user->userlist ? one : zero, space, NULL);
  if (user->userlist)
    strmopencat(buffer, BIG_BUFFER_SIZE * 5, user->userlist->nick, space,
                user->userlist->host, space, user->userlist->channels, space,
                ltoa(user->userlist->flags), space, NULL);
  if (kicker) {
    strmopencat(buffer, BIG_BUFFER_SIZE * 5, kicker->nick, space, kicker->host,
                space, kicker->ip, space, kicker->userlist ? one : zero, NULL);
    if (kicker->userlist)
      strmopencat(buffer, BIG_BUFFER_SIZE * 5, space, kicker->userlist->nick,
                  space, kicker->userlist->host, space,
                  kicker->userlist->channels, space,
                  ltoa(kicker->userlist->flags), NULL);
  }
...
NickList *check_auto(char *channel, NickList *nicklist, ChannelList *chan) {
  ...
  if (get_cset_int_var(chan_ptr->csets, SHITLIST_CSET) && shitptr != NULL &&
      userptr == NULL) {
    char *reason =
        shitptr->reason ? shitptr->reason : get_string_var(SHITLIST_REASON_VAR);
    ...
    case 1: /* never give opz */
      if (nicklist->sent_deop < 4 && nick_isop(nicklist)) {
        add_mode(chan_ptr, "o", 0, nicklist->nick, NULL,
                 get_int_var(NUM_OPMODES_VAR));
        nicklist->sent_deop++;
    ...
    case 4: /* perm ban on offender */
      if (nicklist->sent_deop < 4) {
        send_to_server("MODE %s -o+b %s %s", channel, nicklist->nick,
                       shitptr->filter);
        nicklist->sent_deop++;
        nicklist->sent_deop_time = now;
        if (get_int_var(AUTO_UNBAN_VAR) && shitptr->level != 4)
          add_timer(
              ...
              timer_unban,
              m_sprintf("%d %s %s", from_server, channel, shitptr->filter),
              NULL, -1, "auto-unban");
      }
      add_mode(chan_ptr, NULL, 0, nicklist->nick, reason, 0);
    default:
  ...
  if (userptr && get_cset_int_var(chan_ptr->csets, USERLIST_CSET)) {
    ...
    if (get_cset_int_var(chan_ptr->csets, AOP_CSET)) {
      if ((userptr->flags & ADD_OPS)) {
        ...
        if (!userptr->password && !nicklist->sent_reop &&
            !nick_isop(nicklist)) {
          nicklist->sent_reop++;
          nicklist->sent_reop_time = current;
          if (!(userptr->flags & ADD_IOPS)) {
            malloc_sprintf(&buffer, "%s %s %s %s %d", channel, nicklist->nick,
                           nicklist->host, "o", from_server);
            add_timer(0, empty_string, 10 * 1000, 1, delay_opz, buffer, NULL,
                      -1, "delay-ops");
          } else
    ...
    if ((userptr->flags & USER_FLAG_OPS) &&
        (p = get_string_var(USER_FLAG_OPS_VAR))) {
      done++;
  ...
  if (get_cset_int_var(chan_ptr->csets, KICK_IF_BANNED_CSET) &&
      check_channel_match(get_string_var(PROTECT_CHANNELS_VAR),
                          chan_ptr->channel)) {
    char *ipban = NULL;
    char *u = NULL;
    if (!nicklist->host)
    ...
    if (!isme(nicklist->nick) &&
        (!eban_is_on_channel(ban, chan_ptr) &&
         (ban_is_on_channel(ban, chan_ptr) ||
          (ipban && ban_is_on_channel(ipban, chan_ptr))))) {
      new_free(&ban);
      new_free(&ipban);
      if (nick_isop(nicklist) &&
          !get_cset_int_var(chan_ptr->csets, KICK_OPS_CSET))
        return nicklist;
      my_send_to_server(from_server, "KICK %s %s :Banned", chan_ptr->channel,
                        nicklist->nick);
    }
    new_free(&ban);
    new_free(&ipban);
 ...
 *  4 KickBan offender
 */
int check_prot(char *from, char *person, ChannelList *chan, BanList *thisban,
               NickList *n) {
  NickList *tmp = NULL;
  NickList *kicker;
  char *tmp_ban = NULL;
  ...
  if (!n) {
    if (!(tmp = find_nicklist_in_channellist(person, chan, 0))) {
      for (tmp = next_nicklist(chan, NULL); tmp;
           tmp = next_nicklist(chan, tmp)) {
        if (wild_match(nick, tmp->nick) && wild_match(userhost, tmp->host) &&
            tmp->userlist) {
          n = tmp;
          break;
        }
  ...
  if (get_cset_int_var(chan->csets, USERLIST_CSET) && chan->have_op &&
      ((n && n->userlist) || (tmp && tmp->userlist))) {
    UserList *user = NULL;
    time_t current = now;
    if (n)
      user = n->userlist;
    ...
    if (!(kicker = find_nicklist_in_channellist(from, chan, 0)))
      return 0;
    if ((user->flags &
         (PROT_DEOP | PROT_INVITE | PROT_BAN | PROT_KICK | PROT_REOP))) {
      int do_reop = 0;
      ...
      if (user->flags & PROT_INVITE) {
        ...
        if (thisban && !thisban->sent_unban) {
          ...
          send_to_server("INVITE %s %s", n ? n->nick : tmp->nick,
                         chan->channel);
        }
      }
      if (user->flags & PROT_BAN) {
        ...
        if (kicker->userlist)
          ...
          h = strchr(u, '@');
          *h++ = 0;
          send_to_server("MODE %s -o+b %s %s", chan->channel, kicker->nick,
                         ban_it(kicker->nick, u, h, kicker->ip));
          if (get_int_var(AUTO_UNBAN_VAR))
            add_timer(0, empty_string, get_int_var(AUTO_UNBAN_VAR) * 1000, 1,
                      timer_unban,
                      m_sprintf("%d %s %s", from_server, chan->channel,
                                ban_it(kicker->nick, u, h, kicker->ip)),
                      NULL, -1, "auto-unban");
        }
      }
      ...
      if ((user->flags & PROT_REOP) || do_reop) {
        ...
        if (n && (!n->sent_reop ||
                  (n->sent_reop_time && (current - n->sent_reop_time > 60)))) {
          char *u = NULL;
          malloc_sprintf(&u, "%s %s %s %s %d", chan->channel, n->nick, n->host,
                         "o", from_server);
          add_timer(0, empty_string, 10 * 1000, 1, delay_opz, u, NULL, -1,
                    "delay-ops");
          n->sent_reop++;
    ...
    if ((user->flags & USER_FLAG_PROT) &&
        (p = get_string_var(USER_FLAG_PROT_VAR)))
...
void check_shit(ChannelList *chan) {
  ShitList *Bans;
  if (!chan || !chan->have_op || !shitlist_list || chan->server <= -1 ||
  ...
  for (Bans = shitlist_list; Bans; Bans = Bans->next) {
    ...
    if ((Bans->level == 4) && !ban_is_on_channel(Bans->filter, chan) &&
        !eban_is_on_channel(Bans->filter, chan))
...
void check_hack(char *nick, ChannelList *channel, NickList *ThisNick,
                char *from) {
#ifdef WANT_USERLIST
  UserList *userptr = NULL;
  ShitList *shitptr = NULL;
  ...
  if (!channel || !are_you_opped(channel->channel) || !nick || !ThisNick ||
...
#endif
  {
    ...
    case 3:
      ...
      if (flag == 3)
        bitchsay("NetHack detected on %s by %s!%s", channel->channel, nick,
                 ThisNick->host);
      break;
    case 2:
      bitchsay("NetHack detected on %s by %s!%s", channel->channel, nick,
...
BUILT_IN_COMMAND(reload_save) {
  ...
  if (get_string_var(CTOOLZ_DIR_VAR)) {
    ...
    new_free(&p);
    malloc_sprintf(&buffer, "%s/%s.sav", get_string_var(CTOOLZ_DIR_VAR),
                   version);
  } else
...
BUILT_IN_COMMAND(savelists) {
  ...
  snprintf(thefile, sizeof thefile, "%s/%s.sav", get_string_var(CTOOLZ_DIR_VAR),
           version);
  ...
  count = 0;
  for (ulist = next_userlist(NULL, &size, &location); ulist;
       ulist = next_userlist(ulist, &size, &location)) {
    fprintf(outfile, "ADDUSER %s!%s %s %s%s%s\n", ulist->nick, ulist->host,
            ulist->channels, convert_flags_to_str(ulist->flags),
            ulist->password ? space : empty_string,
            ulist->password ? ulist->password : empty_string);
    count++;
  }
  ...
  while (slist) {
    fprintf(outfile, "ADDSHIT %s %s %d %s\n", slist->filter, slist->channels,
            slist->level, slist->reason ? slist->reason : empty_string);
    slist = slist->next;
    count++;
  }
  ...
  while (ajoin) {
    if (ajoin->ajoin_list == 1) /* actual auto-join entry */
    {
      fprintf(outfile, "AJOIN %s%s%s%s%s\n", ajoin->name,
              ajoin->group ? " -g " : empty_string,
              ajoin->group ? ajoin->group : empty_string,
              ajoin->key ? space : empty_string,
              ajoin->key ? ajoin->key : empty_string);
      count++;
    }
    ajoin = ajoin->next;
  }
