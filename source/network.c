 ...
 *
 */
int socks4_connect(int s, int portnum __attribute__((unused)), struct sockaddr_in *server) {
  ...
  strncpy(sock4_connect.username,
  ...
  if ((red = write(s, &sock4_connect,
                   8 + strlen(sock4_connect.username) + 1)) == -1) {
...
/*
 * try to negotiate a SOCKS5 connection. (with the socket/username, to the
 * server)
 */
int socks5_connect(int s, int portnum __attribute__((unused)), struct sockaddr_in *server) {
  struct _sock_connect {
    char version;
    char type;
    char authtype;
  ...
  if ((red = read(s, tmpbuf, sizeof(tmpbuf) - 1)) == -1) {
    alarm(0);
    bitchsay("Cannot use SOCKS5 proxy, read failed during auth: %s",
             strerror(errno));
    return 0;
  }
  alarm(0);
  ...
  if (tmpbuf[1] != AUTH_NONE) {
    bitchsay("Cannot use SOCKS5 proxy, server wants type %x authentication.",
             tmpbuf[1]);
    return 0;
  }
  ...
  if ((red = read(s, tmpbuf, sizeof(tmpbuf) - 1)) == -1) {
    alarm(0);
    bitchsay("Cannot use SOCKS5 proxy, read failed during bounce: %s. "
             "Attempting SOCKS4",
             strerror(errno));
    return 0;
  }
  ...
  if (tmpbuf[1] != SOCKS5_NOERR) {
    bitchsay("Cannot use SOCKS5 proxy, server failed: %s",
             socks5_error(tmpbuf[1]));
    return 0;
  }
  /*
  ...
  case 1:
    ...
    tmpbuf[3] = '\0';
    tmpI = atoi(tmpbuf);
    bitchsay("SOCKS5 bounce successful, your address will be: %s:%d",
             inet_ntoa(tmpAddr), ntohs(tmpI));
    break;
  case 3: {
    char buffer[256];
    read(s, tmpbuf, 1);
    ...
    tmpbuf[3] = '\0';
    tmpI = atoi(tmpbuf);
    bitchsay("SOCKS5 bounce successful, your address will be: %s:%d", buffer,
             ntohs(tmpI));
    break;
  }
  case 4:
 ...
 */
int BX_connect_by_number(char *hostn, unsigned short *portnum, int service,
                         int protocol, int nonblocking) {
  int fd = -1;
  ...
  if (is_unix) {
    ...
    if (is_unix && (service == SERVICE_SERVER)) {
      if (bind(fd, (struct sockaddr *)&name,
    ...
    /* Unix domain client */
      ...
      alarm(get_int_var(CONNECT_TIMEOUT_VAR));
      if (connect(fd, (struct sockaddr *)&name, strlen(name.sun_path) + 2) <
          0) {
        alarm(0);
        return close(fd), -4;
      }
    ...
    if (!is_unix && (service == SERVICE_SERVER)) {
      ...
      if (getenv("EPIC_USE_HIGHPORTS")) {
        ...
        setsockopt(fd, IPPROTO_IP, IP_PORTRANGE, (char *)&ports, sizeof(ports));
      }
#endif
    ...
    if (LocalHostName) {
      ...
      if (LocalHostName && !getaddrinfo(LocalHostName, NULL, &hints, &res) &&
          res) {
    ...
    server_len = sizeof server.sins.sin;
      ...
      if (use_socks && get_string_var(SOCKS_HOST_VAR)) {
        ...
        fd = handle_socks(fd, *((struct sockaddr_in *)&server),
                          get_string_var(SOCKS_HOST_VAR),
                          get_int_var(SOCKS_PORT_VAR));
        if (fd == -1)
          return -4;
        else
          return fd;
...
#endif
      alarm(get_int_var(CONNECT_TIMEOUT_VAR));
      if (connect(fd, (struct sockaddr *)&server, server_len) < 0 &&
          errno != EINPROGRESS) {
        alarm(0);
        return close(fd), -4;
      }
...
extern char *BX_host_to_ip(const char *host) {
  ...
  if (!getaddrinfo(host, NULL, NULL, &res) && res) {
    ...
    inet_ntop(sf->sf_family,
              (sf->sf_family == AF_INET) ? (void *)&sf->sf_addr
                                         : (void *)&sf->sf_addr6,
              ip, 128);
    freeaddrinfo(res);
...
extern char *BX_ip_to_host(const char *ip) {
  ...
  if (getaddrinfo(ip, NULL, &hints, &res) == 0) {
    if (!res ||
        getnameinfo(res->ai_addr, res->ai_addrlen, host, 128, NULL, 0, 0))
...
extern char *BX_host_to_ip(const char *host) {
  ...
  return (hep ? snprintf(ip, sizeof ip, "%u.%u.%u.%u", hep->h_addr[0] & 0xff,
                         hep->h_addr[1] & 0xff, hep->h_addr[2] & 0xff,
                         hep->h_addr[3] & 0xff),
          ip  : empty_string);
}
