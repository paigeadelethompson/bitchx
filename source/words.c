...
/* move_word_rel()
 *
 * Take a string and return a pointer offset a number of words relative to a
 * given mark. Positive offset N returns a pointer to Nth next word (not
 * counting the current word, if the mark is within a word).  Negative offset -N
 * returns a pointer to the Nth previous word, counting the current word. Offset
 * 0 leaves the mark unchanged.
 */
