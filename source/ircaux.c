 ...
 */
void *n_malloc(size_t size, const char *module, const char *file,
               const int line) {
  char *ptr;
  ...
  if (!ptr) {
    ...
    putlog(LOG_ALL, "*", "*** failed calloc %s %s (%d)",
           module ? module : empty_string, file, line);
    term_reset();
    exit(1);
  }
...
void *n_free(void *ptr, const char *module, const char *file, const int line) {
  if (ptr) {
    ...
    if (alloc_size(ptr) == FREED_VAL) {
      ...
      putlog(LOG_ALL, "*", "*** failed free %s %s (%d)",
             module ? module : empty_string, file, line);
      term_reset();
      exit(1);
    }
...
void *n_realloc(void *ptr, size_t size, const char *module, const char *file,
                const int line) {
  char *ptr2 = NULL;
 ...
 */
char *n_malloc_strcpy(char **ptr, const char *src, const char *module,
...
char *n_malloc_strcat(char **ptr, const char *src, const char *module,
                      const char *file, const int line) {
  size_t msize;
...
int BX_my_strnicmp(const char *str1, const char *str2, size_t n) {
  while (n && *str1 && *str2 &&
         (stricmp_table[(unsigned char)*str1] ==
          stricmp_table[(unsigned char)*str2]))
    str1++, str2++, n--;
  return (n ? (stricmp_table[(unsigned char)*str1] -
               stricmp_table[(unsigned char)*str2])
            : 0);
}

/* my_strnstr: case insensitive version of strstr */
int BX_my_strnstr(const char *str1, const char *str2, size_t n __attribute__((unused))) {
  const char *p = str1;
  if (!p)
    return 0;
  for (; *p; p++)
...
char *BX_chop(char *stuff, int nchar) {
  ...
  if (nchar > 0 && sl > 0 && (size_t)nchar <= sl)
    stuff[sl - nchar] = 0;
  else if ((size_t)nchar > sl)
    stuff[0] = 0;
  return stuff;
}
 ...
 */
char *n_m_strcat_ues(char **dest, char *src, int unescape, const char *module,
                     const char *file, const int line) {
  int total_length;
  char *ptr, *ptr2;
  int z;
  ...
  *dest =
      n_realloc(*dest, sizeof(char) * (total_length + 2), module, file, line);
  if (z == 0)
    **dest = 0;
...
char *BX_check_nickname(char *nick) {
  char *s;
  int len = 0;
  if (!nick || *nick == '-' || isdigit((unsigned char)*nick) || !islegal(*nick))
    return NULL;
...
/* rsindex()
 *
 * Returns a pointer to the last matching character that appears in a string
 * BEFORE the initial search point (to search the entire string, set the initial
 * search point to the terminating NUL). Returns NULL if no matching characters
 * are found.
 *
 * The string to be searched starts at 'start' and the initial search point is
 * 'ptr'.
 *
 * A matching character is any character in 'group', unless the first character
 * of 'group' is a ^, in which case a matching character is any character NOT in
 * 'group'.
 */
...
FILE *BX_uzfopen(char **filename, char *path, int hook) {
  ...
  if ((!end_strcmp(filename_trying, ".gz", 3)) ||
    ...
    if (path_to_gunzip) {
      ...
      if (hook)
        yell("Cannot open file %s because gunzip was not found",
             filename_trying);
      new_free(filename);
      return NULL;
    }
    ...
    if (path_to_gunzip || path_to_uncompress) {
      ...
      if (hook)
        yell("Cannot open file %s because uncompress was not found",
             filename_trying);
      new_free(filename);
      return NULL;
    }
    ...
    if (*path_to_bunzip2) {
      ...
      if (hook)
        yell("Cannot open file %s because bunzip2 was not found",
             filename_trying);
      new_free(filename);
      return NULL;
    }
  ...
  if (ok_to_decompress) {
    ...
    if (hook)
      yell("Cannot open compressed file %s because no uncompressor was found",
           filename_trying);
    new_free(filename);
    return NULL;
  }
...
struct timeval *time_offset(struct timeval *tv, double offset) {
  ...
  offset -= seconds;
  if (offset < 0.0) {
    /* This ensures we never make tv_usec go negative, since it might be
     * unsigned. */
    offset += 1.0;
    seconds -= 1;
  }
...
char *BX_plural(int number) { return (number != 1) ? "s" : empty_string; }

char *BX_my_ctime(time_t when) { return chop(ctime(&when), 1); }
...
char *BX_MatchingBracket(register char *string, register char left,
                         register char right) {
  int bracket_count = 1;
...
char *n_m_strndup(const char *str, size_t len, const char *module,
                  const char *file, const int line) {
 ...
 * figure_out_address
 */
int BX_figure_out_address(char *nuh, char **nick, char **user, char **host,
                          char **domain, int *ip) {
  ...
  *ip = 0;
   ...
   */
  else if (fourthback && (firstback - secondback == 3) &&
           strbegins(thirdback, ".k12.") && strbegins(firstback, ".us")) {
    *host = myhost;
    *domain = fourthback;
    **domain = 0;
    (*domain)++;
   ...
   */
  else if (thirdback && !fourthback && (firstback - secondback == 3) &&
           strbegins(thirdback, ".k12.") && strbegins(firstback, ".us")) {
    *host = empty_string;
    *domain = myhost;
  }
   ...
   */
  else if (thirdback && (endstring - firstback == 3) &&
   ...
   */
  else if (secondback && !thirdback && (endstring - firstback == 3) &&
           (firstback - secondback == 4)) {
    *host = empty_string;
    *domain = myhost;
  }
