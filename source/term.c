 ...
 */
extern void term_pause(char unused __attribute__((unused)),
  ...
  if (!SetConsoleMode(hstdin, dwmode | ((ENABLE_LINE_INPUT | ENABLE_ECHO_INPUT |
...
int term_init(char *term __attribute__((unused))) {
  ...
  *termcap2 = 0;
  ...
  if (tgetent(termcap, term) < 1) {
    fprintf(stderr, "\n");
    fprintf(stderr,
            "Your current TERM setting (%s) does not have a termcap entry.\n",
            term);
    fprintf(stderr, "So we'll be running in dumb mode...\n");
    return -1;
  }
  ...
  SetConsoleMode(
      hinput, dwmode & (~(ENABLE_LINE_INPUT | ENABLE_ECHO_INPUT |
                          ENABLE_PROCESSED_INPUT) |
                        ENABLE_WINDOW_INPUT /* | ENABLE_WRAP_AT_EOL_OUTPUT*/));
   ...
   */
  desired = TERM_CAN_CUP | TERM_CAN_CLEAR | TERM_CAN_CLREOL | TERM_CAN_RIGHT |
            TERM_CAN_LEFT | TERM_CAN_SCROLL;
  ...
  if (current_term->TI_cub || current_term->TI_mrcup || current_term->TI_cub1 ||
      current_term->TI_kbs)
  ...
  if (current_term->TI_csr && (current_term->TI_ri || current_term->TI_rin) &&
      (current_term->TI_ind || current_term->TI_indn))
    termfeatures |= TERM_CAN_SCROLL;
  if (current_term->TI_wind && (current_term->TI_ri || current_term->TI_rin) &&
      (current_term->TI_ind || current_term->TI_indn))
    termfeatures |= TERM_CAN_SCROLL;
  if ((current_term->TI_il || current_term->TI_il1) &&
      (current_term->TI_dl || current_term->TI_dl1))
    termfeatures |= TERM_CAN_SCROLL;
  ...
  if ((current_term->TI_setf && current_term->TI_setb) ||
      (current_term->TI_setaf && current_term->TI_setab))
  ...
  for (i = 0; i < 16; i++) {
    ...
    cbuf[0] = '\0';
    if (i >= 8)
      strlcpy(cbuf, current_term->TI_sgrstrs[TERM_SGR_BOLD_ON - 1],
              sizeof(cbuf));
  ...
  for (i = 0; i < 16; i++) {
    ...
    cbuf[0] = '\0';
    if (i >= 8)
      strlcpy(cbuf, current_term->TI_sgrstrs[TERM_SGR_BLINK_ON - 1],
              sizeof(cbuf));
...
/* These force the default terminal size, and are used when reattached by
   scr-bx. They are then used as the fallback by term_resize(). */
void reset_lines(int nlines) { current_term->TI_lines = nlines; }

void reset_cols(int cols) { current_term->TI_cols = cols; }
...
void term_scroll(int top, int bot, int n) {
  ...
  pair[1] = 7;
  if (n > 0)
    VioScrollUp(top, 0, bot, output_screen->co, n, (PBYTE)&pair,
                (HVIO)vio_screen);
  else if (n < 0) {
    n = -n;
    VioScrollDn(top, 0, bot, output_screen->co, n, (PBYTE)&pair,
                (HVIO)vio_screen);
  }
#ifndef __EMXX__
  return;
  ...
  else if (current_term->TI_wind &&
           (current_term->TI_ri || current_term->TI_rin) &&
           (current_term->TI_ind || current_term->TI_indn)) {
    strcpy(start,
           tparm4(current_term->TI_wind, top, bot, 0, output_screen->co - 1));
    strcpy(final, tparm4(current_term->TI_wind, 0, output_screen->li - 1, 0,
                         output_screen->co - 1));
  ...
  else if ((current_term->TI_il || current_term->TI_il1) &&
           (current_term->TI_dl || current_term->TI_dl1)) {
    if (n > 0) {
      sr = top;
      er = bot;
...
int term_is_utf8(void) {
  ...
  if (!checked) {
    ...
    if (codeset &&
        (!my_stricmp(codeset, "UTF-8") || !my_stricmp(codeset, "UTF8")))
      utf8 = 1;
#endif
  }
  return utf8;
...
void set_meta_8bit(Window *w __attribute__((unused)),
                   char *u __attribute__((unused)), int value) {
  if (dumb_mode)
    return;
  ...
  for (x = 0; x < numcaps; x++) {
    ...
    if (!strcmp(name, compare)) {
      ...
      case CAP_TYPE_STR:
        ...
        if (!control_str)
          return NULL;
        strlcpy(retval, mangle ? control_mangle(control_str) : control_str,
                sizeof retval);
        return retval;
      }
    }
