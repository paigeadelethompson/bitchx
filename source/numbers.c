...
#define BITCHXINFO                                                             \
  "For more information about \002BitchX\002 type \002/about\002"
...
static int check_sync(int comm, char *channel, char *nick, char *whom,
                      char *bantime, ChannelList *chan) {
...
static int handle_stats(int comm, char *from, char **ArgList) {
  ...
  switch (comm) {
    /*
     ^on ^raw_irc "% 211 *" {suecho \(L:line\) $[20]sklnick($3) :
     $4|$5|$6|$7|$8|$strip(: 9)|$strip(: $10);suecho $[8]11
     $[30]sklhost($3)} ^on ^raw_irc "% 212 *" {suecho $[9]3: $4 $5} ^on
     ^raw_irc "% 213 *" {suecho \($3:line\) \($[20]4\)\($[20]6\)
     \($[5]7\)\($strip(: $[4]8)\)} ^on ^raw_irc "% 214 *" {suecho
     \($3:line\) \($[20]4\)\($[20]6\) \($[5]7\)\($strip(: $[4]8)\)} ^on
     ^raw_irc "% 215 *" {suecho \($3:line\) \($[20]4\)\($[20]6\)
     \($[5]7\)\($strip(: $[4]8)\)} ^on ^raw_irc "% 216 *" {suecho
     \($3:line\) \($[20]4\) \($[15]6\)} ^on ^raw_irc "% 218 *" {suecho
     \($3:line\) \($[5]4\) \($[5]5\) \($[5]6\) \($[5]7\) \($strip(:
     $[8]8)\)} ^on ^raw_irc "% 242 *" {suecho $0 has been up for $5- } ^on
     ^raw_irc "% 243 *" {suecho \($3:line\) \($[20]4\) \($[9]6\)} ^on
     ^raw_irc "% 244 *" {suecho \($3:line\) \($[25]4\) \($[25]6\)}
    */
  case 211:
    format_str = "L:line $[20]left($rindex([ $1) $1)  :$[-5]2 $[-5]3 $[-6]4 "
                 "$[-5]5 $[-6]6 $[-8]strip(: $7) $strip(: $8)";
    format2_str = "$[8]9 $[30]mid(${rindex([ $1)+1} ${rindex(] $1) - $rindex([ "
                  "$1) + 1} $1)";
    break;
  case 212:
...
static int trace_oper(char *from, char **ArgList) {
  ...
  put_it("%s", convert_output_format(fget_string_var(FORMAT_TRACE_OPER_FSET),
                                     "%s %s", from, rest));
  return 0;
}
...
static int trace_server(char *from, char **ArgList) {
  char *rest = PasteArgs(ArgList, 0);
  put_it("%s", convert_output_format(fget_string_var(FORMAT_TRACE_SERVER_FSET),
                                     "%s %s", from, rest));
  return 0;
}
...
static int trace_user(char *from, char **ArgList) {
  char *rest = PasteArgs(ArgList, 0);
  put_it("%s", convert_output_format(fget_string_var(FORMAT_TRACE_USER_FSET),
                                     "%s %s", from, rest));
  return 0;
}
...
static int check_server_sync(char *from, char **ArgList) {
  ...
  if (my_stricmp(from, get_server_itsname(from_server)) &&
    ...
    if (do_hook(DESYNC_MESSAGE_LIST, "%s %s", from, ArgList[0]))
      put_it("%s", convert_output_format(fget_string_var(FORMAT_DESYNC_FSET),
...
static void display_msg(char *from, char **ArgList) {
  ...
  if ((s = find_numeric_fset(-current_numeric))) {
    put_it("%s", convert_output_format(s, "%s %d %s %s", update_clock(GET_TIME),
                                       -current_numeric, from, rest));
    return;
  }
  ...
  put_it("%s %s%s%s%s%s%s", numeric_banner(),
         strlen(rest) ? rest : empty_string,
         strlen(rest) ? space : empty_string, ptr ? ptr : empty_string,
         from ? "(from " : empty_string, from ? from : empty_string,
         from ? ")" : empty_string);
}

static void channel_topic(char *from __attribute__((unused)), char **ArgList, int what) {
  char *topic, *channel;
  ChannelList *chan;
  ...
  if (ArgList[1] && is_channel(ArgList[0])) {
    ...
    if (what == 333 && ArgList[2])
      put_it("%s",
             convert_output_format(fget_string_var(FORMAT_TOPIC_SETBY_FSET),
                                   "%s %s %s %l", update_clock(GET_TIME),
                                   channel, topic, my_atol(ArgList[2])));
    else if (what != 333) {
      if ((chan = lookup_channel(channel, from_server, 0)))
        malloc_strcpy(&chan->topic, topic);
      put_it("%s", convert_output_format(fget_string_var(FORMAT_TOPIC_FSET),
                                         "%s %s %s", update_clock(GET_TIME),
                                         channel, topic));
    }
  } else {
    ...
    put_it("%s",
           convert_output_format(fget_string_var(FORMAT_TOPIC_FSET), "%s %s",
                                 update_clock(GET_TIME), ArgList[0]));
  }
}
...
static void not_valid_channel(char *from, char **ArgList) {
  ...
  if (!my_stricmp(from, get_server_itsname(from_server))) {
    ...
    put_it("%s", convert_output_format(fget_string_var(FORMAT_SERVER_MSG2_FSET),
                                       "%s %s %s", update_clock(GET_TIME),
                                       channel, ArgList[1]));
  }
}
...
*/
static void cannot_join_channel(char *from __attribute__((unused)), char **ArgList) {
  ...
  case 437:
    strlcat(buffer, " (Channel is temporarily unavailable)", sizeof buffer);
    break;
  case 471:
    strlcat(buffer, " (Channel is full)", sizeof buffer);
    break;
  case 473:
    strlcat(buffer, " (You must be invited)", sizeof buffer);
    break;
  case 474:
...
static int handle_server_stats(char *from __attribute__((unused)), char **ArgList, int comm) {
  static int norm = 0, invisible = 0, services = 0, servers = 0, ircops = 0,
             unknown = 0, chans = 0, local_users = 0, total_users = 0,
             services_flag = 0;
  int ret = 1;
  char *line;
  ...
  case 255: /* number of local print it out */
    ...
    if (total_users) {
      put_it("%s", convert_output_format(
                       "$G %K[%nlocal users on irc%K(%n\002$0\002%K)]%n $1%%",
                       "%d %d", local_users,
                       divide_rounded(local_users * 100.0, total_users)));
      put_it("%s",
             convert_output_format(
                 "$G %K[%nglobal users on irc%K(%n\002$0\002%K)]%n $1%%",
                 "%d %d", norm, divide_rounded(norm * 100.0, total_users)));
      if (services_flag) {
        put_it("%s",
               convert_output_format(
                   "$G %K[%ninvisible users on irc%K(%n\002$0\002%K)]%n $1%%",
                   "%d %d", invisible,
                   divide_rounded(invisible * 100.0, total_users)));
      } else
        put_it("%s", convert_output_format(
                         "$G %K[%nservices on irc%K(%n\002$0\002%K)]%n", "%d",
                         services));
      put_it("%s",
             convert_output_format(
                 "$G %K[%nircops on irc%K(%n\002$0\002%K)]%n $1%%", "%d %d",
                 ircops, divide_rounded(ircops * 100.0, total_users)));
    }
    put_it("%s", convert_output_format(
                     "$G %K[%ntotal users on irc%K(%n\002$0\002%K)]%n", "%d",
                     total_users));
    put_it("%s", convert_output_format(
                     "$G %K[%nunknown connections%K(%n\002$0\002%K)]%n", "%d",
                     unknown));

    put_it("%s", convert_output_format(
                     "$G %K[%ntotal servers on irc%K(%n\002$0\002%K)]%n "
                     "%K(%navg. $1 users per server%K)",
                     "%d %d", servers, divide_rounded(total_users, servers)));

    put_it("%s", convert_output_format(
                     "$G %K[%ntotal channels created%K(%n\002$0\002%K)]%n "
                     "%K(%navg. $1 users per channel%K)",
  ...
  case 250: {
    ...
    if (ArgList[3] && ArgList[4]) {
      ...
      p++;
      put_it("%s", convert_output_format(
                       "$G %K[%nHighest client connection "
                       "count%K(%n\002$0\002%K) %K(%n\002$1\002%K)]%n",
                       "%s %s", ArgList[3], p));
    }
    break;
  }
...
void whohas_nick(UserhostItem *stuff, char *nick, char *args __attribute__((unused))) {
  if (!stuff || !stuff->nick || !strcmp(stuff->user, "<UNKNOWN>"))
    return;
  put_it("%s",
         convert_output_format("$G Your nick %R[%n$0%R]%n is owned by %W$1@$2",
                               "%s %s %s", nick, stuff->user, stuff->host));
  return;
}
 ...
 */
static void get_nat_address(UserhostItem *stuff, char *nick __attribute__((unused)), char *args __attribute__((unused))) {
...
static void got_initial_version_28(char *server, char *sversion,
                                   char *channel_modes) {
  ...
  if (use_nat_address == 1)
    userhostbase(get_server_nickname(from_server), get_nat_address, 1, "%s",
...
void numbered_command(char *from, int comm, char **ArgList) {
        ...
        if (!tmp->group || is_server_valid(tmp->group, from_server)) {
          if (!in_join_list(tmp->name, from_server)) {
            if ((get_server_version(from_server) > Server2_8hybrid))
              send_to_server("JOIN %s%s%s", tmp->name,
                             tmp->key ? space : empty_string,
                             tmp->key ? tmp->key : empty_string);
            else {
              m_s3cat(&channels, ",", tmp->name);
              m_s3cat(&keys, ",", tmp->key ? tmp->key : empty_string);
            }
            if (get_int_var(JOIN_NEW_WINDOW_VAR)) {
              WhowasChanList *bleah =
                  check_whowas_chan_buffer(tmp->name, -1, 0);
              if (bleah) {
                if (bleah->channellist)
                  add_to_join_list(tmp->name, from_server,
                                   bleah->channellist->refnum);
              } else {
                if (tmp != ajoin_list)
  ...
  case 301: /* #define RPL_AWAY             301 */
  {
    ...
    if (!get_int_var(SHOW_AWAY_ONCE_VAR) || !last_away_msg ||
        strcmp(last_away_nick, ArgList[0]) || strcmp(last_away_msg, away_msg)) {
      ...
      if (do_hook(current_numeric, "%s %s", ArgList[0], away_msg))
        put_it("%s",
               convert_output_format(fget_string_var(FORMAT_WHOIS_AWAY_FSET),
                                     "%s %s", ArgList[0], away_msg));
    }
    break;
  }
  ...
  case 307: /* WHOIS_REGGED */
  {
    if (do_hook(current_numeric, "%s %s %s", from, ArgList[0], ArgList[1]))
      put_it("%s", convert_output_format(
                       fget_string_var(FORMAT_WHOIS_REGISTER_FSET), "%s %s",
                       ArgList[0], " is a registered nick"));
    break;
  }
  case 295:
  case 308: /* WHOIS_ADMIN */
  {
    if (do_hook(current_numeric, "%s %s %s", from, ArgList[0], ArgList[1]))
      put_it("%s", convert_output_format(
                       fget_string_var(FORMAT_WHOIS_ADMIN_FSET), "%s %s",
                       ArgList[0], " is an IRC Server Administrator"));
  ...
  case 309: /* WHOIS_SERVICES */
  {
    if (do_hook(current_numeric, "%s %s %s", from, ArgList[0], ArgList[1]))
      put_it("%s", convert_output_format(
                       fget_string_var(FORMAT_WHOIS_SERVICE_FSET), "%s %s",
                       ArgList[0], " is a Services Administrator"));
    break;
  }
  case 310: /* WHOIS_HELPFUL */
  {
    if (do_hook(current_numeric, "%s %s %s", from, ArgList[0], ArgList[1]))
      put_it("%s",
             convert_output_format(
                 fget_string_var(FORMAT_WHOIS_HELP_FSET), "%s %s", ArgList[0],
                 ArgList[1] ? ArgList[1] : " looks very helpful."));
    break;
...
#endif
      put_it("%s", convert_output_format(
                       fget_string_var(FORMAT_WHOIS_HEADER_FSET), NULL));
      put_it("%s", convert_output_format(
                       fget_string_var(FORMAT_WHOIS_NICK_FSET), "%s %s %s %s",
                       ArgList[0], ArgList[1], host, country(host)));
      put_it("%s",
             convert_output_format(fget_string_var(FORMAT_WHOIS_NAME_FSET),
                                   "%s", ArgList[4]));
      malloc_sprintf(&userhost, "%s@%s", u1, host);
#ifdef WANT_USERLIST
      if ((tmp = lookup_userlevelc("*", userhost, "*", NULL)))
        put_it("%s", convert_output_format(
                         fget_string_var(tmp->flags & ADD_BOT
                                             ? FORMAT_WHOIS_BOT_FSET
                                             : FORMAT_WHOIS_FRIEND_FSET),
                         "%s %s", convert_flags_to_str(tmp->flags), tmp->host));
      if ((tmp1 = nickinshit(ArgList[0], userhost)))
        put_it("%s",
               convert_output_format(fget_string_var(FORMAT_WHOIS_SHIT_FSET),
                                     "%d %s %s %s", tmp1->level, tmp1->channels,
                                     tmp1->filter, tmp1->reason));
      if (tmp || tmp1)
        put_it("%s", convert_output_format(
                         fget_string_var(FORMAT_WHOIS_CHANNELS_FSET), "%s %s",
                         tmp ? "Allowed:" : "Not-Allowed:",
                         tmp ? tmp->channels : tmp1->channels));
#endif
      new_free(&userhost);
    }
  ...
  {
    if (do_hook(current_numeric, "%s %s %s %s", from, ArgList[0], ArgList[1],
                ArgList[2]))
      put_it("%s",
             convert_output_format(fget_string_var(FORMAT_WHOIS_SERVER_FSET),
                                   "%s %s", ArgList[1], ArgList[2]));
    break;
  }
  case 313: /* #define RPL_WHOISOPERATOR    313 */
  {
    ...
    if (do_hook(current_numeric, "%s %s %s", from, ArgList[0], ArgList[1]))
      put_it("%s", convert_output_format(
                       fget_string_var(FORMAT_WHOIS_OPER_FSET), "%s %s",
                       ArgList[0], " (is \002NOT\002 an IRC warrior)"));
    break;
  }
  case 314: /* #define RPL_WHOWASUSER       314 */
  {
    ...
    if (do_hook(current_numeric, "%s %s %s %s %s %s", from, ArgList[0],
      ...
      put_it("%s", convert_output_format(
                       fget_string_var(FORMAT_WHOWAS_HEADER_FSET), NULL));
      put_it("%s", convert_output_format(
                       fget_string_var(FORMAT_WHOWAS_NICK_FSET), "%s %s %s",
  ...
  case 317: /* #define RPL_WHOISIDLE        317 */
  {
    ...
    if (ArgList[3]) /* undernet */
    {
      PasteArgs(ArgList, 3);
      if (nick && idle_str &&
          do_hook(current_numeric, "%s %s %s %s %s", from, nick, idle_str,
                  startup_str, ArgList[3])) {
        int seconds = 0;
        int hours = 0;
        int minutes = 0;
        ...
        put_it("%s", convert_output_format(
                         fget_string_var(FORMAT_WHOIS_IDLE_FSET), "%d %d %d %s",
    ...
    } else /* efnet */
    {
      PasteArgs(ArgList, 2);
      if (nick && idle_str &&
          do_hook(current_numeric, "%s %s %s %s", from, nick, idle_str,
                  ArgList[2] ? ArgList[2] : empty_string)) {
        int seconds = 0;
        int hours = 0;
        int minutes = 0;
        ...
        seconds = secs % 60;
        put_it("%s",
               convert_output_format(fget_string_var(FORMAT_WHOIS_IDLE_FSET),
                                     "%d %d %d %s", hours, minutes, seconds,
                                     ArgList[2] ? ArgList[2] : empty_string));
      }
    }
  ...
  case 318: /* #define RPL_ENDOFWHOIS       318 */
  {
    ...
    if (do_hook(current_numeric, "%s %s", from, ArgList[0]) &&
        fget_string_var(FORMAT_WHOIS_FOOTER_FSET))
      put_it("%s", convert_output_format(
                       fget_string_var(FORMAT_WHOIS_FOOTER_FSET), NULL));
  ...
  case 319: /* #define RPL_WHOISCHANNELS    319 */
  {
    ...
    if (do_hook(current_numeric, "%s %s %s", from, ArgList[0], ArgList[1]))
      put_it("%s",
             convert_output_format(fget_string_var(FORMAT_WHOIS_CHANNELS_FSET),
                                   "%s", ArgList[1]));
  ...
  case 330: /* #define RPL_WHOISLOGGEDIN    330 (hybrid, ratbox, ircu) */
  {
    if (do_hook(current_numeric, "%s %s %s %s", from, ArgList[0], ArgList[1],
                ArgList[2]))
      put_it("%s", convert_output_format(
                       fget_string_var(FORMAT_WHOIS_LOGGEDIN_FSET), "%s %s %s",
                       ArgList[0], ArgList[1], ArgList[2]));
    break;
  }
  ...
  case 378: /* #define RPL_WHOISHOST        378 (unreal, freenode) */
  {
    if (ArgList[2]) {
      ...
      if (do_hook(current_numeric, "%s %s %s %s", from, ArgList[0], ArgList[1],
                  ArgList[2]))
        put_it("%s", convert_output_format(
                         fget_string_var(FORMAT_WHOIS_ACTUALLY_FSET),
                         "%s %s %s", ArgList[0], ArgList[2], ArgList[1]));
    } else {
      /* Bahamut: <nick> :is actually user@host [ip] */
      if (do_hook(current_numeric, "%s %s %s", from, ArgList[0], ArgList[1]))
        put_it("%s", convert_output_format(
                         fget_string_var(FORMAT_WHOIS_ACTUALLY_FSET), "%s %s",
                         ArgList[0], ArgList[1]));
    }
    break;
  }
  case 340: /* #define RPL_INVITING_OTHER	340 */
  {
    if (ArgList[2]) {
      ...
      if (do_hook(current_numeric, "%s %s %s %s", from, ArgList[0], ArgList[1],
                  ArgList[2]))
        put_it("%s %s has invited %s to channel %s", numeric_banner(),
               ArgList[0], ArgList[1], ArgList[2]);
      reset_display_target();
    }
    break;
  ...
  case 341: /* #define RPL_INVITING         341 */
  {
    if (ArgList[1]) {
      ...
      if (do_hook(current_numeric, "%s %s %s", from, ArgList[0], ArgList[1]))
        put_it("%s", convert_output_format(
                         fget_string_var(FORMAT_INVITE_USER_FSET), "%s %s %s",
                         update_clock(GET_TIME), ArgList[0], ArgList[1]));
      reset_display_target();
    }
    break;
  }
  case 348: {
    number_of_exempts++;
    if (check_sync(comm, ArgList[0], ArgList[1], ArgList[2], ArgList[3],
                   NULL) == 0)
      break;
    if (ArgList[2]) {
      time_t tme = (time_t)my_atol(ArgList[3]);
      if (do_hook(current_numeric, "%s %s %s %s %s", from, ArgList[0],
                  ArgList[1], ArgList[2], ArgList[3]))
        put_it("%s",
               convert_output_format(fget_string_var(FORMAT_BANS_FSET),
                                     "%d %s %s %s %l", number_of_exempts,
                                     ArgList[0], ArgList[1], ArgList[2], tme));
    } else if (do_hook(current_numeric, "%s %s %s", from, ArgList[0],
                       ArgList[1]))
      put_it("%s",
             convert_output_format(fget_string_var(FORMAT_BANS_FSET),
                                   "%d %s %s %s %l", number_of_exempts,
                                   ArgList[0], ArgList[1], "unknown", now));
    break;
  }
  case 349: {
    int flag = 1;
    ...
    if ((foo = get_int_var(AUTO_WHOWAS_VAR))) {
      if (foo > -1)
        ...
        send_to_server("WHOWAS %s", ArgList[0]);
                       ...
                       ArgList[1]))
      put_it("%s", convert_output_format(fget_string_var(FORMAT_NONICK_FSET),
  ...
  case 453: {
    ...
    if (is_server_connected(from_server))
      ...
      PasteArgs(ArgList, 0);
      if (do_hook(current_numeric, "%s %s", never_connected ? "-1" : from,
                  *ArgList))
        display_msg(from, ArgList);
    }
    break;
  ...
  case 464: /* #define ERR_PASSWDMISMATCH   464 */
  {
    ...
    if (is_server_connected(from_server)) {
      ...
      if (!dumb_mode) {
        ...
        add_wait_prompt("Server Password:", password_sendline, server_num,
                        WAIT_PROMPT_LINE, 0);
      }
    }
    break;
  ...
  case 367: {
    ...
    if (ArgList[2]) {
      ...
      if (do_hook(current_numeric, "%s %s %s %s %s", from, ArgList[0],
        ...
        put_it("%s",
               convert_output_format(fget_string_var(FORMAT_BANS_FSET),
                                     "%d %s %s %s %l", number_of_bans,
                                     ArgList[0], ArgList[1], ArgList[2], tme));
    } else if (do_hook(current_numeric, "%s %s %s", from, ArgList[0],
                       ArgList[1]))
      put_it("%s", convert_output_format(
                       fget_string_var(FORMAT_BANS_FSET), "%d %s %s %s %l",
                       number_of_bans, ArgList[0], ArgList[1], "unknown", now));
    break;
  }
  case 368: /* #define RPL_ENDOFBANLIST     368 */
  {
    ...
    if (get_int_var(SHOW_END_OF_MSGS_VAR)) {
      if (do_hook(current_numeric, "%s %d %s", from, number_of_bans,
                  *ArgList)) {
        put_it("%s Total Number of Bans on %s [%d]", numeric_banner(),
               ArgList[0], number_of_bans);
      }
    }
    number_of_bans = 0;
    break;
  ...
  case 364: {
    ...
    else if ((do_hook(current_numeric, "%s %s %s", ArgList[0], ArgList[1],
                      ArgList[2] ? ArgList[2] : zero))) {
      if (fget_string_var(FORMAT_LINKS_FSET))
        put_it("%s", convert_output_format(fget_string_var(FORMAT_LINKS_FSET),
                                           "%s %s %s", ArgList[0], ArgList[1],
                                           ArgList[2] ? ArgList[2] : zero));
    }
    break;
  }
  ...
  case 716: /* #define RPL_TARGUMODEG       716 */
  {
    ...
    if (do_hook(current_numeric, "%s %s %s", from, ArgList[0], ArgList[1]))
      put_it("%s",
             convert_output_format(fget_string_var(FORMAT_WHOIS_CALLERID_FSET),
                                   "%s %s", ArgList[0], ArgList[1]));
    break;
  }
  ...
  if (none_of_these) {
    ...
    case 244:
      /* fallthrough */
      if (get_server_stat_flag(from_server) &&
          ((comm > 210 && comm < 219) || (comm > 240 && comm < 245))) {
        ...
        PasteArgs(ArgList, 0);
        if (from && !my_strnicmp(get_server_itsname(from_server), from,
                                 strlen(get_server_itsname(from_server))))
          from = NULL;
        if (from)
          put_it("%s %s from (%s)", numeric_banner(), *ArgList, from);
        else
    ...
    case 328: /* #define RPL_CHANNELURL       328 (atheme, bahamut, freenode) */
    {
      ...
      set_display_target(channel, LOG_CRAP);
      put_it("%s", convert_output_format(
                       fget_string_var(FORMAT_CHANNEL_URL_FSET), "%s %s %s",
                       update_clock(GET_TIME), channel, url));
      reset_display_target();
      break;
    }
    ...
    case 329: /* #define CREATION_TIME	329 */
    {
      ...
      set_display_target(ArgList[0], LOG_CRAP);
      if (!ArgList[2])
        put_it("%s Channel %s was created at %s", numeric_banner(), ArgList[0],
               this_sucks);
      else {
        ...
        put_it("%s Channel %s was created at %s", numeric_banner(), ArgList[0],
               this_sucks);
        put_it("%s Channel %s TS %s Password TS %s opless TS %s",
               numeric_banner(), ArgList[0], cts, pts, ots);
      }
    ...
    case 351: /* #define RPL_VERSION          351 */
      PasteArgs(ArgList, 2);
      put_it("%s", convert_output_format(fget_string_var(FORMAT_SERVER_FSET),
                                         "Server %s %s %s", ArgList[0],
                                         ArgList[1], ArgList[2]));
      break;
    ...
    case 364: /* #define RPL_LINKS            364 */
    {
      ...
      if (ArgList[2]) {
        PasteArgs(ArgList, 2);
        put_it("%s %-20s %-20s %s", numeric_banner(), ArgList[0], ArgList[1],
               ArgList[2]);
      } else {
        PasteArgs(ArgList, 1);
        put_it("%s %-20s %s", numeric_banner(), ArgList[0], ArgList[1]);
      }
      break;
    }
    case 377:
...
#define RPL_SERVLISTEND 235
    ...
    case 365: /* #define RPL_ENDOFLINKS       365 */
      if (comm == 365 && get_int_var(LLOOK_VAR) &&
          get_server_linklook(from_server)) {
        parse_365(ArgList[0], ArgList[1], NULL);
        set_server_linklook(from_server, 0);
        break;
      }
    ...
    case 671: /* #define RPL_WHOISSECURE      671 */
    {
      ...
      put_it("%s",
             convert_output_format(fget_string_var(FORMAT_WHOIS_SECURE_FSET),
                                   "%s %s", ArgList[0], ArgList[1]));
      break;
    }
    default:
