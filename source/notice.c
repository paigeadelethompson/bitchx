...
long oper_kills = 0, nick_collisions = 0, serv_fakes = 0, serv_unauth = 0,
     serv_squits = 0, serv_connects = 0, client_connects = 0, serv_rehash = 0,
     client_exits = 0, serv_klines = 0, client_floods = 0, client_invalid = 0,
     stats_req = 0, client_bot = 0, oper_requests = 0;
...
static void handle_oper_vision(const char *from, const char *line) {
  ...
  */
  ...
  if (sscanf(line,
             "Received KILL message for %500[^.]. From %500s Path: %500s "
             "(%500[^)])",
    ...
    serversay(from, "%s",
              convert_output_format(fget_string_var(kill_fset), "%s %s %s %s",
                                    update_clock(GET_TIME), arg[1], arg[0],
                                    arg[3]));
    up_status = 1;
  }
   ...
   */
    ...
    serversay(from, "%s",
              convert_output_format(
                  fget_string_var(FORMAT_SERVER_NOTICE_NICK_COLLISION_FSET),
                  "%s %s %s", update_clock(GET_TIME), arg[0], arg[1]));
    up_status = 1;
    ...
    serversay(from, "%s",
              convert_output_format("IP Mismatch %C$0-", "%s %s", arg[0]));
  } else if (sscanf(line, "Hacked ops on opless channel: %500s", arg[0]) == 1) {
    if (!(flags & HACK_OPS))
      goto done;

    serversay(from, "%s",
              convert_output_format("Hacked ops on $0", "%s", arg[0]));
    ...
    serversay(
        from, "%s",
        convert_output_format("Connect failure %K[%n$0-%K]", "%s", arg[0]));
    ...
    serversay(from, "%s",
              convert_output_format(
                  fget_string_var(FORMAT_SERVER_NOTICE_FAKE_FSET),
                  "%s %s MODE %s", update_clock(GET_TIME), arg[0], arg[1]));
  }
  /* "Unauthorized connection from %s." */
    ...
    serversay(
        from, "%s",
        convert_output_format(fget_string_var(FORMAT_SERVER_NOTICE_UNAUTH_FSET),
                              "%s %s", update_clock(GET_TIME), arg[0]));
  }
   ...
   */
  else if (sscanf(line, "%500s is rehashing Server config %500s", arg[0],
                  arg[1]) == 2 ||
           sscanf(line, "%500s is rehashing server config %500s", arg[0],
                  arg[1]) == 2 ||
           sscanf(line, "%500s is rehashing configuration %500s", arg[0],
                  arg[1]) == 2) {
    serv_rehash++;
    if (!(flags & REHASH))
      goto done;
    ...
    serversay(
        from, "%s",
        convert_output_format(fget_string_var(FORMAT_SERVER_NOTICE_REHASH_FSET),
                              "%s %s", update_clock(GET_TIME), arg[0]));
  }
   ...
   */
    ...
    if (sscanf(arg[0], "%100[^!]!%*[^@]@%*[^{]{%100s", oper, serv) == 2 &&
        strchr(serv, '.')) {
      chop(serv, 1);
      serversay(from, "%s",
                convert_output_format(
                    fget_string_var(FORMAT_SERVER_NOTICE_GLINE_FSET),
                    "%s %s %s %s %s %s", update_clock(GET_TIME), oper, arg[2],
                    serv, arg[1], arg[3]));
    } else
      serversay(from, "%s",
                convert_output_format(
                    fget_string_var(FORMAT_SERVER_NOTICE_KLINE_FSET),
                    "%s %s %s %s %s", update_clock(GET_TIME), arg[0], arg[2],
                    arg[1], arg[3]));
  }
  /* "User %s (%s@%s) trying to join %s is a possible spambot"
   * "User %s (%s@%s) is a possible spambot"
   */
  else if (sscanf(line, "User %500s (%500s is a possible spam%500s", arg[0],
                  arg[1], arg[2]) == 3 ||
           sscanf(
               line,
               "User %500s (%500s trying to join %*s is a possible spam%500s",
               arg[0], arg[1], arg[2]) == 3) {
    client_bot++;
    ...
    serversay(from, "%s",
              convert_output_format(
                  fget_string_var(FORMAT_SERVER_NOTICE_BOT_FSET), "%s %s %s",
                  update_clock(GET_TIME), arg[0], arg[1]));
  }
  /* ircnet/ircu/bahamut: "%s (%s@%s) is now operator (%c)"
   * hybrid-7/ratbox: "%s (%s@%s) is now an operator"
   * hybrid-8: "%s!%s@%s{%s} is now an operator"
   */
  else if (sscanf(line, "%500s (%500s is now oper%500s", arg[0], arg[1],
                  arg[2]) == 3 ||
           sscanf(line, "%500s (%500s is now an oper%500s", arg[0], arg[1],
                  arg[2]) == 3 ||
           sscanf(line, "%500[^!]!%500s is now an oper%500s", arg[0], arg[1],
                  arg[2]) == 3) {
    size_t len;
    ...
    serversay(from, "%s",
              convert_output_format(fget_string_var(FORMAT_OPER_FSET),
                                    "%s %s %s", update_clock(GET_TIME), arg[0],
                                    arg[1]));
  }
  /* "Received SQUIT %s from %s (%s)" */
    ...
    chop(arg[2], 1);
    serversay(from, "%s",
              convert_output_format("SQUIT of $1 from $2 %K[%R$3-%K]",
  ...
  /* "Received SERVER %s from %s (%d %s)" */
    ...
    serversay(
        from, "%s",
        convert_output_format("Received SERVER %c$1%n from %c$2%n %K[%W$3-%K]",
                              "%s %s %s %s", update_clock(GET_TIME), arg[0],
                              arg[1], arg[2]));
  }
  /* "Sending SQUIT %s (%s)" */
    ...
    chop(arg[1], 1);
    serversay(from, "%s",
              convert_output_format("Sending SQUIT %c$1%n %K[%R$2-%K]",
                                    "%s %s %s", update_clock(GET_TIME), arg[0],
                                    arg[1]));
  }
  /* "Remote CONNECT %s %d from %s" */
  else if (sscanf(line, "Remote CONNECT %500s %500s from %500s", arg[0], arg[1],
                  arg[2]) == 3) {
    serv_connects++;
    ...
    serversay(from, "%s",
              convert_output_format("Remote Connect of $1:$2 from $3",
                                    "%s %s %s %s", update_clock(GET_TIME),
                                    arg[0], arg[1], arg[2]));
  }
   ...
   */
           ...
           sscanf(
               line,
               "Client connecting on port %*[^:]: %500s (%500[^)]) %500[^\n]",
               arg[0], arg[1], arg[2]) == 3) {
    client_connects++;
    ...
    serversay(from, "%s",
              convert_output_format(
                  fget_string_var(FORMAT_SERVER_NOTICE_CLIENT_CONNECT_FSET),
                  "%s %s %s %s", update_clock(GET_TIME), arg[0], arg[1],
                  arg[2]));
  }
   ...
   */
    ...
    serversay(from, "%s",
              convert_output_format(
                  fget_string_var(FORMAT_SERVER_NOTICE_CLIENT_EXIT_FSET),
                  "%s %s %s %s", update_clock(GET_TIME), arg[0], arg[1],
                  arg[2]));
  }
   ...
   */
    ...
    serversay(from, "%s",
              convert_output_format(
                  fget_string_var(FORMAT_SERVER_NOTICE_CLIENT_TERM_FSET),
                  "%s %s %s", update_clock(GET_TIME), arg[0], arg[1]));
  }
  /* !ircnet: "Invalid username: %s (%s@%s)" */
  else if (sscanf(line, "Invalid username: %500s (%500[^)])", arg[0], arg[1]) ==
           2) {
    client_invalid++;
    ...
    serversay(from, "%s",
              convert_output_format(
                  fget_string_var(FORMAT_SERVER_NOTICE_CLIENT_INVALID_FSET),
                  "%s %s %s", update_clock(GET_TIME), arg[0], arg[1]));
  }
  ...
  /* "STATS %c requested by %s (%s@%s) [%s]" */
  else if (sscanf(line, "STATS %500s requested by %500s (%500[^)]) [%500[^]]]",
                  arg[0], arg[1], arg[2], arg[3]) == 4) {
    stats_req++;
    ...
    serversay(
        from, "%s",
        convert_output_format(fget_string_var(FORMAT_SERVER_NOTICE_STATS_FSET),
                              "%s %s %s %s %s", update_clock(GET_TIME), arg[0],
                              arg[1], arg[2], arg[3]));
    ...
    serversay(from, "%s",
              convert_output_format("Nick Flooding %K[%B$1-%K]", "%s %s",
                                    update_clock(GET_TIME), arg[0]));
  }
   ...
   */
    ...
    serversay(from, "%s",
              convert_output_format("Kill line for $1- active", "%s %s",
                                    update_clock(GET_TIME), arg[0]));
  } else {
    if (!(flags & SERVER_CRAP))
      goto done;
    serversay(from, "%s",
              convert_output_format(fget_string_var(FORMAT_SERVER_NOTICE_FSET),
                                    "%s %s %s", update_clock(GET_TIME), from,
                                    stripansicodes(line)));
  }
done:
...
static int check_ignore_notice(char *from, char *to, unsigned long type,
                               char *line, char **high) {
...
static int check_chanwall_notice(const char *from, const char *line, int type) {
  ...
  if (do_hook(type, "%s %s", from, line)) {
    char *s = convert_output_format(fget_string_var(FORMAT_BWALL_FSET),
                                    "%s %s %s %s %s", update_clock(GET_TIME),
                                    channel, from, FromUserHost, newline);
...
void parse_notice(char *from, char **Args) {
  ...
  char *to, *high = empty_string, *target, *line;
  ...
  if (!check_chanwall_notice(from, line, list_type)) {
    ...
    case PUBLIC_NOTICE_LIST:
      s = convert_output_format(
          fget_string_var(check_auto_reply(line) ? FORMAT_PUBLIC_NOTICE_AR_FSET
                                                 : FORMAT_PUBLIC_NOTICE_FSET),
          "%s %s %s %s %s", update_clock(GET_TIME), from, FromUserHost, to,
          line);
      break;

    case NOTICE_GROUP_LIST:
      s = convert_output_format(fget_string_var(FORMAT_NOTICE_GROUP_FSET),
                                "%s %s %s %s", update_clock(GET_TIME), from, to,
                                line);
      break;

    default:
      s = convert_output_format(fget_string_var(FORMAT_NOTICE_FSET),
...
void setup_ov_mode(int on, int hide, int log) {
  ...
  if (on) {
    ...
    if ((tmp = new_window(current_window->screen))) {
      ...
      tmp->absolute_size = 1;
      tmp->skip = 1;
      set_wset_string_var(tmp->wset, STATUS_FORMAT1_WSET,
                          fget_string_var(FORMAT_OV_FSET));
...
#ifdef WANT_OPERVIEW
char *opflags[] = {"COLLIDE", "KILLS",   "MISMATCH", "HACK",    "IDENTD",
                   "FAKES",   "UNAUTHS", "CLIENTS",  "TRAFFIC", "REHASH",
                   "KLINE",   "BOTS",    "OPER",     "SQUIT",   "SERVER",
                   "CONNECT", "FLOOD",   "USER",     "STATS",   "NICK",
                   "ACTIVEK", "CRAP",    NULL};

char all[] = "ALL", none[] = "NONE";
...
void print_ircop_flags(int server) {
  ...
  put_it("%s",
         convert_output_format("$G %bOper%BView%n: $0-", "%s",
                               *buffer ? flag == -1 ? "ALL" : buffer : "NONE"));
}

void convert_swatch(Window *win __attribute__((unused)), char *str, int unused __attribute__((unused))) {
  unsigned long flag;
  char buffer[200];
...
BUILT_IN_COMMAND(s_watch) {
  ...
  if (from_server == -1) {
    put_it("%s", convert_output_format("$G Try connecting to a server first",
                                       NULL, NULL));
    return;
  }
  ...
  if (flag != old_flags)
    set_server_ircop_flags(from_server, flag);
  else if (gotargs && old_flags != (unsigned int)-1) {
    int i;
...
BUILT_IN_COMMAND(ov_window) {
  ...
  if (count == 0)
    put_it("%s", convert_output_format("$G %BOper%bView%n is %K[%W$0%K]", "%s",
                                       on_off(get_int_var(OV_VAR))));
  else {
    if ((!ov && get_int_var(OV_VAR)) || (ov && !get_int_var(OV_VAR))) {
      ...
      put_it("%s", convert_output_format(
                       "$G %BOper%bView%n is now toggled %K[%W$0%K]", "%s",
                       on_off(get_int_var(OV_VAR))));
      return;
    }
    ...
    put_it("%s",
           convert_output_format("$G %BOper%bView%n is already %K[%W$0%K]",
                                 "%s", on_off(get_int_var(OV_VAR))));
    hide = old_hide;
    {
