...
BUILT_IN_COMMAND(nwhois) {
  ...
  while (all || (nick = next_arg(arg, &arg))) {
    while (tmp) {
      if ((all || (nick && (to_chan = is_channel(nick)))) &&
          my_stricmp(tmp->channel, nick)) {
        tmp = tmp->next;
        continue;
      }
      for (user = next_nicklist(tmp, NULL); user;
           user = next_nicklist(tmp, user)) {
        if (all || to_chan || wild_match(nick, user->nick)) {
          if (!once) {
            put_it("%s",
                   convert_output_format(
                       ...
                       " -b  kicks  nicks   pub",
                       "%s",
                       (all || to_chan) ? /*"Channel"*/ tmp->channel : "Nick"));
          ...
          put_it("%s", convert_output_format(
                           "$[10]0 %C$1 $2%w  %B$[16]3  %R$[6]4  %M$[4]5 $[4]6 "
                           ...
                           "none", 0,
#endif
                           user->stat_ops, user->stat_dops, user->stat_bans,
                           user->stat_unbans, user->stat_kicks,
                           user->stat_nicks, user->stat_pub));
          mem_use += sizeof(NickList) +
                     (user->userlist ? sizeof(UserList) : 0) +
                     (user->shitlist ? sizeof(ShitList) : 0);
          if (max_mem < mem_use)
            max_mem = mem_use;
          if (!all)
            break;
    ...
    if (once && nick)
      put_it("%s",
             convert_output_format(
                 "$G End /NWhoIs list for $1- [Mem Usage: $0 bytes per chan]",
                 "%d %s", max_mem, (all || to_chan) ? "All" : nick));
    if (all)
      break;
  }
  if (!all && !once)
    put_it("%s",
           convert_output_format("$G No such nick or channel", NULL, NULL));
  new_free(&t);
}
  ...
  if (args && *args) {
    ...
    if (!command) {
      ...
      if (!ban_words || !new || (new && !wild_match(new->channel, chan))) {
        ...
        bitchsay("[%s] is already in the list for channel %s", args, chan);
    } else {
      int count = 0;
      malloc_strcpy(&word, args);
      while ((new = (WordKickList *)removewild_from_list((List **)&ban_words,
                                                         word))) {
  ...
  if (command && *command && !strcmp(command, "ModeLock")) {
    ...
    if ((chan = lookup_channel(channel, from_server, 0))) {
      ...
      if (*buffer) {
        ...
        bitchsay("%s Mode Locked at: [%s] %s", chan->channel, buffer,
                 (chan->limit > 0) ? "Users" : empty_string);
      } else
    ...
    for (i = 0; i < server_list_size(); i++) {
      for (chan = get_server_channels(from_server); chan; chan = chan->next)
        bitchsay("Lock on %s: %s", chan->channel,
                 chan->modelock_key ? chan->modelock_key : "none");
    }
  }
}
...
BUILT_IN_COMMAND(randomnick) {
  ...
  while (count > 0) {
    send_to_server("NICK %s%s", prefix ? prefix : empty_string,
                   random_str(3, 9 - len));
    count--;
  }
}
...
BUILT_IN_COMMAND(topic_lock) {
  ...
  if ((chan = lookup_channel(channel, from_server, 0))) {
    if (t && *t && !my_stricmp(t, "ON")) {
      ...
      chan->topic_lock = 0;
    bitchsay("Topic lock for [%s] - %s", chan->channel,
             on_off(chan->topic_lock));
  }
}
...
BUILT_IN_COMMAND(sping) {
  ...
  if (!args || !*args) {
    ...
    while ((sname = next_arg(args, &args))) {
      ...
      send_to_server("PING %s :%s", sname, sname);
    }
  }
}
...
BUILT_IN_COMMAND(do_toggle) {
  ...
  if (!args || !*args) {
    ...
    put_it("%s", convert_output_format(
                     ...
                     on_off(get_int_var(CTCP_FLOOD_PROTECTION_VAR)),
    ...
    put_it("%s",
           convert_output_format(
               ...
               on_off(get_int_var(JOINFLOOD_VAR))));
    put_it("%s",
           convert_output_format(
               "%G|   %Cauto_w%chowas   %K[%W$[-3]0%K]    %Cverb%cose_ctcp     "
               "     %K[%W$[-3]1%K]    %Cnickfl%cood   %K[%W$[-3]2%K]",
               "%s %s %s", on_off(get_int_var(AUTO_WHOWAS_VAR)),
               on_off(get_int_var(CTCP_VERBOSE_VAR)),
               on_off(get_int_var(NICKFLOOD_VAR))));
    put_it("%s",
           convert_output_format(
               "%G|   %Ccl%coak         %K[%W$[-3]0%K]    %Coper%cview         "
               "     %K[%W$[-3]1%K]    %Cshit%clist    %K[%W$[-3]2%K]",
               "%s %s %s", on_off(get_int_var(CLOAK_VAR)),
               on_off(get_int_var(OV_VAR)), on_off(get_int_var(SHITLIST_VAR))));
    put_it("%s",
           convert_output_format(
               "%G|   %Ckick_o%cps      %K[%W$[-3]0%K]    %Cannoy%c_kick       "
               "     %K[%W$[-3]1%K]    %Cuser%clist    %K[%W$[-3]2%K]",
               "%s %s %s", on_off(get_int_var(KICK_OPS_VAR)),
               on_off(get_int_var(ANNOY_KICK_VAR)),
               on_off(get_int_var(USERLIST_VAR))));
    put_it("%s",
           convert_output_format(
               "%G|   %Chack%cing       %K[%W$[-3]0%K]    %Cnick_c%completion  "
               "     %K[%W$[-3]1%K]    %Cauto_r%cejoin %K[%W$[-3]2%K]",
               "%s %s %s", on_off(get_int_var(HACKING_VAR)),
               on_off(get_int_var(NICK_COMPLETION_VAR)),
               on_off((get_int_var(AUTO_REJOIN_VAR) ? 1 : 0))));
    put_it("%s",
           convert_output_format(
               "%K|   %Caop           %K[%W$[-3]0%K]    %Cauto_aw%cay          "
               "   %K[%W$[-3]1%K]    %Cauto_rec%conn %K[%W$[-3]2%K]",
               "%s %s %s", on_off((get_int_var(AOP_VAR))),
               on_off((get_int_var(AUTO_AWAY_VAR))),
               on_off(get_int_var(AUTO_RECONNECT_VAR))));
    put_it("%s", convert_output_format(
                     "%K|   %Cbitch         %K[%W$[-3]0%K]    %Cdcc_f%cast     "
                     "         %K[%W$[-3]1%K]    %Ckick_if_ban %K[%W$[-3]2%K]",
                     "%s %s %s", on_off((get_int_var(BITCH_VAR))),
                     on_off((get_int_var(DCC_FAST_VAR))),
                     on_off(get_int_var(KICK_IF_BANNED_VAR))));
    put_it("%s",
           convert_output_format(
               "%g|   %Cftp_g%crab      %K[%W$[-3]0%K]    %Cmircs              "
               "   %K[%W$[-3]1%K]    %Chttp%c_grab   %K[%W$[-3]2%K]",
               "%s %s", on_off((get_int_var(FTP_GRAB_VAR))),
               on_off((get_int_var(MIRCS_VAR))),
               on_off((get_int_var(HTTP_GRAB_VAR)))));
    put_it("%s", convert_output_format(
                     "%G|   %Cdisp%clay_ansi  %K[%W$[-3]0%K]    %Wtype /toggle "
                     "<setting>         %Clog         %K[%W$[-3]1%K]",
                     "%s %s", on_off((get_int_var(DISPLAY_ANSI_VAR))),
                     on_off((get_int_var(LOG_VAR)))));
    ...
    put_it("%s",
           convert_output_format(
               "%K|   %Chack%cing       %K[%W$[-3]0%K]    %Cnick_c%completion  "
               "     %K[%W$[-3]1%K]    %Cauto_rej%coin %K[%W$[-3]2%K]",
               "%s %s %s", on_off(get_int_var(HACKING_VAR)),
               on_off(get_int_var(NICK_COMPLETION_VAR)),
               on_off((get_int_var(AUTO_REJOIN_VAR) ? 1 : 0))));
    put_it("%s",
           convert_output_format(
               "%K:   %Caop           %K[%W$[-3]0%K]    %Cauto_aw%cay          "
               "   %K[%W$[-3]1%K]    %Cauto_rec%conn %K[%W$[-3]2%K]",
               "%s %s %s", on_off((get_int_var(AOP_VAR) ? 1 : 0)),
               on_off((get_int_var(AUTO_AWAY_VAR))),
               on_off(get_int_var(AUTO_RECONNECT_VAR))));
    put_it("%s", convert_output_format(
                     "%K:   %Cbitch         %K[%W$[-3]0%K]    %Cdcc_f%cast     "
    ...
    put_it("%s",
           convert_output_format(
               "%g:   %Cftp_g%crab      %K[%W$[-3]0%K]    %Cmircs              "
               "   %K[%W$[-3]1%K]    %Chttp%c_grab   %K[%W$[-3]2%K]",
               "%s %s %s", on_off((get_int_var(FTP_GRAB_VAR))),
               on_off((get_int_var(MIRCS_VAR))),
               on_off((get_int_var(HTTP_GRAB_VAR)))));
    put_it("%s", convert_output_format(
                     "%K:   %Cdisp%clay_ansi  %K[%W$[-3]0%K]    %Ctimestamp    "
                     "         %K[%W$[-3]1%K]    %Clog         %K[%W$[-3]2%K]",
                     "%s %s %s", on_off((get_int_var(DISPLAY_ANSI_VAR))),
                     on_off((get_int_var(TIMESTAMP_VAR))),
                     on_off((get_int_var(LOG_VAR)))));
    put_it(
        "%s",
        convert_output_format(
            "%K:                             %Wtype /toggle <setting> ", NULL));
    ...
    while ((arg = next_arg(args, &args))) {
      ...
      if (!my_strnicmp(arg, "auto_nslookup", 7)) {
        ...
        toggle_reverse_status(current_window, NULL,
                              get_int_var(DISPLAY_ANSI_VAR));
        if (do_hook(SET_LIST, "%s %s", arg,
                    on_off(get_int_var(DISPLAY_ANSI_VAR))))
          put_it("%s", convert_output_format(
                           "$G %cToggled %GAnsi Display %K[%W$[-3]0%K]", "%s",
                           on_off(get_int_var(DISPLAY_ANSI_VAR))));
      ...
      if (str) {
        if (do_hook(SET_LIST, "%s %s", arg, on_off(get_int_var(var))))
          put_it("%s",
                 convert_output_format(str, "%s", on_off(get_int_var(var))));
      }
      if (var == -1)
...
BUILT_IN_COMMAND(show_version) {
  ...
  version_buf = m_strdup(stripansicodes(convert_output_format(
      fget_string_var(FORMAT_VERSION_FSET), "%s %s %s %s %s", irc_version,
      internal_version, sysname, release, tcl_versionstr)));
  ...
  *match_arg = 0;
  ...
  if (wild_match(nick_arg, match_arg)) {
    ...
    *t_arg = 0;
    ...
    if (t_nick && *t_nick)
      strmopencat(t_arg, 1999, t_nick, ",", ArgList[4], "!", ArgList[1], "@",
                  ArgList[2], NULL);
    else
      strmopencat(t_arg, 1999, ArgList[4], "!", ArgList[1], "@", ArgList[2],
                  NULL);
    malloc_sprintf(&w->who_buff, "%d %s %s :%s", server, nick_arg, t_arg,
                   reason);
  }
}
...
BUILT_IN_COMMAND(whokill) {
  ...
  while ((pattern = next_arg(args, &args))) {
    malloc_sprintf(
        &nick_arg, "%s%s%s", *pattern != '*' ? "*" : empty_string, pattern,
        *(pattern + strlen(pattern) - 1) != '*' ? "*" : empty_string);
    if ((p = strchr(nick_arg, '@')))
      p++;
    whobase(p ? p : nick_arg, who_user_kill, who_user_killend, "%d %s :%s",
            from_server, nick_arg, reason);
    new_free(&nick_arg);
  }
}
...
BUILT_IN_COMMAND(tracekill) {
  ...
  malloc_sprintf(&tnick_arg, "%s%s%s", *pattern != '*' ? "*" : empty_string,
                 pattern,
                 *(pattern + strlen(pattern) - 1) != '*' ? "*" : empty_string);
...
BUILT_IN_COMMAND(traceserv) {
  ...
  if (!(server = next_arg(args, &args)) || !(pattern = next_arg(args, &args)))
    return;
  if (args && *args)
    malloc_strcpy(&treason, args);
  else
  ...
  if ((channel = next_arg(args, &args))) {
    ...
    if ((tmp = new_window(current_window->screen))) {
      ...
      if (channel && (channel = make_channel(channel))) {
        send_to_server("JOIN %s%s%s", channel, args ? space : empty_string,
...
static void gain_nick(UserhostItem *stuff, char *nick, char *args) {
  ...
  old_serv = server;
  if (stuff && stuff->nick &&
      (!strcmp(stuff->user, "<UNKNOWN>") || !strcmp(stuff->host, "<UNKNOWN>")))
    gotit = change_orig_nick(from_server);
  if (!gotit && get_int_var(ORIGNICK_TIME_VAR) > 0 &&
      !timer_callback_exists(delay_gain_nick))
    add_timer(0, empty_string, get_int_var(ORIGNICK_TIME_VAR) * 1000, 1,
              delay_gain_nick, NULL, NULL, -1, "orignick");
...
BUILT_IN_COMMAND(orig_nick) {
  ...
  if (*nick == '-') {
    ...
    if (org_nick && !my_stricmp(nick, org_nick) &&
      ...
      userhostbase(get_server_orignick(from_server), gain_nick, 1, "%d",
                   from_server);
      bitchsay("Trying to regain nick [%s] every %d seconds",
               get_server_orignick(from_server),
               get_int_var(ORIGNICK_TIME_VAR));
    } else
...
BUILT_IN_COMMAND(add_bad_nick) {
  ...
  bitchsay("%s %s LameNick list", add ? "Added" : "Removed",
           add ? "to" : "from");
  while (args && *args) {
    nick = next_arg(args, &args);
    if (add && nick) {
...
int grab_http(char *from, char *to, char *text) {
  ...
  if ((get_int_var(HTTP_GRAB_VAR) && stristr(text, "HTTP:")) ||
      (get_int_var(FTP_GRAB_VAR) &&
       (stristr(text, "FTP:") || stristr(text, "FTP.")))) {
    static int count = 0;
    int url_count = 0;
    char *q = NULL;
    ...
    for (cur_url = url_list, prev_url = NULL; cur_url;
         prev_url = cur_url, cur_url = cur_url->next) {
      url_count++;
    ...
    if (do_hook(URLGRAB_LIST, "%d %d %s %s %s %s", url_count, count, from,
                FromUserHost, to, text))
...
BUILT_IN_COMMAND(url_grabber) {
  ...
  if (args && *args) {
    ...
    while ((p = next_arg(args, &args))) {
      if (!my_stricmp(p, "SAVE")) {
        ...
        snprintf(buffer, BIG_BUFFER_SIZE, "%s/BitchX.url",
        ...
        if (!*++p) {
          ...
          q = my_atol(p);
          for (i = 1, cur_url = url_list, prev_url = NULL; cur_url && i < q;
               prev_url = cur_url, cur_url = cur_url->next, i++)
            ;
          if (cur_url && i == q) {
            if (!prev_url)
              url_list = url_list->next;
        ...
        for (i = 1, cur_url = url_list; cur_url; cur_url = cur_url->next, i++) {
          if (!cur_url->name)
            break;
          put_it("%s", convert_output_format("$G HTTP[$0] $1-", "%d %s", i,
                                             cur_url->name));
        }
        do_display = 0;
      }
  ...
  if (do_display)
    put_it("%s", convert_output_format(
                     "$G HTTP grab %K[%W$0%K]%n FTP grab %K[%W$1%K]", "%s %s",
                     on_off(get_int_var(HTTP_GRAB_VAR)),
                     on_off(get_int_var(FTP_GRAB_VAR))));
#endif
}
...
BUILT_IN_COMMAND(serv_stat) {
  ...
  put_it("%s", convert_output_format("%G| %CC%client exits    %K[%W$[-4]0%K]   "
                                     " %CK%c-lines adds %K[%W$[-4]1%K]",
                                     "%l %l", client_exits, serv_klines));
  put_it("%s", convert_output_format("%G| %CC%client Floods   %K[%W$[-4]0%K]   "
                                     " %CS%ctats reqs   %K[%W$[-4]1%K]",
                                     "%l %l", client_floods, stats_req));
  put_it("%s", convert_output_format("%G| %CI%cnvalid User    %K[%W$[-4]0%K]   "
                                     " %CO%cper Reqs    %K[%W$[-4]1%K]",
                                     "%l %l", client_invalid, oper_requests));
  put_it("%s", convert_output_format("%G| %CP%cossible Bots   %K[%W$[-4]0%K]",
                                     "%l", client_bot));
  put_it("%s", convert_output_format("%G| %CS%cerv Squits     %K[%W$[-4]0%K]   "
                                     " %CS%cerv Connect %K[%W$[-4]1%K]",
                                     "%l %l", serv_squits, serv_connects));
  ...
  put_it("%s", convert_output_format("%G: %CI%cnvalid User    %K[%W$[-4]0%K]   "
                                     " %CO%cper Reqs    %K[%W$[-4]1%K]",
                                     "%l %l", client_invalid, oper_requests));
  put_it("%s", convert_output_format("%K| %CP%cossible Bots   %K[%W$[-4]0%K]",
                                     "%l", client_bot));
  put_it("%s", convert_output_format("%g: %CS%cerv Squits     %K[%W$[-4]0%K]   "
                                     " %CS%cerv Connect %K[%W$[-4]1%K]",
                                     "%l %l", serv_squits, serv_connects));
...
BUILT_IN_COMMAND(set_autoreply) {
  ...
  if (*args == '-') {
    ...
    if (*p == 'd' && strlen(p) == 1) {
      ...
      m_e3cat(&new_args, strip(nickname, not_wanted1),
              (len > 4 ? space : empty_string));
      if (len > 4) {
      ...
      if (!strstr(new_args, strip(nickname, not_wanted2)))
        m_e3cat(&new_args, strip(nickname, not_wanted2),
                (len > 4 ? space : empty_string));
      if (len > 4) {
        memset(new_nick, 0, sizeof(new_nick));
        new_nick[0] = '*';
    ...
    new_free(&new_args);
    bitchsay("Auto-Response now set to [%s]",
             get_string_var(AUTO_RESPONSE_STR_VAR));
  }
}
...
BUILT_IN_COMMAND(add_no_flood) {
  ...
  while (nick && *nick) {
    if (!(nptr =
              find_name_in_genericlist(nick, no_flood_list, FLOOD_HASHSIZE, 0)))
...
BUILT_IN_COMMAND(check_clones) {
  ...
  for (nptr = next_nicklist(chan, NULL); nptr; nptr = next_nicklist(chan, nptr))
    nptr->check_clone = 0;
  for (nptr = next_nicklist(chan, NULL); nptr;
       nptr = next_nicklist(chan, nptr)) {
    char *q;
    ...
    for (nptr1 = next_nicklist(chan, nptr); nptr1;
         nptr1 = next_nicklist(chan, nptr1)) {
      char *p;
      ...
      if (!strcmp(p, q)) {
        /* clone detected */
        if (!clones &&
            do_hook(CLONE_LIST, "Clones detected on %s", chan->channel))
          bitchsay("Clones detected on %s", chan->channel);
        if (!nptr->check_clone++) {
          if (do_hook(CLONE_LIST, "%s %s %s", nptr->nick, nptr->host,
                      nick_isop(nptr) ? "@" : empty_string))
            put_user(nptr, chan->channel);
          clones++;
        }
        if (do_hook(CLONE_LIST, "%s %s %s", nptr1->nick, nptr1->host,
                    nick_isop(nptr1) ? "@" : empty_string))
...
BUILT_IN_COMMAND(pastecmd) {
  ...
  for (start_pos = start_pos ? start_pos->prev : win->display_ip->prev;
       line > 1; start_pos = start_pos->prev)
    line--;
  ...
  while (count) {
    ...
    if (start_pos && start_pos->line) {
      if (do_hook(PASTE_LIST, "%s %s", channel, start_pos->line))
        send_text(channel,
                  convert_output_format(fget_string_var(FORMAT_PASTE_FSET),
                                        "%s %d %s", channel, line,
                                        start_pos->line),
                  STXT_LOG);
...
#if defined(GUI)
void parsemenuitem(char *pargs, char **itemtext, char **options,
                   char **itemalias, int *refnum) {
  char *text;
  *refnum = 0;
...
/*
 * Returns a pointer to the menu you are looking for, or returns NULL if it does
 * not exist
 */
...
static int null_cmp_func(List *s, List *s1) { return 0; }
...
void removemenu(char *menuname) {
  MenuStruct *thismenu;
  if ((thismenu =
           (MenuStruct *)remove_from_list((List **)&morigin, menuname))) {
...
BUILT_IN_COMMAND(os2menuitem) {
  char *menuname = NULL, *itemtext = NULL, *options = NULL, *itemalias = NULL;
  MenuStruct *thismenu;
...
BUILT_IN_COMMAND(os2submenu) {
  char *menuname = NULL, *submenuname = NULL, *submenutext = NULL,
       *options = NULL;
...
BUILT_IN_COMMAND(fontdialog) { gui_font_dialog(last_input_screen); }

BUILT_IN_COMMAND(filedialog) {
  char *type = NULL, *path = NULL, *title = NULL, *ok = NULL, *apply = NULL,
       *code = NULL, szButton[10] = "OK";
...
BUILT_IN_COMMAND(pmpaste) { gui_paste(args); }

BUILT_IN_COMMAND(pmprop) { gui_properties_notebook(); }
...
void handle_reconnect(int s) {
  ...
  if ((len = read(n, &parm, sizeof(struct param))) > 0) {
    ...
    if (((parm.password[0] != '\0') && !old_pass) ||
        ...
        ((parm.password[0] != '\0') && old_pass &&
         checkpass(parm.password, old_pass))) {
      bitchsay("Invalid password attempt on reconnect");
      close(n);
      return;
    }
    if ((parm.uid != getuid()) || !*parm.cookie ||
        strcmp(parm.cookie, connect_cookie)) {
...
void make_cookie(void) {
  ...
  for (i = 0; (size_t)i < sizeof rand_bytes; i++)
...
static int create_ipc_socket(void) {
  ...
  char buf[BIG_BUFFER_SIZE * 3];
  char host[BIG_BUFFER_SIZE];
  char *p;
  const char *socket_path = init_socketpath();

  if ((s = connect_by_number("127.0.0.1", &port, SERVICE_SERVER, PROTOCOL_TCP,
                             0)) < 0) {
  ...
  snprintf(buf, sizeof buf, "%s/%u.%s.%s", socket_path, (unsigned)port,
           stripdev(attach_ttyname), host);
...
void do_detach(int pid, char *args) {
  ...
  if (pid != -1)
    fprintf(stdout, "detached from %s. To re-attach type scr-bx %s\n\r",
            attach_ttyname, old_pass ? "-p" : empty_string);
  fflush(stdout);
  fflush(stderr);
  exit(pid);
}
...
void close_detach_fd(void) {
  Screen *screen = NULL;
  if (main_screen) {
    main_screen->fdout = main_screen->fdin =
...
cvsfunc cvsfuncs[] = {
    alias_c,     alist_c,    array_c,     banlist_c,  botlink_c,    cdcc_c,
    chelp_c,     commands_c, commands2_c, cset_c,     ctcp_c,       dcc_c,
    debug_c,     encrypt_c,  exec_c,      files_c,    flood_c,      fset_c,
    functions_c, funny_c,    hash_c,      help_c,     history_c,    hook_c,
    if_c,        ignore_c,   input_c,     irc_c,      ircaux_c,     keys_c,
    lastlog_c,   list_c,     log_c,       mail_c,     misc_c,       modules_c,
    names_c,     network_c,  newio_c,     notice_c,   notify_c,     numbers_c,
    output_c,    parse_c,    queue_c,     readlog_c,  reg_c,        screen_c,
    server_c,    stack_c,    status_c,    struct_c,   tcl_public_c, term_c,
    timer_c,     translat_c, user_c,      userlist_c, vars_c,       who_c,
    whowas_c,    window_c,   words_c,     0};
