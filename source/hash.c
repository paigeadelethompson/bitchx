 ...
 */
static inline void move_link_to_top(NickList *tmp, NickList *prev,
                                    HashEntry *location) {
  if (prev) {
    NickList *old_list;
 ...
 */
static inline void remove_link_from_list(NickList *tmp, NickList *prev,
                                         HashEntry *location) {
  if (prev) {
...
void BX_add_name_to_genericlist(char *name, HashEntry *list,
                                unsigned int size) {
  List *nptr;
 ...
 * to the top of the list in the specific array location
 */
static inline void move_gen_link_to_top(List *tmp, List *prev,
                                        HashEntry *location) {
  if (prev) {
    List *old_list;
 ...
 * from our chain of hashed entries.
 */
static inline void remove_gen_link_from_list(List *tmp, List *prev,
                                             HashEntry *location) {
  if (prev) {
    /* remove the link from the middle of the list */
    prev->next = tmp->next;
...
List *BX_find_name_in_genericlist(const char *name, HashEntry *list,
                                  unsigned int size, int remove) {
  HashEntry *location;
  List *tmp, *prev = NULL;
...
NickList *BX_find_nicklist_in_channellist(const char *nick, ChannelList *cptr,
                                          int remove) {
  HashEntry *location;
...
void show_nicklist_hashtable(ChannelList *cptr) {
  ...
  for (count = 0; count < NICKLIST_HASHSIZE; count++) {
    if (cptr->NickListTable[count].links == 0)
      continue;
    say("HASH DEBUG: %d   links %d   hits %d", count,
        cptr->NickListTable[count].links, cptr->NickListTable[count].hits);

    for (ptr = (NickList *)cptr->NickListTable[count].list, count2 = 0; ptr;
         count2++, ptr = ptr->next) {
...
void show_whowas_debug_hashtable(WhowasWrapList *cptr) {
  ...
  for (count = 0; count < WHOWASLIST_HASHSIZE; count++) {
    ...
    for (ptr = (WhowasList *)cptr->NickListTable[count].list, count2 = 0; ptr;
         count2++, ptr = ptr->next) {
      say("HASH_DEBUG: %d:%d  %10s %s!%s", count, count2, ptr->channel,
...
static unsigned long hash_userhost_channel(char *userhost, char *channel,
                                           unsigned int size) {
 ...
 * to the top of the list in the specific array location
 */
static inline void move_link_to_top_whowas(WhowasList *tmp, WhowasList *prev,
                                           HashEntry *location) {
  if (prev) {
    WhowasList *old_list;
 ...
 * from our chain of hashed entries.
 */
static inline void remove_link_from_whowaslist(WhowasList *tmp,
                                               WhowasList *prev,
                                               HashEntry *location) {
  if (prev) {
    /* remove the link from the middle of the list */
...
void BX_add_whowas_userhost_channel(WhowasList *wptr, WhowasWrapList *list) {
  unsigned long hvalue = hash_userhost_channel(
      wptr->nicklist->host, wptr->channel, WHOWASLIST_HASHSIZE);
...
WhowasList *BX_find_userhost_channel(char *host, char *channel, int remove,
                                     WhowasWrapList *wptr) {
  HashEntry *location;
   ...
   * "We found the bucket, now search the chain"
   */
  for (tmp = (WhowasList *)(&(wptr->NickListTable[hvalue]))->list; tmp;
       prev = tmp, tmp = tmp->next) {
    if (!tmp->nicklist->host || !tmp->channel || !host || !channel)
      continue;
    if (!my_stricmp(host, tmp->nicklist->host) &&
...
WhowasList *BX_next_userhost(WhowasWrapList *cptr, WhowasList *nptr) {
  ...
  if (!cptr)
    ...
    int hvalue;
    /* hit end of chain, go to next bucket */
    hvalue = hash_userhost_channel(nptr->nicklist->host, nptr->channel,
                                   WHOWASLIST_HASHSIZE) +
             1;
    if (hvalue >= WHOWASLIST_HASHSIZE) {
      /* end of list */
...
void show_whowas_hashtable(WhowasWrapList *cptr, char *list) {
  ...
  say("WhoWas %s Cache Stats: %lu hits  %lu  links  %lu unlinks", list,
      cptr->total_hits, cptr->total_links, cptr->total_unlinks);
  for (count = 0; count < WHOWASLIST_HASHSIZE; count++) {

    if (cptr->NickListTable[count].links == 0)
      continue;
    for (ptr = (WhowasList *)cptr->NickListTable[count].list; ptr;
         count2++, ptr = ptr->next)
      put_it("%s",
             convert_output_format("%K[%W$[3]0%K] %Y$[10]1 %W$2%G!%c$3",
                                   "%d %s %s %s", count2, ptr->channel,
                                   ptr->nicklist->nick, ptr->nicklist->host));
  }
}

int show_wholeft_hashtable(WhowasWrapList *cptr, time_t ltime, int *total,
                           int *hook, char *list __attribute__((unused))) {
  int count, count2;
  WhowasList *ptr;
  ...
  for (count = 0; count < WHOWASLIST_HASHSIZE; count++) {
    ...
    if (cptr->NickListTable[count].links == 0)
      continue;
    for (ptr = (WhowasList *)cptr->NickListTable[count].list, count2 = 1; ptr;
         count2++, ptr = ptr->next) {
      if (ptr->server1 /* && ptr->server2*/) {
        if (!(*total)++ &&
            (*hook = do_hook(WHOLEFT_HEADER_LIST, "%s %s %s %s %s %s", "Nick",
                             "Host", "Channel", "Time", "Server", "Server")))
          put_it("%s", convert_output_format(
                           fget_string_var(FORMAT_WHOLEFT_HEADER_FSET), NULL));
        if (do_hook(WHOLEFT_LIST, "%s %s %s %ld %s %s", ptr->nicklist->nick,
                    ptr->nicklist->host, ptr->channel, ltime - ptr->time,
                    ptr->server1 ? ptr->server1 : "Unknown",
                    ptr->server2 ? ptr->server2 : "Unknown"))
          put_it("%s",
                 convert_output_format(
                     fget_string_var(FORMAT_WHOLEFT_USER_FSET),
                     "%s %s %s %l %s", ptr->nicklist->nick, ptr->nicklist->host,
                     ptr->channel, (long)ltime - ptr->time,
                     ptr->server1 ? ptr->server1 : empty_string));
      }
    }
  }
  if (*total)
...
int BX_remove_oldest_whowas_hashlist(WhowasWrapList *list, time_t timet,
                                     int count) {
  WhowasList *ptr;
  int total = 0;
  ...
  if (!count) {
    for (x = 0; x < WHOWASLIST_HASHSIZE; x++) {
      ...
      while (ptr) {
        if ((ptr->time + timet) <= now) {
          if (!(ptr = find_userhost_channel(ptr->nicklist->host, ptr->channel,
                                            1, list)))
            break;
    ...
    while ((ptr = next_userhost(list, NULL)) && count) {
      x = hash_userhost_channel(ptr->nicklist->host, ptr->channel,
                                WHOWASLIST_HASHSIZE);
      if (!(ptr = find_userhost_channel(ptr->nicklist->host, ptr->channel, 1,
                                        list)))
        break;
      if (ptr->nicklist) {
        new_free(&(ptr->nicklist->ip));
...
int cmp_stat(List *a, List *b) {
  NickList *a1 = (NickList *)a, *b1 = (NickList *)b;
  int a_status = nick_isop(a1)       ? 0
                 : nick_ishalfop(a1) ? 1
                 : nick_isvoice(a1)  ? 2
                                     : 3;
  int b_status = nick_isop(b1)       ? 0
                 : nick_ishalfop(b1) ? 1
                 : nick_isvoice(b1)  ? 2
                                     : 3;
  int cmp;
...
Flooding *BX_add_name_to_floodlist(char *name, char *host, char *channel __attribute__((unused)),
                                   HashEntry *list, unsigned int size) {
  Flooding *nptr;
  unsigned long hvalue = hash_nickname(name, size);
...
Flooding *BX_find_name_in_floodlist(char *name, char *host __attribute__((unused)), HashEntry *list,
                                    unsigned int size, int remove) {
  HashEntry *location;
