...
#define cparse(format, str)                                                    \
  convert_output_format(fget_string_var(format), "%s", str)
...
char *away_format = NULL, *hold_lines_format = NULL, *channel_format = NULL,
     *notify_format = NULL, *cpu_saver_format = NULL, *kill_format = NULL,
     *lag_format = NULL, *mail_format = NULL, *nick_format = NULL,
     *query_format = NULL, *server_format = NULL, *clock_format = NULL,
     *dcccount_format = NULL, *cdcc_format = NULL, *msgcount_format = NULL,
     *umode_format = NULL, *mode_format = NULL, *topic_format = NULL,
     *user_format = NULL;
...
#define NUMBER_OF_EXPANDOS                                                     \
  (sizeof(status_expandos) / sizeof(struct status_formats))
#endif
 ...
 */
static char *convert_sub_format(const char *format, int args, char c,
                                const char *padded) {
  size_t i = 0;
  ...
  while (*format && pos < BIG_BUFFER_SIZE - 4) {
    *padded = 0;
    ...
    for (i = 0; i < (int)NUMBER_OF_EXPANDOS; i++) {
      ...
      win->status_func[k][(*cp)++] = status_expandos[i].callback_function;
      break;
    }
  }
...
void BX_build_status(Window *win, char *format __attribute__((unused)), int unused __attribute__((unused))) {
  int i, k;
  char *f = NULL;
  int ofs = from_server;
  if (!win)
...
/* Take a built status line and justify it to window size, handling the >%
 * right-justify status format. */
static void justify_status(Window *win, char *buffer, size_t buffer_size) {
  ...
  while (*ptr) {
     ...
     */
    else if ((*ptr == REV_TOG) || (*ptr == UND_TOG) || (*ptr == ALL_OFF) ||
             (*ptr == BOLD_TOG) || (*ptr == BLINK_TOG))
      ptr++;
    /*
  ...
  /*
   * Now if we have a rhs, or the user wants padding anyway, then we have to pad
   * it.
   */
  padding[0] = 0;
  ...
  snprintf(buffer, buffer_size, "%s%s%s%s", str, padding, rhs ? rhs : "",
           ALL_OFF_STR);
  new_free(&str);
}
...
void make_status(Window *win) {
   ...
   */
  for (status_line = 0;
       status_line < 1 + win->double_status + win->status_lines;
       status_line++) {
    char *str;
    ...
    if (get_int_var(REVERSE_STATUS_VAR))
      ...
      len = 0;
    str = &buffer[len];
    snprintf(str, BIG_BUFFER_SIZE - 1, win->wset->status_format[line],
             func_value[0], func_value[1], func_value[2], func_value[3],
             func_value[4], func_value[5], func_value[6], func_value[7],
             func_value[8], func_value[9], func_value[10], func_value[11],
             func_value[12], func_value[13], func_value[14], func_value[15],
             func_value[16], func_value[17], func_value[18], func_value[19],
             func_value[20], func_value[21], func_value[22], func_value[23],
             func_value[24], func_value[25], func_value[26], func_value[27],
             func_value[28], func_value[29], func_value[30], func_value[31],
             func_value[32], func_value[33], func_value[34], func_value[35],
    ...
    do_hook(STATUS_UPDATE_LIST, "%d %d %s", win->refnum, status_line, buffer);
    if (dumb_mode) {
...

    {
      ...
      st = cparse((line == 3)   ? FORMAT_STATUS3_FSET
                  : (line == 2) ? FORMAT_STATUS2_FSET
                  : (line == 1) ? FORMAT_STATUS1_FSET
                                : FORMAT_STATUS_FSET,
                  buffer);
      if (!ansi)
        st = stripansicodes(st);
      if (win->status_lines &&
          (line == win->double_status + win->status_lines) && win->status_split)
...
static char *status_nickname(Window *window) {
  ...
  snprintf(my_buffer, MY_BUFFER / 2, window->wset->nick_format,
...
static char *status_query_nick(Window *window) {
  ...
  if (window->query_nick && window->wset->query_format) {
    snprintf(my_buffer, BIG_BUFFER_SIZE, window->wset->query_format,
             window->query_nick);
    return my_buffer;
  } else
    RETURN_EMPTY;
...
static char *status_right_justify(Window *window __attribute__((unused))) {
...
static char *status_clock(Window *window) {
  ...
  if ((get_int_var(CLOCK_VAR) && window->wset->clock_format) &&
      (get_int_var(SHOW_STATUS_ALL_VAR) ||
       (window == window->screen->current_window)))
    snprintf(my_buf, MY_BUFFER, window->wset->clock_format,
...
static char *status_hold_lines(Window *window) {
  ...
  if ((num = (window->lines_held / 10) * 10)) {
    snprintf(my_buffer, MY_BUFFER / 2, window->wset->hold_lines_format,
             ltoa(num));
    return (my_buffer);
  }
  RETURN_EMPTY;
...
static char *status_msgcount(Window *window) {
  ...
  if (get_int_var(MSGCOUNT_VAR) && window->wset->msgcount_format) {
    snprintf(my_buffer, MY_BUFFER / 2, window->wset->msgcount_format,
             ltoa(get_int_var(MSGCOUNT_VAR)));
    return my_buffer;
  }
  RETURN_EMPTY;
}
...
static char *status_channel(Window *window) {
  ...
  if (window->current_channel /* && chan_is_connected(s, window->server)*/) {
    ...
    if (get_int_var(HIDE_PRIVATE_CHANNELS_VAR) &&
        is_channel_mode(window->current_channel, MODE_PRIVATE | MODE_SECRET,
                        window->server))
    ...
    if ((num = get_int_var(CHANNEL_NAME_WIDTH_VAR)) &&
      ...
      channel[num] = (char)0;
    snprintf(my_buffer, IRCD_BUFFER_SIZE, window->wset->channel_format,
             channel);
    return my_buffer;
  }
  RETURN_EMPTY;
...
static char *status_mail(Window *window) {
  ...
  if (window &&
      (get_int_var(MAIL_VAR) && (number = check_mail()) &&
       window->wset->mail_format) &&
      (get_int_var(SHOW_STATUS_ALL_VAR) ||
       (window == window->screen->current_window))) {
...
static char *status_away(Window *window) {
  ...
  if (window && connected_to_server && !get_int_var(SHOW_STATUS_ALL_VAR) &&
    ...
    if (window->server != -1 && get_server_away(window->server)) {
      snprintf(my_buffer, MY_BUFFER, window->wset->away_format,
               ltoa(get_int_var(MSGCOUNT_VAR)));
      return my_buffer;
    } else
      RETURN_EMPTY;
...
static char *status_hold(Window *window) {
  ...
  if (window->holding_something &&
      (text = get_wset_string_var(window->wset, STATUS_HOLD_WSET)))
    return (text);
  RETURN_EMPTY;
}
    ...
    if ((chan = lookup_channel(window->current_channel, window->server, 0))) {
      ...
      if ((t = chan->topic)) {
        ...
        if (get_int_var(STATUS_DOES_EXPANDOS_VAR))
          ...
          strcpy(t2, t);
        snprintf(my_buffer, sizeof my_buffer, window->wset->topic_format,
                 stripansicodes(t2));
      } else
...
static char *status_oper(Window *window) {
  ...
  if (get_server_operator(window->server) &&
      ...
      (get_int_var(SHOW_STATUS_ALL_VAR) || connected_to_server ||
       (window->screen->current_window == window)))
    return (text);
  RETURN_EMPTY;
}
...
static char *status_refnum(Window *window) {
  ...
  strlcpy(my_buffer, window->name ? window->name : ltoa(window->refnum),
          sizeof my_buffer);
  return (my_buffer);
}
...
static char *status_oper_kills(Window *window) {
  ...
  if (window->wset->kills_format && (nick_collisions || oper_kills)) {
    ...
    snprintf(my_buffer, MY_BUFFER, window->wset->kills_format, tmp,
             ltoa(oper_kills));
    return my_buffer;
  }
  RETURN_EMPTY;
...
static char *status_dcccount(Window *window) {
  ...
  if (window->wset->dcccount_format && send_count_stat) {
    ...
    snprintf(my_buffer, 2 * MY_BUFFER, window->wset->dcccount_format,
             ltoa(get_count_stat), tmp);
    return my_buffer;
  }
  RETURN_EMPTY;
}
...
static char *status_cdcccount(Window *window) {
  ...
  if (window->wset->cdcc_format && cdcc_numpacks) {
    ...
    snprintf(my_buffer, 2 * MY_BUFFER, window->wset->cdcc_format,
             ltoa(send_numpacks), tmp);
    return my_buffer;
  }
#endif
  RETURN_EMPTY;
...
static char *status_users(Window *window) {
  ...
  if (window->server != -1 && window->wset->status_users_format) {
    if ((chan = prepare_command(&serv, NULL, PC_SILENT))) {
      ...
      snprintf(my_buffer, MY_BUFFER * 2, window->wset->status_users_format,
...
static char *status_null_function(Window *window __attribute__((unused))) { RETURN_EMPTY; }
...
static char *status_percent(Window *window __attribute__((unused))) {
  static char percent[] = "%";
  return percent;
}
