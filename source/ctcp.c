...
#define CTCP_HANDLER(x)                                                        \
  static char *x(CtcpEntry *ctcp __attribute__((unused)),                      \
                 char *from __attribute__((unused)),                           \
...
static CtcpEntry ctcp_cmd[] = {
    {"SED", CTCP_SED, CTCP_INLINE | CTCP_NOLIMIT,
     "contains simple_encrypted_data", do_sed, do_sed_reply},
    {"UTC", CTCP_UTC, CTCP_INLINE | CTCP_NOLIMIT,
     "substitutes the local timezone", do_utc, do_utc},
    {"ACTION", CTCP_ACTION, CTCP_SPECIAL | CTCP_NOLIMIT,
     "contains action descriptions for atmosphere", do_atmosphere,
     do_atmosphere},

    {"DCC", CTCP_DCC, CTCP_SPECIAL, "requests a direct_client_connection",
     do_dcc, do_dcc_reply},

    {"CDCC", CTCP_CDCC, CTCP_SPECIAL | CTCP_TELLUSER,
     "checks cdcc info for you", do_bdcc, NULL},
    ...
    {"VERSION", CTCP_VERSION, CTCP_REPLY | CTCP_TELLUSER,
     "shows client type, version and environment", do_version, NULL},

    {"CLIENTINFO", CTCP_CLIENTINFO, CTCP_REPLY | CTCP_TELLUSER,
     "gives information about available CTCP commands", do_clientinfo, NULL},
    {"USERINFO", CTCP_USERINFO, CTCP_REPLY | CTCP_TELLUSER,
     "returns user settable information", do_userinfo, NULL},
    {"ERRMSG", CTCP_ERRMSG, CTCP_REPLY | CTCP_TELLUSER,
     "returns error messages", do_echo, NULL},
    {"FINGER", CTCP_FINGER, CTCP_REPLY | CTCP_TELLUSER,
     "shows real name, login name and idle time of user", do_finger, NULL},
    {"TIME", CTCP_TIME, CTCP_REPLY | CTCP_TELLUSER,
     "tells you the time on the user's host", do_time, NULL},
    {"PING", CTCP_PING, CTCP_REPLY | CTCP_TELLUSER,
     "returns the arguments it receives", do_ping, do_ping_reply},
    {"ECHO", CTCP_ECHO, CTCP_REPLY | CTCP_TELLUSER,
     "returns the arguments it receives", do_echo, NULL},

    {"INVITE", CTCP_INVITE, CTCP_SPECIAL, "invite to channel specified",
     do_cinvite, NULL},
    {"WHOAMI", CTCP_WHOAMI, CTCP_SPECIAL, "user list information", do_whoami,
     NULL},
    {"OP", CTCP_OPS, CTCP_SPECIAL, "ops the person if on userlist", do_ctcpops,
     NULL},
    {"OPS", CTCP_OPS, CTCP_SPECIAL, "ops the person if on userlist", do_ctcpops,
     NULL},
    {"UNBAN", CTCP_UNBAN, CTCP_SPECIAL, "unbans the person from channel",
     do_ctcpunban, NULL},
    {"IDENT", CTCP_IDENT, CTCP_SPECIAL, "change userhost of userlist",
     do_ctcpident, NULL},
    {"XLINK", CTCP_BOTLINK, CTCP_SPECIAL, "x-filez rule", do_botlink, NULL},

    {"UPTIME", CTCP_UPTIME, CTCP_SPECIAL, "my uptime", do_ctcp_uptime, NULL},

    {NULL, CTCP_CUSTOM, CTCP_REPLY | CTCP_TELLUSER, NULL, NULL, NULL}};
...
static char *ctcp_type[] = {"PRIVMSG", "NOTICE"};
...
void print_ctcp(char *from, char *uh, char *to, char *str, char *cmd) {
  put_it("%s",
         convert_output_format(
             fget_string_var(is_channel(to) ? FORMAT_CTCP_CLOAK_FUNC_FSET
                                            : FORMAT_CTCP_CLOAK_FUNC_USER_FSET),
             "%s %s %s %s %s %s", update_clock(GET_TIME), from, uh, to, str,
...
CTCP_HANDLER(do_ctcpident) {
  ...
  if (nick && passwd && ThisNick && ThisNick->password) {
    if (!checkpass(passwd, ThisNick->password)) {
      ...
      if (!h || !*h)
        return NULL;
      malloc_sprintf(&ThisNick->host, "*%s@%s", clear_server_flags(u),
                     cluster(h));
    }
  }
  if (!ok && !get_int_var(CLOAK_VAR))
    ...
    logmsg(LOG_CTCP, from, 0, "Successful %s from %s!%s", "IDENT", from,
           FromUserHost);
  }
#endif
  return NULL;
...
got_good_pass1:
  ...
  send_to_server("NOTICE %s :try /ctcp %s cdcc help", from,
...
CTCP_HANDLER(do_botlink) {
  ...
  if (nick && password) {
    char *t = NULL;
    UserList *n = NULL;
    if (get_string_var(BOT_PASSWD_VAR) ||
        (n = lookup_userlevelc(nick, FromUserHost, "*", password))) {
      char *pass;
      if (!n)
      ...
      if (pass && !my_stricmp(pass, password)) {
        ...
        logmsg(LOG_CTCP, from, 0, "Successful %s from %s!%s to %s [%s]",
               "XLINK", from, FromUserHost, to, cmd ? cmd : empty_string);
      } else
    ...
    log_denied("XLINK", from, FromUserHost, to, cmd);
    return m_sprintf("Invalid Bot Link request %s %s %s", nick ? nick : "null",
...
CTCP_HANDLER(do_cinvite) {
  ...
  if (chan && tmp && (tmp->flags & ADD_INVITE) &&
    ...
    if (im_on_channel(channel, from_server) &&
        is_chanop(channel, get_server_nickname(from_server))) {
      if (chan->key)
        my_send_to_server(
            serv, "NOTICE %s :\002Ctcp-inviting you to %s. Key is [%s]\002",
            from, channel, chan->key);
      else
        my_send_to_server(serv, "NOTICE %s :\002Ctcp-inviting you to %s\002",
                          from, channel);
      my_send_to_server(serv, "INVITE %s %s", from, channel);
      logmsg(LOG_CTCP, from, 0, "Successful INVITE from %s!%s to %s", from,
             FromUserHost, channel);
    } else {
      if (get_int_var(SEND_CTCP_MSG_VAR))
        my_send_to_server(
            serv, "NOTICE %s :\002%s\002: I'm not on %s, or I'm not opped",
    ...
    if (!chan) {
      if (get_int_var(SEND_CTCP_MSG_VAR))
        my_send_to_server(serv,
                          "NOTICE %s :\002%s\002: I'm not on that channel",
                          from, version);
      log_denied("INVITE", from, FromUserHost, to, cmd);
    } else {
      if (get_int_var(SEND_CTCP_MSG_VAR))
        my_send_to_server(serv, "NOTICE %s :\002%s\002: Access Denied", from,
                          version);
      log_denied("INVITE", from, FromUserHost, to, cmd);
    }
  } else
...
CTCP_HANDLER(do_whoami) {
  ...
  if (Nick && Nick->flags) {
    send_to_server("NOTICE %s :\002%s\002: Userlevel - %s ", from, version,
                   convert_flags_to_str(Nick->flags));
    send_to_server(
        "NOTICE %s :\002%s\002: Host Mask - %s Channels Allowed - %s   %s",
        from, version, Nick->host, Nick->channels,
        Nick->password ? "Password Required" : empty_string);
    logmsg(LOG_CTCP, from, 0, "Successful %s from %s!%s to %s", "WHOAMI", from,
           FromUserHost, to);
...
CTCP_HANDLER(do_ctcp_uptime) {
  ...
  Nick = lookup_userlevelc("*", FromUserHost, "*", NULL);
  if (Nick) {
    send_to_server("NOTICE %s :\002%s\002: Current Uptime is %s", from, version,
                   convert_time(now - start_time));
    logmsg(LOG_CTCP, from, 0, "Successful %s from %s!%s to %s", "UPTIME", from,
           FromUserHost, to);
  } else if (get_int_var(SEND_CTCP_MSG_VAR))
  ...
  if (chan && get_cset_int_var(chan->csets, USERLIST_CSET) && Nick) {
    if (Nick->flags & ADD_OPS) {
      ...
      if (im_on_channel(channel, from_server) &&
        ...
        logmsg(LOG_CTCP, from, 0, "Successful %s from %s!%s", "OPS", from,
               FromUserHost);
      } else {
        if (get_int_var(SEND_CTCP_MSG_VAR))
          my_send_to_server(
      ...
      if (im_on_channel(channel, from_server) &&
        ...
        logmsg(LOG_CTCP, from, 0, "Successful %s from %s!%s", "VOICE", from,
               FromUserHost);
      } else {
        if (get_int_var(SEND_CTCP_MSG_VAR))
          my_send_to_server(
      ...
      if ((!get_int_var(CLOAK_VAR) || (Nick && (Nick->flags & ADD_CTCP))) &&
          get_int_var(SEND_CTCP_MSG_VAR))
        my_send_to_server(
            serv, "NOTICE %s :\002%s\002: I'm not on %s, or I'm not opped",
            from, version, channel ? channel : empty_string);
      log_denied("OPS", from, FromUserHost, to, cmd);
    }
  } else if (!get_int_var(CLOAK_VAR) || (Nick && (Nick->flags & ADD_CTCP))) {
    if (get_int_var(SEND_CTCP_MSG_VAR))
      my_send_to_server(
          serv, "NOTICE %s :\002%s\002: I'm not on %s, or I'm not opped", from,
          version, channel ? channel : empty_string);
    log_denied("OPS", from, FromUserHost, to, cmd);
  }
#endif
  return NULL;
  ...
  if (chan && get_cset_int_var(chan->csets, USERLIST_CSET) && Nick &&
    ...
    if (chan && chan->have_op) {
      if ((b = ban_is_on_channel(ban, chan))) {
        ...
        logmsg(LOG_CTCP, from, 0, "Successful %s from %s!%s on %s", "UNBAN",
               from, FromUserHost, channel);
      } else {
        if (get_int_var(SEND_CTCP_MSG_VAR))
          my_send_to_server(
              server, "NOTICE %s :\002%s\002: you %s are not banned on %s",
              from, version, ban, channel);
        log_denied("OPS", from, FromUserHost, to, cmd);
      }
    } else {
      my_send_to_server(
          server, "NOTICE %s :\002%s\002: I'm not on %s, or I'm not opped",
          from, version, channel);
      log_denied("OPS", from, FromUserHost, to, cmd);
    }
  } else if (!get_int_var(CLOAK_VAR) || (Nick && (Nick->flags & ADD_CTCP))) {
    if (!chan)
      my_send_to_server(server,
                        "NOTICE %s :\002%s\002: I'm not on that channel", from,
                        version);
    else
      my_send_to_server(server, "NOTICE %s :\002%s\002: Access Denied", from,
                        version);
    log_denied("OPS", from, FromUserHost, to, cmd);
  }
#endif
    ...
    if (*ptr && do_hook(ENCRYPTED_PRIVMSG_LIST, "%s %s %s", from, to, ptr)) {
      if (is_channel(to))
        put_it("%s", convert_output_format(
                         fget_string_var(FORMAT_ENCRYPTED_PUBLIC_FSET),
                         "%s %s %s %s %s", update_clock(GET_TIME), from,
                         FromUserHost, to, ptr));
      else
        put_it("%s", convert_output_format(
                         fget_string_var(FORMAT_ENCRYPTED_PRIVMSG_FSET),
...
CTCP_HANDLER(do_sed_reply) {
  ...
  if (ret) {
    ...
    if (*ptr && do_hook(ENCRYPTED_NOTICE_LIST, "%s %s %s", from, to, ptr)) {
      if (is_channel(to))
        put_it("%s", convert_output_format(
                         fget_string_var(FORMAT_ENCRYPTED_PUBLIC_NOTICE_FSET),
                         "%s %s %s %s %s", update_clock(GET_TIME), from,
                         FromUserHost, to, ptr));
      else
        put_it("%s",
               convert_output_format(
                   fget_string_var(FORMAT_ENCRYPTED_NOTICE_FSET), "%s %s %s %s",
  ...
  if (is_channel(to)) {
    ...
    if (do_hook(ACTION_LIST, "%s %s %s", from, to, cmd)) {
      ...
      if (is_current_channel(to, from_server, 0)) {
        ...
        put_it("%s", convert_output_format(s, "%s %s %s %s %s",
    ...
    if (do_hook(ACTION_LIST, "%s %s %s", from, to, cmd))
      put_it("%s", convert_output_format(
                       fget_string_var(ac_reply ? FORMAT_ACTION_AR_FSET
                                                : FORMAT_ACTION_FSET),
                       "%s %s %s %s %s", update_clock(GET_TIME), from,
                       FromUserHost, to, ptr1));
  }
...
CTCP_HANDLER(do_dcc) {
  ...
  if (offer.type && offer.description && *offer.description && offer.address &&
      offer.port)
  ...
  if (cmd && *cmd) {
    ...
    for (dll = dll_ctcp; dll; dll = dll->next) {
      if (!my_stricmp(cmd, dll->name)) {
        send_ctcp(CTCP_NOTICE, from, CTCP_CLIENTINFO, "%s %s", dll->name,
                  dll->desc ? dll->desc : "none");
        return NULL;
      }
    }
#endif
    for (i = 0; i < NUMBER_OF_CTCPS; i++) {
      if (!my_stricmp(cmd, ctcp_cmd[i].name)) {
        send_ctcp(CTCP_NOTICE, from, CTCP_CLIENTINFO, "%s %s", ctcp_cmd[i].name,
                  ctcp_cmd[i].desc);
        return NULL;
      }
    }
    send_ctcp(CTCP_NOTICE, from, CTCP_ERRMSG, "%s: %s is not a valid function",
...
CTCP_HANDLER(do_version) {
  ...
  malloc_strcpy(
      ...
      stripansicodes(convert_output_format(
          ...
          internal_version, the_unix, the_version, tcl_versionstr)));
  ...
  malloc_strcpy(
      ...
      stripansicodes(convert_output_format(
          fget_string_var(FORMAT_VERSION_FSET), "%s %s %s %s %s", irc_version,
          internal_version, "*IX", " ", tcl_versionstr)));
  send_ctcp(CTCP_NOTICE, from, CTCP_VERSION, "%s :%s", version_reply,
#endif
...
CTCP_HANDLER(do_time) {
  ...
  if (get_int_var(CLOAK_VAR))
    return NULL;
  send_ctcp(CTCP_NOTICE, from, CTCP_TIME, "%s", my_ctime(now));
  return NULL;
}
...
#define GECOS_DELIMITER ','
  ...
  send_ctcp(CTCP_NOTICE, from, CTCP_FINGER, "%s (%s@%s) Idle %ld second%s",
...
extern char *do_ctcp(char *from, char *to, char *str) {
  ...
  for (;; strlcat(local_ctcp_buffer, last, sizeof local_ctcp_buffer - 2)) {
    ...
    if (!dll && ctcp_cmd[i].id == CTCP_ACTION)
      check_flooding(from, CTCP_ACTION_FLOOD, ctcp_argument,
                     is_channel(to) ? to : NULL);
    else if (!dll && ctcp_cmd[i].id == CTCP_DCC)
      check_flooding(from, CTCP_FLOOD, ctcp_argument,
                     is_channel(to) ? to : NULL);
#else
    if (ctcp_cmd[i].id == CTCP_ACTION)
      ...
      if (get_int_var(NO_CTCP_FLOOD_VAR) &&
          (now - get_server_last_ctcp_time(from_server) <
           get_int_var(CTCP_DELAY_VAR))) {
        if (get_int_var(FLOOD_WARNING_VAR))
          put_it("%s",
                 convert_output_format(fget_string_var(FORMAT_FLOOD_FSET),
...
#endif
    {
       ...
       */
#ifdef WANT_TCL
      if (check_tcl_ctcp(from, FromUserHost, from, to, ctcp_command,
                         ctcp_argument))
        continue;
#endif

      if (do_hook(CTCP_LIST, "%s %s %s %s", from, to, ctcp_command,
                  ctcp_argument) &&
          allow_ctcp_reply) {
      ...
      if (do_hook(CTCP_LIST, "%s %s %s %s", from, to, ctcp_command,
        ...
        if (get_int_var(CTCP_VERBOSE_VAR))
          ...
          put_it("%s",
                 convert_output_format(
                     fget_string_var(
                         (!lookup_userlevelc("*", FromUserHost, "*", NULL))
                             ? get_int_var(CLOAK_VAR) ? FORMAT_CTCP_CLOAK_FSET
                                                      : FORMAT_CTCP_FSET
                         : get_int_var(CLOAK_VAR) ? FORMAT_CTCP_CLOAK_USER_FSET
...
#endif
#ifdef WANT_TCL
    check_tcl_ctcr(from, FromUserHost, from, to, ctcp_command,
                   ctcp_argument ? ctcp_argument : empty_string);
#endif
    /* Set up the window level/logging */
    set_display_target(from, LOG_CTCP);
    /* Toss it at the user.  */
    if (do_hook(CTCP_REPLY_LIST, "%s %s %s", from, ctcp_command,
                ctcp_argument)) {
      put_it("%s", convert_output_format(
                       fget_string_var(FORMAT_CTCP_REPLY_FSET),
                       "%s %s %s %s %s", update_clock(GET_TIME), from,
                       FromUserHost, ctcp_command, ctcp_argument));
      add_last_type(&last_ctcp_reply[0], 1, from, FromUserHost, ctcp_command,
                    ctcp_argument);
    }
...
extern void send_ctcp(int type, char *to, int datatag, char *format, ...) {
  char putbuf[BIG_BUFFER_SIZE + 1], *putbuf2;
  int len;

  if ((len = IRCD_BUFFER_SIZE - (12 + strlen(to))) < 0)
    return;

  if ((size_t)len < strlen(ctcp_cmd[datatag].name) + 3)
    return;
  ...
  if (format) {
    ...
    do_hook(SEND_CTCP_LIST, "%s %s %s %s", ctcp_type[type], to,
            ctcp_cmd[datatag].name, putbuf);
    snprintf(putbuf2, len, "%c%s %s%c", CTCP_DELIM_CHAR, ctcp_cmd[datatag].name,
             putbuf, CTCP_DELIM_CHAR);
  } else {
    do_hook(SEND_CTCP_LIST, "%s %s %s", ctcp_type[type], to,
            ctcp_cmd[datatag].name);
    snprintf(putbuf2, len, "%c%s%c", CTCP_DELIM_CHAR, ctcp_cmd[datatag].name,
             CTCP_DELIM_CHAR);
  }
